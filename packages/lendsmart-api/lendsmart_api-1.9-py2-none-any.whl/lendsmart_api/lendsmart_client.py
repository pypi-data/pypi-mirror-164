...
class LendsmartClient:
    ...
    def get_response(self, method, url, headers, body):
        ...
        print("======Trying to connect api ", url)
        self.retry_count +=1
        print("Retrying ",self.retry_count, RETRY_COUNT)
        response = method(url, headers=headers, data=body)
        if response.status_code in RETRY_STATUS_CODES and RETRY_COUNT != self.retry_count:
            raise ApiError("Not found", status=response.status_code, json=None)
        return response


    def should_retry(self, endpoint):
        """
            This function will return true if retry needed for this endpoint

            Here we will check the root endpoint is configured in retry or not

            for example:
                1. Endpoint = "/a4cbdev/loanapps/2095309757140738047"
                2. This will be splitted into [a4cbdev, loanapps, 2095309757140738047]
                3. we remove the namespace from list [loanapps, 2095309757140738047]
                4. Then we check the loanapps is configured in RETRY_ENDPOINTS
        """
        namespace = self.get_namespace()
        for retry_endpoint in RETRY_ENDPOINTS:
            substring1 = str("/" + namespace + "/" + retry_endpoint + "/")
            substring2 = str("/" + retry_endpoint + "/")
            if substring1 in endpoint:
                return True
            if substring2 in endpoint:
                return True
        return False
    ...
    def _api_call(self, endpoint, model=None, method=None, data=None, filters=None):
        ...
        is_retry_needed = self.should_retry(endpoint)
        print("is_retry_needed", is_retry_needed)
