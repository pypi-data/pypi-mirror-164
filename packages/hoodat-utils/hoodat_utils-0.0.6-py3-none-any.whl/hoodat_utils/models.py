...
class Video(db.Model):
    ...
    actor_group_id = db.Column(db.Integer,
                               db.ForeignKey("actor_groups.actor_group_id"))
    frames = db.relationship("Frame", backref="Video", lazy="dynamic")
    frames_complete = db.relationship("Frame_Complete",
                                      backref="Video",
                                      lazy="dynamic")
    objects = db.relationship("Object", backref="Video", lazy="dynamic")
    objects_complete = db.relationship("Object_Complete",
                                       backref="Video",
                                       lazy="dynamic")
    triggers = db.relationship("Trigger", backref="Video", lazy="dynamic")
    faces = db.relationship("Face", backref="Video", lazy="dynamic")
    facenet_haar_predictions = db.relationship("Facenet_Haar_Prediction",
                                               backref=backref("Video",
                                                               uselist=False))

    __table_args__ = (db.UniqueConstraint("video_id",
                                          "episode_id",
                                          name="episode_video"), )
...
class Frame(db.Model):
    ...
    face = db.relationship("Face",
                           backref=backref("Frame", uselist=False),
                           lazy="dynamic")
...
class Frame_Complete(db.Model):
    ...
    frame_number = db.Column(db.Integer, nullable=False)
    objects_complete = db.relationship("Object_Complete",
                                       backref="Frame_Complete",
                                       lazy="dynamic")
    faces_complete = db.relationship("Face_Complete",
                                     backref=backref("Frame_Complete",
                                                     uselist=False),
                                     lazy="dynamic")
...
class Object(db.Model):
    ...
    character_label = db.relationship("Character_Label",
                                      backref=backref("Object", uselist=False))
    object_pca_features = db.relationship("Object_PCA_Features",
                                          backref=backref("Object",
                                                          uselist=False))
    object_umap_features = db.relationship("Object_UMAP_Features",
                                           backref=backref("Object",
                                                           uselist=False))
...
class Object_Complete(db.Model):
    ...
    object_h = db.Column(db.Integer, nullable=False)
    faces_complete = db.relationship("Face_Complete",
                                     backref=backref("Object_Complete",
                                                     uselist=False),
                                     lazy="dynamic")
    facenet_haar_predictions = db.relationship("Facenet_Haar_Prediction",
                                               backref=backref(
...
class Face_Complete(db.Model):
    ...
    object_id = db.Column(db.Integer,
                          db.ForeignKey("objects_complete.object_id"))
    video_id = db.Column(db.Integer, db.ForeignKey("videos.video_id"))
    frame_id = db.Column(db.Integer, db.ForeignKey("frames_complete.frame_id"))
    episode_id = db.Column(db.Integer, db.ForeignKey("episodes.episode_id"))
    object_number = db.Column(db.Integer, nullable=False)
...
class Character_Label(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("object_id",
                                          "character_id",
                                          name="object_label"), )
...
class Season(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("series_id",
                                          "season_number",
                                          name="series_season"), )
...
class Episode(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint("episode_number",
                                          "season_id",
                                          name="season_episode"), )
...
class Actor(db.Model):
    ...
    actor_group_lookups = db.relationship("Actor_Group_Lookup",
                                          backref="Actor",
                                          lazy="dynamic")
...
class Trigger(db.Model):
    ...
    process_id = db.Column(db.Integer,
                           db.ForeignKey("processes.process_id"),
                           nullable=False)
...
class Haar_Cascade(db.Model):
    ...
    haar_cascade_id = db.Column(db.Integer,
                                primary_key=True,
                                autoincrement=True)
...
class Object_PCA_Features(db.Model):
    ...
    object_pca_features_id = db.Column(db.Integer,
                                       primary_key=True,
                                       autoincrement=True)
    object_id = db.Column(db.Integer, db.ForeignKey("objects.object_id"))
...
class Object_UMAP_Features(db.Model):
    ...
    object_umap_features_id = db.Column(db.Integer,
                                        primary_key=True,
                                        autoincrement=True)
    object_id = db.Column(db.Integer, db.ForeignKey("objects.object_id"))
    feature_1 = db.Column(db.Numeric(7, 6), nullable=False)
...
class Facenet_Haar_Model(db.Model):
    ...
    facenet_haar_model_id = db.Column(db.Integer,
                                      primary_key=True,
                                      autoincrement=True)
    model_name = db.Column(db.String(24), unique=True, nullable=False)
    series_id = db.Column(db.Integer, db.ForeignKey("series.series_id"))
    timestamp_added = db.Column(db.String(10), nullable=False)
    model_input_data = db.relationship("Facenet_Haar_Model_Input_Data",
                                       backref="Facenet_Haar_Model",
                                       lazy="dynamic")
    model_predictions = db.relationship("Facenet_Haar_Prediction",
                                        backref="Facenet_Haar_Model",
                                        lazy="dynamic")
...
class Facenet_Haar_Model_Input_Data(db.Model):
    ...
    facenet_haar_model_input_data_id = db.Column(db.Integer,
                                                 primary_key=True,
                                                 autoincrement=True)
    object_id = db.Column(db.Integer,
                          db.ForeignKey("objects.object_id"),
                          nullable=False)
    character_id = db.Column(db.Integer,
                             db.ForeignKey("characters.character_id"),
                             nullable=False)
    model_name = db.Column(db.String(24),
                           db.ForeignKey("facenet_haar_models.model_name"),
                           nullable=False)
    timestamp_added = db.Column(db.String(10), nullable=False)
    ...
    __table_args__ = (db.UniqueConstraint(
        "model_name",
        "object_id",
        name="facenet_haar_model_input_model_object"), )
...
class Facenet_Haar_Prediction(db.Model):
    ...
    facenet_haar_prediction_id = db.Column(db.Integer,
                                           primary_key=True,
                                           autoincrement=True)
    object_id = db.Column(db.Integer,
                          db.ForeignKey("objects.object_id"),
                          nullable=False)
    object_complete_id = db.Column(db.Integer,
                                   db.ForeignKey("objects_complete.object_id"),
                                   nullable=True)
    video_id = db.Column(db.Integer,
                         db.ForeignKey("videos.video_id"),
    ...
    __table_args__ = (
        db.UniqueConstraint("object_id",
                            "video_id",
                            "facenet_haar_model_name",
                            name="facenet_haar_prediction_and_model"),
        db.UniqueConstraint("facenet_haar_model_name",
                            "object_id",
...
class Facenet_Haar_Prediction_Complete(db.Model):
    ...
    facenet_haar_prediction_complete_id = db.Column(db.Integer,
                                                    primary_key=True,
                                                    autoincrement=True)
    object_id = db.Column(db.Integer,
                          db.ForeignKey("objects_complete.object_id"),
    ...
    __table_args__ = (
        db.UniqueConstraint("object_id",
                            "video_id",
                            name="facenet_haar_prediction_complete"),
        db.UniqueConstraint("facenet_haar_model_name",
                            "object_id",
