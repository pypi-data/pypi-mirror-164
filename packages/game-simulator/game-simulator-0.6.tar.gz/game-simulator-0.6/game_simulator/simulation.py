...
def simulation_function(num_rows,
    ...
    if len(total_permutations) > 50:
        total_permutations = random.sample(total_permutations,50)
    else:
    ...
    if iterate_all == "NO":
        ...
        for t in range(timeperiod):
            ...
            for j in range(len(cart_prod2)):
                ...
                flat_list_row = random.choices(flat_list_row+zz1,weights=[random_multiplier*10]*len(flat_list_row)+[10]*len(zz1),k=1)
                
                zz1 = [ i for i in range_lists[1] if i not in flat_list_column]
                flat_list_column = random.choices(flat_list_column+zz1,weights=[random_multiplier*10]*len(flat_list_column)+[10]*len(zz1),k=1)
