...
class Post(object): # can be a post or channel info wrap
	...
	def getActiveUser(self):
		text = getText(self.extra)
		if not text:
			return -1
		if 'online' not in text.lower():
			return 0
		try:
			return getNumber(''.join(text.split(',')[-1].split())[:-6])
		except:
			return -1

	def getViews(self):
		try:
			return getNumber(getText(self.views))
		except:
			return 0
...
def getPostFromSoup(name, soup):
	...
	post.extra = getField(soup, 'tgme_page_extra')
	...
	post.views = soup.find('span', 'tgme_widget_message_views')
