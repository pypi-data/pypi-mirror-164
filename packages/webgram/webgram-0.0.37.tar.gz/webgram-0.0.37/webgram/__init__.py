...
def _getPostsSoup(name, post_id, direction, ttl):
	link = 'https://t.me/s/' + name
	if post_id:
		link += '?%s=%d' % (direction, post_id)
	return getSoup(link, ttl=ttl or post_id)

# force_cache flag is deprecated
def getPosts(name, post_id=None, direction='after', force_cache = False, ttl = 1):
	if force_cache:
		ttl = float('Inf')
	soup = _getPostsSoup(name, post_id, direction, ttl)
