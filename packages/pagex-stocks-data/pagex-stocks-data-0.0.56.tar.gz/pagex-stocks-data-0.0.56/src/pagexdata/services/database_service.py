...
def read_all_as_dataframe(entity_class_) -> DataFrame:
    ...
    df = pd.read_sql(entity_class_.__tablename__, url.render_as_string(hide_password=False))
    pd.to_datetime(df.import_date, format=datetime_service.date_format)
    return df
...
def update_reddit_hits_entry(entity) -> None:
    ...
    try:
        ...
        db_session.execute(update(RedditHits)
           .where(RedditHits.date == entity.date)
    ...
    try:
        ...
        for subreddit in subreddits:
            for symbol in ticker_symbols:
                ...
                db_session.execute(update(RedditHits)
                    .where(RedditHits.date == import_date)
