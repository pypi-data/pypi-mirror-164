...
class CollageGenerator:
    ...
    def generate(self, entity: str, username: str, cols: int, rows: int,
                 period: str) -> Image:
        self._validate_parameters(entity, rows, cols, period)
        if entity == self.ENTITY_ALBUM:
            return self.generate_top_albums_collage(username, cols, rows,
                                                    period)
        elif entity == self.ENTITY_ARTIST:
            return self.generate_top_artists_collage(username, cols, rows,
                                                     period)
        elif entity == self.ENTITY_TRACK:
            return self.generate_top_tracks_collage(username, cols, rows,
                                                    period)
        else:
            raise ValueError(f"Invalid entity: {entity}")

    def generate_top_albums_collage(self, username: str, cols: int, rows: int,
                                    period: str) -> Image:
        user = self.lastfm_client.get_user(username)
        tiles = self._get_tiles_from_top_albums(user, limit=cols * rows,
                                                period=period)
        return self._create_image(tiles, cols, rows)

    def generate_top_artists_collage(self, username: str, cols: int, rows: int,
                                     period: str) -> Image:
        user = self.lastfm_client.get_user(username)
        tiles = self._get_tiles_from_top_artists(user, limit=cols * rows,
                                                 period=period)
        return self._create_image(tiles, cols, rows)

    def generate_top_tracks_collage(self, username: str, cols: int, rows: int,
                                    period: str) -> Image:
        user = self.lastfm_client.get_user(username)
        tiles = self._get_tiles_from_top_tracks(user, limit=cols * rows,
                                                period=period)
        return self._create_image(tiles, cols, rows)

    def _get_tiles_from_top_albums(self, user: User, limit: int,
                                   period: str) -> List:
        top_albums = self.lastfm_client.get_top_albums(user, limit, period)
        tiles = self._create_tiles_from_top_items(top_albums,
                                                  entity=self.ENTITY_ALBUM)
        return tiles

    def _get_tiles_from_top_artists(self, user: User, limit: int,
                                    period: str) -> List:
        top_artists = self.lastfm_client.get_top_artists(user, limit, period)
        tiles = self._create_tiles_from_top_items(top_artists,
                                                  entity=self.ENTITY_ARTIST)
        return tiles

    def _get_tiles_from_top_tracks(self, user: User, limit: int,
                                   period: str) -> List:
        top_tracks = self.lastfm_client.get_top_tracks(user, limit, period)
        tiles = self._create_tiles_from_top_items(top_tracks,
                                                  entity=self.ENTITY_TRACK)
        return tiles

    def _create_image(self, tiles: List[CollageTile], cols: int,
                      rows: int) -> Image:
        """
        ...
        for tile in tiles:
            ...
            self._insert_tile_title(new_image,
                                    f"{tile.title}. ({tile.playcount})",
                                    cursor)
    ...
    @classmethod
    def _create_tile_from_top_item(cls, top_item: TopItem,
                                   entity: str) -> CollageTile:
    ...
    def _insert_tile_title(self, image: Image, title: str,
    ...
    def _get_artist_image(cls, artist: Artist) -> bytes:
        ...
        if soup.find(class_="header-new-background-image"):
            url = str(
                soup.find(class_="header-new-background-image").get("content"))
        if not url:
            img = cls._generate_blank_tile()
        else:
    ...
    def _validate_parameters(self, entity: str, rows: int, cols: int,
                             period: str):
