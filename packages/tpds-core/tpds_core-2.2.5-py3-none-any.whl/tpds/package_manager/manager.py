...
from typing import Any, Sequence, MutableMapping, Union, Iterator
...
class PackageManager():
    ...
    __shared_state: dict[str, Any] = {}
    __licenses = {
        'Proprietary': open(os.path.join(os.path.dirname(__file__), 'licenses', 'mchp.txt'), 'r').read()
    }
    ...
    def __init__(self, processor: Any = None) -> None:
        if not self.__shared_state:
            ...
            self._clients['pypi'] = PipPackageClient(self._manifest.get_by_source('pypi'), 
        ...
        for pn, pd in self._manifest.packages.items():
            if pn in self._packages:
                ...
                if 'conda' in pd['source']:
                    ...
                    chan = 'pypi'
                self._packages[pn] = PackageDetails(name=pn, description=pd['description'], channel=chan)

    def _refresh_package_list(self, _client, local: bool = True, remote: bool = True, **kwargs: Any) -> None:
        if local:
        ...
        for avail in _client.get_available_packages():
            if avail.license_text is None:
                avail.license_text = self.__licenses.get(avail.license if avail.license else 'Proprietary', None)
    ...
    def get_upgradable(self) -> Sequence[PackageDetails]:
        return list(filter(lambda p: p.installed and p.latest and p.installed < p.latest, self._packages.values()))
    ...
    def get_packages_formatted(self) -> str:
        return tabulate(TableIterator(self._packages.values(), ['name', ('channel', prettify_channel), 'installed', 'latest']), headers=self.__headers)
...
def package_manager_main():
    ...
    parser = argparse.ArgumentParser(description='Package Manager for the Trust Platform Development Suite')
