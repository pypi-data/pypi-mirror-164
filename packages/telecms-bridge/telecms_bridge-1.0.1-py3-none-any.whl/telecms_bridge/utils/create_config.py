...
from telecms_bridge import settings
from telecms_bridge_base.db import build_engine_url
...
def create_config(config_not_set):
    ...
    if 'token' not in settings.USE_DEFAULT_CONFIG:
        token = prompt(
            promt_message('<green><b>Enter your Service Exchange token</b></green>'),
    ...
    if 'address' not in settings.USE_DEFAULT_CONFIG:
        address = prompt(
            promt_message('<green><b>Which host to run Service Exchange on?</b></green>\n<i>Default is {}</i>'.format('0.0.0.0 (any IP)')),
    ...
    port = prompt(
        promt_message('<green><b>Which port to run Service Exchange on?</b></green>\n<i>Default is {}</i>'.format('8888')),
    ...
    print_formatted_text('')
    print_formatted_text(HTML('<skyblue>You can now start <b>Service Exchange</b></skyblue>'))
    print_formatted_text(HTML('<skyblue><b>===========================================</b></skyblue>\n'))
