...
def _get_google_auth_token(url='https://sample-metadata-api-mnrpw3mdza-ts.a.run.app/') -> str:
    ...
    # try to see if we're on a VM
    google_identity_message = ''
    # use "format=full" here to ensure email is included in the response, by default it is not!
    gurl = f'http://metadata.google.internal/computeMetadata/v1/instance/service-accounts/default/identity?format=full&audience={url}'
    try:
        ...
        # Not on a VM, might be useful for later
        google_identity_message = str(e)
    ...
    try:
        ...
        # mfranklin: I noticed this when running on a GCP VM, I don't really
        #       know why this happens, but I assume this approach only works
        #       for user-specific identities, not service-accounts.
        raise RuntimeError(
            'There was an error when getting an identity token, if you are '
            'using a service-account, you should set the '
            'GOOGLE_APPLICATION_CREDENTIALS environment variable to the path to your '
            'credentials file, as the call to metadata.google.internal failed with '
            f'{google_identity_message}.'
        )
