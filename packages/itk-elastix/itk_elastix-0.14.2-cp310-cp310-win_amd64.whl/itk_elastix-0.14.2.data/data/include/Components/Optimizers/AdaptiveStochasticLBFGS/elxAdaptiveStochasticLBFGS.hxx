...
AdaptiveStochasticLBFGS<TElastix>::BeforeEachResolution()
{
  ...
  this->GetConfiguration()->ReadParameter(memory, "LBFGSMemory", this->GetComponentLabel(), level, 0);
  this->m_LBFGSMemory = memory;

  /** Set the updateFrequenceL. */
  SizeValueType updateFrequenceL = 5;
  this->GetConfiguration()->ReadParameter(updateFrequenceL, "UpdateFrequenceL", this->GetComponentLabel(), level, 0);
  ...
  double beta = 20.0;
  this->GetConfiguration()->ReadParameter(beta, "SP_beta", this->GetComponentLabel(), level, 0);
  ...
  this->GetConfiguration()->ReadParameter(
    useAdaptiveLBFGSStepSizes, "UseAdaptiveLBFGSStepSizes", this->GetComponentLabel(), level, 0);
  ...
  this->GetConfiguration()->ReadParameter(
    this->m_AutomaticLBFGSStepsizeEstimation, "AutomaticLBFGSstepsizeEstimation", this->GetComponentLabel(), level, 0);

  /** Set the scale of the windowa for H0. */
  double windowScale = 5;
  this->m_Configuration->ReadParameter(windowScale, "WindowScale", this->GetComponentLabel(), level, 0);
...
AdaptiveStochasticLBFGS<TElastix>::ResumeOptimization()
{
  ...
  this->GetConfiguration()->ReadParameter(curvatureSamplerType, "CurvatureSampler", this->GetComponentLabel(), 0, 0);
  ...
  while (!this->m_Stop)
  {
    ...
    else if (m_CurrentIteration < m_NumberOfIterations)
    {
      ...
      if (this->GetAutomaticLBFGSStepsizeEstimation() && (this->m_CurrentIteration + 0) % this->m_UpdateFrequenceL == 0)
      //&& (this->m_CurrentIteration  == this->m_UpdateFrequenceL * 2) )
      {
        // This takes a significant amount of time!!
        itk::TimeProbe timer_ape;
        timer_ape.Start();
        ...
        this->GetConfiguration()->ReadParameter(useSearchDirForAdaptiveStepSize,
                                                "UseSearchDirForAdaptiveStepSize",
    ...
    if ((this->m_CurrentIteration + 1) % this->m_UpdateFrequenceL == 0)
    {
      ...
      for (unsigned int m = 0; m < M; ++m)
      {
        ...
        this->GetElastix()->GetElxMetricBase(m)->SetAdvancedMetricImageSampler(curvatureSamplers[m]);
      }

      /** Get current and previous curvature gradients. */
      itk::TimeProbe timer_gvad;
      timer_gvad.Start();
      ...
      this->m_CurrentT++;
      if (this->m_CurrentT >= this->m_LBFGSMemory)
      {
        this->m_CurrentT = 0;
      }

      /** Update the bound. */
      if (this->m_Bound < this->m_LBFGSMemory)
      {
        this->m_Bound++;
      }
...
AdaptiveStochasticLBFGS<TElastix>::AutomaticLBFGSStepsizeEstimation()
{
  ...
  computeDisplacementDistribution->ComputeUsingSearchDirection(
    this->m_SearchDir, jacg, maxJJ, maximumDisplacementEstimationMethod);
