...
def _process_doc(
    ...
    if p.returncode != 0:
        ...
        with open(doc_file_no_extension + "_05a_floatingText.xml", "wb") as f:
            f.write(out)
            logger.info(
                "Wrote {}".format(
                    os.path.basename(doc_file_no_extension + "_05a_floatingText.xml")
                )
            )
            transient_files.append(doc_file_no_extension + "_05a_floatingText.xml")
    ...
    cli_args = [
        ...
        "-jar",
        SAXON_PATH,
        doc_file_no_extension + "_05a_floatingText.xml",
    ...
    if p.returncode != 0:
        ...
        with open(doc_file_no_extension + "_08_tei_openedition.xml", "wb") as f:
            f.write(out)
            logger.info(
                "Wrote {}".format(
                    os.path.basename(doc_file_no_extension + "_08_tei_openedition.xml")
                )
            )
    ...
    cli_args = [
        ...
        doc_file_no_extension + "_08_tei_openedition.xml",
        "--noout",
    ]
    logger.debug(" ".join(cli_args))
    p = Popen(
    ...
    if p.returncode != 0:
        ...
        logger.info(
            "Validate {}".format(
                os.path.basename(doc_file_no_extension + "_08_tei_openedition.xml validates")
            )
        )
