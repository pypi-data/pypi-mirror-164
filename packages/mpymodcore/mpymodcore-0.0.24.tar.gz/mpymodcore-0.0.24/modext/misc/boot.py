...
def start_auto_config(tz_handler_cls=None, enable_sd=True):
...
def loop_async(config=None, debug=True):
...
def run_loop(config=None, debug=True):
    print("deprecated: use loop_async")
    return loop_async(config=config, debug=debug)
...
def print_main_info():
    ...
    print()
    print("call loop_async() - run in async mode ")
    print()
