...
class LineageClient:
    ...
    async def update_entity(
        ...
        Summary:
            ...
            Note this function will only apply to file_data type which is ONLY one in used
        ...
        Parameter:
            ...
            - namespace(str): the namespace be longs to greenroom or core
        ...
        attributes = FileDataAttribute(_id, file_name, path, size, owner, namespace, project_code)
        entity = Entity('file_data', attributes, owner)
    ...
    async def delete_entity(self, _id: str, entity_type: str = 'file_data'):
        """
        Summary:
        ...
        async with httpx.AsyncClient(verify=False) as client:
            response = await client.delete(
                ...
                params={'attr:global_entity_id': _id},
                auth=(self.username, self.password),
                timeout=60,
            )
    ...
    async def get_lineage(self, _id: str, direction: str = 'INPUT', entity_type: str = 'file_data') -> dict:
        """
        Summary:
        ...
        async with httpx.AsyncClient(verify=False) as client:
            response = await client.get(
                ...
                params={'attr:global_entity_id': _id, 'depth': 50, 'direction': direction},
                auth=(self.username, self.password),
                timeout=60,
            )
    ...
    async def create_lineage(
        ...
        lineage_attirbute = LineageAttirbute(name, 'file_data', input_id, 'file_data', output_id, description)
