...
try:  # 加载默认设置
    from zmirror.config_default import *
...
# 
# 覆盖原先代码中的config.py文件导入的变量（因为这这设计看起来不合理）
my_host_scheme = os.environ.get("MTSM_HOST_SCHEME","http://")
my_host_name = os.environ.get("MTSM_HOST_NAME",'127.0.0.1')
my_host_port = os.environ.get("MTSM_HOST_PORT",8080)
target_domain = os.environ.get("MTSM_TARGET_DOMAIN","archive.org")
target_scheme = os.environ.get("MTSM_TARGET_SCHEME","https://")
# 这里面大部分域名都是通过 `enable_automatic_domains_whitelist` 自动采集的, 我只是把它们复制黏贴到了这里
# 实际镜像一个新的站时, 手动只需要添加很少的几个域名就可以了.
# 自动采集(如果开启的话)会不断告诉你新域名
external_domains = ( 'analytics.archive.org', 'web.archive.org',)
# 强制所有 archive.org 站点使用HTTPS
force_https_domains = 'ALL'
# 自动动态添加域名
enable_automatic_domains_whitelist = True
domains_whitelist_auto_add_glob_list = ('*.archive.org',)
# 完成config.py 变量覆盖，不过，以后需要继续改进。
#########################################################
