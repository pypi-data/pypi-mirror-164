...
def regression(y_true, y_pred):
    assert isinstance(y_true, (pandas.DataFrame, pandas.Series))
    assert isinstance(y_pred, (pandas.DataFrame, pandas.Series))

    if isinstance(y_true, pandas.Series):
        # [for pandas.Series]: Scalar regression
        return ScalarRegression.evaluation(y_true, y_pred, y_prob)
    else:
        # [for pandas.DataFrame]: Scalar regression
        if y_true.shape[1] == 1:
            return ScalarRegression.evaluation(y_true, y_pred, y_prob)
        else:
            return VectorRegression.evaluation(y_true, y_pred, y_prob)
