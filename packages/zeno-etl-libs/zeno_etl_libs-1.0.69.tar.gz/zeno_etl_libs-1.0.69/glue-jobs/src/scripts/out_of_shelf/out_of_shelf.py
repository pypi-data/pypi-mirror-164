...
def main(debug_mode, as_inactive_exclude_types, mature_delta_days, db,
    ...
    try:
        ...
        out_of_shelf_group = out_of_shelf_group[
            ['store_id', 'store_name', 'drug_id', 'drug_name', 'type', 'bucket',
             'drug_grade', 'closing_date', 'max_set', 'mature_flag',
             'customer_type', 'segment_calculation_date', 'oos_count',
             'drug_count', 'oos_min_count', 'oos_min_drug_count',
             'oos_std_qty_count', 'oos_std_qty_drug_count',
             'oos_sales_loss_30d', 'sales_30d']]
        ...
        if debug_mode == 'N':
            ...
            table_info = helper.get_table_info(db=rs_db_write,
                                               ...
                                               schema=write_schema)
            columns = list(table_info['column_name'])
            out_of_shelf_drug_group = out_of_shelf_drug_group[columns]  # required column order
        ...
        df_max_set = out_of_shelf_drug_group.loc[
            out_of_shelf_drug_group["max-set"] == 'Y']
...
def get_inventory(store_id, db, read_schema):
    q_inventory = """
...
def get_past_30d_sales(store_id, db, run_date, read_schema):
...
def get_buckets(store_id, db, read_schema):
    q_latest_buckets = f"""
    select "store-id" , "drug-id" , bucket 
    from "{read_schema}"."ipc2-segmentation" is2 
    where "store-id" = {store_id}
    and "reset-date" = (select max("reset-date") 
        from "{read_schema}"."ipc2-segmentation" where "store-id" = {store_id})
    """
...
def get_customer_segment(db, read_schema, interval=90):
    q_customer_seg = """
...
def get_drug_list(store_id, db, read_schema, max_set='Y', drugs=None):
    if max_set == 'Y':
        max_condition = "max > 0"
        drugs_condition = ""
...
def mature_drugs_flag(store_id, run_date, mature_delta_start_date,
                      mature_delta_days, db, read_schema):
    q_mature_days = """
            select "store-id" , "drug-id" , sum("drug-count") as mature_days
            from "{read_schema}"."out-of-shelf-drug-level" oosdl 
            ...
            """.format(run_date.strftime("%Y-%m-%d"),
                       ...
                       store_id, read_schema=read_schema)
...
if __name__ == '__main__':
    ...
    oos_generic, oos_others = main(
        debug_mode, as_inactive_exclude_types, mature_delta_days, rs_db,
        rs_db_write, read_schema, write_schema, logger)
