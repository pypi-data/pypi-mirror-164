...
barcoded_ga = pd.read_sql(q_8,cnxn)
...
non_barcoded_ga = pd.read_sql(q_9,cnxn)



# Concatenating the barcoded, non barcoded and wh_returns

warehouse_all=pd.concat([barcoded,non_barcoded,wh_returns,barcoded_ga,non_barcoded_ga],sort=False,ignore_index=False)
...
warehouse_merge=warehouse_merge.groupby(['entity-id', 'entity-name', 'entity-type','inventory-type',
