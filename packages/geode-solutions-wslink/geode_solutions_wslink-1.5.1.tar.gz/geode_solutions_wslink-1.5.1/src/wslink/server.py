...
import asyncio
import logging
from python.src.wslink.ssl_context import *
...
def add_arguments(parser):
    ...
    parser.add_argument(
        ...
        default="",
        dest="ssl",
        help="add a tuple file (certificate, key) or adhoc string",
    )
...
def start_webserver(
    ...
    if len(options.ssl) > 0:
        ...
        if type(options.ssl) is tuple:
            tokens = options.ssl.split(",")
            if len(tokens) != 2:
                raise Exception(f"ssl configure must be \"adhoc\" or a tuple of files \"cert,key\"")
