...
class Retrievable(Resource):
    ...
    @classmethod
    def retrieve_keys(cls, id: str, env: str) -> Resource:
        if cls._resource != "organizations":
            raise Exception("Resource not allowed")
        """Retrieve a resource given its ID
        ...
        """
        response = client.get(f'/{cls._resource}/{id}/apikeys/{env}')
        return response
...
class Updatable(Resource):
    ...
    @classmethod
    def _update_certs(cls, id: str, files:Any , data) -> Resource:
        """Update an specific resource with new data.
        ...
        """
        response = client.certs(f'/{cls._resource}/{id}/certificate', data, files)
        return cls._from_dict(response)
