...
class Client:
    ...
    def certs(self, endpoint: str, data: Optional[Dict[str, Any]], files: Any):
        headers = {
        'Authorization': 'Bearer '+self.api_key
        }
       
        response = requests.request("PUT",  url=('https://' + self.host + urljoin('/', endpoint)), headers=headers, data=data, files=files)
