...
class Config(object):
    ...
    def validate(self): # type: () -> None
        ...
        if all([s.pollers for s in self._sections.values()]) and \
                (self.globals.busypoll or self.globals.busyread):
            self.globals.busypoll = None
            self.globals.busyread = None
            _printhead("when using pollers exclusively, " \
                "busypoll and/or busyread are not applicable - " \
                "ignoring these values", 93)
