    ...
    def menu_get_entries(self, prefilled:dict=None) -> dict:
        ...
        if self.menu_result != None:
                ...
                if result_count > 0:    
                    ...
                    entries[SAPAbapODataConnector.MENU_RESULT_PANDAS] = f"Get current result as pandas dataframe (entity {self.menu_entity}, endpoint {self.endpoint})"

        # pandas support
        if self.menu_pandas_df != None:
            entries[SAPAbapODataConnector.MENU_PANDAS_MENU] = f"Open menu for current pandas dataframe {self.menu_pandas_df.get_name()}"
    ...
    def menu_process_selection(self, selected:str, text:str=None):
        try:
            if selected == SAPAbapODataConnector.MENU_FIND_ENDPOINTS:
                ...
                self.menu_pandas_df = PandasResultDataframe(df, self.menu_entity)                           
                print(df)
