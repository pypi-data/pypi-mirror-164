...
from droppy.numerics import ContourGenerator



class ContourWeSpeed( ContourGenerator ):
    def __init__(self, speed , w_range = np.arange(0.2 , 1.8, 0.01), side = 2 ):
        ...
        self.speed = speed
        ContourGenerator.__init__(self , lambda w , b : np.abs(sp.w2we( w, b, speed = speed )), x , y )

    def plot( self, values = None, **kwargs ) :
        if values is None :
            values = np.arange( 0.1 , sp.w2we( np.max(self.x) , b = np.pi, speed =self.speed) , 0.1 )
        ax = ContourGenerator.plot(self, values = values, **kwargs)
        ax.set(xlabel = "Frequency (rad/s)" , ylabel = "Heading (rad)" )
        return ax

class ContourWeW( ContourGenerator ):
    def __init__(self, w , speed_range, side = 2):
        x = speed_range
        y = np.linspace( 0. , side*np.pi , side*200 )
        ContourGenerator.__init__(self , lambda speed, b : np.abs(sp.w2we( w, b, speed = speed )), x , y )
...
class ContourWeW_t( ContourGenerator ):
    def __init__(self, w , speed_range, to_ms = 1.0, side = 2):
        x = np.linspace( 0. , side * np.pi , side * 200 )
        y = speed_range
        ContourGenerator.__init__(self , lambda b, speed : np.abs(sp.w2we( w, b, speed = speed*to_ms )), x , y )
