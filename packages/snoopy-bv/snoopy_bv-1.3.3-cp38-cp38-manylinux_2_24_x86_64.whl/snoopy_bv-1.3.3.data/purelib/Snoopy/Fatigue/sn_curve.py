...
class SnCurve(object):
    ...
    def damage_from_distribution(self, rangeDist, rtz, duration):
        ...
        for iSlope in range(self.nbSlope) :
            ...
            m = self.df.m[iSlope]
            k = self.df.K[iSlope]
            dam += quad( lambda x : rangeDist.pdf(x) * x**m / k , s_min, s_max  )[0] * duration / rtz
        return dam


    def fatigueLife_from_distribution(self, rangeDist, rtz ) :
        """Return fatigue life on the sea-state (in year)
        """
        return 1. / self.damage_from_distribution( rangeDist, rtz , 365.24*24*3600)
