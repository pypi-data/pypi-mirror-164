...
class DateParser(DateData):
    ...
    def date_split(self, DATE):
        ...
        for i in split_date:
            if re.match( self['match sep'], str(i), flags = re.IGNORECASE ):
                self.data[c] = str(i)
                self.alldata[c] = self.data[c]
            else:
                self.alldata[c] = str(i)
                self.separators.append(str(i))

            c += 1

        return ( self.data, self.alldata )
    ...
    def parse_date(self, str_, list_all = False):
        """
        ...
        for i in self['format codes']['full']:
            ...
            if re.match( i[0], str_ ):
                ...
                self.isValid = True
                self.alldata = [ str_ ]
                self.dateObject = dt.strptime( str_, i[1] )
                self.formatting = i[1]
        ...
        for i in self.data:
            num = chk_spelled(self['spelled'], self.data[i])
        ...
        for i in self.data:
            ...
            if re.match( '^[0-9]+$', DN ):
                ...
                for F in self['format codes']['single']:
                    if re.match( F[0], self.data[i], flags = re.IGNORECASE ):
                        log.debug(f"Found a match for {self.data[i]} - adding {F[1]} to list of possibilities")
        ...
        log.debug(f"self.data = {str(self.data)}")
        for i in combos:
            ...
            alldata = []
            chk_str = []
            for c in range( 0, len( self.alldata )):
                if c in self.data:
                    fmt.append( str(i[count]) )
                    count += 1

                    if fmt[c] in ('%d', '%m', '%H', '%I', '%M', '%S') and re.match( '^[0-9]{1}$', str(self.alldata[c]) ):
                        log.debug(f"Changing date/time data to 2 digits instead of one - ({fmt[c]}) ({self.alldata[c]})")
                        alldata.append( '0' + str(self.alldata[c]) )

                    elif self.alldata[c].lower() in ('am,', 'pm'):
                        log.debug(f"Changing am/pm data to uppercase")
                        alldata.append( self.alldata[c].upper() )

                    else:
                        alldata.append( str(self.alldata[c]) )

                else:
                    fmt.append( str(self.alldata[c]) )
                    alldata.append( str(self.alldata[c]) )
            ...
            if self.try_fmt( ''.join( alldata ), formatting ):
                ...
                if DObj.strftime( formatting ) == ''.join(alldata):
                    ...
                    if list_all:
                        self.format_list.append(formatting)
                        continue

                    self.alldata = alldata
                    self.dateObject = DObj
        ...
        if list_all:
            ...
            self.formatting = self.format_list[0]
