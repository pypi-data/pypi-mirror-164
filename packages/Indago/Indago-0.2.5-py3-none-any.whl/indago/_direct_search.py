...
#                    self.cS[0].X = self.X0[0]
        ...
        n = self.params['n']
        self.X = np.zeros([self.dimensions, n])
        for i in range(self.dimensions):
            self.X[i, :] = np.linspace(self.lb[i], self.ub[i], n)
                
        # Generate array of candidates
        self.CN = np.array([CandidateState(self) for _ in range(self.dimensions)], \
                            dtype=CandidateState)
        self.CP = np.array([CandidateState(self) for _ in range(self.dimensions)], \
                            dtype=CandidateState)
            
        #self.C[0].X = self.X0
        #self.x = self.C[0]
        #self.collective_evaluation(self.Y[:1])
        self.H = []
        ...
        self.x_last = CandidateState(self)
    
        for i in range(self.dimensions):
            self.x_next.X[i] = self.X[i, self.y_next[i]]
        self.collective_evaluation([self.x_next])
        self.x_last = self.x_next.copy()
        self.H.append(list(self.y_next))
    ...
    def _run(self):
        ...
        for self.it in range(self.iterations):
            ...
            if find_dir:
                ...
                C = np.append(self.CN, self.CP)
                self.collective_evaluation(C)
                # print(self.x_next)
                #print(np.min(C))
                x_best = np.argmin(C)
                
                # print('old', self.x_next.X, self.x_next.f)
                # for i in range(d):
                #     print(f'{i}-', self.CN[i].X, self.CN[i].f)
                #     print(f'{i}+', self.CP[i].X, self.CP[i].f)
                ...
                for i in range(d):                    
                    ...
                    #L = np.argsort([c.f for c in self.CN])
                    #self.dy[L[:5]] = -2
                    # print(self.CN[L[0]].f, self.CN[L[1]].f)
                    #L = np.argsort([c.f for c in self.CP])
                    #self.dy[L[:5]] = 2
            ...
            # print(self.it, self.dy, self.y_next)
            if np.sum(np.abs(self.dy)) == 0:
                print('Converged to local optima')
                break
            ...
            for i in range(d):
                self.x_next.X[i] = self.X[i, self.y_next[i]]
            self.collective_evaluation([self.x_next])
            # print('new', self.x_next.X, self.x_next.f)
            # input(' > Press return to continue.')
            find_dir = self.x_next >= self.x_last
            
            y_best = np.zeros(d, dtype=int)
            for i in range(d):
                y_best[i] = np.argmin(np.abs(self.best.X[i] - self.X[i, :]))
                    
            if list(self.y_next) in self.H:
                # print(list(self.y_next))
                # print(self.H)
                # print('Loop convergence')
                # input('> Press return to continue.')  
                #break
                    
                if np.sum(self.y_next - y_best) == 0 or list(y_best) in self.H and find_dir:
                    print('Finished')
                    # input('> Press return to continue.')    
                    break            
                else:
