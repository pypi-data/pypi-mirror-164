...
def generate_feature_weight2(
    ...
    multipoly = _quadrat_cut_geometry(
        npoly.loc[row_index, "geometry"], quadrat_width=quadrat_width
    )
    geoms = set()
    ...
    for spindex, poly in enumerate(multipoly.geoms):
        ...
        if possible_matches_index:
            ...
            print("no intersection: ", row_index, spindex, poly, flush=True)
    i_index = []
    j_index = []
    p_index = []
    wghts = []
    if len(precise_matches) != 0:
        ...
        res_intersection = npoly.loc[[row_index]].overlay(
            grid_cells.iloc[list(geoms)], how="intersection"
        )
