...
class BaseCodeConverter:
    ...
    def convert(self, script_file_path: str | Path) -> tuple[str, list[str]]:
        ...
        # --------------------------------------------------------------------
        basecode_dpath = CONST.Path.DirPath.BaseCode
        for prefix in [f'{i}' for i in script_fpath.parts[script_fpath.parts.index(CONST.Path.ACTUALLY_CWD.parts[-1])+1:]][:-1]:
    ...
    def _parsing_script_and_write_codeline_file(self, script_fpath: Path, basecode_fpath: Path) -> None:
        ...
        if not self._temp_case_is_assert:
            error_message = CONST.Error.Interpreter.GrammarUseCaseNoAssertion.replace('#CASE_NAME#', self._temp_case_name)
    ...
    def _parse_currently_statement(self) -> None:
        ...
        # 根据符号执行对应的解析逻辑
            ...
            try:
                ...
                error_message = CONST.Error.Interpreter.GrammarSymbolWrongful.replace('#SYMBOL#', symbol)
                raise AwakenAnalysisBaseCodeError(error_message)
