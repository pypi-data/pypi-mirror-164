...
class DFSBase(object):
    ...
    def __init__(self, file_name):
        ...
        if hllDll is None:
            ...
            os.chdir(mz)    
            hllDll = WinDLL(os.path.join(mz, "ufs.dll"))
    ...
    def from_template_file(cls, file_name, TemplateFileName):
        """
        Creates a new file copying all info from the Templatefile except the data and the timesteps.
        """
        TemplateFile = DFSBase.open_file(TemplateFileName)
        cls = TemplateFile.new_file(file_name, TemplateFile.number_of_items)
        cls._copy_item_info(TemplateFile)
        cls.__copy_from_template(TemplateFile)
    ...
    def __copy_from_template(self, dfs):
        """
        ...
        self.number_of_columns = dfs.number_of_columns;
        self.number_of_rows = dfs.number_of_rows;
        self.number_of_layers = dfs.number_of_layers
        self.dx = dfs.dx
        self.dy = dfs.dy
        self.x_origin = dfs.x_origin
        self.y_origin = dfs.y_origin
        self.orientation = dfs.orientation
    ...
    def write_item_timestep(self, TimeStep, Item, data):
        ...
        if self.time_axis is TimeAxisType.CalendarNonEquidistant and self._currentTimeStep > 0 :
            ...
            ts = self.time_step_size * TimeStep

        if self.time_step_unit is TimeInterval.Second:
            time = ts.total_seconds()
        elif self.time_step_unit is TimeInterval.Minute:
            time = ts.total_seconds()/60
        elif self.time_step_unit is TimeInterval.Hour:
            time = ts.total_seconds()/3600
...
class DFS0(DFSBase):
    ...
    def from_file(cls, file_name):
        """
        Deprecated. Use open_file(file_name) instead
    ...
    def get_data(self, TimeStep, Item):
        ...
        if Item in self._cache:
            currentts = self._cache[Item]
            if TimeStep in currentts:
                return self._cache[Item][TimeStep] #We have already read this item
        else:
            currentts = {}
            self._cache[Item]=currentts
        data = super(DFS0, self).read_item_time_step(TimeStep, Item)[0]
        if self.use_cache:
            currentts[TimeStep]=data
        return data
        ...
        if not self.__data_read:
            for t in range(0, self.number_of_time_steps):
                for inumber in range(1, self.number_of_items +1):
                    self.get_data(t,inumber)
            self.__data_read = True
        return list(self._cache[Item].values())
    ...
    def get_all_data(self):
        """
        Gets all data as a Pandas series
        """
         # initialize dataframe
        dat = pd.DataFrame(index=self.time_steps, columns=[it.name for it in self.items])

        # read item by item
        for i, it in enumerate(self.items):
            dat[it.name] = self.get_timeseries(i+1)

        # set delete value to np.nan
        dat[dat==self.delete_value] = np.nan
        return dat
...
class DFS2(DFSBase):
    ...
    def get_data(self, TimeStep, Item):
        """
        Gets the data for a TimeStep and Item as masked numpy array.
        Uses cache if use_cache is set to true
        """
        if Item in self._cache:
            currentts = self._cache[Item]
            if TimeStep in currentts:
            ...
            currentts = {}
            self._cache[Item]=currentts
        data = ma.array(self.read_item_time_step(TimeStep, Item), mask=self.compression_array,fill_value= self.delete_value)
        if self.use_cache:
            currentts[TimeStep]=data
                
        return data

    def get_values(self, Item, row, column):
        """
        Gets all data for a particular item. Reads through the entire file at the first call
        """
        toreturn =[]
        for t in range(0, self.number_of_time_steps):
            toreturn.append(self.get_data(t, Item)[row, column])
        return toreturn


    def get_timeseries(self, Item, row, column):
        """
        Gets all data for a particular item and cell as a Pandas series
        """
        return pd.Series(self.get_values(Item, row, column), index=self.time_steps)
...
class DFS3(DFSBase):
    ...
    def get_data(self, TimeStep, Item):
        ...
        if Item in self._cache:
            currentts = self._cache[Item]
            if TimeStep in currentts:
                return currentts[TimeStep] #We have already read this item
        else:
            currentts = {}
            self._cache[Item]=currentts
        data=None
        ...
        if self.use_cache:
            currentts[TimeStep]=toreturn
                
        return toreturn

    def get_values(self, Item, row, column, Layer):
        """
        Gets all data for a particular item. Reads through the entire file at the first call
        """
        toreturn =[]
        for t in range(0, self.number_of_time_steps):
            toreturn.append(self.get_data(t, Item)[Layer][row, column])
        return toreturn


    def get_timeseries(self, Item, row, column, Layer):
        """
        Gets all data for a particular item and cell as a Pandas series
        """
        return pd.Series(self.get_values(Item, row, column, Layer), index=self.time_steps)
