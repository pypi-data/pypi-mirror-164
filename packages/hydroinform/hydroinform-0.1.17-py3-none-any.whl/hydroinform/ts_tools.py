...
#        Q50 =self.time_series.median()
    ...
    def durmedmin(self):
        ...
        return self.Duration(self.time_series, val)
...
def qh_function(H, A, H0, N):
    return A *  max(0, H-H0)^N


def read_sensorthings(url):
    """
    Reads a pandas time series from a VandWeb URL
    """
    jData = read_json_data(url)
    l=list(map(lambda s:s['Result'], jData['value']))
    d=list(map(lambda s:pd.Timestamp(s['ResultTime']), jData['value']))
    return pd.Series(l, d)

def read_hydroinform_ts(sensorid, start=datetime(2020,1,1), end=datetime.now(), zoomlevel=3):
    """
    Returns a time series from hydroinform
    """
    url = r'http://hydroinform.cloudapp.net/api/TimeSeries/' + str(sensorid) + '?start=' + str(start) + '&end=' + str(end) + '&zoomlevel=' + str(zoomlevel) 
    jdata = read_json_data(url)
    l=list(map(lambda s:s['Value'], jdata))
    d=list(map(lambda s:pd.Timestamp(s['Time']), jdata))
    return pd.Series(l, d)
