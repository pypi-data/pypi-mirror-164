...
from ..capi import (DataEngine, Modelling, License, Selection, Viewer, Vulcan,
                    DrillholeModel)
...
from ..capi.scan import Scan as ScanDll
...
from ..internal.util import to_utf8, default_type_error_message
...
ObjectIdT = typing.TypeVar("ObjectIdT")
...
class Project:
  ...
  def __init__(
    ...
    self.__types_for_data: list[tuple[int, type]] = [
      (7, Scan),
  ...
  def __load_dlls_with_dataengine_types(self) -> None:
    ...
    try:
      drillhole_version = DrillholeModel().version
  ...
  @contextmanager
  def read(self, path_or_id: str | ObjectID[ObjectIdT]
  ...
  @contextmanager
  def edit(self, path_or_id: str | ObjectID[ObjectIdT]
      ) -> Generator[DataObject | ObjectIdT, None, None]:
  ...
  def copy_object(
    ...
    if isinstance(object_to_clone, str):
      ...
      # :TODO: SDK-747. This would be more readable if it was something
      # like: 'if object_to_clone.is_closed'
      ...
      if (object_to_clone.lock_type is LockType.READWRITE
          and not object_to_clone._lock.is_closed):
  ...
  def get_selected(self) -> list[ObjectID[DataObject]]:
