...
class FieldInformation(BaseField):
  ...
  def description(self, value: str):
    ...
    if self._read_only:
      raise ReadOnlyError("Cannot edit description on read-only field.")
  ...
  def unit(self, value: DistanceUnit | AngleUnit | NO_UNIT):
    if self.__deleted:
      raise DeletedFieldError(self)
    if self._read_only:
      raise ReadOnlyError("Cannot edit unit in read-only mode.")
  ...
  def delete(self):
    ...
    if self.__deleted:
      return
    if self._read_only:
      raise ReadOnlyError("Cannot delete field in read-only mode.")
