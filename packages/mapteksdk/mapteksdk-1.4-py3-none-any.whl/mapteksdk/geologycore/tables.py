...
class BaseTable:
  ...
  @property
  def _read_only(self):
    """True if the object is open for read-only."""
    # pylint: disable=protected-access
    return self._parent.lock_type is LockType.READ
...
class BaseTableInformation(BaseTable):
  ...
  def add_field(
    ...
    if self._read_only:
      raise ReadOnlyError(
        "Cannot add fields to database not open for editing.")
...
class BaseDrillholeTable(BaseTable):
  ...
  def add_rows(self, count: int=1, *, index: int | None=None):
    ...
    """
    if self._read_only:
      raise ReadOnlyError("Cannot add rows in read-only mode.")
    if index is None:
  ...
  def remove_rows(self, index: int, count: int):
    ...
    """
    if self._read_only:
      raise ReadOnlyError("Cannot remove rows in read-only mode.")
    index = int(index)
    count = int(count)
  ...
  def dataframe(self, *,
    ...
    """
    if save_changes and self._read_only:
      raise ReadOnlyError("Cannot edit dataframe of read-only drillhole.")
    if fields is None:
      fields = self.fields
    ...
    try:
      ...
      if save_changes and not self._read_only:
        LOG.info(
  ...
  def _put_pandas(self, frame: pd.DataFrame):
    ...
    """
    # :TODO: SDK-679: Implement saving a pandas dataframe.
    raise ValueError(
      "Saving changes made by a pandas dataframe is not yet implemented.")
