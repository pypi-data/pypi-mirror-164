...
class ObjectID(typing.Generic[T]):
  ...
  def _from_string(cls, oid_string):
    ...
    if isinstance(oid_string, str):
      ...
      return cls(obj)
    raise TypeError(
      f"Incorrect type provided for oid_string: '{type(oid_string).__name__}'.")
  ...
  def from_path(cls, object_path):
    ...
    raise TypeError(
      "Incorrect type provided for object_path. "
      f"Expected {str.__name__}, got {type(object_path).__name__}.")
