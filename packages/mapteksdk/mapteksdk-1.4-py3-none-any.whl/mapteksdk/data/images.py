...
class Raster(DataObject):
  ...
  image : string, pathlib.Path or PIL.Image.Image
    ...
    if self.__is_new:
      if image is not None:
        ...
        if isinstance(image, Image.Image):
          self.__construct_from_pillow_image(image)
        else:
          self.__construct_from_image_path(image)
      else:
        self.__construct_from_width_and_height(width, height)
      try:
  ...
  def __construct_from_width_and_height(self, width, height):
  ...
  def __construct_from_image_path(self, image_path):
  ...
  def __construct_from_pillow_image(self, image):
    ...
    self.__pixels = pixels.reshape(-1, 4)
