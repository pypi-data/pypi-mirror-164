...
class StringColourMap(DataObject):
  ...
  ...    # Get the colour associated with the key "Gold".
  ...    # (Will raise a KeyError if "Gold" is not part of the map.)
  ...    _ = read_map["Gold"]
  ...    # Determine if a key is part of a map.
  ...    _ = "Stone" in read_map
  ...    # Delete a key from the colour map.
  ...    del read_map["Iron"]
  ...
  def __getitem__(self, key):
    ...
    if not isinstance(key, str):
      raise TypeError(default_type_error_message(
        "colour map key", key, str
      ))
    indices = np.where(self.legend == key)[0]
    if indices.size == 0:
  ...
  def __setitem__(self, key, value):
    ...
    value = convert_to_rgba(value)
    indices = np.where(self.legend == key)[0]
    if indices.size == 0:
      # Bypass the setter because the arrays should be the correct shape.
      self.__legend = np.append(self.legend, key)
      self.__colours = np.row_stack((self.colours, value))
    else:
      index = indices[0]
  ...
  def __delitem__(self, key):
    ...
    indices = np.where(self.legend == key)[0]
    if indices.size == 0:
      raise KeyError(f"Cannot delete nonexistent key: {key}")
    index = indices[0]
    # Bypass the setter because the arrays should be the correct shape.
    self.__legend = np.delete(self.__legend, index)
    self.__colours = np.delete(self.__colours, index, axis=0)

  def __contains__(self, key):
    self._raise_if_invalid()
    # Ensure the key is the right type before calling in on the legend.
    if not isinstance(key, str):
      return False
    return key in self.legend
  ...
  def legend(self, legend):
    if legend is None:
      self.__legend = np.full((0,), "", dtype=np.str_)
    else:
  ...
  def colours(self, colours):
    if colours is None:
      self.__colours = np.zeros((self.intervals, 4), dtype=np.uint8)
      self.__colours[:, 1] = 255
      self.__colours[:, 3] = 255
    else:
      self.__colours = convert_array_to_rgba(colours, len(colours))
