...
class Scan(Topology, PointProperties, CellProperties, RotationMixin):
  ...
  def point_count(self):
    ...
    # If the dimensions of the scan are known, use them to determine
    # the point count.
    ...
    if super().point_count != 0:
      return super().point_count
    # The dimensions were unspecified, but the user may have set
    # ranges/vertical/horizontal angles. Assume all points are valid.
    # The angle count is considered more important than the range count.
    vertical_angles_count = (
      self.__vertical_angles.shape[0] if self.__vertical_angles is not None
      else 0)
    horizontal_angles_count = (
      self.__horizontal_angles.shape[0] if self.__horizontal_angles is not None
      else 0)
    angle_count = max(horizontal_angles_count,
                      vertical_angles_count)
    if angle_count != 0:
      return angle_count

    # Use the shape of the ranges array as a last resort.
    if self.__ranges is not None:
      return self.point_ranges.shape[0]

    # If none of the above worked, return 0 because no points have been set.
    return 0
  ...
  def __dimensions_known(self):
    ...
    return (self.__major_dimension_count is not None) and \
           (self.__minor_dimension_count is not None)
  ...
  def point_intensity(self):
    ...
    if self.__point_intensity.shape[0] != self.point_count:
      self.__point_intensity = trim_pad_1d_array(
        self.__point_intensity, self.point_count, 0)
