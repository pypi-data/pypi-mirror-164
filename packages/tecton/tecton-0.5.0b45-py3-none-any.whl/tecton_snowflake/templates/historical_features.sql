...
{%- macro join(name, spine, spine_timestamp_key, feature_timestamp_key, feature_columns, join_keys, ttl_seconds, prefix) %}
...
{%- macro join_window(name, spine, spine_timestamp_key, feature_timestamp_key, feature_columns, join_keys, time_aggregation, last_join, prefix) %}
...
WITH
    ...
    {%- for item in feature_set_items %}
        {%- if item.aggregation %}
            {{- join_window(item.name, "_TT_SPINE_TABLE", spine_timestamp_key, item.timestamp_key, item.features, item.join_keys, item.aggregation, loop.last, item.namespace) | indent }}
        {% else %}
            {{ fv_cte_name(feature_set_items, loop.index0) }} AS (
                {{- join(item.name, "_TT_SPINE_TABLE", spine_timestamp_key, item.timestamp_key, item.features, item.join_keys, item.ttl_seconds, item.namespace) | indent }}
    ...
    {%- if include_feature_view_timestamp_columns -%}
        {%- for item in feature_set_items -%}
            {%- if item.aggregation -%}
                ...
                DATEADD('SECOND', -MOD(DATE_PART(EPOCH_SECOND, _TT_SPINE_TABLE.{{ spine_timestamp_key }}), {{ item.aggregation.slide_interval.ToSeconds() }}), DATE_TRUNC('SECOND', _TT_SPINE_TABLE.{{ spine_timestamp_key }})) AS {{ item.name|upper + separator + item.timestamp_key|upper }}
            {%- else %}
                {{ final_select_name(item.namespace + separator + item.timestamp_key, false, true) }}
            {%- endif -%}
        {%- endfor %}
    {%- endif -%}
    {%- for item in feature_set_items -%}
        ...
        {%- if item.aggregation -%}
            {%- for feature in item.aggregation.features %}
                {{- final_select_name(item.namespace + separator + feature.output_feature_name, feature.function|snowflake_function == "count", not (loop.last and last_item), ("" if append_prefix else feature.output_feature_name)) | indent }}
            {%- endfor -%}
        {%- else %}
            {%- for feature in item.features %}
                {{- final_select_name(item.namespace + separator + feature, false, not (loop.last and last_item), ("" if append_prefix else feature)) | indent }}
            {%- endfor -%}
        {%- endif -%}
    {%- endfor %}
