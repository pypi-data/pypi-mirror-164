...
class SherlockExplorer:
    ...
    def explore_object(self, object_id, input_lc_file=None, time_units=1, auto_detrend_periodic_signals=False,
        ...
        object_info = lc_builder.build_object_info(object_id, None, sectors, input_lc_file, cadence, None,
                                                   None, None, None, None,
                                                   high_rms_enabled=False, smooth_enabled=smooth)
        ...
        while True:
            ...
            duration_days = duration / 24 / 60
            #fig_transit.suptitle("Transit centered plots for TIC " + str(object_id))
            for i in np.arange(t0, t0 + period * 100, period):
                args_plot = np.argwhere((i - duration_days * 2 < time) & (time < i + duration_days * 2)).flatten()
                if len(args_plot) > 10:
            ...
            fig.show()
            mask = wotan.transit_mask(time, period, duration_days * 2, t0)
