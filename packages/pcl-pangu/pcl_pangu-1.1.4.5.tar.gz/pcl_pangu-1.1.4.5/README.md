...
## 模型Saas服务
通过我们提供Python API获取我们的模型Saas服务，使用方法请参考：[**使用方法**](docs/README_PGSaas.md)。
**现阶段为免费使用，但因为服务资源有限会对总流量做限制，请勿高频调用。** 主要功能包括：

- 获取所有模型信息，相关接口为model_list() 
- 获取指定模型信息，model_info(model) 
- 在线推理， generate(model, prompt_input, max_token=None, top_k=None, top_p=None, api_key=None, **kwargs)
