...
if LEADERBOARD:
    ...
    def leaderboard(info, title, round_, xxx):
...
if MAIN:
    ...
    def main(BoTs=[], xxx=100):
        ...
        with Live(leaderboard(info, f"\n"
                                    f"[purple]Round [biu]{roundr}[/] completed. ", roundr, xxx),
                  refresh_per_second=100) as live:
            for roundr in range(1, 10000 + 1):
                for bot in shuffled(bots):
                    if random()<info[bots.index(bot)][2]:
                        # call the bot
                        response = bot(shuffled(copy(info)), bot, roundr)
                        # update the info
            ...
            with open("out", "w") as f:
                ...
                for rank, bot in enumerate(info):
                    writer.writerow((rank+1, bot[0].__name__, bot[1], round(bot[2],1)))
            while True: pass
