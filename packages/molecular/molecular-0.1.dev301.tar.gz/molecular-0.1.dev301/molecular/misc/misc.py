...
def numpy_pivot(a, axes, index_names, value_name='value', aggfunc='mean'):
    """
    Pivot numpy arrays with an arbitrary number of axes.
    ...
    # Create DataFrame
    df = pd.DataFrame(dict(dict(zip(index_names, a_sparse.coords))), **{value_name: a_sparse.data})

    # Aggregate
    return df.pivot_table(index=index_names, values=value_name, aggfunc=aggfunc)
