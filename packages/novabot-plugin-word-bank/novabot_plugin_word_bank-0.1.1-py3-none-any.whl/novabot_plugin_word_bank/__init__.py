...
Modified from word_bank `https://github.com/kexue-z/nonebot-plugin-word-bank2`
...
PERM_EDIT = GROUP_ADMIN | GROUP_OWNER | PRIVATE_FRIEND | SUPERUSER | GROUP_MEMBER
...
async def _(
    ...
    index = get_session_id(event)
    index = "0" if "全局" in flag else index
    if wb.delete(index, type_, Message(key), require_to_me):
...
wb_clear_cmd_gl = on_command(
    "删除全局词库", block=True, priority=2, permission=PERM_GLOBAL, handlers=[wb_clear("全局")]
)
wb_clear_bank = on_command(
    "删除全部词库", block=True, priority=2, permission=PERM_GLOBAL, handlers=[wb_clear("全部")]
)
...
async def wb_search(
    ...
    if len(matched) == 2:
        type_ = "群" if isinstance(event, GroupMessageEvent) else "用户"
        id_ = event.group_id if isinstance(event, GroupMessageEvent) else event.user_id
        flag, key = matched
    else:
        type_, id_, flag, key = matched

    nickname = event.sender.card or event.sender.nickname

    if type_ and not id_:
        await matcher.finish(f"请填写{type_}ID")

    index = "0" if "全局" in flag \
        else {"群": "group", "用户": "private"}[type_] + f"_{id_}" if type_ \
    ...
    if isinstance(event, GroupMessageEvent):
        ...
        for entry in entrys:
            ...
            forward_msg.extend(to_json(value, "bot回复", str(bot.self_id), ) for value in entry.values)
