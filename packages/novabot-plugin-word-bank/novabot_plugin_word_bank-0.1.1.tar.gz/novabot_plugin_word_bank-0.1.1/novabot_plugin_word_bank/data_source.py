...
class WordBank(object):
        ...
        if self.bank_path.exists() and self.bank_path.is_file():
            ...
            for t in MatchType:
                ...
                for user_id in data.get(t.name, {}).keys():
                    self.__data[t.name][user_id] = [WordEntry.load(key, value)
    ...
    def match(
        ...
        for type_ in MatchType:
            if res := self.__match(index, msg, type_, to_me):
                return res
        return []

    def __get_entries(self, index: str, match_type: MatchType) -> Iterator[WordEntry]:
        yield from self.__data[match_type.name].get(index, [])
        if index != "0":
            yield from self.__data[match_type.name].get("0", [])

    def __match(
            self, index: str, msg: Message, match_type: MatchType, to_me: bool = False
    ) -> List[Message]:
        return next((entry.get_values()
    ...
    def keys(self, index: str, match_type: MatchType) -> List[Message]:
        ...
        """
        name = match_type.name
        return [entry.get_key() for entry in self.__data[name][index]]
