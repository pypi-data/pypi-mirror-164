...
class WordEntry:
    ...
    def match(self, msg: Message, match_type: MatchType, to_me: bool = False) -> bool:
        ...
        if match_type == MatchType.congruence:
            return False if len(self.key) != len(msg) else compare_msg(self.key, msg)
        elif match_type == MatchType.include:
            msg = Message(str(msg).strip())
            diff_len = len(msg) - len(self.key)
            if diff_len < 0:
                return False
            return any(include_msg(msg[start: start + len(self.key)], self.key) for start in range(diff_len + 1))
