...
from manimlib.shader_wrapper import ShaderWrapper, get_colormap_code
...
if TYPE_CHECKING:
    ...
    T = TypeVar("T", "Mobject")
    TimeBasedUpdater = Callable[[T, float], None | T]
    NonTimeUpdater = Callable[[T], None | T]
...
class Mobject(object):
    ...
    def resize_points(self, new_length: int, resize_func: Callable[[np.ndarray, int], np.ndarray] = resize_array):
    ...
    def compute_bounding_box(self) -> np.ndarray:
        all_points = np.vstack(
            [self.get_points(), *(mob.get_bounding_box() for mob in self.get_family()[1:] if mob.has_points())]
        )
        if len(all_points) == 0:
    ...
    def are_points_touching(self, points: np.ndarray, buff: float = 0) -> np.ndarray:
    ...
    def add_updater(self, update_function: Updater, index: int | None = None, call_updater: bool = True):
    ...
    def rotate(self, angle: float, axis: np.ndarray = OUT, about_point: np.ndarray | None = None, **kwargs):
    ...
    def apply_function_to_submobject_positions(self, function: Callable[[np.ndarray], np.ndarray]):
        for submob in self.submobjects:
    ...
    def wag(self, direction: np.ndarray = RIGHT, axis: np.ndarray = DOWN, wag_factor: float = 1.0):
        for mob in self.family_members_with_points():
            ...
            mob.set_points(
                mob.get_points() + np.dot(alphas.reshape((len(alphas), 1)), np.array(direction).reshape((1, mob.dim)))
            )
        return self
    ...
    def align_on_border(self, direction: np.ndarray, buff: float = DEFAULT_MOBJECT_TO_EDGE_BUFFER):
        """
    ...
    def to_corner(self, corner: np.ndarray = LEFT + DOWN, buff: float = DEFAULT_MOBJECT_TO_EDGE_BUFFER):
        return self.align_on_border(corner, buff)

    def to_edge(self, edge: np.ndarray = LEFT, buff: float = DEFAULT_MOBJECT_TO_EDGE_BUFFER):
    ...
    def surround(self, mobject: Mobject, dim_to_match: int = 0, stretch: bool = False, buff: float = MED_SMALL_BUFF):
    ...
    def set_rgba_array(self, rgba_array: npt.ArrayLike, name: str = "rgbas", recurse: bool = False):
        for mob in self.get_family(recurse):
            mob.data[name] = np.array(rgba_array)
        return self

    def set_color_by_rgba_func(self, func: Callable[[np.ndarray], Sequence[float]], recurse: bool = True):
        """
    ...
    def set_color_by_rgb_func(
        self, func: Callable[[np.ndarray], Sequence[float]], opacity: float = 1, recurse: bool = True
    ):
        """
    ...
    def add_background_rectangle(self, color: ManimColor | None = None, opacity: float = 0.75, **kwargs):
    ...
    def get_continuous_bounding_box_point(self, direction: np.ndarray) -> np.ndarray:
        ...
        return center + direction / np.max(
            np.abs(np.true_divide(direction, corner_vect, out=np.zeros(len(direction)), where=((corner_vect) != 0)))
        )
    ...
    def get_pieces(self, n_pieces: int) -> Group:
        ...
        return Group(
            *[template.copy().pointwise_become_partial(self, a1, a2) for a1, a2 in zip(alphas[:-1], alphas[1:])]
        )
    ...
    def match_coord(self, mobject_or_point: Mobject | np.ndarray, dim: int, direction: np.ndarray = ORIGIN):
        if isinstance(mobject_or_point, Mobject):
    ...
    def match_x(self, mobject_or_point: Mobject | np.ndarray, direction: np.ndarray = ORIGIN):
        return self.match_coord(mobject_or_point, 0, direction)

    def match_y(self, mobject_or_point: Mobject | np.ndarray, direction: np.ndarray = ORIGIN):
        return self.match_coord(mobject_or_point, 1, direction)

    def match_z(self, mobject_or_point: Mobject | np.ndarray, direction: np.ndarray = ORIGIN):
        return self.match_coord(mobject_or_point, 2, direction)

    def align_to(self, mobject_or_point: Mobject | np.ndarray, direction: np.ndarray = ORIGIN):
        """
        Examples:
    ...
    def set_color_by_xyz_func(
        self, glsl_snippet: str, min_value: float = -5.0, max_value: float = 5.0, colormap: str = "viridis"
    ):
        """
        ...
        self.set_color_by_code(
            "color.rgb = float_to_color({}, {}, {}, {});".format(
                glsl_snippet, float(min_value), float(max_value), get_colormap_code(rgb_list)
            )
        )
        return self
    ...
    def get_shader_wrapper_list(self) -> list[ShaderWrapper]:
        shader_wrappers = it.chain(
            [self.get_shader_wrapper()], *[sm.get_shader_wrapper_list() for sm in self.submobjects]
        )
    ...
    def check_data_alignment(self, array: Iterable, data_key: str):
        ...
        if d_len != 1 and d_len != len(array):
            self.data[data_key] = resize_with_interpolation(self.data[data_key], len(array))
        return self
    ...
    def read_data_to_shader(self, shader_data: np.ndarray, shader_data_key: str, data_key: str):
