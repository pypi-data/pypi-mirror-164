...
"""
class BoltzmannGRNTest():
  def __init__(self, size = None, on_p = None, weight = None, sparsity = None, directed = True, type_state = 'binary', state_list = None, edge_list = None):
    #size = size, on_p = gene_start_probability, weight = weight, sparsity = sparsity, directed = directed, type_state = 'binary'
    """
    模型初始化
    1. 定义模型大小（基因数量）
    2. 定义基因关系矩阵
    ...
    """
    if edge_list != None:
      self.size = len(edge_list)
    elif size != None:
      self.size = size
    else:
      exit("Please Define a size for the graph")
    self.size = size
    ...
    #self, edgeList = None, weightList = None, stateList = None, numNodes = None, sparsity = 0.9, onProb = 0.5, directed = False
  ...
  def Generate_Gene_Network(self, n, sparsity, on_p, directed):
    ...
    if type(directed) == bool and (type(sparsity) == float or type(sparsity) == int) and type(n) == int and n > 0 and (self.type_state == "spin" or self.type_state == "binary"):
      if sparsity >= 0 and sparsity <= 1:
        ...
        print(f"Model Sparsity is set to", self.sparsity)
  ...
  def createGraph(self):
    if self.edge_list != None:
      G = self.generate_G_via_list()
    else:
      G = self.Generate_Gene_Network(self.size, self.sparsity, self.on_p, self.directed)
    return G

  def generate_G_via_list(self):
    if type(self.edge_list) == dict:
      lst = self.edge_list.values()
    else:
      pass
    lst = self.edge_list
    n = len(lst)
    ...
    for i in range(1, n): #Iterate through number of nodes to generate a spanning Tree
      ...
      G.add_node(i)
    for node in lst:
      index = lst.index(node)
      for item in node:
        G.add_edge(index, item)
  ...
  def addState(self, G):
    ...
    state0 = 0
    if len(self.state_list) == 0:
      for node in G.nodes: #Iterate through nodes and set state depending on initial Probability
        chance = random.random() #Random Chance
        if chance < self.on_p:
          G.nodes[node]["state"] = state1
        else:
          G.nodes[node]["state"] = state0
    else:
       for node in G.nodes: #Iterate through nodes and set state depending on initial Probability
          chance = random.random() #Random Chance
          if chance < self.on_p:
  ...
  def createMatrix(self, weightlist = None, style = 'random'):
    """
    ...
    wmatrix = np.zeros((self.size, self.size))#Create a weight matrix
    #print(wmatrix)
    complete = False
    while complete == False:
      if style == 'random':
        ...
        for sublist in weightlist:
          index = weightlist.index(sublist)
          for item in sublist:
            indexw = sublist.index(item)
            wmatrix[index][indexw]
        complete = True
      else:
        for node in G.nodes(): #Iterate through all nodes
          for neighbour in G.neighbors(node): #Iterate through all its neighbours
            wmatrix[node][neighbour] = self.weight
  ...
  def update(self, G, timesteps, field, showevery):
    """
    更新timesteps次模型
    size - 模型大小
    G - 模型图
    ...
    G - Model Graph
    """
    if type(showevery) != int:
      exit(f'showevery needs to be an int, not', type(showevery))
    print("================================================================")
    print(f"Received Input graph of size", self.size)
    ...
    for t in range(0, timesteps+1): #While below the set timestep
      ...
      if t%showevery == 0:
        print(f"Timestep:", t, 'mean state is', sum_index)
