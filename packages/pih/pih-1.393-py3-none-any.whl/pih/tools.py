...
class DataTools:
    ...
    @staticmethod
    def to_result(value: str, cls = None) -> Result:
        ...
        return Result(FieldItemList(field_list), list(map(lambda x: DataTools.from_data(cls(), x), data)) if isinstance(data, List) else DataTools.from_data(cls(), data) if cls else data)
    ...
    def get_first(value: List, default_value: Any = None) -> Any:
        return value[0] if value and len(value) > 0 else default_value
