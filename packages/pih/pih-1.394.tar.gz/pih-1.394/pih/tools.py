...
from pih.collection import FieldItem, FieldItemList, FullName, LoginPasswordPair, Result
...
class DataTools:
    ...
    @staticmethod
    def to_result(value: str, cls = None, first_data_item: bool = False) -> Result:
        object: dict = json.loads(value)
        fields = object["fields"]
        data = DataTools.get_first(
            object["data"]) if first_data_item else object["data"]
        field_list: List = []
        ...
        return Result(FieldItemList(field_list), (list(map(lambda x: DataTools.from_data(cls(), x), data)) if isinstance(data, List) and cls else DataTools.from_data(cls(), data) if cls else data) if data else None)
...
class ResultUnpack:
    ...
    def data_is_empty(result: Result) -> bool:
        return result is None or (isinstance(result.data, list) and len(result.data) == 0)
...
class PIHEncoder(json.JSONEncoder):
    def default(self, obj):
        ...
        if isinstance(obj, FieldItemList):
            return obj.list
        if isinstance(obj, FullName) or isinstance(obj, LoginPasswordPair) or isinstance(obj, Result):
