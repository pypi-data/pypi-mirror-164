...
from pih.collection import ActionValue, CommandChainItem, CommandLinkItem, FieldItem, CommandItem, FieldItemList, FullName, LoginPasswordPair, PasswordSettings, Result, User
...
class PIH:
    ...
    class AUTH:
        ...
        def login():
            ...
            username = PIH.INPUT.input("Enter username:")
            ...
            try:
                ...
                PR.red("\nSorry, wrong user or password. Good bye...")
    ...
    class RESULT:
        ...
        class MARK:
            ...
            @staticmethod
            def by_tab_number(value: str) -> Result:
                return DataTools.to_result(RPC_COMMANDS.MARK.get_mark_by_tab_number(value))

            @staticmethod
            def mark_by_person_name(value: str) -> Result:
                return DataTools.to_result(RPC_COMMANDS.MARK.get_mark_by_person_name(value))

            @staticmethod
            def free_marks() -> Result:
                return DataTools.to_result(RPC_COMMANDS.MARK.get_free_marks())

            @staticmethod
            def free_marks_by_group(group: dict) -> Result:
                return DataTools.to_result(RPC_COMMANDS.MARK.get_free_marks_by_group(group))

            @staticmethod
            def free_marks_group_statistics() -> Result:
                return DataTools.to_result(RPC_COMMANDS.MARK.get_free_marks_group_statistics())
        ...
        class USER:
            ...
            @staticmethod
            def by_login(value: str) -> Result:
                return DataTools.to_result(RPC_COMMANDS.USER.get_user_by_login(value))

            @staticmethod
            def template_list() -> Result:
                return DataTools.to_result(RPC_COMMANDS.USER.get_template_list())

            @staticmethod
            def containers() -> Result:
                return DataTools.to_result(RPC_COMMANDS.USER.get_containers())

            @staticmethod
            def by_full_name(value: FullName) -> Result:
                return DataTools.to_result(RPC_COMMANDS.USER.get_user_by_full_name(value), User)

            @staticmethod
            def by_name(value: str) -> Result:
                return DataTools.to_result(RPC_COMMANDS.USER.get_user_by_name(value), User)
    ...
    class INPUT:
        ...
        def input(caption: str = None) -> str:
            try:
                return input() if caption else input(caption)
            ...
            while True:
                if item_label:
                    for index, item in data:
                        PR.index(index, item_label(item))
                if length == 1:
                    return 0
        ...
        @staticmethod
        def item_by_index(caption: str, data: dict, item_label: Callable = None) -> dict:
        ...
        def full_name() -> FullName:
            ...
            full_name.last_name = PIH.INPUT.full_name_part("Введите фамилию")
            full_name.first_name = PIH.INPUT.full_name_part("Введите имя")
            full_name.middle_name = PIH.INPUT.full_name_part("Введите отчество")
            return full_name
    ...
    class VISUAL:
        ...
        @staticmethod
        def table_with_caption(result: Result, caption: str = None, use_index: bool = False, modify_table_handler: Callable = None, modify_data_handler: Callable = None) -> None:
            if caption is not None:
                PR.cyan(caption)
            field_list = result.fileds
            data = result.data
...
class PR:
    ...
    @staticmethod
    def input(caption: str) -> str:
        PR.color(Back.LIGHTWHITE_EX, caption)
