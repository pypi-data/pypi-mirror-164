...
from pih.collection import ActionValue, CommandChainItem, CommandLinkItem, FieldItem, CommandItem, FieldItemList, FullName, LoginPasswordPair, Mark, MarkGroup, MarkGroupStatistics, PasswordSettings, Result, User, UserContainer
from pih.const import CONST, EXECUTOR, FIELD_NAME_COLLECTION, FIELD_COLLECTION, PASSWORD, PATHS, USER_PROPERTY
...
class PIH:
    ...
    class RESULT:
        ...
        class MARK:
            ...
            @staticmethod
            def by_name(value: str) -> Result[Mark]:
    ...
    class INPUT:
        ...
        def indexed_field_list(list: FieldItemList, caption: str) -> str:
            ...
            return PIH.INPUT.item_by_index(caption, name_list, lambda item: list.get_item_by_name(item).caption)
            ...
            while True:
                if item_label:
                    for index, item in enumerate(data):
                        PR.index(index + 1, item_label(item))
                if length == 1:
                    return 0
        ...
        class USER:
            ...
            def search_attribute() -> str:
                return PIH.INPUT.indexed_field_list(
                    FIELD_COLLECTION.AD.SEARCH_ATTRIBUTE, "Выберите по какому критерию искать, введя индекс")
    ...
    class VISUAL:
        ...
        @staticmethod
        def mark_by_tab_number_for_result(result: Result) -> None:
            PIH.VISUAL.table_with_caption_first_title_is_centered(
        ...
        @staticmethod
        def free_marks_by_group_for_result(result: Result, group: dict, use_index: bool) -> None:
        ...
        @staticmethod
        def table_with_caption(result: Result | dict, caption: str = None, use_index: bool = False, modify_table_handler: Callable = None, modify_data_handler: Callable = None) -> None:
                            ...
                            if field_item.name == FIELD_COLLECTION.INDEX.name:
                                ...
                                if modify_data_handler is not None:
                                    modify_item_data = modify_data_handler(
                                        field_item, item)
                                    row_data.append(item_data if modify_item_data is None else modify_item_data)
                                else:
    ...
    class ACTION:
        ...
        class USER:
            ...
            @staticmethod
            def set_status(user: User, status: str, container_dn: str) -> bool:
                return RPC_COMMANDS.USER.set_status(user.distinguishedName, status, container_dn)
    ...
    def call_actions_by_index(self, index: int = 0, change: bool = False):
        ...
        while True:
            try:
                ...
                PR.red("Повтор предыдущего действия")
                if index > 0:
