...
class SchemaIndex():
    ...
    def tables_attributes(self, nested_structures=False):
        collection_structure_path = ''.join([
                DefaultConfiguration.TEMP_FILES_PATH,
                DefaultConfiguration.DATABASE + '.pickle'
            ]
        )
        ...
        for table in self:
            for attribute in self[table]:
                ...
                if not nested_structures:
                    if collection_structure[table][attribute] not in [type(list()), type(dict())]:
    ...
    def persist_to_file(self, file_directory, filename):
        ...
        with shelve.open(filepath) as storage:
