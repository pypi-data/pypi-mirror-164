...
def update_truth_settings(given: Union[List[Dict], Dict], default: List[Dict] = None) -> List[Dict]:
    ...
    for truth_setting in truth_settings:
        ...
        for n in range(len(index)):
            if type(index[n]) == str:
                values = index[n].split(':')
                if len(values) != 2:
                    raise SonusAIError(f"Unable to parse truth_setting['index'] of {index[n]}")
                index[n] = list(range(int(values[0]), int(values[1]) + 1))
            elif type(index[n]) != int:
                raise SonusAIError(f"Unable to parse truth_setting['index'] of {index[n]}")
        ...
        for sublist in index:
            if isinstance(sublist, list):
                for item in sublist:
                    truth_setting['index'].append(item)
            else:
                truth_setting['index'].append(sublist)
