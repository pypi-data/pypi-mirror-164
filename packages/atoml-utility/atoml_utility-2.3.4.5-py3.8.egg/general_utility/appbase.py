...
class AppBase:
    ...
    def is_update(self, field, fields):
        result = None
        if (field in fields) or (isinstance(fields, str) and fields.lower() == '*'): # 白名单
    ...
    def get_local_infos(self, fields = "*"):
        ...
        if (b): # 需要返回的字段
            if (v is None): # 需要取值的字段
                ...
                v = ("%s/%s %d%%" % (used, total, psutil.virtual_memory().percent))
            result['virtual_memory'] = v

        b, v = self.is_update('swap_memory', fields)
        if (b): # 需要返回的字段
            if (v is None): # 需要取值的字段
                swap_free = str(round(psutil.swap_memory().used / (1024.0 * 1024.0 * 1024.0), 2))
                swap_total = str(round(psutil.swap_memory().total / (1024.0 * 1024.0 * 1024.0), 2))
                v = ("%s/%s %d%%" % (swap_free, swap_total, psutil.swap_memory().percent))
