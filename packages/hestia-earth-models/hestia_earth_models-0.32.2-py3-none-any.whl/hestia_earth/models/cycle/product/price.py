...
def get_liveAnimal_lookup_values(product: dict, country_id: str, year: int = None):
    ...
    for animal_product in animal_products:
        grouping = get_animalProduct_lookup_value(MODEL, animal_product, FAO_LOOKUP_COLUMN)
        if not grouping:
            continue

        debugValues(product, model=MODEL, term=term_id, key=MODEL_KEY, by='liveAnimal',
...
def _run_by_liveAnimal(product: dict, country_id: str, year: int = None):
    ...
    value = price * carcass_weight if all([price_per_carcass_weight, carcass_weight]) else None
    return None if value is None else _product(product, value)


def _should_run_product_by_liveAnimal(cycle: dict, product: dict):
...
def _should_run_product_by_country(cycle: dict, product: dict):
    term_id = product.get('term', {}).get('@id')
...
def _should_run_by_country(cycle: dict):
...
def run(cycle: dict):
    should_run_by_country, country_id = _should_run_by_country(cycle)
    ...
    return non_empty_list([
        (
            (
                (
                    _run_by_liveAnimal(p, country_id, year) if _should_run_product_by_liveAnimal(cycle, p)
                    else _run_by_country(p, country_id, year) if _should_run_product_by_country(cycle, p)
                    else None
