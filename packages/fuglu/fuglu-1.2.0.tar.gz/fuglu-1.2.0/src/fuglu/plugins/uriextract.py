...
from fuglu.shared import ScannerPlugin, DUNNO, DEFER, string_to_actioncode, apply_template, FileList, AppenderPlugin, HAVE_BEAUTIFULSOUP, Suspect, get_default_cache
...
class URIExtract(ScannerPlugin):
    ...
    def _check_skiplist(self, uris, extractfuncname='extracturis', use_hacks=False):
        ...
        newuris = []
        for uri in uris:
            exturis = extractfunc(uri, use_hacks)
    ...
    def _ip_convert_base10(self, netloc):
        if is_ipv4(netloc):
            ipbytes = netloc.split('.')
            intvals = []
            for ipbyte in ipbytes:
                if self._re_hexip.match(ipbyte):
                    intvals.append(str(int(ipbyte, 16)))
                elif self._re_octip.match(ipbyte):
    ...
    def _normalise_components(self, uri):
        ...
        try:
            ...
            netloc = self._ip_convert_base10(netloc)
    ...
    def _quickfilter(self, uris):
        ...
        uris = [self._normalise_components(self._removeprefixes(u.strip().lstrip('*').strip('"').rstrip('-'), ['blob:', ': ', '3D"', '=3D"', '='])) for u in uris]
                ...
                if attObj.content_fname_check(contenttype_contains="html") \
                    ...
                    if HAVE_BEAUTIFULSOUP:
                        ...
                        if isinstance(hrefs, list):
                            try:
                                atagshtml = BeautifulSoup.BeautifulSoup(io.StringIO(decoded_payload), "lxml").find_all('a')
                            except Exception as e:
                            ...
                            # add to hrefs list
                            try:
                                imgtagshtml = BeautifulSoup.BeautifulSoup(io.StringIO(decoded_payload), "lxml").find_all('img')
                            except Exception as e:
                        ...
                        try:
                            basetags = BeautifulSoup.BeautifulSoup(io.StringIO(decoded_payload), "lxml").find_all('base')
                        except Exception as e:
...
class EmailExtract(URIExtract):
    ...
    def _run(self,suspect):
        ...
        for content in self.get_decoded_textparts(suspect, ignore_words_without="@", maxsize=maxsize, maxsize_analyse=maxsize_analyse, hrefs=hrefs):
            ...
            try:
                for email in content.split(' '):
                    part_emails = self.extractor.extractemails(email, usehacks)
        ...
        for email in hrefs_all:
            ...
            if not suspect.stimeout_continue(self.section):
                self.logger.warning(
                    f"{suspect.id} Timeout in email href loop: {suspect.stimeout_string(self.section)}")
                return DUNNO
...
class DomainAction(ScannerPlugin):
    ...
    def _get_rest_userpref(self, fugluid):
        ...
        if restapi_endpoint:
            ...
            content = restapi.get(self.config, restapi_endpoint, fugluid, restapi_timeout)
            userpref_types = self.config.getlist(self.section, 'userpref_types')
    ...
    def _gen_userskiplist(self, fugluid, recipient):
        skiplist = []
        ...
        if SQL_EXTENSION_ENABLED:
            ...
            userprefs = self._get_sa_userpref(fugluid)
    ...
    def examine(self,suspect):
        ...
        if not urls:
            self.logger.debug('%s no urls to check' % suspect.id)
            return DUNNO
        try:
            skiplist = self._gen_userskiplist(suspect.id, suspect.to_address)
...
class EmailAction(DomainAction):
    def __init__(self,config,section=None):
        ...
        self.requiredvars['blacklistconfig']['default'] = '${confdir}/rblemail.conf'
        self.requiredvars['message']['default'] = '5.7.1 block listed email address ${address} by ${blacklist}'
