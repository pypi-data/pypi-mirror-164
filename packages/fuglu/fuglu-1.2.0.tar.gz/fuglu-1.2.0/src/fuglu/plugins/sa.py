...
#
#
from fuglu.shared import ScannerPlugin, DUNNO, DEFER, Suspect, string_to_actioncode, apply_template, actioncode_to_string
...
"""
    ...
    def _lint_spam(self):
        spamflag, score, rules = self.safilter_symbols(GTUBE, 'test', fid="<no-suspect>")
        if 'GTUBE' in rules:
    ...
    def check_sql_blocklist(self, suspect, runtimeconfig=None):
        ...
        try:
            ...
            self.logger.error('%s Could not read UserPrefs: %s' % (suspect.id, str(e)))
            action = self._problemcode()
            message = 'Internal Server Error'
            return action, message
        
        for result in blocklistings:
            self.logger.debug('%s checking blocklist value %s' % (suspect.id, result.value))
            # build regex
            ...
            if pattern.search(suspect.from_address):
                self.logger.debug('%s Blocklist match : %s for sa pref %s' % (suspect.id, suspect.from_address, result.value))
                ...
                suspect.add_header("%sBlacklisted" % prependheader, result.value) # deprecated
                suspect.add_header("%sBlocklisted" % prependheader, result.value)
                suspect.debug('Sender is Blocklisted: %s' % result.value)
    ...
    def _extract_spamstatus(self, msgrep, spamheadername, suspect):
        ...
        if spamheader is None:
            ...
            patt = re.compile(r'Score=([\-\d.]{1,10})', re.IGNORECASE)
    ...
    def examine(self, suspect):
        ...
        if suspect.size > maxsize and not strip_oversize:
            ...
            prependheader = self.config.get('main', 'prependaddedheaders')
            suspect.add_header("%sSA-SKIP" % prependheader, 'Too big for spamchecks. %s > %s' % (suspect.size, maxsize))
        ...
        if forwardoriginal:
            ...
            if ret is None:
                ...
                self.logger.warning('%s SA report scan FAILED' % suspect.id)
                suspect.add_header('%sSA-SKIP' % self.config.get('main', 'prependaddedheaders'), 'SA scan failed')
            ...
            suspect.tags['SAPlugin.report'] = report # deprecated
            self.logger.debug('suspect %s %s %s %s' % (suspect.id, isspam, spamscore, report))
        
        action = DUNNO
        message = None
        try:
            ishighspam = self._is_highspam(runtimeconfig, spamscore)
            if isspam:
                values = dict(spamscore=spamscore)
                message = apply_template(self.config.get(self.section, 'rejectmessage'), suspect, values)
            
            if isspam and not ishighspam:
            ...
            self._spamreport(suspect, isspam, ishighspam, report, spamscore, enginename='SpamAssassin')
        except RESTAPIError as e:
    ...
    def _is_highspam(self, runtimeconfig, spamscore):
        if spamscore is not None:
    ...
    def _safilter_content(self, messagecontent, user, command, fid: str = ""):
        ...
        while attempts:
            ...
            try:
                ...
                version, number, status = answer
                if status != 'EX_OK':
                    self.logger.error(f"{fid} Got bad status from spamd: %s" % status)
                    if attempts:
                        time.sleep(retry_sleep)
                    continue
