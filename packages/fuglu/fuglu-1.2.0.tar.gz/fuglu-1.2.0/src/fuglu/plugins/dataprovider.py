...
#
from fuglu.shared import ScannerPlugin, DUNNO, FileList, apply_template, SuspectFilter
...
class URIUDPSend(ScannerPlugin):
    ...
    def _initfilter(self):
        ...
        if filename is None or filename == "":
            self.logger.debug('Filter regex file not defined')
            return True

        if not os.path.exists(filename):
            self.logger.error(f'Filter regex file not found: {filename}')
            return False
    ...
    def _run(self, suspect):
        ...
        self.logger.info(f'{suspect.id} sent {len(urilist)} URIs')
        return
