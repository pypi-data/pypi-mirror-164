...
class ElasticBackend(AbstractBackend):
    ...
    def get_elastic_connection(self, reload=False):
        es = None
    ...
    def archive(self, suspect, fugluid_override=None, reload=False, retry=3):
        result = {}
        try:
    ...
    def do_archive(self, suspect):
        ...
        for backend_name in self.backends:
            ...
            try:
                ...
                #self.logger.error(f'{suspect.id} failed to archive using backend {backend_name} due to {e.__class__.__name__}: {str(e)}')
            ...
            if arg is not None and arg.lower().strip() == 'no':
                ...
                if suspect.get_tag('debug'):
                    ...
                    if result:
                        ...
                        self.logger.info('%s archived successfully: %s' % (suspect.id, result))
                    if errors:
                        self.logger.warning('%s archiving errors in backend %s' % (suspect.id, ','.join(list(errors.keys()))))
                        for key in errors:
                            self.logger.error('%s archiving errors in backend %s: %s' % (suspect.id, key, errors[key]))
