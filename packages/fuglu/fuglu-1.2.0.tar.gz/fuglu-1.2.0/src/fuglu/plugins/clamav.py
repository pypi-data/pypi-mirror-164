...
"""
    ...
    def scan_stream(self, content, suspectid ="(NA)"):
        ...
        self.logger.debug('%s: All chunks sent, sending 0 - size to tell ClamAV the whole message has been sent' % suspectid)
        s.sendall(struct.pack(b'!L', 0))
        self.logger.debug('%s: 0 has been sent, now wait for answer' % suspectid)
        dr = {}
