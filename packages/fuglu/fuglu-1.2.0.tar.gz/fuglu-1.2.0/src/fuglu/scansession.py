...
class SessionHandler(TrackTimings):
    ...
    def handlesession(self, worker=None):
        ...
        while keep_connection:
            ...
            try:
                ...
                message_prefix = "(#%u)" % isuspect  # identifier (receiving multiple suspects in one session)

                self.set_workerstate(message_prefix+'receiving message')
                ...
                if suspect is None:
                    ...
                    self.logger.debug(message_prefix+'Health Check Suspect retrieved')
                ...
                if len(suspect.recipients) != 1:
                    self.logger.warning(message_prefix+'Notice: Message from %s has %s recipients. Plugins supporting only one recipient will see: %s' % (
                ...
                try:
                    ...
                    self.logger.warning(message_prefix+'Could not get incoming port: %s' % str(e))
                ...
                if suspect.tempfilename() != "(buffer-only)":
                    try:
                        ...
                        self.logger.debug(message_prefix+'Removed tempfile %s' % (suspect.tempfile if suspect.tempfile else "(not available)"))
                        suspect.tempfile = None
                        self.tracktime("Remove-tempfile")
                    except OSError:
                        self.logger.warning(message_prefix+'Could not remove tempfile %s' % suspect.tempfile)
            except KeyboardInterrupt:
                sys.exit(0)
            except ValueError as e:
                # Error in envelope send/receive address
                try:
                    self.logger.warning(message_prefix+"Invalid send/receive address -> %s" % force_uString(e))
                except Exception:
                    pass
                ...
                if suspect is None:
                    ...
                    if remove_tmpfiles_on_error:
                        if tmpfilename:
                            self.logger.debug(message_prefix+'Remove tmpfile: %s for failed message' % tmpfilename)
                        self.protohandler.remove_tmpfile()
                    else:
                        if tmpfilename:
                            self.logger.warning(message_prefix+'Keep tmpfile: %s for failed message' % tmpfilename)
                        else:
                            self.logger.warning(message_prefix+'No tmpfile to keep for failed message')
                    ...
                    if remove_tmpfiles_on_error:
                        try:
                            ...
                            self.logger.debug(message_prefix+'Removed tempfile %s' % suspect.tempfilename())
                        except OSError:
                            self.logger.warning(message_prefix+'Could not remove tempfile %s' % suspect.tempfilename())
                    else:
                        self.logger.warning(message_prefix+'Keep tempfile %s for failed message' % suspect.tempfilename())

                # try to remove the suspect
                try:
                    self.logger.debug(message_prefix+'Remove suspect (current refs): %u' % sys.getrefcount(suspect))
        ...
        if isuspect > 2:
            self.logger.info(message_prefix+'Session finished for %u sessions in same connection' % (isuspect-1))
    ...
    def run_plugins(self, suspect, pluglist):
        ...
        for plugin in pluglist:
            try:
                self.logger.debug(f'{suspect.id} Running plugin {str(plugin)}')
                self.set_workerstate("%s : Running Plugin %s" % (suspect, plugin))
    ...
    def run_prependers(self, suspect):
        ...
        for plugin in self.prependers:
            try:
                self.logger.debug(f'{suspect.id} Running prepender {str(plugin)}')
    ...
    def run_appenders(self, suspect, finaldecision, applist):
        ...
        for plugin in applist:
            try:
                self.logger.debug(f'{suspect.id} Running appender {str(plugin)}')
