...
class Mailattachment(Cachelimits):
        ...
        if not self.is_archive:
            ...
            try:
                ...
                try:
                    ...
                    if noextractinfo is not None:
                        noextractinfo.append(fname, "archivehandle", "exception: %s" % force_uString(e))
                    return None
        ...
        if self.buffer is not None:
            try:
                ...
                self.logger.warning(f"{self.fugluid}, Problem creating Archivehandle for file: "
                                    ...
                                    f"(message: {force_uString(e)}) -> ignore")
    ...
    def get_mangled_filename(self, prefix: str = '', maxchars: int = 15, keepending: bool = False):
        first=self.filename.split('.')[0]
        #replace spaces with underscore
        first = self._re_ws.sub('_', first)
        #remove all non alnum chars
        first=''.join([x for x in first if x in string.ascii_letters or x in string.digits])
        ...
        if keepending and "." in self.filename:
...
Location     : %s        
Archive type : %s        
Content type : %s""" % (self.filename,'(unknown)' if self.filesize is None else str(self.filesize),
                        self.filename + element_of +
                        element_of.join(["{" + obj().filename +"}" for obj in self.parent_archives]),
                        self.archive_type,
                        self.contenttype)
