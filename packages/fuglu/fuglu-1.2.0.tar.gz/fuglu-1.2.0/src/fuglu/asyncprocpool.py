...
def fuglu_async_process_worker(queue, config, shared_state, child_to_server_messages, logQueue):
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=async_threadpoolworkers) as pool:
            ...
            asworkers = [fuglu_async_process_worker_main(queue, workerstate, logger, controller, i, logtimings, pool, async_prepare_timeout) for i in range(1, 1+async_coroutines)]
...
async def fuglu_async_process_worker_main(queue, workerstate, logger, controller, asyncid, logtimings, pool, async_prepare_timeout):
    ...
    while True:
        try:
            ...
            future = loop.create_connection(lambda: protocol, sock=sock)
