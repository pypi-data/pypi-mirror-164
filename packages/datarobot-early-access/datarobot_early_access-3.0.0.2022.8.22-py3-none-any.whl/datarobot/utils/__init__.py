...
def _from_api_dict(
    ...
    keep_attrs = keep_attrs or []  # type: ignore[assignment]
    # prepare attributes in format 'top.middle.bottom' for processing
    parsed_attrs = []
    # Casting because we know from above line that if keep_attrs is None we set to empty list
    for attr in cast(Union[List[str], List[List[str]]], keep_attrs):
