...
class Project(APIObject, BrowserMixin):
    ...
    def create_from_data_source(
        ...
        if not any(auth_type in new_payload.keys() for auth_type in AuthenticationType.ALL):
            raise ValueError(
                "One of the followng must be specified: `username` and `password`, `credential_id`, \
or `credential_data`."
            )
