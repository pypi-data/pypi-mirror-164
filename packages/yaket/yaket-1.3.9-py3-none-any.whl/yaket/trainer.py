...
class Trainer:
    ...
    def _check_shapes(self, train_dataset, val_dataset):
        """Check that the shapes of the train and validation datasets are the same"""
        if isinstance(train_dataset, tf.data.Dataset) and isinstance(val_dataset, tf.data.Dataset):
            train_shape = train_dataset.element_spec
            val_shape = val_dataset.element_spec
            if len(train_shape) > 4 or len(val_shape) > 4:
                raise ValueError("The shape of the input data must be lower than 4 dimensions")
            if train_shape != val_shape:
                raise ValueError(
                    f"Shapes of train and validation datasets are different. Train: {train_shape}, Val: {val_shape}"
                )
        elif isinstance(train_dataset, tuple) and isinstance(val_dataset, tuple):

            if len(train_dataset) > 4 or len(val_dataset) > 4:
                raise ValueError("The shape of the input data must be lower than 4 dimensions")
            if len(train_dataset) != len(val_dataset):
                raise ValueError(
                    f"Shapes of train and validation datasets are different."
                )

    def add_callback(self, callback: tf.keras.callbacks.Callback):
        """Add a callback to the model

        Args:
            callback:  Callback to add to the model
        """
    ...
    def _get_input_shape(self):
        ...
        if isinstance(self.train_dataset, tf.data.Dataset):
            for a in self.train_dataset.take(1):
               
                arr = a[0] if isinstance(a[0], tuple) else a
                self.input_shape = (None, *arr[0].shape[1:])
        else:
            arr = self.train_dataset[0] if isinstance(self.train_dataset[0], tuple) else self.train_dataset
            self.input_shape = (None, *arr[0].shape[1:])
    ...
    def _save_model(self):
        ...
        if self.config.save_model is not None:
            self._out_path = self.config.save_model
            os.makedirs(self._out_path, exist_ok=True)

        elif self._model_checkpoint is not None:
    ...
    def __post_init__(self):
        ...
        self._check_shapes(self.train_dataset, self.val_dataset)
