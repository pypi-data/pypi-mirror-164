...
class TrainingModel(BaseModel, extra=Extra.allow):
    ...
    metrics: Optional[
        conlist(item_type=Union[str, Dict], min_items=1, unique_items=True)
    ]
    ...
    save_model: Optional[constr(strict=True)] = './outputs/model/'
    ...
    def accelerator_validator(cls, v):
        ...
        if v not in Accelerator.list():
            raise ValueError(
                f"{v} is not a valid accelerator.\nPlease use: {Accelerator.list()}"
            )
        if v in Accelerator.list_gpu() and len(tf.config.list_physical_devices("GPU")) == 0:
