...
def emissions_factor_combustor(
    ...
    Calculate the emissions factor of a combustor (e.g. gas turbine, flare, etc.). Three different methods can be used
...
def cumulative_co2_production(rate_of_emissions: pd.Series, start_date: Optional[pd.Timestamp] = None) -> pd.Series:
    ...
    Calculate total CO2 production according to the rate of emissions. The total is calculated by performing trapezoidal integration
    ...
    then it will default to the start of the current year.
...
def cumulative_co2_cost(
    ...
    Calculate the cumulative cost of CO2 for either a combustor or power consumer. This function calculates the rate of CO2 emitted
    and then uses that to calculate to total CO2 emitted. This is then multiplied by the cost factor to get total cumulative cost. Note
    that the co2_cost_factor, emissions_factor and data have to have consistent units to generate a currency output.
