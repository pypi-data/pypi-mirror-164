...
class GeometryBase(abstract_base):
    ...
    def project(self, spatial_reference):
        ...
        if isinstance(self.geometry, ShapelyPoint):
            ...
            return src.oogeo.objects.point.Point({'x': new_x, 'y': new_y}, spatial_reference)
        elif isinstance(self.geometry, MultiLineString):
            parts = []
            for part in self.geometry.geoms:
                ...
                parts.append(points)
            return src.oogeo.objects.line.Line(parts, spatial_reference=spatial_reference)
        else:
            parts = []
            for part in self.geometry.geoms:
                exterior_points = []
            ...
            return src.oogeo.objects.polygon.Polygon(parts, spatial_reference)
