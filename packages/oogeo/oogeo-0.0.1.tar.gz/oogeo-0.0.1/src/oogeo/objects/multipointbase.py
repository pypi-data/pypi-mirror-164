...
def get_near_points(target_geometry, near_geometry, tolerance):
    ...
    for index, point in enumerate(target_points):
        ...
        nearest_point = edge.nearest_point(point) if isinstance(edge, oogeo.objects.line.Line) else edge
        if not nearest_point.equals(point):
            test_line = oogeo.objects.line.Line([point, nearest_point], point.spatial_reference)
        ...
        for test_point in [previous_point, next_point]:
            ...
            if not test_point.equals(test_nearest_point):
                test_line = oogeo.objects.line.Line([test_point, test_nearest_point], point.spatial_reference)
                if not test_line.is_perpendicular(edge, tolerance=3):
                    continue
            ...
            # point for analysis
            test_line = oogeo.objects.line.Line([test_point, point], point.spatial_reference)
...
class MultipointBase(GeometryBase, abstract_base):
    ...
    def intersection(self, other, exact=False, allow_invalid=False):
        ...
        for geometry in intersection_geometries:
            if isinstance(geometry, ShapelyPoint):
                return_geometries.append(oogeo.objects.point.Point(geometry, self.spatial_reference, allow_invalid=allow_invalid))
            elif (isinstance(geometry, LineString) or isinstance(geometry, MultiLineString)) and not geometry.is_empty:
                return_geometries.append(oogeo.objects.line.Line(geometry, self.spatial_reference, allow_invalid=allow_invalid))
            elif isinstance(geometry, ShapelyPolygon) or isinstance(geometry, MultiPolygon):
                if geometry:
                    return_geometries.append(oogeo.objects.polygon.Polygon(geometry, self.spatial_reference, allow_invalid=allow_invalid))
            elif isinstance(geometry, MultiPoint):
                for point in geometry.geoms:
                    return_geometries.append(oogeo.objects.point.Point(point, self.spatial_reference, allow_invalid=allow_invalid))
    ...
    def buffer(self, distance, end_style='FLAT', join_style='BEVEL', allow_invalid=False):
        ...
        if distance == 0:
            return oogeo.objects.polygon.Polygon(self.geometry.buffer(distance), self.spatial_reference, allow_invalid=allow_invalid)

        buffer_polygon = oogeo.objects.polygon.Polygon(
    ...
    def nearest_point(self, point):
        ...
        return oogeo.objects.point.Point(nearest_point, self.spatial_reference)
    ...
    def contains(self, other_geometry, exact=False):
        ...
        """
        if exact or isinstance(other_geometry, oogeo.objects.point.Point):
    ...
    def scale(self, scale_factor):
        ...
        for point in self.points:
            new_geometry = new_geometry.replace_point(point, oogeo.objects.point.Point({'x': point.x * scale_factor, 'y': point.y * scale_factor}, self.spatial_reference), self.allow_invalid)
        return new_geometry
    ...
    def transform(self, delta_x, delta_y):
        ...
        new_geometry = self.clone()
        for point in self.points:
            new_geometry = new_geometry.replace_point(point, oogeo.objects.point.Point({'x': point.x + delta_x, 'y': point.y + delta_y}, self.spatial_reference), allow_invalid=True)
