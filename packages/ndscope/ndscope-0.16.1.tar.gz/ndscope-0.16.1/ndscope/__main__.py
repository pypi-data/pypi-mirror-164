...
from . import __version__, pg
from . import const
from . import template as _template
from .util import resolve_ndsserver
...
parser.add_argument(
    '--title', '--window-title', dest='window_title', metavar='TITLE',
    help="application window title")
parser.add_argument(
    '--light', dest='color_mode', action='store_const', const='light',
    help="light color mode (black-on-white plots)")
parser.add_argument(
    '--dark', dest='color_mode', action='store_const', const='dark',
    help="dark color mode (white-on-black plots) [default]")
parser.add_argument(
    '-bw', '--black-on-white', action='store_true',
    help=argparse.SUPPRESS)
parser.add_argument(
    '--font-size', type=int, metavar='SIZE',
    help="label font size")
parser.add_argument(
    '--line-width', type=int, metavar='WIDTH',
...
def main():
    ...
    if args.black_on_white:
        print("Warning: the --black-on-white option is deprecated.  Please use --light instead.")
