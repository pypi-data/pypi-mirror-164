...
if getattr(PyTango, "__version_number__", 0) < 722:
    ...
    print(
        "PyTango Version is %s: fandango.dynamic.USE_STATIC_METHODS = %s"
...
class DynamicDSImpl(PyTango.LatestDeviceImpl, Logger):
    ...
    def __init__(
        ...
        if getattr(self, "UseTaurus", False):
            self.UseTaurus = bool(tango.loadTaurus())
    ...
    def check_property_extensions(prop, value, db=None, extensions=EXTENSIONS):
        ...
        return tango.check_property_extensions(
            prop,
            value,
...
class DynamicDSAttrs(DynamicDSImpl):
    ...
    def dyn_attr(self):
        ...
        for line in self.DynamicAttributes:
            try:
                ...
                if 0:
                    # Works on PyTango7
                    _is_allowed = lambda s, req_type, a=aname: type(
                        self
                    ).is_dyn_allowed(s, req_type, a)
                elif 0:
    ...
    def check_attribute_events(self, aname, poll=False):
        ...
        """
        self.UseEvents = list(
            filter(bool, (u.split("#")[0].strip().lower() for u in self.UseEvents))
        )
        ...
        try:
            ...
            if 1:
                print(
                    "\n".join(
                        [
                            "DynamicDS_evalAttr_WrongFormulaException",
                            "%s is not a valid expression!" % (formula,),
                        ]
                    )
                )
...
class DynamicDSHelpers(DynamicDSAttrs):
    ...
    def getXAttr(self, aname, default=None, write=False, wvalue=None, full=False):
        ...
        """
        params = tango.parse_tango_model(
            ...
            if not device:
                ...
                if device in devs_in_server:
                    ...
                    if full_name not in self._external_attributes:
                        self.debug(
                            "%s.getXAttr: creating %s proxy to %s"
                            % (
                                self._locals.get("ATTRIBUTE"),
                                "taurus" if self.UseTaurus else "PyTango",
                                full_name,
                            )
                        )
                        ...
                        if self.UseTaurus:
                            ...
                            if self._locals.get(
                                "ATTRIBUTE"
                            ) and self.check_attribute_events(
                                self._locals.get("ATTRIBUTE")
                            ):
                                ...
                                self.info(
                                    "\t%s.addListener(%s)"
                                    % (full_name, self._locals["ATTRIBUTE"])
                                )
                                if (
                                    tango.get_model_name(a)
                                    not in self._external_listeners
                                ):
                                    self._external_listeners[
                                        tango.get_model_name(a)
                                    ] = set()

                                self._external_listeners[tango.get_model_name(a)].add(
                                    self._locals["ATTRIBUTE"]
                                )

                            self._external_attributes[full_name].addListener(
                                self.event_received
                            )
                        else:
                            # USING PLAIN PYTANGO (POLLING UNCACHED VALUES)
                            cap = tango.CachedAttributeProxy(
                                full_name, keeptime=max((100, self.KeepTime))
                            )  # keeptime=self.KeepTime)
                            self._external_attributes[full_name] = cap
                    else:
                        self.debug(
                            "%s.getXAttr: using %s proxy to %s"
                            % (
                                self._locals.get("ATTRIBUTE"),
                                "taurus" if self.UseTaurus else "PyTango",
                                full_name,
                ...
                if device == self.get_name():
                    ...
                    if device in devs_in_server:
                        ...
                        if full_name not in self._external_commands:
                            if self.UseTaurus:
                                self._external_commands[full_name] = tango.TAU.Device(
                                    device
                                )
        ...
        for C, devs in list(classes.items()):
            for d in devs:
                if C not in self.classes or d not in self.classes[C]:
                    ...
                    self.classes[C].append(d)
        self.path = (
            self.db.get_property(self.PROPERTY, ["DeviceClasses"])["DeviceClasses"]
            or [""]
        )[0]
        if self.path:
