...
class EventThread(Logger, ThreadedObject):
    ...
    def process(self):
        ...
        for nxt, s in reversed(sorted(pollings)):
...
class EventSource(Logger, SingletonMap):
    ...
    DEFAULT_EVENTS = [
        "change"
    ]  # [ 'periodic', 'change', 'archive', 'quality' ] #'user_event',
    ...
    def __init__(self, name, keeptime=1000.0, fake=False, parent=None, **kw):
        ...
        **kw used to keep compatibility with old api
        ...
        self.setLogLevel(
            kw.get(
                "loglevel", kw.get("log_level", kw.get("logLevel", self.DEFAULT_LOG))
            )
        )
        ...
        self.polling_period = kw.get(
            "pollingPeriod",
            kw.get(
                "polling_period", iif(isNumber, self.forced, self.DefaultPolling, float)
            ),
        )
    ...
    def set_cache(self, value, t=None):
        ...
        self.attr_value = (
            hasattr(value, "value")
            and value
            or fakeAttributeValue(self.full_name, value, error=False)
        )
