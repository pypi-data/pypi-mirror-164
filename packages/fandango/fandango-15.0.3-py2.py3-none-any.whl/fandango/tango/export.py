...
def export_attribute_to_dict(
    model, attribute=None, value=None, keep=False, as_struct=False
):
    """
    ...
    try:
        v = value or check_attribute(
            attr.database + "/" + attr.device + "/" + attr.name
        )
        ...
        if v and "DevFailed" not in str(v):
            ...
            try:
                ...
                attr.unit = ac.unit
                attr.description = (
                    ac.description if ac.description != "No description" else ""
                )
                ...
                attr.enum_labels = list(getattr(ac, "enum_labels", []))  # New in T9
            except:
                traceback.print_exc()
            ...
            if attr.data_format == "SCALAR":
                ...
                if v.value is None or not v.dim_x:
                    ...
                    attr.string = sep.join("[%s]" % sep.join(vv) for vv in svalue)
...
def export_commands_to_dict(device, target="*"):
    ...
    name, proxy = (
        (device, get_device(device)) if isString(device) else (device.name(), device)
    )
    dct = {}
    for c in proxy.command_list_query():
        if not fandango.matchCl(target, c.cmd_name):
            continue
...
def export_properties_to_dict(device, target="*"):
    ...
    if "/" in device:
        ...
        db = get_database()
        props = [
            p for p in db.get_class_property_list(device) if fandango.matchCl(target, p)
        ]
        return dict(
            (k, v if isString(v) else list(v))
            for k, v in list(db.get_class_property(device, props).items())
        )
...
def export_device_to_dict(device, commands=True, properties=True):
    ...
    dct = Struct(
        fandango.obj2dict(
            i, fltr=lambda n: n in "dev_class host level name server".split()
        )
    )
...
def import_device_from_dict(
    ...
    if properties:
        props = dict(
            (k, v) for k, v in list(dct["properties"].items()) if clmatch(properties, k)
        )
    ...
    attrs = dict(
        (k, v) for k, v in list(dct["attributes"].items()) if clmatch(attributes, k)
    )
    dp = get_device(name)
    alist = dp.get_attribute_list()
    alist = list(map(lowstr, alist))
...
def tango2table(filters, opts=[]):
    ...
    table = fd.arrays.dict2array(
        output,
        branch_filter=(lambda x: str(x) != "extensions"),
        leave_filter=(lambda y: y and str(y).split()[0] not in excluded),
        empty="",
    )
