...
def get_matching_devices(
    expressions, limit=0, exported=False, fullname=False, trace=False
):
    """
    ...
    condition = lambda d: any(
        matchCl("(%s/)?(%s)" % (defhost, e), d, terminate=True) for e in expressions
    )
