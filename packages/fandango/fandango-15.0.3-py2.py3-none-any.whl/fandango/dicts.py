...
class ThreadDict(dict):
    ...
    def run(self):
        self.tracer("In ThreadDict.run(%s)" % str(self.threadkeys()))
            ...
            for k in keys:
                try:
                    if (time.time() - self._updates.get(k, 0)) > self._periods.get(
                        ...
                        if self.trace:
                            self.tracer("ThreadDict::run(): updating %s" % str(k))
...
class CaselessDict(dict):
            ...
            try:
                ...
                if hasattr(other, "items"):
                    for k, v in list(other.items()):
                        dict.__setitem__(
                            self, k.lower() if hasattr(k, "lower") else k, v
                        )
                else:
                    for k, v in other:
                        dict.__setitem__(
    ...
    def setdefault(self, key, def_val=None):
        ...
        return dict.setdefault(
            self, key.lower() if hasattr(key, "lower") else key, def_val
        )
...
class ReversibleDict(object):  # dict):
    ...
    def __getitem__(self, key, raw=False):
        ...
        if not ks.get(key, []):
            raise_(Exception, "KeyNotFound(%s)" % str(key))
        if self.nextlevel() == self.depth() - 1:
            i = ks[key].pop()
