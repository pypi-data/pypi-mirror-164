        ...
        ranges = list(
            map(
                ...
                (
                    ...
                    old_div(256, (i + 0.01)),
                    256 % (i + 0.01),
                    256 - old_div(256, (i + 0.01)),
                )
                if i < 256
                else (170, i / 256.0, i % 256),
            )
        )
...
def getQwtPlot(series, xIsTime=False):
    ...
    if xIsTime:
        qpt.setAxisScaleDraw(qpt.xBottom, getQwtTimeScaleDraw())
    return qpt
...
class QDialogWidget(Qt.QDialog):
    ...
    def setWidget(self, widget, accept_signal=None, reject_signal=None):
        ...
        self._widget = widget
        if accept_signal:
            self.connect(widget, Qt.SIGNAL(accept_signal), self.accept)
        if reject_signal:
            self.connect(widget, Qt.SIGNAL(reject_signal), self.reject)
        self.updateGeometry()
        return self
...
class QColorDictionary(SortedDict, Singleton):
    ...
    def __init__(self, sort=False):
        ...
        for n in Qt.QColor.colorNames():
            ...
            if not sum(rgb):
                ...
                v.getRgb()[:3] == c.getRgb()[:3] for v in list(colors.values())
            ):
                colors[n] = c
            else:
                last[n] = c
        if sorted:
            ...
            dark = [
                ...
                if max(colors[n].getRgb()[:3]) < 0.25 * (255 * 3)
            ]
            light = [
                (n, colors.pop(n))
                for n in list(colors.keys())
                if min(colors[n].getRgb()[:3]) > 0.75 * (255 * 3)
            ]
            self.update(
                sorted(
                    (
                        (n, colors.pop(n))
                        for n in list(colors.keys())
                        if all(c in (0, 255) for c in colors[n].getRgb()[:3])
                    ),
            ...
            x = len(colors)
            for i in range(x):
                n = sorted(list(colors.items()), key=lambda t: -self.get_diff_code(*t))[
                    0
                ][0]
                self[n] = colors.pop(n)
            self.update(light)
    ...
    def show(self):
        ...
        for n, c in list(self.items()):
            l = Qt.QLabel(
                "%s: %s, %s" % (n, str(c.getRgb()[:3]), self.distances.get(n, None))
            )
            l.setStyleSheet(
                "QLabel { background-color: rgba(%s) }"
                % (",".join(str(x) for x in c.getRgb()))
            )
    ...
    def get_diff_code(self, name, color):
        ...
        self.distances[name] = [
            sum(
                (abs(x - y) if abs(x - y) > MinDiff else 0)
        ...
        return (
            sum(self.distances[name])
            if not any(d < MinDiff * 1.5 for d in self.distances[name])
            else 0
        )
...
class QCustomPushButton(Qt.QPushButton):
    ...
    def setBold(self, bold):
        if bold:
            ...
            self.label.setText(
                str(self.label.text()).replace("<b>", "").replace("</b>", "")
            )
...
class QCustomTabWidget(Qt.QWidget):
    ...
    def __init__(self, parent=None, icon_builder=None):
        ...
        self.rcount = 0
        self.connect(
            self.stackWidget, Qt.SIGNAL("currentChanged(int)"), self.emitCurrentChanged
        )
    ...
    def addTab(self, widget, label, icon=None, width=50, height=60):
        ...
        if label in self.buttons:
            print(
                "======> Button(%s) already exists!,\n\t %s rejected!!"
                % (label, widget)
            )
        else:
            if icon is None and self.icon_builder is not None:
                try:
                    icon = self.icon_builder(label)
                except:
            ...
            self.connect(
                button,
                Qt.SIGNAL("pressed()"),
                lambda w=widget, l=label: self.setCurrentWidget(w, l),
            )

    def insertTab(self, index, page, label):
        return self.addTab(page, label)

    def insertTab(self, index, page, icon, label):
        return self.addTab(page, label, icon)
...
class TauFakeEventReceiver(object):
    def event_received(self, source, type_, value):
        print(
            "%s: Event from %s: %s(%s)"
            % (time.ctime(), source, type_, shortstr(getattr(value, "value", value)))
        )
...
def getColorsForValue(value, palette=getattr(colors, "QT_DEVICE_STATE_PALETTE", None)):
    ...
    if value is None:
        ...
        if value.type == PyTango.ArgType.DevState:
            ...
            bg_brush, fg_brush = colors.QT_DEVICE_STATE_PALETTE.qbrush(
                (PyTango.DevState.FAULT, PyTango.DevState.ON)[value.value]
            )
        else:
            bg_brush, fg_brush = colors.QT_ATTRIBUTE_QUALITY_PALETTE.qbrush(
                value.quality
            )
    else:
...
class TauColorComponent(TaurusBaseComponent):
    ...
    def handleEvent(self, evt_src=None, evt_type=None, evt_value=None):
        ...
        self.info(
            "In TauColorComponent(%s).handleEvent(%s,%s)"
            % (self.__name, evt_src, evt_type)
        )
    ...
    def updateStyle(self):
        ...
        try:
            ...
            if value is not None:
                value = value.value
                self.info(
                    "In TauColorComponent(%s).updateStyle(%s)" % (self.__name, value)
                )
                self._currBgColor, self._currFgColor = getColorsForValue(value)
            else:
                self.info(
                    "In TauColorComponent(%s).updateStyle(%s), using defaults"
                    % (self.__name, value)
                )
        except:
            self.info(
                "TauColorComponent(%s).updateStyle(): Unable to getColorsForValue(%s):\n%s"
                % (self.__name, value, traceback.format_exc())
            )
...
class QSignalHook(Qt.QObject):
    ...
    def setModel(self, model):
        ...
        try:
            ...
            self._model = model
            self._new_model = (
                self.function(model) if self.function is not None else model
            )
    ...
    def fire_targets(self, targets=[]):
        ...
        for ts in targets:
            try:
                ts.fireEvent(
                    ts,
                    taurus.core.TaurusEventType.Change,
                    ts.getModelValueObj(cache=False),
                )
            except:
                print(ts)
                traceback.print_exc()
...
class QWorker(Qt.QThread):
    ...
    def __init__(
        self, parent=None, name="", queue=None, method=None, cursor=None, sleep=5000
    ):
        """
        ...
        self.emitter.setParent(
            Qt.QApplication.instance()
        )  # Mandatory!!! if parent is set before changing thread it could lead to segFaults!
        self._done = 0
    ...
    def getDone(self):
        ...
        return (
            float(self._done) / (self._done + self.getQueue().qsize())
            if self._done
            else 0.0
        )
    ...
    def _doSomething(self, args):
        ...
        if method:
            try:
                ...
                self.log.error(
                    "At TauEmitterThread._doSomething(): %s" % traceback.format_exc()
                )
    ...
    def __next__(self):
        queue = self.getQueue()
        (queue.empty() and self.log.info or self.log.debug)(
            "At TauEmitterThread.next(), %d items remaining." % queue.qsize()
        )
        try:
            if not queue.empty():
...
try:
    ...
    TAURUS_ATTR_MIME_TYPE = (
        TAURUS_DEV_MIME_TYPE
    ) = TAURUS_MODEL_MIME_TYPE = TAURUS_MODEL_LIST_MIME_TYPE = TEXT_MIME_TYPE
...
def Dropable(QtKlass):
    ...
    class DropableQtKlass(QtKlass):  # ,Decorated):
            ...
            if not getattr(self, "_supportedMimeTypes", []):
                try:
                    self.setSupportedMimeTypes(
                        [
                            self.TAURUS_DEV_MIME_TYPE,
                            self.TAURUS_ATTR_MIME_TYPE,
                    ...
                    self._dlog(
                        "Unable to import TAURUS MIME TYPES: %s"
        ...
        def getDropEventCallback(self, mimetype=None):
            ...
            return getattr(self, "_dropeventcallback", None) or getattr(
                self, "setText", None
            )
                    ...
                    try:
                        if data.strip():
                            try:
                                ...
                                self._dlog(
                                    "Invalid data (%s,%s) for MIMETYPE=%s"
                                    % (repr(mimeData), repr(data), repr(mtype))
                                )
                                traceback.print_exc()
                                return None
                    except:
                        self.info(traceback.warning_exc())
                except:
                    self.debug("\tNot dropable data (%s)" % (data))
    ...
    class DoubleClickableQtKlass(QtKlass):  # ,Decorated):
        ...
        def mouseDoubleClickEvent(self, event):
            if getattr(self, "_doubleclickhook", None) is not None:
                ...
                try:
                    QtKlass.mouseDoubleClickEvent(self)
                except:
                    pass
    ...
    class DraggableQtKlass(QtKlass):  # ,Decorated):
        ...
        def getDragEventCallback(self):
            ...
            if not getattr(self, "_drageventcallback", None):
                self.setDragEventCallback(
                    lambda s=self: str(s.text() if hasattr(s, "text") else "")
                )
        ...
        def mousePressEvent(self, event):
            ...
            tracer(
                "In Draggable(%s,%s).mousePressEvent"
                % (name, self._qt_klass__.__name__)
            )
...
class QOptionChooser(Qt.QDialog):
    ...
    def launch(self):
        cmd = (
            " ".join(
                map(str, [self.command[0], self.combo.currentText()] + self.command[1:])
            )
            + " &"
        )
        print(cmd)
...
def QConfirmAction(
    ...
    options=Qt.QMessageBox.Ok | Qt.QMessageBox.Cancel,
):
    """
    ...
    """
    if Qt.QMessageBox.Ok == QtGui.QMessageBox.warning(
        parent, title, message, QtGui.QMessageBox.Ok | QtGui.QMessageBox.Cancel
    ):
        action()
...
class QTextBuffer(Qt.QDialog):
    ...
    def __init__(self, title="TextBuffer", maxlen=1000, parent=None):
        ...
        self._savebutton.connect(
            self._savebutton, Qt.SIGNAL("clicked()"), self.saveLogs
        )
        self.layout().addWidget(self._savebutton)

    def append(self, text):
        if self._buffer and text == self._buffer[-1]:
            test = "+1"
        ...
        self._count.setText(
            "%d/%d" % (min((self._maxlen, len(self._buffer))), self._maxlen)
        )
    ...
    def saveLogs(self):
        ...
        if fname:
            open(fname, "w").write(str(self.text()))
...
class QDropTextEdit(Qt.QTextEdit):
    ...
    def mouseDoubleClickEvent(self, event):
        if self.double_click_hook is not None:
            ...
            try:
                Qt.QTextEdit.mouseDoubleClickEvent(self)
            except:
                pass
    ...
    def dropEvent(self, event):
        ...
        if any(s in event.mimeData().formats() for s in self.mimeTypes()):
            mtype = self.handleMimeData(
                event.mimeData(), self.addModels
            )  # lambda m:self.addModels('^%s$'%m))
            event.acceptProposedAction()
        else:
...
class QGridTable(Qt.QFrame):  # Qt.QFrame):
    ...
    def setHorizontalHeaderLabels(self, labels):
        ...
        for i, l in enumerate(labels):
            ...
            if ql not in self._widgets:
                self._widgets.append(ql)
...
class QDictToolBar(Qt.QToolBar):
    ...
    def build_action(self, name, icon, action):
        ...
        if action:
            Qt.QObject.connect(qaction, Qt.SIGNAL("triggered()"), action)
        return qaction

    def set_toolbar(self, toolbar):
        """The toolbar argument must be dictionary {name:(icon,action)} or a list of ('Name','icon.jpg',action) tuples."""
        if hasattr(toolbar, "items"):
            toolbar = [(k, v[0], v[1]) for k, v in list(toolbar.items())]
    ...
    def add_to_main_window(self, MainWindow, where=Qt.Qt.TopToolBarArea):
        try:
            MainWindow.addToolBar(where, self)
        except:
...
def GetFramedTaurusValue(model=None, label=True, hook=None):
    ...
    if hook:
        frame.taurusvalue.connect(
            frame.taurusvalue, QtCore.SIGNAL("itemClicked(QString)"), hook
        )
    if label:
...
class QTableOnWidget(Qt.QWidget):
    ...
    def setCells(self, data=None):
        ...
        if data and not isSequence(data[0]):
            data = [fandango.toList(r) for r in data]
...
class QEvaluator(Qt.QWidget):
    ...
    def __init__(
        self, parent=None, model="", filename="~/.qeval_history"
    ):  #'import fandango'):
        ...
        try:
            ...
            self._locals.update(
                [
                    (k, v)
                    for k, v in list(fandango.functional.__dict__.items())
                    if isCallable(v)
                ]
            )
            ...
            self._locals.update(
                (k, getattr(fandango.web, k)) for k in ("table", "bold", "color")
            )
            self._modules = {}
            self._instances = {}
            self.history = []
            self.filename = (
                filename.replace("~", os.getenv("HOME"))
    ...
    def dir_module(self, *args):
        """
        This method gets first argument that is an string as filter
        Then gets the next arguments that are not string as module
        It applies a filter on dir(module) and returns matching methods
        ...
        if "*" not in f and not f.startswith("_"):
            f = "*%s*" % f
        ks = fandango.first(
            (dir(a) for a in args if not isString(a)), list(self._locals.keys())
        )
        return sorted(
            (
                t
                for t in fandango.matchAll(f, ks)
                if f.startswith("_") or not t.startswith("__")
            ),
            key=lowstr,
        )
    ...
    def help(self, m=None):
        try:
            if m:
                ...
                return "%s\n%s" % (
                    inspect.getsource(m).split(":")[0].replace("\n", ""),
                    m.__doc__,
                )
            else:
                return self.__doc__
    ...
    def setModel(self, model=None):
        ...
        try:
            if model:
                ...
                self.combo.clear()
                self.combo.addItems(sorted(self.commands, key=lowstr))
            else:
                self.model = None
                self.target = None
                self.commands = []
    ...
    def setup_ui(self):
        ...
        self.result.setHtml(
            "<html><head><style>%s</style></head><body></body></html>" % (self.css)
        )
        ...
        self.connect(
            self.shortcut, Qt.SIGNAL("activated()"), self.button.animateClick
        )  # self.execute)
        self.check = Qt.QCheckBox("Append")
        self.check.setChecked(True)
        self.setLayout(Qt.QGridLayout())
        self.layout().addWidget(
            Qt.QLabel("Write python code or load(module) and select from the list"),
            0,
            0,
            1,
            4,
    ...
    def multiline_exec(self):
        [
            self.execute(l.strip(), "")
            for l in str(self.mledit.toPlainText()).split("\n")
        ]
    ...
    def execute(self, cmd=None, args=None):
        ...
        if not cmd and not args:
            cmd, args = str(self.combo.currentText()), str(
                self.args.currentText()
            )  # text())
        print("execute: %s(%s)" % (cmd, args))
        if self.filename:
            try:
                open(self.filename, "a").write("%s.%s(%s)\n" % (self.model, cmd, args))
            except:
                pass
        try:
            ...
            print(o)
        txt = "\n".join(
            list(map(str, list(o.items())))
            if hasattr(o, "items")
            else (
                [(str(t).strip("\n\r ") or getattr(t, "__name__", "?")) for t in o]
                if isSequence(o)
                else [str(o)]
            )
        )
        isHtml = (
            txt.strip().startswith("<") and txt.strip().endswith(">") and "</" in txt
        )
        txt = (
            ("execute: %s(%s) => " % (cmd, args))
            + type(o).__name__
            + ":\n"
            + "-" * 80
            + "\n"
        ...
        if self.model is None and not any(
            i > 1 and cmd == str(self.combo.itemText(i))
    ...
    def printf(self, txt, append=True, highlight=False, curr=""):
        if append:
            try:
                ...
                curr = ""
        txt = (
            self.highlight(txt) if (highlight is True) else txt
        )  # highlight=1 can still be used to force highlighting
        if (self.css and "<style>" not in curr) or not curr or not append:
            txt = "<html><head><style>%s</style></head><body>%s</body></html>" % (
                self.css,
                txt,
            )
        else:
        ...
        try:
            self.result.keyPressEvent(
                Qt.QKeyEvent(Qt.QEvent.KeyPress, Qt.Qt.Key_End, Qt.Qt.NoModifier)
            )
    ...
    def main(args=None):
        ...
        if args and os.path.isfile(args[0]):
            args = list(filter(bool, (l.strip() for l in open(args[0]).readlines())))
