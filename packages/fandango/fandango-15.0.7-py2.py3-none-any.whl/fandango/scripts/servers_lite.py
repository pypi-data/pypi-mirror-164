...
def get_device_attribute_properties(device, attribute):
    props = get_database().get_device_attribute_property(device, {attribute: "*"})[
        attribute
    ]
    return dict((k, list(p)) for k, p in list(props.items()))


def clean_device_attribute_properties(device):
    attrs = get_device_attributes(device)
    props = dict(
        (a, list(get_device_attribute_properties(device, a).keys())) for a in attrs
    )
...
def clean_device_properties(device):
    ...
    get_database().delete_device_property(device, list(props.keys()))


def remove_device_server(server, clean_db=True):
    s = Astor(server)
    assert len(s), "ServerNotFound!"
    assert len(s) == 1, "RegexpNotAllowed:%s" % list(s.keys())
    if clean_db:
        assert None not in list(s.states().values()), "DeviceNotRunning"
    devs = s.get_all_devices()
    for device in devs:
        try:
            if clean_db:
                clean_device_attribute_properties(device)
        except:
    ...
    for device in devs:
        if clean_db:
            clean_device_properties(device)
    ...
    def __default_factory__(self, dev_name):
        ...
        if dev_name not in list(self.keys()):
            try:
                ...
                dev = (
                    attrklass
                    if str(dev_name).count("/") == (4 if ":" in dev_name else 3)
                    else devklass
                )(dev_name)
...
class TServer(Object):
    ...
    def update_level(self, host, level=0):
        ...
        if type(level) is str:
            level = level.strip()
        if self.controlled:
            self.host, self.level = host.split(".")[0].strip(), int(level or 0)
        else:
...
class ServersDict(CaselessDict, Object):
    ...
    def __init__(
        ...
        self.db = (
            PyTango.Database()
            if not tango_host
            else PyTango.Database(*(tango_host.split(":")))
        )
    ...
    def get_db_device(self):
        ...
        dev = [
            d
            for d in list(self.proxies.keys())[:]
            if d.lower().startswith("sys/database/")
        ]
        if dev:
    ...
    def get_all_devices(self):
        ...
        return list(chain(*[s.get_device_list() for s in list(self.values())]))
    ...
    def start_servers(self, servers_list=None, host="", wait=10.0):
        ...
        for s_name in servers_list:
            ...
            if not host:
                try:
                    if s_name in self:
                        self[s_name].init_from_db(self.db)
        ...
        for level, host, s_name in sorted(full_servers_list):
            ...
            try:
                ...
                ct = int(old_div(wait, 3))
                dp = self.proxies.get(
                    s_name if "dserver" in s_name else "dserver/" + s_name
                )
                while ct > 0:
                    event.wait(3)
                    try:
            ...
            if host and "tango/admin/%s" % host in self.proxies:
                try:
                    self.proxies["tango/admin/%s" % host].UpdateServersInfo()
                except Exception as e:
                    print("Unable to update %s Starter: %s" % (host, e))
        return done
    ...
    def restart_servers(self, servers_list=None, wait=5.0):
    ...
    def set_server_level(self, server_name, host, level):
        ...
        print(
            "DUE TO export_device ISSUES THIS METHOD IS NOT SUITABLE FOR ADDING NEW SERVERS, USE devs2starter.jy INSTEAD"
        )
