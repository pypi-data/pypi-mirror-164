...
from fandango.functional import isString
...
class TangoCommand(object):
    ...
    def __init__(
        ...
        if process:
            ...
            self.process = fandango.threads.WorkerThread(
                device + "/" + command, process=True
            )
        else:
            self.process = None
        pass
    ...
    def _parse_feedback(self, feedback):
        ...
        elif isString(feedback):
            if "/" in feedback:
                device, target = (
                    feedback.rsplit("/", 1)
                    if feedback.count("/") >= (4 if ":" in feedback else 3)
                    else (feedback, state)
                )
            else:
                device, target = self.device, feedback
            proxy = TangoCommand.Proxies[device]
            attrs, comms = proxy.get_attribute_list(), [
                cmd.cmd_name for cmd in proxy.command_list_query()
            ]
            if inCl(target, comms):
                self.feedback = lambda d=device, c=target: TangoCommand.Proxies[
                    d
                ].command_inout(c)
            elif inCl(target, attrs):
                self.feedback = (
                    lambda d=device, a=target: TangoCommand.Proxies[d]
                    .read_attribute(a)
                    .value
                )
            else:
                raise TangoCommand.CommandException(
                    "UnknownFeedbackMethod_%s" % feedback
                )
    ...
    def execute(
        ...
        if isString(self.command):
            if not asynch:
                if args:
                    result = self.proxy.command_inout(self.command, args)
                else:
                    result = self.proxy.command_inout(self.command)
            else:
                self.trace("Using asynchronous commands")
                if args:
                    cid = self.proxy.command_inout_asynch(self.command, args)
                else:
                    cid = self.proxy.command_inout_asynch(self.command)
        ...
        if feedback is not None:
            ...
            tt, tw = min((timeout, (t1 - t0 + wait))), max((0.025, wait / 10.0))
            now, got = t1, None
            while True:
                self.event.wait(tw)
                now = time.time()
                ...
                if not wait or expected is None or got == expected:
                    self.trace(
                        "Feedback (%s) obtained after %s s" % (got, time.time() - t0)
                    )
                    break
                if now > (t0 + timeout):
                    raise TangoCommand.CommandTimeout(str(self.timeout * 1000) + " ms")
            ...
            if expected is None:
                ...
                self.trace(
                    "Result (%s,%s==%s) verified after %s s"
                    % (result, got, expected, time.time() - t0)
                )
