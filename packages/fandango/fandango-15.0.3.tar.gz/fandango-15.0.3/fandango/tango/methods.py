...
def add_new_device(server, klass, device):
    ...
    for c in server + klass + device:
        if re.match("[^a-zA-Z0-9\-\/_\+\.\:]", c):
            raise_(Exception, "CharacterNotAllowed('%s')" % c)
...
def get_device_info(dev, db=None):
    ...
    if ":" in dev:
        db = get_database(dev)
        dev = get_normal_name(dev)

    try:
        dd = get_database_device(db=db)
        dev = get_normal_name(get_device_name(dev))
        vals = dd.DbGetDeviceInfo(dev)
        di = Struct(
            [
                (k, v)
                for k, v in zip(
                    ("name", "ior", "level", "server", "host", "started", "stopped"),
                    vals[1],
                )
            ]
        )
        ...
        raise Exception(
            "get_device_info(%s,%s,%s): %s" % (dev, db, vals, traceback.format_exc())
        )
...
def get_server_version(target):
    return get_device(target).info().server_version


def get_device_for_alias(alias):
    """returns the device name for a given alias"""
    try:
        return get_database().get_device_alias(alias)
...
def get_alias_dict(exp="*"):
    ...
    tango = get_database()
    return dict(
        (k, tango.get_device_alias(k)) for k in tango.get_device_alias_list(exp)
    )
...
def attr2str(attr_value):
    ...
    if hasattr(attr_value, "value"):
        return "%s%s(%s)" % (
            att_name,
            type(attr_value.value).__name__,
            attr_value.value,
        )
    else:
...
def get_real_name(dev, attr=None):
    ...
    for a in get_device_attributes(dev):
        if matchCl(attr, a):
            return dev + "/" + a
        if matchCl(attr, get_attribute_label(dev + "/" + a)):
            return dev + "/" + a
    return None
...
def parse_tango_model(name, use_host=False, fqdn=None, *args, **kwargs):
    ...
    if m and "/" not in name[m.end() :]:  # Name should end at attribute
        ...
        gd = m.groupdict()
        r.device = "/".join([s for s in gd["device"].split("/") if ":" not in s])
        if gd.get("attribute"):
...
def get_polled_attrs(device, others=None):
    ...
    if isSequence(device):
        return CaselessDict(
            zip(map(str.lower, device[::2]), map(float, device[1::2]))
        )
    elif isinstance(device, DeviceProxy):
        attrs = device.get_attribute_list()
        periods = [(a.lower(), int(device.get_attribute_poll_period(a))) for a in attrs]
...
def get_polling_stats(device, brief=False, full=False):
    """
    ...
    """
    dp = get_device(device)
    stats = CaselessDefaultDict(fn.Struct)
    pst = dp.polling_status()
    for st in pst:
        st = [s.rsplit("=", 1) for s in st.split("\n")]
        name = st[0][-1].strip()
        stats[name].polling = [
            float(s[-1]) for s in st if s[0].startswith("Polling period")
        ][0]
        stats[name].read_ms = [
            float(s[-1]) for s in st if s[0].startswith("Time needed")
        ][0]
        stats[name].periods = [
            list(map(float, s[-1].split(","))) for s in st if "last records" in s[0]
        ][0]

    p = min(t.polling for t in list(stats.values()) if t.polling)
    s = sum(t.read_ms for t in list(stats.values()) if t.polling)
    r = {"read_time_ms": s, "polling": p, "usage": s and old_div(s, (p or s))}
    print("%s totals: %s" % (device, r))
    ...
    if brief:
        ...
        for a in dp.get_attribute_list():
            if a in stats:
                continue
            stats[a].polling = 0
            stats[a].read_ms = fn.timeit(dp.read_attribute, [a], number=1)
            stats[a].periods = []
...
def get_device_stats(device):
    return get_polling_stats(device, brief=False, full=True)
...
def get_device_labels(target, filters="*", brief=True):
    ...
    if isString(target):
        ...
        d, target = target, target.name()
    if brief:
        db = get_database()
        attrlist = PyTango.StdStringVector()
        db.get_device_attribute_list(target, attrlist)
    else:
        attrlist = d.get_attribute_list()

    for a in attrlist:
        l = (
            get_attribute_label(target + "/" + a, use_db=True)
            if brief
            else d.get_attribute_config(a).label
        )
        if (not filters or any(map(matchCl, (filters, filters), (a, l)))) and (
            not brief or l != a
        ):
            labels[a] = l
...
def get_attribute_info(model, attribute=None):
    """
    ...
    if isString(model):
        if attribute is None:
            model, attribute = model.rsplit("/", 1)
        model = get_device(model)

    ai = model.get_attribute_config(attribute)
    types = [CmdArgType.values.get(ai.data_type), ai.data_format, ai.writable]

    if ai.max_dim_x > 1:
        types.append(ai.max_dim_x)
    if ai.max_dim_y > 0:
        types.append(ai.max_dim_y)
...
def set_attribute_config(device, attribute, config, events=True, verbose=False):
    ...
    """
    print("fandango.tango.set_attribute_config(%s,%s,%s)" % (device, attribute, config))
...
def check_attribute_events(
    model, ev_type=None, verbose=False, asynch=False, keep=False, wait=0.01
):
    """
    ...
    try:
        ...
        ev_type = fandango.notNone(
            ev_type, (EventType.CHANGE_EVENT, EventType.ARCHIVE_EVENT)
        )
        result = dict.fromkeys(toSequence(ev_type))

        if check_device(dp):
            for ev_type in list(result.keys()):
                try:
...
def set_attribute_events(
    ...
    if any(e is not None for e in (arch_rel_event, arch_abs_event, arch_per_event)):
...
def set_attribute_value(attr=None, value=None, date=None, quality=None):
    """
    helper method for internal command reusage:
        replace attr.set_value(value) by
            return set_attribute_value(attr,value)
    """
    try:
        if attr is not None:
            if date is not None:
                attr.set_value_date_quality(
                    value, date, quality or PyTango.AttrQuality.ATTR_VALID
                )
            else:
...
def parse_black_box_row(row):
    token = "[0-9a-zA-Z]+"
    rip = "(" + token + "\." + token + "\." + token + "(P:" + "\." + token + ")?)"
    pid = "(?:PID[\ =])([0-9]+)"
    date, cmd = list(map(str.strip, row.split(" : ", 1)))
...
def get_black_box(device, n=100):
    """
    ...
    data = sorted((t["time"], t) for t in map(parse_black_box_row, bb))
    j, t, l = 0, 0, ""
    for i, row in enumerate(data[1:]):
        r = row[0] - data[i][0]  # time between commands
        if r > t:
            j, t, l = i, r, row[1]["row"]
        row[1]["rtime"] = r
        row[1]["load"] = r if r > 0 else 0
    print("slower command (%f s) was: %s" % (t, l))
    return dict(data)


parse_black_box = get_black_box  # backwards compatibility
...
def get_attribute_property(device, attribute, prop, db=None):
    ...
    return (db or get_databse()).get_device_attribute_property(device, attribute, prop)[
        prop
    ]
...
def get_devices_properties(device_expr, properties, hosts=[], port=10000):
    ...
    """
    expr = device_expr
    if not isSequence(properties):
        properties = [properties]
    get_devs = lambda db, reg: [
        d
        for d in db.get_device_name("*", "*")
        if not d.startswith("dserver") and matchCl(reg, d)
    ]
    if hosts:
        tango_dbs = dict(
            ("%s:%s" % (h, port), PyTango.Database(h, port)) for h in hosts
        )
    else:
        tango_dbs = {get_tango_host(): get_database()}
    return dict(
        ("/".join((host, d) if hosts else (d,)), db.get_device_property(d, properties))
        for host, db in list(tango_dbs.items())
        for d in get_devs(db, expr)
    )
...
def property_undo(dev, prop, epoch, db=None):
    ...
    if valids and news:
        ...
        print(
            "No property values found for %s/%s before %s"
            % (dev, prop, time2str(epoch))
        )
    elif not news:
        print("Property %s/%s not modified after %s" % (dev, prop, time2str(epoch)))
...
EXTENSIONS = {
    "@COPY:": _copy_extension,
    "@FILE:": _file_extension,
    "@ATTR:": _attr_extension,
}


def check_property_extensions(
    prop,
    value,
    db=None,
    ...
    if (
        (not filters or prop in filters)
        and fandango.isSequence(value)
        ...
        for v in value:
            try:
                ...
                ext, f = first(
                    [(e, f) for e, f in list(extensions.items()) if v.startswith(e)]
                    or [(None, None)]
                )
                if ext:
                    parsed.extend(f(prop, v))
                else:
                    parsed.append(v)
            except:
                print(
                    "check_property_extensions(%s,%s): %s"
...
def get_server_pid(server):
    try:
        ...
        if not pid:
            pid = fandango.linos.get_process_pid(
                server.replace("/", " "), exclude="javaArch|grep|screen|0:00"
            )
        return pid
...
def check_attribute(attr, readable=True, timeout=0, brief=False, trace=False, expire=0):
    ...
    try:
        if hasattr(attr, "read"):
            ...
            if att not in [
                str(s).lower() for s in DeviceProxy(dev).get_attribute_list()
            ]:
                raise Exception("AttrNotFound")
            proxy = readable and AttributeProxy(attr)
        ...
        try:
            ...
            if trace:
                traceback.print_exc()
            return e if (readable and not brief) else None

    except Exception as e:
        if trace:
            traceback.print_exc()
        return e if not brief else None
...
def read_attribute(attr, timeout=0, full=False):
    ...
    return check_attribute(attr, timeout=timeout, readable=True, brief=not full)
...
def check_device_list(devices, attribute=None, command=None):
    ...
    for host, servs in list(hosts.items()):
        if not check_host(host):
            print(
                "Host %s failed, discarding %d devices"
            ...
            for server, devs in list(servs.items()):
                if not check_device("dserver/%s" % server, full=False):
                    print(
                        "Server %s failed, discarding %d devices" % (server, len(devs))
                    )
                    result.update((d, False) for d in devs)
                else:
                    for d in devs:
                        result[d] = check_device(
                            d, attribute=attribute, command=command, full=False
                        )
    return result
...
def read_internal_attribute(device, attribute):
    ...
    if isString(device):
        device = get_internal_devices().get(
            device,
            (
                getattr(attribute, "parent", None)
                or get_device(device, keep=True)
            ),
        )

    attr = (
        attribute
        if isinstance(attribute, fakeAttributeValue)
    ...
    if aname == "state":
        ...
        if isProxy:
            print(
                "fandango.read_internal_attribute(): "
                "calling DeviceProxy(%s).read_attribute(%s)" % (attr.device, attr.name)
            )
            ...
            for s in dir(device):
                if s.lower() == "is_%s_allowed" % aname:
                    allow_method = s
                if s.lower() == "read_%s" % aname:
                    read_method = s
            print(
                "fandango.read_internal_attribute():"
                " calling %s.is_%s_allowed()" % (attr.device, attr.name)
            )
            is_allowed = (not allow_method) or getattr(device, allow_method)(
                AttReqType.READ_REQ
            )
            if not is_allowed:
                attr.throw_exception(
                    "%s.read_%s method is not allowed!!!" % (device, aname)
                )
                ...
                if isDyn:
                    ...
                    attr.throw_exception(
                        "%s.read_%s method not found!!!"
                        % (device, aname, [d for d in dir(device)])
                    )
            else:
                # Returning valid values
                msg = "fandango.read_internal_attribute():" " calling %s.read_%s()" % (
                    attr.device,
                    aname,
                )
                print(msg)
    ...
    if args:
        if args and args[0] == "help":
            ...
            try:
                if callable(f):
                    args = [
                        fandango.trial(lambda: eval(a) if isString(a) else a, excepts=a)
                        for a in args[1:]
                    ]
                    print(f(*args))
                return 0
