...
import sys, time, traceback, pickle, urllib.request, urllib.parse, urllib.error
import fandango
...
try:
    ...
    PyQt4 = Qt = QColor = fandango.Object()
...
def get_url(url, filename=None):
    # downloads a url content
    s = urllib.request.urlretrieve(url, filename=filename)
    return s[0]
...
title = lambda s, n=1: '<a name="%s"><h%d>%s</h%d></a>' % (
    iname(s),
    n,
    s,
    n,
)  # <a> allows to make titles linkable
title1 = lambda s: "<h1>%s</h1>" % s

row, cell = (lambda s: "<tr>%s</tr>" % s), (lambda s: "<td>%s</td>" % s)
table = (
    lambda s: "<table border=1>\n"
    + "\n".join(
        [
            row(
                "".join(
                    [
                        (
                            cell("%s" % c)
                            if (len(s) == 1 or j or (len(toList(r)) < 3 and i))
                            else cell(bold("%s" % c))
                        )
...
def dicts2table(dcts, keys=None, formatter=None):
    ...
    """
    if not keys:
        keys = sorted(set(k for dct in dcts for k in list(dct.keys())))
    if not formatter:
...
def dict2dict2table(seq, keys=None, formatter=None):
    ...
    """
    if not keys:
        keys = [""] + sorted(set(k for v in list(seq.values()) for k in list(v.keys())))
    if not formatter:
        formatter = lambda s: s
    lines = [
        keys,
...
def jqplot(title, ats):
    ...
    series = "series:[\n%s\n]" % ",\n".join(
        [
            serie.replace("$ATTR", a).replace(
                "$COLOR", "rgba(%d,%d,%d,1)" % all_colors[i][:3]
            )
            for i, a in enumerate(ats)
        ]
    )
    # data = """[[[1, 2],[3,5.12],[5,13.1],[7,33.6],[9,85.9],[11,219.9]]]"""
    data = str([list([fandango.time2str(t[0]), t[1]] for t in vals[k]) for k in ats])
    return (
        jqp_template.replace("$DATA", data)
