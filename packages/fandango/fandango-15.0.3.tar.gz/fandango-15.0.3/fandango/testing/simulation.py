...
def load(
    tango_host, instance, devices, replace={}, overwrite=False, def_class="SimulatorDS"
):
    """
    ...
    for dd, df in list(filenames.items()):
        ...
        if data["dev_class"] == "PyAlarm":
            ...
            for k, v in sorted(vals.items()):
                ...
                attrprops[dd][k] = dict(
                    (p, data["attributes"][k].get(p, ""))
                    for p in ("format", "label", "unit", "min_alarm", "max_alarm")
                )

            ft.put_device_property(dd, "DynamicAttributes", dynattrs)
            try:
                ft.get_database().put_device_attribute_property(
                    dd, dict((k, v) for k, v in list(attrprops[dd].items()) if v)
                )
            except:
                fn.time.sleep(3.0)
