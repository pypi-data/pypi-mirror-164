...
def api_request(r_type, url, auth, data=None, api_alert=None, api_alert_info=None, status_code=[1]):
    try:
        ...
        elif r_type not in ["get", "post", "put"]:
        ...
        res = {"status": False, "response": None, "exc": str(traceback.format_exc()), "time": datetime.now().timestamp() - s_time, "err_msg": "", "exc": ""}
        if api_alert is not None:
            api_alert["e_time"] = datetime.now().timestamp()
        return res
