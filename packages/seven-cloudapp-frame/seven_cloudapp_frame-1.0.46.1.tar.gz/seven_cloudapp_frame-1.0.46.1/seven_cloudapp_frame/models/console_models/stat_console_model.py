...
@LastEditTime: 2022-08-24 18:01:43
            ...
            try:
                ...
                if len(stat_queue_list) > 0:
                    for stat_queue in stat_queue_list:
                        try:
                            stat_log_model = StatLogModel(db_transaction=db_transaction)
            ...
            try:
                ...
                try:
                    ...
                    if stat_report_total == 0:
                        ...
                        stat_report.act_id = stat_queue_dict["act_id"]
                        stat_report.module_id = stat_queue_dict["module_id"]
                        stat_report.user_id = stat_queue_dict["user_id"]
                        stat_report.open_id = stat_queue_dict["open_id"]
                        stat_report.key_name = field_name
                        stat_report.key_value = stat_queue_dict["key_value"]
                        stat_report.create_date = create_date
                        stat_report.create_year = create_year_int
                        stat_report.create_month = create_month_int
                        stat_report.create_day = create_day_int
                        stat_user_report_model.add_entity(stat_report)

                    db_transaction.begin_transaction()
                    key_value = stat_queue_dict["key_value"]
                    stat_user_report_model.update_table(f"{field_name}={field_name}+{key_value}", stat_report_condition, params=stat_report_param)
                    stat_user_log_model.add_entity(stat_log)
                    result,message = db_transaction.commit_transaction(True)
                    if result == False:
                        raise Exception("执行事务失败", message)
...
class StatUserDataWork(TimingWork):
    ...
    def clear_log(self):
        ...
        while True:
            is_del = stat_log_model.del_entity("create_day<%s", params=[ SevenHelper.get_now_day_int(-24 * 30)], limit="100")
            if is_del == False:
                break
    ...
    def clear_report(self):
        ...
        while True:
            stat_user_report_list = stat_user_report_model.get_dict_list(where="create_day<%s", order_by="id asc", limit="100", params=[SevenHelper.get_now_day_int(-24 * 30 * space_day)], limit="100")
