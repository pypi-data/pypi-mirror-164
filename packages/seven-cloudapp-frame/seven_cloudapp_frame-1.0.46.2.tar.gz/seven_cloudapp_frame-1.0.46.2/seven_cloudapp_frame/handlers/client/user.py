...
@LastEditTime: 2022-07-27 18:16:54
...
class LoginHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if RiskManageHelper.check_current_limit(app_id, app_info_dict["current_limit_count"]) == True:
        ...
        invoke_result_data = user_base_model.save_user_by_openid(app_id, act_id, open_id, user_nick, avatar, union_id, login_ip)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
...
class GetCouponPrizeHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        invoke_result_data = order_base_model.get_coupon_prize(app_id, act_id, user_id, user_prize_id, is_log)
        if invoke_result_data.success == False:
            return self.response_json_error(invoke_result_data.error_code, invoke_result_data.error_message)
        else:
