...
def benchmark_cpu(executable: Callable[[List[dict], bool], Iterable[dict]], depth: int = 2, validate: bool = False):
    ...
    # Execute 10 times to get an initial estimate to make the run time 1 second each INCLUDING making the data
    start = float(time())
    ...
    n = round(1.0 / estimate)
    print(f'Iterations: {n}, time reported in milliseconds:')

    best = float('inf')
    for i in range(10):
        data = [make_stuff_data(depth) for _ in range(n)]
        start = time()
        for _ in executable(data, validate):
            pass
