...
from pyzeebe_fork.errors import (
    ActivateJobsRequestInvalidError,
    JobAlreadyDeactivatedError,
    JobNotFoundError,
)
from pyzeebe_fork.grpc_internals.grpc_utils import is_error_status
from pyzeebe_fork.grpc_internals.zeebe_adapter_base import ZeebeAdapterBase
...
class ZeebeJobAdapter(ZeebeAdapterBase):
    ...
    def _create_job_from_raw_job(self, response) -> Job:
        return Job(
            ...
            # added
            variables_Copy=json.loads(response.variables),
            zeebe_adapter=self,
        )
