...
if TYPE_CHECKING:  # Only import for type checking (is False at runtime).
    from alphaui import Blocks, Interface
    from alphaui.blocks import BlockContext
    from alphaui.components import Component

analytics_url = "https://api.gradio.app/"
PKG_VERSION_URL = "https://api.gradio.app/pkg-version"
analytics.write_key = "uxIFddIEuuUcFLf9VgH2teTEtPlWdkNy"
JSON_PATH = os.path.join(os.path.dirname(alphaui.__file__), "launches.json")
...
def show_tip(interface: alphaui.Blocks) -> None:
    if interface.show_tips and random.random() < 1.5:
        tip: str = random.choice(alphaui.strings.en["TIPS"])
        print(f"Tip: {tip}")
    ...
    try:
        if not os.path.exists(JSON_PATH):
            ...
            if launches["launches"] in [25, 50, 150, 500, 1000]:
                print(alphaui.strings.en["BETA_INVITE"])
            with open(JSON_PATH, "w") as j:
...
def component_or_layout_class(cls_name: str) -> Component | BlockContext:
    ...
    """
    import alphaui.components
    import alphaui.layouts
    import alphaui.templates

    components = [
        (name, cls)
        for name, cls in alphaui.components.__dict__.items()
        if isinstance(cls, type)
    ]
    templates = [
        (name, cls)
        for name, cls in alphaui.templates.__dict__.items()
        if isinstance(cls, type)
    ]
    layouts = [
        (name, cls)
        for name, cls in alphaui.layouts.__dict__.items()
        if isinstance(cls, type)
    ]
    for name, cls in components + templates + layouts:
        if name.lower() == cls_name.replace("_", "") and (
            issubclass(cls, alphaui.components.Component)
            or issubclass(cls, alphaui.blocks.BlockContext)
        ):
            return cls
