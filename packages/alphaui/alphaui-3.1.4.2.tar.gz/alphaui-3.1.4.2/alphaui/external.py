...
import alphaui
from alphaui import components, utils
...
def load_blocks_from_repo(name, src=None, api_key=None, alias=None, **kwargs):
    ...
    blocks: alphaui.Blocks = factory_methods[src](name, api_key, alias, **kwargs)
    return blocks
...
def get_models_interface(model_name, api_key, alias, **kwargs):
    ...
    interface = alphaui.Interface(**kwargs)
    return interface
...
def get_spaces_blocks(model_name, config):
    ...
    for d, dependency in enumerate(config["dependencies"]):
        if dependency["backend_fn"]:
            ...
            fns.append(None)
    return alphaui.Blocks.from_config(config, fns)
