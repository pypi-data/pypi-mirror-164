...
#from .tools import get_outputs_and_metrics
from .tools import get_outputs_labels_and_metrics
from .scores import get_scores

#def get_thresholds(model, in_valid_loader, score_type="MMLES", batches_to_accumulate=32):
#def get_thresholds(model, loader, score_type="MMLES"):
#def get_thresholds(outputs, score_type="MMLES"):
def get_thresholds_from_scores(scores):
    print("====>>>> getting thresholds <<<<====")
    #model.eval()
    ##accumulated_batches = 0
    #accumulated_scores = None
    ##batches_to_accumulate = batches_to_accumulate

    thresholds = torch.Tensor(25)
    #results = get_outputs_labels_and_metrics(model, loader)
    #scores = get_scores(outputs, score_type=score_type)
