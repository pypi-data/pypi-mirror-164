...
class OdbcDatabase(implements(DbInterface)):
    ...
    def _update_record(self, dbx, table_name, ws_name, id, values, col_dict=None, actual_cols=None, ensure_record_found=True):
        ...
        if ensure_record_found:
            ...
            if rowcount != 1:
                print("WARNING: _update_record failed; rowcount={}, cmd={}".format(rowcount, cmd), flush=True)
                assert rowcount == 1
    ...
    def _add_new_cols_to_table(self, dbx, table_name, new_cols, record, cols_dict, force_vchar=False, is_for_bag=True, new_col_types=None):
        ...
        if new_col_types:
            ...
            for key in new_cols:
                cols_dict[key] = self._get_col_type(record[key], is_for_bag=is_for_bag)
    ...
    def _init_core_tables(self, dbx, create=False):
        ...
        # RUN_INFO
        col_dict["run_info"] = {"_id": VARCHAR, 'app_name': VARCHAR, 'box_name': VARCHAR, 'compute': VARCHAR, 
            'create_time': TIME, 'description': VARCHAR, 'display_name': VARCHAR, 'exper_name': VARCHAR, 
        ...
        # JOB_INFO
        # NEW COLS: sla, low_pri, vm_size, sku, service_name, service_type, compute, compute_target, num_dgd_seeds
        col_dict["job_info"] =  {"_id": VARCHAR, "compute": VARCHAR, "compute_target": VARCHAR, 
            ...
            "node_count": INT, "num_dgd_seeds": INT, "primary_metric": VARCHAR, "run_count": INT, "repeat": INT,
    ...
    def _find_null_columns(self, docs, groups_only=True):
        ...
        for doc in docs:
            for col, value in doc.items():
                ...
                elif not groups_only or in_group:
                    # top level value
                    if value is None:
    ...
    def _remove_null_columns(self, docs, groups_only=True):
        # remove any col with all NULL values (this col was most likely never defined for this set of records)
        # not knowing for sure is a limitation of the way we represent TAGS, HPARAMS, and METRICS
        all_null_cols = self._find_null_columns(docs, groups_only)
    ...
    def _get_info_for_runs(self, ws_name, filter_dict, fields_dict, sort_col=None, sort_dir=1, skip=None, 
        ...
        # TODO: why is this same call made twice??  it IS needed, but we should explain why here.
        ...
        self._remove_null_columns(docs, groups_only=True)
