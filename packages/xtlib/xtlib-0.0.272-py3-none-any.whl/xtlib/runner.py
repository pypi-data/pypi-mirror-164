...
class Runner():
    ...
    def create_run(self, job_id, user_cmd_parts, box_name="local", parent_name=None, rerun_name=None, node_index=0, 
        ...
        if log_to_store:
            ...
            if fake_submit:
                ...
                display_name = utils.expand_xt_vars(display_name, job_runs=None, run_id=run_name, args=args)
    ...
    def process_run_command(self, args):
        ...
        args["job_id"] = job_id
        experiment = utils.expand_xt_vars(experiment, job_runs, args=args)
        ...
        if not fake_submit:
            ...
            dd = {
                ...
                "compute": utils.safe_value(compute_def, "compute"),
...
def update_compute_def_from_cmd_options(compute_def, hold=False):
    ...
    compute_props = ["vm-size", "azure-image", "low-pri", "box-class", "docker", "setup", "boxes", 
        "vc", "cluster", "queue", "sku", "nodes", "sla", "compute", "service"]
