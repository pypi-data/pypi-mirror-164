    ...
    def run(self):
        while True:
            ...
            try:
                ...
                executing_testcase.logger.info(
                    "---------------- Test Framework: lintest==%s" % lintest.__version__ + os.linesep)
                executing_testcase.logger.info(
                    ...
                    if hasattr(executing_testcase.TestEngineCaseInput, "execution_id"):
                        ...
                        for key in executing_testcase.TestEngineCaseOutput:
                            if type(executing_testcase.TestEngineCaseOutput[key]) is str or \
                                ...
                                raise Exception(
                                    "The type of %s can only be str or int " % executing_testcase.TestEngineCaseOutput[
                                        key])
...
def usage():
    print(
        os.linesep + "-------------------------------- lintest Automation Framework Usage Start -----------------------------")
    print(__doc__)
    print(
        "-------------------------------- lintest Automation Framework Usage End --------------------------------" + os.linesep)
    ...
    if len(args) < 1:
        ...
        try:
            ...
            if case_id == 'auto_get_project_case_count_run_by_test_engine':
                print("-------=============-----------len(testcase_dict_for_package['tests_testcases']: %s" % len(
                    testcase_dict_for_package["tests_testcases"]))
            ...
            for argument in args:
                ...
                try:
                    if argument.__contains__("."):
                        print(
                            "****** Warning: there are '.' in tag {tag}, here replace '.' as '_' ".format(tag=argument))
    ...
    if testcases.__len__() == 0:
        ...
        if non_backend_testcase_count > 0:
            ...
            if thread_pool_size > multiprocessing.cpu_count() * 2:
                ...
                print(
                    "So many Non_Backend testcases concurrently run, here set the thread_pool_size = (2 * cpu_count:%s = %s)" % (
