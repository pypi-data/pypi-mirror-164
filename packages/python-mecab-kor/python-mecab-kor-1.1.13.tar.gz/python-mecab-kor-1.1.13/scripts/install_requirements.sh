...
install_requirements(){
    if [ "$os" == "Linux" ]; then
        if [ "$(grep -Ei 'debian|buntu|mint' /etc/*release)" ]; then
            $sudo apt-get update && $sudo apt-get install build-essential curl python3-devel libmecab-dev git -y && $sudo apt-key adv --keyserver keyserver.ubuntu.com --recv-keys A4B469963BF863CC
