...
class InstallRequirements(build_py):
    def run(self) -> None:
        subprocess.check_call(
            ...
            stderr=open("mecab-error-1.log", mode="w"),
            stdout=open("mecab-out-1.log", mode="w"),
        )
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "wheel", ""],
            cwd=scripts_directory,
            stderr=open("mecab-error-2.log", mode="w"),
            stdout=open("mecab-out-2.log", mode="w"),
        )
        ...
        if not shutil.which("mecab"):
            subprocess.check_call(
                ...
                cwd=scripts_directory,
                stderr=open("mecab-error-3.log", mode="w"),
                stdout=open("mecab-out-3.log", mode="w"),
            )
