...
class MySSML:
    ...
    #['晓晓', '云扬', '晓辰', '晓涵', '晓墨', '晓秋', '晓睿', '晓双', '晓萱', '晓颜', '晓悠', '云希', '云野']
    VALID_VOICE_NAMES_MAP = {'zh-CN-XiaoxuanNeural': {'LocalName': '晓萱', 'VoiceStyleNames': ['general', 'calm', 'fearful', 'cheerful', 'disgruntled', 'serious', 'angry', 'gentle', 'depressed', 'Default']}, 'zh-CN-XiaochenNeural': {'LocalName': '晓辰', 'VoiceStyleNames': ['general', '']}, 'zh-CN-XiaoxiaoNeural': {'LocalName': '晓晓', 'VoiceStyleNames': ['general', 'assistant', 'chat', 'customerservice', 'newscast', 'affectionate', 'angry', 'calm', 'cheerful', 'disgruntled', 'fearful', 'gentle', 'lyrical', 'sad', 'serious', 'poetry-reading', 'Default']}, 'zh-CN-XiaohanNeural': {'LocalName': '晓涵', 'VoiceStyleNames': ['general', 'calm', 'fearful', 'cheerful', 'disgruntled', 'serious', 'angry', 'sad', 'gentle', 'affectionate', 'embarrassed', 'Default']}, 'zh-CN-XiaozhenNeural': {'LocalName': '晓甄', 'VoiceStyleNames': ['general', 'angry', 'disgruntled', 'cheerful', 'fearful', 'sad', 'serious', 'Default']}, 'zh-CN-YunjianNeural': {'LocalName': '云健', 'VoiceStyleNames': ['general', 'Narration-relaxed', 'Sports_commentary', 'Sports_commentary_excited', 'Default']}, 'zh-CN-XiaoyanNeural': {'LocalName': '晓颜', 'VoiceStyleNames': ['general', '']}, 'zh-CN-XiaoyiNeural': {'LocalName': '晓伊', 'VoiceStyleNames': ['general', 'angry', 'disgruntled', 'affectionate', 'cheerful', 'fearful', 'sad', 'embarrassed', 'serious', 'gentle', 'Default']}, 'zh-CN-YunhaoNeural': {'LocalName': '云皓', 'VoiceStyleNames': ['general', 'Advertisement_upbeat', 'Default']}, 'zh-CN-XiaoshuangNeural': {'LocalName': '晓双', 'VoiceStyleNames': ['general', 'chat', 'Default']}, 'zh-CN-XiaomengNeural': {'LocalName': '晓梦', 'VoiceStyleNames': ['general', 'chat', 'Default']}, 'zh-CN-YunfengNeural': {'LocalName': '云枫', 'VoiceStyleNames': ['general', 'angry', 'disgruntled', 'cheerful', 'fearful', 'sad', 'serious', 'depressed', 'Default']}, 'zh-CN-YunzeNeural': {'LocalName': '云泽', 'VoiceStyleNames': ['general', 'calm', 'fearful', 'cheerful', 'disgruntled', 'serious', 'angry', 'sad', 'depressed', 'documentary-narration', 'Default']}, 'zh-CN-XiaoqiuNeural': {'LocalName': '晓秋', 'VoiceStyleNames': ['general', '']}, 'zh-CN-YunyeNeural': {'LocalName': '云野', 'VoiceStyleNames': ['general', 'embarrassed', 'calm', 'fearful', 'cheerful', 'disgruntled', 'serious', 'angry', 'sad', 'Default']}, 'zh-CN-XiaoruiNeural': {'LocalName': '晓睿', 'VoiceStyleNames': ['general', 'calm', 'fearful', 'angry', 'sad', 'Default']}, 'zh-CN-YunxiNeural': {'LocalName': '云希', 'VoiceStyleNames': ['general', 'narration-relaxed', 'embarrassed', 'fearful', 'cheerful', 'disgruntled', 'serious', 'angry', 'sad', 'depressed', 'chat', 'assistant', 'newscast', 'Default']}, 'zh-CN-XiaoyouNeural': {'LocalName': '晓悠', 'VoiceStyleNames': ['general', '']}, 'zh-CN-YunxiaNeural': {'LocalName': '云夏', 'VoiceStyleNames': ['general', 'calm', 'fearful', 'cheerful', 'angry', 'sad', 'Default']}, 'zh-CN-YunyangNeural': {'LocalName': '云扬', 'VoiceStyleNames': ['general', 'customerservice', 'narration-professional', 'newscast-casual', 'Default']}, 'zh-CN-XiaomoNeural': {'LocalName': '晓墨', 'VoiceStyleNames': ['general', 'embarrassed', 'calm', 'fearful', 'cheerful', 'disgruntled', 'serious', 'angry', 'sad', 'depressed', 'affectionate', 'gentle', 'envious', 'Default']}}
    ...
    VALID_VOICE_STYLES = {
        ...
        'Sports_commentary_excited':['体育评论-兴奋'],
    ...
    def __init__(self):
        ...
        self.tmpstring = ['<speak xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="http://www.w3.org/2001/mstts" xmlns:emo="http://www.w3.org/2009/10/emotionml" version="1.0" xml:lang="zh-CN">']
    ...
    def get_speakers(self):
        return MySSML.VALID_VOICE_NAMES_MAP

    # 中英文分句函数
    def cut_sent(self, para, drop_empty_line=True, strip=True, deduplicate=False,language = 'cn'):
        '''cut_sentences
        :param para: 输入文本
    ...
    def check_speaker_style(self,speaker,style):
        ...
        if self.is_all_chinese(speaker):
            for s,v in self.VALID_VOICE_NAMES_MAP.items():
                if speaker == v["LocalName'"]:
                    speaker = s
                    foundSpeaker = True
                    break
        ...
        if speaker not in self.VALID_VOICE_NAMES_MAP:
            ...
            speakerStyles = self.VALID_VOICE_NAMES_MAP[speaker]["VoiceStyleNames"]
    ...
    # s 元素可包含文本和以下元素：audio、break、phoneme、prosody、say-as、sub、mstts:express-as。
    # prosody 元素可包含文本和以下元素：audio、break、p、phoneme、prosody、say-as、sub 和 s。
    # say-as 元素只能包含文本。
    # lexicon 元素必须位于 voice 元素内部。lexeme 元素在自定义词典中区分大小写。
    # 使用 IPA 时，phoneme 元素不能包含空格。IPA 中的音节边界为“.”。
    ...
    def voice(self, text='', name=""):
        ...
        if name not in self.VALID_VOICE_NAMES_MAP:
            raise ValueError('The name provided to voice is not valid')
        ...
        self.obj_list.append({
            ...
            "text"  :   '<voice name="{}"></voice>'.format(name)
        })
        if text != "":
            self.p(text)
        return self
    ...
    def silence(self, value="1000ms", type="Sentenceboundary"):
        ...
        # 返回当前对象
        self.current_silence = {"type":type,"sid":id,"value":value}
    ...
    def style(self, text="", style="general", styledegree=2, role = None):
        ...
        self.obj_list.append({
            ...
            "pid"   :   pid,
            "type"  :   TYPE,
            "text"  :   '<mstts:express-as{} style="{}" styledegree="{}" >'.format(role,style, styledegree) + '</mstts:express-as>'
        })
        if text != "":
            self.p(text)
        return self  
    ...
    # 添加一个指定名称的书签到SSML
    def mark(self, text, addto_objlist = True):
        ...
        if self.current_silence is not None:
            ...
            sid = self.current_silence["sid"]
            stype = self.current_silence["type"]
            svalue = self.current_silence["value"]
            if sid not in self.mark_list:
                self.mark_list[sid] = {}
                self.mark_list[sid]["type"] = stype
                self.mark_list[sid]["value"] = svalue
                self.mark_list[sid]["marks"] = [self._escape(text)]
            else:
                self.mark_list[sid]["marks"].append(self._escape(text))

        return self 


    # 返回marks列表
    def get_marks(self,all_marks = False):
        if all_marks:
            return self.mark_list
        res = {}
        #print(self.mark_list)
        for sk,sv in self.mark_list.items():
            if sv["type"] == "Leading" and len(sv["marks"])>0:
                for i,mk in enumerate(sv["marks"]):
                    res[mk] = {"offset":"0ms","text":mk}
                res[sv["marks"][0]] = {"offset":sv["value"],"text":sv["marks"][0]}
            elif sv["type"] == "Tailing" and len(sv["marks"])>0:
                for i,mk in enumerate(sv["marks"]):
                    res[mk] = {"offset":"0ms","text":mk}
                res[sv["marks"][-1]] = {"offsetend":sv["value"],"text":sv["marks"][-1]}
            elif sv["type"] == "Sentenceboundary" and len(sv["marks"])>0:
                for i,mk in enumerate(sv["marks"]):
                    if i>0 and i<len(sv["marks"]):
                        res[mk] = {"offset":sv["value"],"text":mk}
                    else:
                        res[mk] = {"offset":"0ms","text":mk}
            else:
                pass
    ...
    def prosody(self, attributes, word):
        ...
        try:
            ...
            TYPE = "PROSODY"
            id,pid = self.get_objIdPid(TYPE)
            content = self._escape(word)

            if self.automark:
                tmpcontent = []
                tmps = self.cut_sent(word)
                for i,sen in enumerate(tmps):
                    tmpcontent.append(sen)
                    self.mark(id + "_m"+str(i),addto_objlist=False)
                    tmpcontent.append('<bookmark mark="'+ id + "_m"+str(i) +'" />')
            ...
            self.obj_list.append({
                ...
                "pid"   :   pid,
                "type"  :   TYPE,
                "text"  :   "<prosody%s>%s</prosody>" % (tag_attributes, content)
            })
            return self
        except AttributeError:
            raise AttributeError('Parameters must be strings')
