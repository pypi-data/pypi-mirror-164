...
class Twins:
    ...
    def _get_percent(self, x, y):
        if x * y <= 0:
            return -1
        else:
            return min(abs(x), abs(y))/ max(abs(x), abs(y))

    def _get_data_process(self, result_eval):
        data_process = []
        for i in range(len(result_eval)):
            if result_eval[i] != 3.1415:
                data_process.append([i, self._ID[i], result_eval[i]])

        return sorted(data_process, key = lambda x : x[-1])

    def _defaul_dict(self,st, end):
        df_dict = {}
    ...
    def _get_index_SS(self, result_eval):
        df_dict = self._defaul_dict(1, self._time_moment)
        n, id_prev, id_next = len(result_eval), len(result_eval) * [-1], len(result_eval) * [-1]

        for i in range(n-1):
            df_dict[result_eval[i][1]].append(i)

            if result_eval[i][1] > result_eval[i+1][1]:
                for j in range(self._time_moment,  int(result_eval[i+1][1]), -1):
                    if len(df_dict[j]) != 0:
                        for k in df_dict[j]:
                            id_next[k] = i+1
                        df_dict[j] = []
        ...
        id = self._index_test[-2] * [0]
        percent = self._index_test[-2] * [0]

        for i in range(n):
            if id_prev[i] == -1 and id_next[i] == -1:
                pass

            elif id_prev[i] == -1:
                pr_next = self._get_percent(result_eval[id_next[i]][-1], result_eval[i][-1])
                id[result_eval[i][0]] = result_eval[id_next[i]][0]
                percent[result_eval[i][0]] = pr_next

            elif id_next[i] == -1:
                pr_prev = self._get_percent(result_eval[id_prev[i]][-1], result_eval[i][-1])
                id[result_eval[i][0]] = result_eval[id_prev[i]][0]
                percent[result_eval[i][0]] = pr_prev
            else:
                pr_prev = self._get_percent(result_eval[id_prev[i]][-1], result_eval[i][-1])
                pr_next = self._get_percent(result_eval[id_next[i]][-1], result_eval[i][-1])
    ...
    def _getIndex(self):
        arr = np.array(self.data_test['TIME'])
        if self._type_data == 'YEAR':
            for s in arr:
                yield int((int(s)-np.min(arr))+1)
        else:
            for s in arr:
                yield int((int(s)-np.min(arr))+1)
