...
inline bool vmath_almostEqual(float a, float b, float margin = 0.00001f) {
	return fabs(a - b) < margin;
}

inline bool vmath_almostEqual(const float* v0, const float* v1, int d, float margin = 0.00001f) {

	for (int i = 0; i < d; i++) {
		if (!vmath_almostEqual(v0[i], v1[i], margin)) return false;
	}
	return true;
}
...
// [IGE] based on http://answers.unity3d.com/questions/467614/what-is-the-source-code-of-quaternionlookrotation.html
inline void vmath_quat_look_rotation(const float* forward, const float* up, float* quaternion)
{
	float vector[3], vector2[3], vector3[3];
	vmath_normalize(forward, 3, vector);
	vmath_cross3D(up, vector, vector2);
	vmath_normalize(vector2, 3, vector2);
	vmath_cross3D(vector, vector2, vector3);

	float m00 = vector2[0], m01 = vector2[1], m02 = vector2[2];
	float m10 = vector3[0], m11 = vector3[1], m12 = vector3[2];
	float m20 = vector[0], m21 = vector[1], m22 = vector[2];

	float num8 = (m00 + m11) + m22;
	if (num8 > 0.f)
	{
		float num = (float)sqrt(1.0 + num8);
...
inline void vmath_mat4_orthographic_ZO(float left, float right, float bottom, float top, float zNear, float zFar, float* out)
{
	float sum_rl, sum_tb, sum_nf, inv_rl, inv_tb, inv_nf;
	sum_rl = (right + left);
	sum_tb = (top + bottom);
	...
	vmath_vect_set((inv_rl + inv_rl), 0.0f, 0.0f, 0.0f, 4, &(out[0]));
	vmath_vect_set(0.0f, (inv_tb + inv_tb), 0.0f, 0.0f, 4, &(out[4]));
	vmath_vect_set(0.0f, 0.0f, (inv_nf), 0.0f, 4, &(out[8]));
	vmath_vect_set((-sum_rl * inv_rl), (-sum_tb * inv_tb), (zNear * inv_nf), 1.0f, 4, &(out[12]));
}
...
inline void vmath_quatToEuler(const float* quat, float* euler) {
	...
	// special cases: (0.5f, 0.5f, 0.5f, -0.5f) and (0.5f, 0.5f, -0.5f, 0.5f)
	if (vmath_almostEqual(quat[0], 0.5f) && vmath_almostEqual(quat[1], 0.5f)) {
		if (vmath_almostEqual(quat[2], 0.5f) && vmath_almostEqual(quat[3], -0.5f)) {
			euler[0] = 1.5708f;
			euler[1] = 1.5708f;
		}
		else if (vmath_almostEqual(quat[2], -0.5f) && vmath_almostEqual(quat[3], 0.5f)) {
			euler[0] = -1.5708f;
			euler[1] = -1.5708f;
		}
		euler[2] = 0.0f;
	...
	if (test > 0.49999f * unit) {
		euler[0] = 0.0f;
		euler[1] = 2.0f * atan2f(quat[0], quat[3]);
		euler[2] = PI / 2.0f;
		return;
	}
	...
	euler[2] = vmath_almostEqual(unit, 0.f) ? 0.f : asinf(2.0f * test / unit);
	return;
}
...
inline void vmath_mat4_fromTransform(const float* pos, const float* rot, int rotSize, const float* scale, const float* shear, float* out) {
	...
	rottransmat[15] = 1.0f;
	if(shear) vmath_mul_matrix_matrix(rottransmat, shear, 4, 4, tmp);
