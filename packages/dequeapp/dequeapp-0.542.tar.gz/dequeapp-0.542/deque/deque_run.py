...
from deque.datatypes import Image, Audio, Histogram, BoundingBox2D, Table
from deque.util import MODEL, CODE, DATA, ENVIRONMENT, RESOURCES,TRACKING_ENDPOINT
import requests
import glob
import time
import psutil
...
class Run:
    ...
    def log(self, data, step=None, commit=True):
        ...
        p = multiprocessing.Process(target=self._log_artifact_task, args=(full_data, self._run_meta_data))
        p.start()
