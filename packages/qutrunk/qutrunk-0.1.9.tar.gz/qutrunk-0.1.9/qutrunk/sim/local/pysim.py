...
class Simulator:
    ...
    def init_plus_state(self):
        ...
        stateVecSize = chunk_size
        normFactor = 1.0 / math.sqrt(stateVecSize)
    ...
    def init_classical_state(self, state_ind):
        """Init classical state"""
        # dimension of the state vector
        stateVecSize = self.total_num_amps
    ...
    def cy(self, control_bit, target_bit):
        ...
        chunkSize = self.total_num_amps
        size_half_block = 2**target_bit
        size_block = size_half_block * 2
        for this_task in range(num_task):
            thisBlock = this_task // size_half_block
            indexUp = thisBlock * size_block + this_task % size_half_block
            indexLo = indexUp + size_half_block

            controlBit = self.extract_bit(control_bit, indexUp)
            if controlBit:
                stateRealUp = self.real[indexUp]
                stateImagUp = self.imag[indexUp]

                # update under +-{{0, -i}, {i, 0}}
                self.real[indexUp] = conjFac * self.imag[indexLo]
                self.imag[indexUp] = conjFac * -self.real[indexLo]
                self.real[indexLo] = conjFac * -stateImagUp
                self.imag[indexLo] = conjFac * stateRealUp

    def cz(self, control_bits, num_control_bits):
        state_vec_size = self.total_num_amps
        mask = self.getQubitBitMask(control_bits, num_control_bits)
        for index in range(state_vec_size):
            if mask == (mask & index):
    ...
    def unitary(self, target_bit, ureal, uimag):
        ...
        for this_task in range(num_task):
            ...
            stateRealLo = self.real[indexLo]
            stateImagLo = self.imag[indexLo]

            # state[indexUp] = u00 * state[indexUp] + u01 * state[indexLo]
            self.real[indexUp] = (
                ureal[0][0] * stateRealUp
                - uimag[0][0] * stateImagUp
                + ureal[0][1] * stateRealLo
                - uimag[0][1] * stateImagLo
            )
            self.imag[indexUp] = (
                ureal[0][0] * stateImagUp
                + uimag[0][0] * stateRealUp
                + ureal[0][1] * stateImagLo
                + uimag[0][1] * stateRealLo
            )

            # state[indexLo] = u10  * state[indexUp] + u11 * state[indexLo]
            self.real[indexLo] = (
                ureal[1][0] * stateRealUp
                - uimag[1][0] * stateImagUp
                + ureal[1][1] * stateRealLo
                - uimag[1][1] * stateImagLo
            )
            self.imag[indexLo] = (
                ureal[1][0] * stateImagUp
                + uimag[1][0] * stateRealUp
                + ureal[1][1] * stateImagLo
                + uimag[1][1] * stateRealLo
            )

    def crx(self, control_bit, target_bit, angle):
        unitAxis = [1, 0, 0]
        self.controlled_rotate_around_axis(control_bit, target_bit, angle, unitAxis)
    ...
    def controlled_rotate_around_axis(self, control_bit, target_bit, angle, unitAxis):
        mag = math.sqrt(
            unitAxis[0] * unitAxis[0]
            + unitAxis[1] * unitAxis[1]
            + unitAxis[2] * unitAxis[2]
        )
        unitVec = [unitAxis[0] / mag, unitAxis[1] / mag, unitAxis[2] / mag]

        alpha_real = math.cos(angle / 2.0)
        alpha_imag = -math.sin(angle / 2.0) * unitVec[2]
        beta_real = math.sin(angle / 2.0) * unitVec[1]
        beta_imag = -math.sin(angle / 2.0) * unitVec[0]
        ...
        for this_task in range(num_task):
            ...
            if controlBit:
                ...
                self.real[indexUp] = (
                    alpha_real * stateRealUp
                    - alpha_imag * stateImagUp
                    - beta_real * stateRealLo
                    - beta_imag * stateImagLo
                )
                self.imag[indexUp] = (
                    alpha_real * stateImagUp
                    + alpha_imag * stateRealUp
                    - beta_real * stateImagLo
                    + beta_imag * stateRealLo
                )

                # state[indexLo] = beta  * state[indexUp] + conj(alpha) * state[indexLo]
                self.real[indexLo] = (
                    beta_real * stateRealUp
                    - beta_imag * stateImagUp
                    + alpha_real * stateRealLo
                    + alpha_imag * stateImagLo
                )
                self.imag[indexLo] = (
                    beta_real * stateImagUp
                    + beta_imag * stateRealUp
                    + alpha_real * stateImagLo
                    - alpha_imag * stateRealLo
                )
    ...
    def getQubitBitMask(self, qubits, numqubit):
        mask = 0
