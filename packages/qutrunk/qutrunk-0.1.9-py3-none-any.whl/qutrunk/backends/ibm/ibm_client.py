...
class IBMQ(Session):
    ...
    def get_result(self, device, execution_id, num_retries=3000, interval=1, verbose=False):
        """获取执行结果"""
...
def send(info, device='ibmq_qasm_simulator', token=None, shots=None, num_retries=3000,
    ...
    try:
        ...
        if not online:
            print("The device is offline. Use the simulator instead or try again later.")
...
if __name__ == '__main__':
    ...
    #
    # print(i.authenticate(token))
    # i.get_list_devices(verbose=True)
    # print(i.is_online("ibmq_qasm_simulator"))
    # i.run(device="ibmq_qasm_simulator")
