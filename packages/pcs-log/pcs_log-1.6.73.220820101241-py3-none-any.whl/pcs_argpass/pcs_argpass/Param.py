...
#           Text += "\n"
    ...
    def __Prepare(self) -> None:
        ...
        AktualName = self.__Prefix
        
        for c in self.__Children.values():
            if not c.__IsPrepared:
                c.__Prepare()
...
#            opts, args = getopt.getopt(self.__Argumente[1:],self.__ShortStr,self.__LongList)
            ...
            if o in self.__Glob_ImportList:
                ...
                if n.exists():
                    if n.is_file():
                        ...
                        self.__AssignImportValues(wGlobDict, FileName=str(n))
                    else:
        ...
        for o, a in opts:
            ...
            if o in self.__ExportList:
                if self.__Prefix is not None:
                    if self.__Prefix != '':
                        print(f"//{'-'*60}\n// {self.__Prefix}\n//{'-'*60}\n")
                print(json.dumps(self.__WorkDict, sort_keys=True, indent=4, cls=self.PathEncoder))
                if self.__Parent is None:
                    sys.exit(0)
                return True
    ...
    def __AssignImportValues(self, wGlobDict:dict, FileName:str) -> None:
        ...
        for k in self.__WorkDict.keys():        # versuche Daten für all unsere Keys zu bekommen
            IsOk = True
            try:
                iVal = wDict[k]
            except:
            ...
            if IsOk:
                ...
                if Res is not None:                     # der übergebene Wert war ungültig
                        raise self.ParamError(Res)
        for n,c in self.__Children.items():
            c.__AssignImportValues(wGlobDict,FileName=FileName) # löse auch für alle Child-Klassen auf
