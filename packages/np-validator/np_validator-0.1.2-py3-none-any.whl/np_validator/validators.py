...
def meets_filesize_threshold(
    ...
    """
    Validates whether or not a filepath meets a given
    filesize threshold
...
def has_dict_key(value: dict, path: List[str]) -> Tuple[Any, bool]:  # type: ignore
    ...
    A single element path will grab from the top level keys.
    Multiple path elements are used to index into nested dictionaries.
...
def meets_value_threshold(
    ...
    Note
    ----
    - meant to mirror ``get_data_subtype`` for ``synchronization_data``
    in ``NP_pipeline_validation``, an earlier implementation this tool is based off of
    """
...
def meets_vsync_rate_threshold(
    value: Dataset,
    threshold: int,
    tolerance: int,
    line_label: str,
) -> Tuple[float, bool]:
    vsyncs_rising, vsyncs_falling = get_sync_line_data(value, line_label)
    vsync_rate = 1 / np.median(np.diff(vsyncs_falling))
    return vsync_rate, (threshold - tolerance) < vsync_rate < (threshold + tolerance)


def meets_barcode_threshold(
    value: Dataset,
    threshold: int,
    line_label: str,
):
    r, f = get_sync_line_data(value, line_label)
    return len(f), len(f) >= threshold


def meets_cam_sync_threshold(
    value: Dataset,
    threshold: int,
    tolerance: int,
    line_label: str,
    ...
    return cam_rate, (threshold - tolerance) < cam_rate < (threshold + tolerance)
