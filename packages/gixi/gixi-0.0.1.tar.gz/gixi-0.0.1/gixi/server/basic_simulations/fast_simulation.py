...
class FastSimulation(object):
    def __init__(self, device='cuda'):
        ...
        self.mask_coords = torch.flip(
            (self.x.to(torch.float) * torch.cos(self.y.to(torch.float) / 512 * pi / 2)).squeeze(), (0,)
        )
    ...
    def simulate_labels(self):
        ...
        except AttributeError:  # torch 1.6.0 or below
            indices = torch.ones_like(pos, dtype=torch.bool)
...
def mul_perlin(img):
    ...
    return img * perlin(rates, weights, amp=1, device=img.device)
...
def background_perlin(img):
    return img + perlin((1,), device=img.device) * img.max() / 2
...
def simulate_labels(device='cuda'):
