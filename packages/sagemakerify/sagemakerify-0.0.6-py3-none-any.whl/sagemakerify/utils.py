...
class Defaults():
    def __init__(self, 
        ...
        globals.CACHE = Cache(os.path.join(os.path.expanduser('~'),'.sagemakerify'), 'cache.json', self.clear_cache)
...
def get_function_outputs(fnc):    
    outputs = [line.strip() for line in inspect.getsourcelines(fnc)[0] if ' return ' in line ]
    if len(outputs)>0:
