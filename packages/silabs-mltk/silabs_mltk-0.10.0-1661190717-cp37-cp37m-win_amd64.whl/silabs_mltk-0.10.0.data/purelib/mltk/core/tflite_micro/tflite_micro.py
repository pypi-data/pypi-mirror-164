...
class TfliteMicro:
    ...
    def record_model(
        ...
        if enable_accelerator_recorder:
            ...
            tflm_model.accelerator.enable_recorder()
        ...
        try:
            ...
            accelerator_recorded_data = None
            if enable_accelerator_recorder:
                accelerator_recorded_data = tflm_model.accelerator.get_recorded_data()
            if reenable_simulator_backend:
                tflm_model.accelerator.set_simulator_backend_enabled(True)

            retval = []

            for layer_index, layer_data in enumerate(recorded_data):
                # pylint: disable=protected-access
                tf_layer = copy.deepcopy(tflite_model.layers[layer_index])
                retval.append(tf_layer)
                for input_index, input_bytes in enumerate(layer_data.inputs):
                ...
                for output_index, output_bytes in enumerate(layer_data.outputs):
                ...
                if accelerator_recorded_data is not None:
                    tf_layer.metadata['accelerator_data'] = None if not accelerator_recorded_data[layer_index] else accelerator_recorded_data[layer_index]
