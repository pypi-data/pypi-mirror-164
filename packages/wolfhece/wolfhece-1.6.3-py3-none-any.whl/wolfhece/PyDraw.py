...
class imagetexture():
    ...
    def load(self):
        ...
        if self.parent.canvas.SetCurrent(self.parent.context):
            ...
            raise NameError('Opengl setcurrent -- maybe a conflict with an existing opengl32.dll file')
...
class WolfMapViewer(wx.Frame):
    ...
    def OnMenubar(self,event:wx.CommandEvent):
        ...
        if id==wx.ID_OPEN:
            ...
            curvec.find_minmax()
            mybounds=[[curvec.minx,curvec.maxx],[curvec.miny,curvec.maxy]]
        ...
        if len(self.myarrays)+len(self.myvectors)+len(self.myclouds)+len(self.myres2D)==2:
        ...
        if self.action is None:
            ...
            if alt:
                if self.active_array is not None:
                    ...
                    self.active_vector.add_vertex(wolfvertex(x,y,z))
                else:
                    self.active_vector.add_vertex(wolfvertex(x,y))    
            else:
                self.active_vector.add_vertex(wolfvertex(x,y))  
    ...
    def OnButton(self, e:wx.MouseEvent):
        ...
        if self.action=='dynamic parallel' and altdown:
