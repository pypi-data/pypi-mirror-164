...
class SubBasin:
    ...
    def get_outFlowRaw_noDelay(self, unit='m3/s'):
        ...
        if(index==0): 
            ...
            print("ERROR: the simulation time is not long enough for this subbasin to be taken into account")
            print("Error informations : ")
            print("Function name : get_outFlowRaw_noDelay()")
            print("index = ", index)
            print("len(myOutFlow) = ", len(myOutFlow))
            print("self.timeDelay = ", self.timeDelay)
            sys.exit()
    ...
    def compute_hydro(self):
        ...
        if(self.model==cst.tom_UH or self.model==cst.measures or self.model==cst.tom_GR4):
            ...
            if(index==0): 
                ...
                print("ERROR: the simulation time is not long enough for this subbasin to be taken into account")
                print("Error informations : ")
                print("Function name : compute_hydro()")
                print("Name = ", self.name)
                print("ID = ", self.iD)
                print("index = ", index)
                print("len(myOutFlow) = ", len(self.myHydro))
                print("self.timeDelay = ", self.timeDelay)
                print("All inlets timeDelay :")
                for element in self.intletsObj:
                    curObj = self.intletsObj[element]
                    print(curObj.timeDelay)
                print("=================")
                sys.exit()
            # Raw hydrograph
    ...
    def activate(self, effSubs:list=[], effSubsSort:list=[], mask:list=[], onlyItself:bool=False):
        ...
        if self.alreadyUsed == False:
            ...
            if onlyItself == False:
                for element in self.intletsObj:
                    effSubs, effSubsSort = self.intletsObj[element].activate(mask=mask, effSubs=effSubs, effSubsSort=effSubsSort)
    ...
    def init_timeDelays(self):
        ...
        if os.path.exists(fileTitle):
            self.transferParam = Wolf_Param(filename=fileTitle)
            ...
            if transferModel == cst.tom_transf_no:
                ...
                print("ERROR in 'init_timeDelays' -> SubBasin : this model is not implemented yet!")
