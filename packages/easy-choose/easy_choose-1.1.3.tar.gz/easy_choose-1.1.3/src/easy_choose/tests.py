...
# Import chooser() from chooser.py
...
def testascii(num: int=1, only: bool=False, low: int=32, up: int=127):
    '''Select from the ASCII code table.
    Optional - low/up : Sets the selected range.'''
    names = [chr(n) for n in range(low, up)]
    return chooser(num, only, *names)