...
def gitup(giturl:str):
    ...
    targetDir = "/tmp/gittmp_1";
    gitclone(owner,repo,parsed.username,targetDir)

    logger.info(f"gitup 入口文件: file")

def gitclone(owner, repo, token, dest_dir):
    """克隆一个github仓库"""
    process_helper.exec(f"rm -rdf {dest_dir}")
    cmd = f"git clone --depth=1 https://{token}@github.com/{owner}/{repo}.git {dest_dir}/{repo}"
    cp = process_helper.exec(cmd, check=False)
    if cp.returncode != 0:
        logger.log(f"clone 失败 {owner}, {repo},{dest_dir}")
        logger.log(cp.stderr)
    else:
        logger.log("clone 成功")
