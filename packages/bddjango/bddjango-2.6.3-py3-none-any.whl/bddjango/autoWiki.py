...
class AutoWiki(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for view_id in range(len(vs)):
            ...
            self.ret_dc['result']['data'] = dc_ls
            for k, v in self.response_render_format.items():
                self.ret_dc[v] = self.ret_dc.pop(k)

            example_data = json.dumps(self.ret_dc, sort_keys=False, indent=4, separators=(', ', ': '), ensure_ascii=False)
