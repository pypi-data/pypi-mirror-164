import numpy as np
from _typeshed import Incomplete
from edgeiq import bounding_box as bounding_box, edge_tools as edge_tools, processing as processing
from edgeiq._production_client import PRODUCTION_CLIENT as PRODUCTION_CLIENT
from edgeiq.base_service import BaseService as BaseService
from edgeiq.edge_tools import markup_image as markup_image, to_json_serializable as to_json_serializable
from edgeiq.engine_accelerator import Engine as Engine
from typing import Any, List

class InstanceSegmentationPrediction(bounding_box.BoundingBoxPrediction):
    def __init__(self, box: bounding_box.BoundingBox, mask: np.ndarray, contours: list, hierarchy: list, confidence: float, label: str, index: int) -> None: ...
    def __eq__(self, other): ...
    @property
    def label(self) -> str: ...
    @label.setter
    def label(self, label: str): ...
    @property
    def index(self) -> int: ...
    @property
    def mask(self) -> np.ndarray: ...
    @property
    def contours(self) -> list: ...
    @property
    def hierarchy(self) -> list: ...

class InstanceSegmentationResults:
    def __init__(self, predictions: List[InstanceSegmentationPrediction], duration: float, image: np.ndarray, **kwargs) -> None: ...
    def __eq__(self, other): ...
    @property
    def duration(self) -> float: ...
    @property
    def predictions(self) -> List[InstanceSegmentationPrediction]: ...
    @predictions.setter
    def predictions(self, predictions: List[InstanceSegmentationPrediction]): ...
    @property
    def image(self) -> np.ndarray: ...

RESULT_TYPE: Incomplete

class InstanceSegmentation(BaseService):
    def __init__(self, model_id: str) -> None: ...
    def segment_image(self, image: np.ndarray, confidence_level: float = ...) -> InstanceSegmentationResults: ...
    def markup_image(self, image: np.ndarray, predictions: List[InstanceSegmentationPrediction], show_labels: bool = ..., show_confidences: bool = ..., show_masks: bool = ..., colors: list = ..., line_thickness: int = ..., font_size: float = ..., font_thickness: int = ...) -> np.ndarray: ...
    def publish_analytics(self, results: InstanceSegmentationResults, tag: Any = ...): ...
