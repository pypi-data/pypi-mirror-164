...
@magicgui(
    ...
    active={
        ...
        "label": "Start/Stop patch selection",
        "visible": True,
        "value": False,
    },
    store_path={
...
def create_patch(layer, event):
    if event.type == "mouse_press" and event.button == 1:
        ...
        rectangle = create_rectangle(
            cords,
            patch_creation.patch_size.value,
            patch_creation.img_layer.value.data.shape,
...
def create_rectangle(cords, patch_size, img_layer_shape, cursor_pos):
    ...
    rectangle = sanitize_rectangle(rectangle, img_layer_shape, patch_size - 1)
