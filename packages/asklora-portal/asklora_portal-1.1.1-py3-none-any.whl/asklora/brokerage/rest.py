...
from . import logs, vars
from .common import URL
from .exceptions import APIError, RetryException
from .journals import JournalEntries, ReverseJournalEntries
...
class Broker(ConnectionBroker):
    ...
    def batch_transfer_reverse(
        self,
        to_account: str,
        entries: list[ReverseJournalEntries],
        entry_type: str = "JNLC",
    ) -> list[dict]:
        """
        many to one journal, suitable for deposit.
        """
        is_valid: bool = all(
            isinstance(entrie, ReverseJournalEntries) for entrie in entries
        )
        if not is_valid:
            raise APIError({"code": "0000", "message": "error type entries"})
        entries = [asdict(entry) for entry in entries]
        payload = dict(entry_type=entry_type, to_account=to_account, entries=entries)
        logs.logger.debug({"batch_transfer_reverse": {"payload": payload}})
        return self.post("v1/journals/reverse_batch", payload)

    def batch_transfer(
        self,
        from_account: str,
        entries: list[JournalEntries],
        entry_type: str = "JNLC",
    ) -> list[dict]:
        """
        one to many journal, suitable for withdraw.
        """
        is_valid: bool = all(isinstance(entrie, JournalEntries) for entrie in entries)
        if not is_valid:
            raise APIError({"code": "0000", "message": "error type entries"})
        entries = [asdict(entry) for entry in entries]
        payload = dict(
            entry_type=entry_type, from_account=from_account, entries=entries
        )
        logs.logger.debug({"batch_transfer_payload": {"payload": payload}})
        return self.post("v1/journals/batch", payload)
