...
class KafkaHelper:
    def __init__(self, **kwargs):
        ...
        if kwargs.get("username"):
            ...
            self.sasl_config.update({"sasl_plain_username": username})
        else:
            username = options.rabbitmq_config.get("username")
        if kwargs.get("password"):
            password = kwargs.pop("password")
            self.sasl_config.update({"sasl_plain_password": username})
        else:
    ...
    def publish(self, value=None, key=None, headers=None, partition=None, timestamp_ms=None, **configs):
        producer = KafkaProducer(bootstrap_servers=self.bootstrap_servers, api_version=(2, 0.2), **self.sasl_config,
                                 **configs)
        try:
    ...
    def consume(self, callback, **configs):
        consumer = KafkaConsumer(self.topic, bootstrap_servers=self.bootstrap_servers, api_version=(2, 0.2),
