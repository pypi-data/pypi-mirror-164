    ...
    def __init__(self, load, irr_data, fresnel_eff, tnoct, gamma, panel_power, num_panel,
        ...
        if not isinstance(self.load.index, pd.DatetimeIndex) or not isinstance(self.load.index, pd.DatetimeIndex):
            ...
            try:
                ...
                print(self.irr_data.index.hour)

                # if not self.load.index.hour or not self.irr_data.index.hour:
                #     raise ValueError('Los datos deben ser horarios')

            except:
                raise TypeError('Index must be pd.DateTimeIndex')
            
        print(self.load.index)
