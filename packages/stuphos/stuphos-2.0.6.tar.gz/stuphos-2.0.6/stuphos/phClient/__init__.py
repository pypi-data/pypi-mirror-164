...
class apiClient:
    ...
    class session:
        ...
        def interpretAgent(self, script):
            return self.command('do-interpret-agent', script)
        def interpretScript(self, script):
...
class nativeSession:
    ...
    def __init__(self, frame, host, playerName, password, passwordSend = True):
        ...
        task = frame.task
        task.checkAccess(['system:network', 'https', host, 'rpc'], 'write')
        ...
        def connect():
            ...
            try: self._session = client.login(playerName, password, passwordSend = passwordSend)
                ...
                # XXX Todo: hide system traceback.
                raise e
    ...
    def interpretAgent(self, script):
        return self._action(self._session.interpretAgent, script)
    def interpretScript(self, script):
        return self._action(self._session.interpretScript, script)
