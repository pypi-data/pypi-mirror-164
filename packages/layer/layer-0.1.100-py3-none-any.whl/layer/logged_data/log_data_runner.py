...
class LogDataRunner:
    ...
    def log(  # pylint: disable=too-many-statements
        ...
        for tag, value in data.items():
            ...
            # boolean check must be done before numeric check as it also returns true for booleans.
                ...
                self._log_image(
                    tag=tag, img_value=value, epoch=epoch, category=category, break_url=break_url
                )
                ...
                self._log_plot_figure(tag=tag, figure=value, category=category, break_url=break_url)
                ...
                self._log_plot_figure(
                    tag=tag, figure=value.get_figure(), category=category, break_url=break_url
                )
            elif self._is_pyplot(value):
                assert isinstance(value, ModuleType)
                self._log_current_plot_figure(tag=tag, plt=value, category=category, break_url=break_url)
            elif isinstance(value, Path):  # This must be below image and video!
                self._log_file_or_directory_from_path(
                    tag=tag, path=value, category=category, break_url=break_url
                )
            else:
        ...
        if Image.is_image_path(img):
            ...
            if self._train_id and epoch is not None:
                self._log_image_from_path(
                    tag=tag, path=img, epoch=epoch, category=category, break_url=break_url
                )
            else:
                self._log_image_from_path(tag=tag, path=img, category=category, break_url=break_url)
            ...
            if self._train_id and epoch is not None:
                self._log_pil_image(tag=tag, image=img, epoch=epoch, category=category, break_url=break_url)
            else:
                self._log_pil_image(tag=tag, image=img, category=category, break_url=break_url)
    ...
    def _log_metric(
        ...
        _epoch: int = epoch if epoch is not None else -1

        self._logged_data_destination.receive(
            func=lambda client: client.log_model_metric(
                train_id=train_id,
                tag=tag,
                points=[ModelMetricPoint(epoch=_epoch, value=float(numeric_value))],
    ...
    def _log_text(self, tag: str, text: str, category: Optional[str] = None) -> None:
        self._logged_data_destination.receive(
            lambda client: client.log_text_data(
                train_id=self._train_id,
    ...
    def _log_markdown(
        ...
        self._logged_data_destination.receive(
            lambda client: client.log_markdown_data(
                train_id=self._train_id,
                dataset_build_id=self._dataset_build_id,
                tag=tag,
                data=text,
    ...
    def _log_number(
        ...
        self._logged_data_destination.receive(
            func=lambda client: client.log_numeric_data(
                train_id=self._train_id,
                dataset_build_id=self._dataset_build_id,
                tag=tag,
                data=str(number),
    ...
    def _log_boolean(
        ...
        self._logged_data_destination.receive(
            lambda client: client.log_boolean_data(
                train_id=self._train_id,
                dataset_build_id=self._dataset_build_id,
                tag=tag,
                data=str(bool_val),
    ...
    def _log_dataframe(
        ...
        self._logged_data_destination.receive(
            lambda client: client.log_table_data(
                train_id=self._train_id,
                dataset_build_id=self._dataset_build_id,
                tag=tag,
                data=df_json,
    ...
    def _log_file_or_directory_from_path(
        self, tag: str, path: Path, category: Optional[str] = None, break_url: bool = False
    ) -> None:
        if path.is_file():
    ...
    def _log_binary_from_path(
        ...
        *,
        ...
        with open(path, "rb") as binary_file:
            self._logged_data_destination.receive(
                func=lambda client: client.log_binary_data(
                    train_id=self._train_id,
                    dataset_build_id=self._dataset_build_id,
                    tag=tag,
    ...
    def _log_current_plot_figure(
        self, tag: str, plt: ModuleType, category: Optional[str] = None, break_url: bool = False,
    ) -> None:
        if len(plt.get_fignums()) > 0:
            self._log_plot_figure(tag, plt.gcf(), category=category, break_url=break_url)
        else:
