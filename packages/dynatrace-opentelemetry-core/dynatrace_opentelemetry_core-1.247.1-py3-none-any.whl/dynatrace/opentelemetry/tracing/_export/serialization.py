...
SpanGroupKey = collections.namedtuple(
    "SpanGroupKey", ("server_id", "path_info", "trace_id")
)

SpanGroup = collections.namedtuple(
    "SpanGroup",
    ("trace_id_serialized", "parent_custom_tag", "spans_serialized"),
)
...
def _store_span_in_protobuf(
    ...
    Args:
        ...
        span: The :class:`ReadableSpan` which`s fields will be
            applied to the given protobuf span.
    ...
    _set_pb_span_parent(pb_span, span, tag)
    ...
    _add_pb_attrs(span.attributes, pb_span.attributes)
...
def _serialize_resource_and_meta_info(
    ...
    pb_resource = _create_pb_resource_from(resource)
...
def _create_pb_resource_from(resource: Resource) -> resource_pb2.Resource:
...
def _group_spans(
    ...
    for watched_span in spans:
        ...
        _store_span_in_protobuf(
            watched_span, sdk_span, group.trace_id_serialized, pb_span, tag
        )
...
def serialize_for_export(
    ...
    _serialize_resource_and_meta_info(export_data, reference_resource)
