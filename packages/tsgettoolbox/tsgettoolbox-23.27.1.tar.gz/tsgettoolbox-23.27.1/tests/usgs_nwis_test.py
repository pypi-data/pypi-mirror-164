...
def test_get_sites_by_state_code():
    mocked_urls = {
        ...
        "http://waterservices.usgs.gov/nwis/iv/": "usgs/nwis/RI_instantaneous.xml",
    }
...
def test_get_sites_by_county_code():
    ...
    sites_data_file = f"usgs/nwis/sites_county_{county_code}_daily.xml"
...
def test_get_sites_by_huc():
    huc = "02070010"
    sites_data_file = f"usgs/nwis/sites_huc_{huc}_daily.xml"
    with test_util.mocked_urls(sites_data_file):
...
def test_get_sites_single_site():
    site_code = "08068500"
    site_data_file = f"usgs/nwis/site_{site_code}_daily.xml"
...
def test_get_site_data_bad_service_raises_error():
    ...
    with test_util.mocked_urls(site_data_file):
        with pytest.raises(ValueError):
            ulmo.usgs.nwis.get_site_data(site_code, service="bad_service")
...
def test_get_site_data_single_site_with_period():
    ...
    with test_util.mocked_urls(site_data_file):
        site_data = ulmo.usgs.nwis.get_site_data(
            site_code, period="P45D", service="daily"
        )
...
def test_get_sites_multiple_sites():
    ...
    sites_data_file = f"usgs/nwis/sites_{'_'.join(site_codes)}_daily.xml"
    with test_util.mocked_urls(sites_data_file):
...
def test_get_sites_by_bounding_box():
    ...
    sites_data_file = f"usgs/nwis/sites_{bounding_box_values}_daily.xml"
    with test_util.mocked_urls(sites_data_file):
...
def test_get_sites_by_serving_parameter_code():
    ...
    sites_data_file = f"usgs/nwis/sites_{site_code}_{parameter_code_value}_daily.xml"
    with test_util.mocked_urls(sites_data_file):
        sites = ulmo.usgs.nwis.get_sites(
            sites=site_code, parameter_code=parameter_code_value, service="dv"
        )
    assert len(sites) == 1
...
def test_get_sites_by_multiple_serving_parameter_code():
    ...
    sites_data_file = f"usgs/nwis/sites_{site_code}_{parameter_code_values}_daily.xml"
    with test_util.mocked_urls(sites_data_file):
        sites = ulmo.usgs.nwis.get_sites(
            sites=site_code, parameter_code=parameter_code_values, service="dv"
        )
    assert len(sites) == 1
...
def test_get_site_data_multiple_methods():
    ...
    with test_util.mocked_urls(site_data_file):
        site_data = ulmo.usgs.nwis.get_site_data(
            site_code, methods={"62614": "all", "45592": "all"}
        )
