...
from tsgettoolbox.ulmo.nasa import daymet
...
def test_get_data_as_dataframes():
    for test_pixel_kwargs in test_pixels:
        ...
        assert pixel_data.shape == (
            len(test_pixel_kwargs["years"]) * 365,
            len(test_pixel_kwargs["variables"]) + 2,
        )
...
def test_get_data_as_dicts():
    for test_pixel_kwargs in test_pixels:
        ...
        assert (
            f"{test_pixel_kwargs['years'][0]}-01-01"
            in pixel_data[test_pixel_kwargs["variables"][0]].keys()
        )
        assert (
            f"{test_pixel_kwargs['years'][-1]}-12-30"
            in pixel_data[test_pixel_kwargs["variables"][0]].keys()
        )
        for var in test_pixel_kwargs["variables"]:
            assert var in pixel_data.keys()
