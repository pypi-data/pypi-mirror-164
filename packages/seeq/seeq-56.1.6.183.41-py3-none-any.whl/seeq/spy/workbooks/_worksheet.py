...
class Worksheet(Item):
    ...
    def pull(item_id, *, workbook=None, extra_workstep_tuples=None, include_images=True,
             session: Optional[Session] = None, errors='raise', status=None):
        ...
        worksheet.pull_worksheet(session, item_id, extra_workstep_tuples=extra_workstep_tuples,
                                 include_images=include_images, errors=errors, status=status)
        return worksheet
    ...
    def pull_worksheet(self, session: Session, worksheet_id, extra_workstep_tuples=None, include_images=True,
        ...
        self.document.pull(session, include_images=include_images, errors=errors, status=status)
    ...
    def find_workbook_links(self, session: Session, errors, status):
        return self.document.find_workbook_links(session, errors, status)
    ...
    def push_fixed_up_workbook_links(self, session: Session, item_map, label, datasource_output, errors, status):
        ...
        self.document.push(session, item_map[self.workbook.id], item_map[self.id], item_map, datasource_output,
                           push_images=False, label=label, item_push_errors=None, errors=errors, status=status)
...
class AnalysisWorksheet(Worksheet):
    ...
    def pull_worksheet(self, session: Session, worksheet_id, extra_workstep_tuples=None, include_images=True,
                       errors='raise', status=None):
        super().pull_worksheet(session, worksheet_id, errors, status)
        ...
        worksheet_output = safely(
            ...
            errors=errors, status=status)  # type: WorksheetOutputV1
        ...
        if extra_workstep_tuples:
            for workbook_id, worksheet_id, workstep_id in extra_workstep_tuples:
                if workbook_id == self.workbook.id and worksheet_id == self.id and workstep_id is not None:
                    workstep_tuples_to_pull.add((workbook_id, worksheet_id, workstep_id))
        self._pull_worksteps(session, workstep_tuples_to_pull, errors, status)

        if not self.worksteps:
            workstep = AnalysisWorkstep(self)
            self.definition['Current Workstep ID'] = workstep['ID']

    def _pull_worksteps(self, session: Session, workstep_tuples, errors, status):
    ...
    def push(self, session: Session, pushed_workbook_id, item_map, datasource_output, existing_worksheet_identifiers,
        ...
        safely(lambda: workbooks_api.set_current_workstep(workbook_id=pushed_workbook_id,
               ...
               errors='catalog')  # type: WorksheetOutputListV1
...
def get_analysis_worksheet_from_url(url, include_archived=False, quiet=False):
    ...
    workbook_search = spy.workbooks.search({'ID': workbook_id}, errors='catalog', quiet=quiet)
