...
def process_properties(session: Session, df, display_template_map, datasource, status, errors, existing_tree_df=None,
    ...
    df.loc[row_type == 'pre-existing', :] = df.loc[row_type == 'pre-existing', :] \
        .apply(safely(pull_calculation), axis=1, session=session)
    ...
    df.loc[row_type != 'pre-existing', :] = df.loc[row_type != 'pre-existing', :] \
        .apply(safely(make_node_reference), axis=1, display_template_map=display_template_map, session=session)
...
def push_and_replace_display_templates(session: Session, df, display_template_map, datasource, workbook_id):
    ...
    for original_id, display_template_input in display_template_map.items():
        ...
        display_template_output = display_templates_api.create_display_template(body=display_template_input)
        template_id_map[original_id] = display_template_output.id
