...
def search(query, *, content_filter='owner', all_properties=False, recursive=False, include_archived=False,
    ...
    try:
        results_df = _search(session, query, content_filter=content_filter, all_properties=all_properties,
                             recursive=recursive, include_archived=include_archived, errors=errors, status=status)
...
def _search(session: Session, query, *, content_filter, all_properties, recursive, include_archived, parent_id=None,
            parent_path='', search_folder_id=None, errors='raise', status=None):
    ...
    if 'ID' in query:
        workbook_output = safely(lambda: workbooks_api.get_workbook(id=query['ID']),
    ...
    @request_safely(action_description=_request_folder_contents_description, errors=errors, status=status)
        ...
        if props_match and len(path_filter_parts) == 0:
            ...
            if all_properties:
                ...
                @request_safely(action_description=f'get all properties for Workbook "{content.name}" {content.id}',
                                errors=errors, status=status, on_error=_add_error_message_and_warn)
