...
class Item:
    ...
    @staticmethod
    def pull(item_id, *, allowed_types=None, session: Session = None, errors='raise', status=None):
        session = Session.validate(session)
        item_output = safely(lambda: Item._get_item_output(session, item_id),
                             action_description=f'pull Item {item_id}', errors=errors, status=status)
        ...
        item._pull(session, item_id, errors, status)
