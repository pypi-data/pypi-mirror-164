...
class Workbook(ItemWithOwnerAndAcl):
    ...
    def _pull(self, session: Session, *, workbook_id=None, extra_workstep_tuples=None, include_inventory=True,
              include_images=True, specific_worksheet_ids: Optional[List[str]] = None, errors='raise', status=None):
        ...
        workbook_output = safely(lambda: workbooks_api.get_workbook(id=workbook_id),
                                 ...
                                 errors=errors, status=status)  # type: WorkbookOutputV1
        ...
        self._pull_owner_and_acl(session, workbook_output.owner, errors, status)
        ...
        if specific_worksheet_ids is not None:
            ...
            worksheet_ids = Workbook._pull_worksheet_ids(session, workbook_id, errors, status,
        ...
        for worksheet_id in worksheet_ids:
            ...
            Worksheet.pull(worksheet_id, workbook=self, extra_workstep_tuples=extra_workstep_tuples,
                           include_images=include_images, session=session, errors=errors, status=status)
        ...
        if include_inventory:
            self._scrape_item_inventory(session, errors, status)
            ...
            self._scrape_folder_inventory(session, errors, status)
    ...
    @staticmethod
    def _pull_worksheet_ids(session: Session, workbook_id: str, errors, status, *, get_archived_worksheets=False):
        workbooks_api = WorkbooksApi(session.client)

        @request_safely(action_description=f'gather all Worksheets within Workbook {workbook_id}',
                        errors=errors, status=status)
    ...
    @staticmethod
    def find_by_name(session: Session, workbook_name, workbook_type, folder_id, errors, status) \
            -> Optional[WorkbookOutputV1]:
    ...
    def push(self, *, owner=None, folder_id=None, item_map=None, label=None, datasource=None,
             ...
             scope_globals_to_workbook=False, errors='raise', status=None, session: Optional[Session] = None):
        session = Session.validate(session)
        self.status = Status.validate(status)
        self.item_push_errors = set()
        ...
        if not workbook_item:
            ...
            for is_archived in [False, True]:
                ...
                while True:
                    worksheet_output_list = safely(
                        ...
                        errors=errors, status=status)  # type: WorksheetOutputListV1
                    if worksheet_output_list is None:
                        break

                    for worksheet_output in worksheet_output_list.worksheets:  # type: WorksheetOutputV1
                        @request_safely(
                            action_description=f'get Data ID for worksheet {workbook_output.id}/{worksheet_output.id}',
                            errors=errors, status=status)
            ...
            self._push_owner_and_location(session, workbook_output, owner_id, folder_id, errors, status)
        ...
        if access_control:
            self._push_acl(session, workbook_output.id, self.datasource_maps, item_map, access_control, errors, status)
        ...
        for worksheet in self.worksheets:  # type: Worksheet
            ...
            worksheet_output = safely(
                ...
                errors=errors, status=status)
            if first_worksheet_id is None and worksheet_output is not None:
                first_worksheet_id = worksheet_output.id

        # Pull the set of worksheets and re-order them
        maybe_worksheet_ids = Workbook._pull_worksheet_ids(session, workbook_output.id, errors, status)
    ...
    def push_containing_folders(self, session: Session, item_map, datasource_output, use_full_path, parent_folder_id,
                                owner, label, access_control, errors, status):
        if 'Ancestors' not in self:
            ...
            if create_folders_now:
                if ancestor_id in self.item_inventory:
                    ...
                    try:
                        ...
                        if e.status != 404 and errors != 'catalog':
                            raise e
    ...
    def find_workbook_links(self, session: Session, errors, status):
        ...
        links = dict()
        for worksheet in self.worksheets:
            links.update(worksheet.find_workbook_links(session, errors, status))

        return links

    def push_fixed_up_workbook_links(self, session: Session, item_map, label, datasource_output, errors, status):
        for worksheet in self.worksheets:
            worksheet.push_fixed_up_workbook_links(session, item_map, label, datasource_output, errors, status)
    ...
    def _scrape_item_inventory(self, session: Session, errors: str, status: Status):
    ...
    def _scrape_folder_inventory(self, session: Session, errors: str, status: Status):
        if 'Ancestors' not in self:
            return
        ...
        for ancestor_id in self['Ancestors']:
            ...
            try:
                item = Item.pull(ancestor_id, allowed_types=['Folder'], session=session, errors=errors, status=status)
            except ApiException as e:
                if e.status == 404:
                    continue
