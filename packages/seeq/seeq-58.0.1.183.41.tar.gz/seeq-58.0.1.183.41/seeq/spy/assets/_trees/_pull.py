...
def pull_tree(session: Session, node_id, workbook_id, display_template_map, datasource, status, errors):
    """
    ...
    df = _properties.process_properties(session,
                                        ...
                                        display_template_map, datasource, status, errors, existing_tree_df)

    df = pull_all_children_of_all_nodes(session, df, workbook_id, display_template_map, datasource, status, errors,
                                        existing_tree_df)
...
def pull_all_children_of_all_nodes(session: Session, df, workbook_id, display_template_map, datasource, status, errors,
...
def pull_all_children_of_node(session: Session, df, node_id, node_full_path, workbook_id, display_template_map,
                              datasource, existing_tree_df, item_ids_to_ignore, parent_value, status, errors):
    """
    ...
    insert_df = _properties.process_properties(session, search_results, display_template_map, datasource,
                                               status, errors, existing_tree_df, pull_nodes=False)
...
def get_existing_spy_tree(session: Session, df, workbook_id, datasource):
    ...
    def _spy_tree_root_filter(root):
        return root.scoped_to is not None and item_output_has_sdl_datasource(root, datasource)
...
def get_active_conditions(session: Session, condition_ids, pd_start, pd_end, errors, status):
