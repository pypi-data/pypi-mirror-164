...
class Annotation:
    ...
    def find_workbook_links(self, session: Session, errors='raise', status=None):
        if not self.html:
            return dict()
            ...
            for match in matches:
                ...
                if 'workbook' not in group_dict:
                    ...
                    item_output = safely(
                        ...
                        errors=errors, status=status)  # type: ItemOutputV1
    ...
    def pull(self, session: Session, *, include_images=True, errors='raise', status=None):
        ...
        annotations = annotations_api.get_annotations(annotates=[self.worksheet.id])  # type: AnnotationListOutputV1
        ...
        for annotation_item in annotations.items:  # type: AnnotationOutputV1
            candidate = safely(lambda: annotations_api.get_annotation(id=annotation_item.id),
                               ...
                               errors=errors, status=status)  # AnnotationOutputV1
    ...
    def push(self, session: Session, pushed_workbook_id, pushed_worksheet_id, item_map, datasource_output,
             push_images, label, item_push_errors=None, errors='raise', status=None):
        self.render_plots()
        ...
        annotations_api = AnnotationsApi(session.client)
        # Note: get_annotations() won't error if the user doesn't have access to the `annotates` IDs
        annotations_output = annotations_api.get_annotations(
            annotates=[pushed_worksheet_id])  # type: AnnotationListOutputV1

        relevant_annotations = [a for a in annotations_output.items if a.type == self.annotation_type]
        if len(relevant_annotations) == 0:
            ...
            relevant_annotation = safely(lambda: annotations_api.create_annotation(body=new_annotation),
                                         ...
                                         errors=errors, status=status)  # type: AnnotationOutputV1
        ...
        self._push_specific(session, item_map, datasource_output, label, new_annotation, relevant_annotation,
                            item_push_errors, errors=errors, status=status)
    ...
    def _push_specific(self, session: Session, item_map, datasource_output, label, new_annotation, annotation,
                       item_push_errors=None, errors='raise', status=None):
...
class Report(Annotation):
    ...
    def pull(self, session: Session, include_images=True, errors='raise', status=None):
        session = Session.validate(session)
        annotation_output = super().pull(session, include_images=include_images, errors=errors, status=status)
        ...
        for content in contents.content_items:
            ...
            if content.date_range is not None:
                new_date_range = DateRange.pull(content.date_range, report=self, errors=errors, status=status)
        ...
        for date_range_id in annotation_output.date_range_ids:
            if date_range_id not in self.date_ranges.keys():
                new_date_range = DateRange.pull(date_range_id, report=self, errors=errors, status=status)
                self.date_ranges[new_date_range.id] = new_date_range
        ...
        for asset_selection_id in annotation_output.asset_selection_ids:
            if asset_selection_id not in self.asset_selections.keys():
                new_asset_selection = AssetSelection.pull(asset_selection_id, report=self, errors=errors, status=status)
                self.asset_selections[new_asset_selection.id] = new_asset_selection
    ...
    def _push_specific(self, session: Session, item_map, datasource_output, label, new_annotation, existing_annotation,
        ...
        for date_range_id in existing_annotation.date_range_ids:
            existing_date_range = DateRange.pull(date_range_id, session=session, errors=errors, status=status)
        ...
        for content_id in existing_annotation.content_ids:
            existing_content = Content.pull(content_id, session=session, errors=errors, status=status)
        ...
        def _push_it(_item_type, _item_dict, _existing, _ids_to_archive):
            for _item_object in _item_dict.values():  # type: Union[DateRange, AssetSelection, Content]
                try:
                    # This push has custom item_push_errors handling so always use errors='raise'
                    _item_output = _item_object.push(session, item_map, _existing, errors='raise', status=None)
                    if _item_output.id in _ids_to_archive:
                        _ids_to_archive.remove(_item_output.id)
                except ApiException as e:
                    if item_push_errors is not None:
        ...
        for content_id_to_archive in content_ids_to_archive:
            @request_safely(action_description=f'archive Content {content_id_to_archive}', errors=errors, status=status)
