...
try:
    from sqlalchemy import Column, engine, cast, String
...
class ManagedFsMixin:
    ...
    # Custom filters
    @staticmethod
    def policies_filter(query: Query, value: str):
        query = query.filter(cast(ManagedFs.policies, String).contains(f'"name": "{value}"'))
        return query
