...
def get_bed_df(bed):
    ...
    if not isinstance(bed, pd.DataFrame):
        msg = "[get_bed_df]: bed does not appear to be a data frame. type: {}".format(
            type(bed)
        )
...
def get_longest_features_by_end(bed_df):
    ...
    longest_orfs = pd.concat(
        [longest_orf_in_forward_group, longest_orf_in_reverse_group]
    )
...
def parse_exon_start_end_length(exon, delimiter="-"):
    ...
    if len(exon_start_end) != 2:
        msg = (
            "There was a problem parsing the exon coordinates: '{}'. Inner "
    ...
    if not utils.is_int(start):
        msg = (
            "There was a problem parsing the exon coordinates: '{}'. Could "
            "not interpret \"start\" as an integer: '{}'".format(
                exon, exon_start_end[0]
            )
        )
        raise ValueError(msg)
    ...
    if not utils.is_int(end):
        msg = (
            "There was a problem parsing the exon coordinates: '{}'. Could not "
    ...
    if length < 0:
        msg = (
            "The length for the exon was negative: '{}'. Please ensure the coordinates "
...
def get_block_and_offset(pos, block_starts, block_lengths):
    ...
    if (offset < 0) or (offset > block_lengths[i]):
        msg = (
            "The pos does not fall within the block structure. pos: {}, "
            "block_starts: {}, block_lengths: {}".format(
                pos, block_starts, block_lengths
            )
        )
...
def convert_genomic_coords_to_bed_blocks(coords, outer_sep=",", inner_sep="-"):
    ...
    if num_exons < 1:
        msg = (
            "There was a problem parsing the outer coordinates: '{}'. Outer "
    ...
    if not math_utils.is_monotonic(relative_starts):
        msg = (
            "There was a problem parsing the coordinates: '{}'. The outer "
...
def split_bed12_blocks(bed_entry):
    ...
    exon_lengths = np.fromstring(bed_entry["exon_lengths"], sep=",", dtype="int")
    exon_rel_starts = np.fromstring(
    ...
    for i, (start, end, transcript_start) in enumerate(
        zip(exon_starts, exon_ends, transcript_starts)
    ):
        exon = {
...
def split_bed12(bed_df, num_cpus=1, progress_bar=False):
    ...
    if len(bed_df) == 0:
        msg = "[bed_utils.split_bed12]: Attempting to split empty bed data " "frame"
        logger.warning(msg)
        return pd.DataFrame(columns=columns)

    exons = parallel.apply_parallel(
        bed_df, num_cpus, split_bed12_blocks, progress_bar=progress_bar
    )
...
def retain_all_thick_only(bed_df, prune_no_thick=True, progress_bar=True, num_cpus=1):
    ...
    ret = parallel.apply_parallel(
        bed_df[~m_no_thick], num_cpus, retain_thick_only, progress_bar=progress_bar
    )
...
def retain_all_five_prime_of_thick(
    bed_df: pd.DataFrame, progress_bar: bool = True, num_cpus: int = 1
):
    ...
    ret = retain_all_thick_only(bed_df, progress_bar=progress_bar, num_cpus=num_cpus)

    if len(ret) == 0:
        msg = (
            "[bed_utils.retain_all_five_prime_of_thick]: No annotated 5' "
...
def retain_all_three_prime_of_thick(
    bed_df: pd.DataFrame, progress_bar: bool = True, num_cpus: int = 1
):
    """Given a bed12+ data frame, this function retains only the portion of
    ...
    if len(ret) == 0:
        msg = (
            "[bed_utils.retain_all_three_prime_of_thick]: No annotated 3' "
    ...
    while next_p_site_position != np.inf:
        ...
        if next_p_site_position < next_exon_start:
            ...
            rel_pos = [
                position_interval_intersection(
                    c,
                    next_p_site_position - interval_starts[c],
                    interval_info[c],
                    position_info[next_p_site_index],
                )
                for c in cache
            ]
    ...
    * IT DOES NOT SPLIT INTERVALS_BED BASED ON BED12 FIELDS *
    ...
    for seqname in seqnames:
        for strand in strands:
            ...
            matches = get_position_intersections(
                positions,
                interval_starts,
                interval_ends,
...
def get_exact_bed_matches(bed_a, bed_b, seqname, strand):
    ...
    if len(a_starts) == 0 or len(b_starts) == 0:
        msg = (
            "seqname: {}, strand: {}. Did not find any transcripts for a or "
            "b. len(a_starts): {}, len(b_starts): {}".format(
                seqname, strand, len(a_starts), len(b_starts)
            )
        )
        logger.warning(msg)
        return []

    matches = get_exact_interval_matches(
        a_starts, a_ends, a_info, b_starts, b_ends, b_info
    )
    exact_block_matches = get_exact_block_matches(
        matches, block_counts_a, block_counts_b
    )
...
def _get_bed_exons(bed_a, bed_b, exons, exons_a, exons_b):
    ...
    if exons is not None:
        if exons_a_passed or exons_b_passed:
            msg = (
                "[get_bed_overlaps]: only exons or exons_{a,b} may be "
    ...
    if only_a or only_b:
        msg = (
            "[get_bed_overlaps]: either both or neither of exons_{a,b} must "
            "be provided."
        )
        raise RuntimeError(msg)
...
#   among bed entries.
###
interval_overlap = collections.namedtuple("interval_overlap", "a_info,b_info,overlap")

transcript_overlap = collections.namedtuple(
    "transcript_overlap", "a_info,b_info,overlap,a_fraction,b_fraction"
)
    ...
    while (next_a_interval < num_a_intervals) or (len(a_cache) != 0):
        ...
        if next_a_start < next_b_start:
            ...
            for b in b_cache:
                ...
                io = interval_overlap(a_info[next_a_interval], b_info[b], overlap)
            ...
            for a in a_cache:
                ...
                io = interval_overlap(a_info[a], b_info[next_b_interval], overlap)
...
def get_transcript_overlap_fractions(
    transcript_overlaps, a_lengths_map, b_lengths_map, min_a_overlap=0, min_b_overlap=0
):
    ...
    for (a_info, b_info), overlap in transcript_overlaps.items():
        ...
        to = transcript_overlap(a_info, b_info, overlap, a_fraction, b_fraction)
    ...
    *** Alternatively, if the exons are passed in, then the 'id's from A and
    ...
    for seqname in seqnames:
        for strand in strands:
            ...
            overlaps = get_interval_overlaps(
                a_starts, a_ends, a_info, b_starts, b_ends, b_info
            )
            transcript_overlaps = get_transcript_overlaps(overlaps)

            transcript_fractions = get_transcript_overlap_fractions(
                transcript_overlaps,
                a_lengths_map,
                b_lengths_map,
...
def get_entries_with_upstream_overlaps(
    ...
    *** Alternatively, if the exons are passed in, then the 'id's from A and
    ...
    upstream_overlaps = {(o.a_info, o.b_info): o for o in upstream_overlaps}
...
def get_bed_sequence(bed_entry, seq_sequence, split_exons=True):
    ...
    if not split_exons:
        ...
        transcript_sequence = [
            seq_sequence[s:e] for s, e in zip(genomic_starts, genomic_ends)
        ]
...
def get_all_bed_sequences(
    bed, fasta_file, split_exons=True, bed6=False, progress_bar=True
):
    ...
    if (num_columns < 12) and (not bed6):
        msg = (
            "The bed file must contain at least twelve columns."
    ...
    if bed6:
        msg = (
            "The bed6 argument is set to True, hence the exons of"
    ...
    for (seqname, sequence) in fasta:
        ...
        transcript_sequences = parallel.apply_df_simple(
            bed[m_seqname],
            get_bed_sequence,
            sequence,
            split_exons,
    ...
    if seqname_order is not None:
        if isinstance(seqname_order, str):
            if os.path.exists(seqname_order):
                ...
                msg = (
                    "The chr_name_file was specified, but the file does not "
