...
def start_end_code(parser: Parser, content: str):
  if parser.in_code:
    # when parsing under the "default" rule
...
def main():
  ...
  # find the .maddog directory 
  maddog_path = os.getcwd()
  cwd = maddog_path

  contents = os.listdir()
  while '.maddog' not in contents:
    os.chdir("..")

    if maddog_path == os.getcwd(): 
      # we can't go any further
      logging.error("This is not a maddog project (could not find .maddog), aborting.")

    maddog_path = os.getcwd() 

    contents = os.listdir()
  
  maddog_path = os.path.join(maddog_path, '.maddog')
  maddog_contents = os.listdir(maddog_path)

  # register extensions
  if 'maddog_extensions.py' in maddog_contents:
    logging.info("Registering extensions.")
    sys.path.append(maddog_path)

    import maddog_extensions
    maddog_extensions.register_extensions(parser)
  
  include_path = os.path.join(maddog_path, 'include')

  # go back into original working directory
  os.chdir(cwd)
