...
class DatasetRequestModel(object):
    ...
    def __init__(self, name=None, description=None, default_profile_id=None, schema=None, region=None, cloud_platform=None, enable_secure_monitoring=False, phs_id=None, experimental_self_hosted=False, properties=None, dedicated_ingest_service_account=False, local_vars_configuration=None):  # noqa: E501
    ...
    def dedicated_ingest_service_account(self):
        ...
        If true, for GCP-backed datasets, a service account will be created in the dataset's primary GCP project. This service account must be granted `storage.objects.get` permissions on any source buckets that TDR will ingest data from. The service account can be obtained when retrieving the dataset.  If this value is false and the dataset is a GCP-backed dataset, then the global service account must be granted `storage.objects.get` permissions to source buckets. Note: this can be done by granting the \"Storage Object Viewer\", \"Storage Legacy Object Reader\", or \"Storage Legacy Object Owner\" role to the source buckets This might be useful to set as a security improvement since this provides a dedicated account that explicitly does not have access to any other buckets.  It's also recommended if you need to authorize a large number of buckets since the main service account could run into Google group membership quota violations.   # noqa: E501
