...
class SpawnTicket(dd.Action):
    ...
    def run_from_ui(self, ar, **kw):
        ...
        # parent.comments.create(body="@" + ar.get_user().username + " " + _("created new child ticket") + " " + "#" + str(child.id))
        comment = rt.models.comments.Comment(
            body="@" + ar.get_user().username + " " + _("created new child ticket") + " " + "#" + str(child.id)
        )
...
def setup_memo_commands(sender=None, **kwargs):
    ...
    def py2html(parser, s, cmdname, usages):
