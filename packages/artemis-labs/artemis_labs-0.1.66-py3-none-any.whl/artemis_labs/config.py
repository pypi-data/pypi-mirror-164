...
def line_graph(arr, named_args_dict : Dict) -> Tuple:
    ...
    plots a line graph from those values, using named_args_dict
    to customize the graph, and returns a serialized form
...
def line_graph_x_y_z(arr , named_args_dict : Dict) -> Tuple:
    '''
    This takes in an array of 3 containers of numerical values and named args,
    plots a line graph using the first container as the x's, the second
    as the y's, the third as the z's, and using named arguments to customize the graph,
    and returns a serialized form of the matplotlib figure
    :param arr: Container of 3 containers of numerical values to graph (List or Numpy)
    :param named_args_dict: Named arguments provided when invoking decorator
    :return: Serialized line graph
    '''
