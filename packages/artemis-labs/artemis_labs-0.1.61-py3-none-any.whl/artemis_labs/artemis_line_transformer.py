    ...
    def transform_script(lines : List[str], init_offset=10) -> List[str]:
        ...
        for i, line in enumerate(lines):
            ...
            if 'app = Artemis' in line:
                ...
                run_info_str = '### *System Information* \\n'
                for field_name, field_value in run_info.items():
                    run_info_str += f'__**{field_name}**__: {field_value} \\n'
                transformed_lines.append(f"app.create_output(0, 0, '', '{run_info_str}', 'markdown', '')")
                continue
            ...
            if doc_accum_active and (line.lstrip().startswith("\'\'\'") or line.lstrip().startswith("\"\"\"")):
                ...
                transformed_lines.append(
                    f"{doc_accum_indentation}app.create_output({doc_accum_line_start - 1}, {i - init_offset + 1}, '', {str_prefix}'{doc_accum}', 'markdown', '')")
                doc_accum_active = False
                doc_accum_line_start = -1
                doc_accum = ''
                doc_accum_indentation = ''
            ...
            if i > 0 and not doc_accum_active and line.lstrip().startswith("\'\'\'") or line.lstrip().startswith("\"\"\"") and "@blockdoc" in line:
            ...
            if token_chain.is_valid() and not in_block_quote:
                ...
                if '@input' == token_chain.get_command() and has_data_arg:
                    ...
                    # append fodder
                    transformed_lines.append(line)
                    transformed_lines.append(f"{token_chain.get_indentation()}app.create_input({i - init_offset + 1}, {i - init_offset + 2}, 'Variable {data}', '')")
