...
class Artemis:
    ...
    def callback_handler(self, message : str) -> None:
        ...
        if message['type'] != 'ping':
            ...
            if message['type'] == 'query':
                ...
                t = time.time()
                t_ms = int(t * 1000)
                with open(os.path.join(os.getcwd(), f'archive_{t_ms}.html'), 'w', errors='ignore') as file:
                   file.write(archive)
                ...
                if os.name == 'nt':
                    ...
                    if is_file:
                        ...
                        print('[Artemis] Opening dir: ' + message['content'])
                        os.popen('explorer "' + message["content"] + '"', 'r')
                else:
    ...
    def wait_for_next(self) -> None:
        ...
        :return: None
        '''
        self.artemis_socket.send(json.dumps({"type" : "wait-for-next"}))
