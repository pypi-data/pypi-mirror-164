...
class ArtemisHelper:
    ...
    def serialize(data : Any, data_type : ArtemisType, named_args=[]) -> str:
        ...
        if data_type == ArtemisType.MATPLOTLIB_PLOT:
            ...
            graph_serialized = {
                'data' : ArtemisHelper.__b64_encode_bytes(ArtemisHelper.__matplotlib_fig_to_bytes(data)),
                'width' : graph_size[0],
                'height' : graph_size[1]
            }
            if 'max-width' in named_args:
                graph_serialized['max-width'] = named_args['max-width']
            if 'max-height' in named_args:
                graph_serialized['max-height'] = named_args['max-height']
            return ('graph', graph_serialized)
