...
def draw(rng, n, params, dep_params=None, dep_fn=None, values=None):
    ...
    # Sort parameter names to ensure a consistent sampling order.
    indep_names = sorted(list(params.keys()))
    dep_names = sorted(list(dep_params.keys()))
    ...
    subspace_samples = sample_subspaces(rng, n, param_shapes)
    ...
    for name in indep_names:
        dist = params[name]
        values[name] = lhs_values(name, dist, subspace_samples[name])
