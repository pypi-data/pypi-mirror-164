...
class DBAPICursor(with_metaclass(abc.ABCMeta, object)):
    ...
    def _fetch_while(self, fn):
        _logger.info("Commons - fetch while.... fn type - {}".format(type(fn)))
        while fn():
            _logger.info("fetching next fetch more from fetch while")
            self._fetch_more()
            if fn():
    ...
    def fetchone(self):
        ...
        if not self._data:
            ...
            #     _logger.info("Commons - Breaking the loop after 1000Commons - fetch many records")
            #     return None
            self._rownumber += 1
