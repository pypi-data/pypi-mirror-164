...
WEDATA_AUTH_SERVICE = "https://wedata-api-fusion-test.cloud.tencent.com/safe/v1/queryAccountMapping"
WEDATA_SYSTEM_CONFIG_FILES_SERVICE = "https://wedata-api-fusion-test.cloud.tencent.com/cloudapi/platformgateway/v1/platform/DescribeConfigFiles"
WEDATA_CUSTOM_CONFIG_FILES_SERVICE = "https://wedata-api-fusion-test.cloud.tencent.com/cloud/datasource/v1/DescribeDataSourceConfigFiles"
WEDATA_DATA_SOURCE = "https://wedata-api-fusion-test.cloud.tencent.com/cloud/datasource/v1/DescribeDatasource"
...
class HiveInitial(object):
    def __init__(self, client):
        ...
        self._hdfs_site = "/tmp/hdfs-site.xml"
        self._krb_conf = "/etc/krb5.conf"
        self._jwt_key = "tbds@2022"
        self._wedata_auth_info = {}
        self._secret_info = client.get_secret_info()
        self._hive_conn_cache = {}

        if not os.path.exists(self._hive_site):
            os.mknod(self._hive_site)
    ...
    def spark_hive_initial(self, emr_id, username=None):
       ...
       :rtype:
       """
        emr_conn_info = self._get_emr_security_auth_info(emr_id, username)
        if emr_conn_info["keytab"] != "":
            self._init_kerberos_ticket(emr_conn_info["principal"], emr_conn_info["keytab"])

    def hive_initial(self, emr_id, username=None):
        """获取emr hive连接句柄
    ...
    def _get_hdfs_client(self):
        ...
        root = tree.getroot()
        for v in root:
            if v[0].text == "dfs.namenode.http-address":
                hdfs_address = "http://" + v[1].text
        ...
        if open_kerberos:
            client = KerberosClient(hdfs_address)
            return client

        client = HdfsClient(hdfs_address)
        return client

    def _get_emr_security_auth_info(self, emr_id, username):
        if username and username in ["root", "hadoop"]:
            raise Exception("Not support username %s" % username)
        ...
        cred = credential.Credential(self._secret_info["secret_id"], self._secret_info["secret_key"])
        client = emr_client.EmrClient(cred, self._secret_info["region"])
        ...
        master_host = node_rsp.NodeList[0].Ip
        ...
        for v in conf_rsp.ServiceConfList:
            ...
            if v.FileName == "krb5.conf":
                ...
                ret["krb5.conf"] = self._krb_conf
                config = configparser.ConfigParser()
                config.optionxform = lambda option: option
                config.read(ret["krb5.conf"])
                config["realms"] = {
                    realm.upper(): """{
                        kdc = %s
                        admin_server = %s
                    }""" % (kdc + ":" + port, admin_server)
                }
                config.write(open(ret["krb5.conf"], "w"))
    ...
    def _get_wedata_security_auth_info(self, wedata_id, source_account):
        ...
        datasource_rsp = requests.post(WEDATA_DATA_SOURCE, json=body,
                                        headers={"Authorization": self._get_wedata_jwt_token(jwt_payload),
                                                 "x-wedata-trace-id": "666666"})
        datasource_rsp = json.loads(datasource_rsp.text)
        ...
        params = json.loads(datasource["ParamsString"])
        ...
        if datasource["Category"] == "DB":
            jwt_payload = {
                "exp": int(time.time() + 3600),
                "user_id": datasource["OwnerAccount"],
                "tenant_id": str(datasource["AppId"]),
                "owner_user_id": datasource["OwnerAccount"],
                "user_name": datasource["OwnerAccountName"],
                "from_type": "OPEN_API"
            }
            body = {
                "Id": wedata_id
            }
            custom_file_rsp = requests.post(WEDATA_CUSTOM_CONFIG_FILES_SERVICE, json=body,
                                            headers={"Authorization": self._get_wedata_jwt_token(jwt_payload),
                                                     "x-wedata-trace-id": time_stamp})
            host, port, config_file_empty = self._save_config_files(custom_file_rsp)
        ...
        jwt_payload = {
            "owner_user_id": source_account,
            "user_id": source_account,
            "exp": int(time.time() + 3600),
            "tenant_id": str(datasource["AppId"]),
            "user_name": "tikit_agent"
        }
        body = {
            "ClusterId": datasource["ClusterId"]
        }
        config_jwt_token = self._get_wedata_jwt_token(jwt_payload)
        config_file_rsp = requests.post(WEDATA_SYSTEM_CONFIG_FILES_SERVICE, json=body,
                                        headers={"Authorization": config_jwt_token, "x-wedata-trace-id": time_stamp})
        host, port, config_file_empty = self._save_config_files(config_file_rsp)
        ...
        auth_jwt_token = self._get_wedata_jwt_token(jwt_payload)
        auth_rsp = requests.post(WEDATA_AUTH_SERVICE, json=body,
                                 headers={"Authorization": auth_jwt_token, "x-wedata-trace-id": time_stamp})
        auth_rsp = json.loads(auth_rsp.text)
    ...
    def _save_config_files(self, config_file_rsp):
        if config_file_rsp.text is None:
            raise Exception("Failed to get configuration file")
        config_file_rsp = json.loads(config_file_rsp.text)
