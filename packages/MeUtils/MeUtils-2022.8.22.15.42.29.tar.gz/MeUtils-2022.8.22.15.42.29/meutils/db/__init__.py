...
class Redis(object):
    ...
    def __init__(self, ips=None, password=None, decode_responses=False, cache_fn=None):
        ...
        if cache_fn:
            self.rc.get = cache_fn(self.rc.get)
...
class SQL(object):
    ...
    def __init__(self, cache_fn=None, **kwargs):
        self.conn = None  # 重写
        if cache_fn:
            pd.read_sql = cache_fn(pd.read_sql)  # todo: 需要重新设计逻辑
