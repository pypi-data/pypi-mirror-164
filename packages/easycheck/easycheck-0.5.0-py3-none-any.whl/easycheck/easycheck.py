...
def check_if(condition,
    ...
    Args:
        ...
        *args, **kwargs: optional arguments passed to the exception class
    ...
    if not condition:
        _raise(handle_with, message, *args, **kwargs)


def check_if_not(condition,
                 handle_with=AssertionError,
                 message=None,
                 *args, **kwargs):
...
def _raise(error, message=None, *args, **kwargs):
    ...
    if issubclass(error, Warning):
        ...
        if message:
            raise error(message, *args, **kwargs)
        else:
            if args or kwargs:
