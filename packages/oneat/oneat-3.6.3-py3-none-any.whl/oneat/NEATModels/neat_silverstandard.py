...
class NEATCynamic(object):
    ...
    def loadData(self):
        ...
        self.X = X
        self.X = tf.reshape(self.X, (self.X.shape[0], self.X.shape[2], self.X.shape[3], self.X.shape[1]))
        self.Y = Y[:, :, 0]
        self.X_val = X_val
        self.X_val = tf.reshape(self.X_val, (self.X_val.shape[0], self.X_val.shape[2], self.X_val.shape[3], self.X_val.shape[1]))
        self.Y_val = Y_val[:, :, 0]
                                ...
                                for p in range(0,len(predictions)):   
                                  ...
                                  if sum_time_prediction is not None:
                                     ...
                                     for i in range(0, sum_time_prediction.shape[0]):
                                          ...
                                          self.key_categories, 
                                          self.key_cord, 
                                          self.nboxes, 'detection', 'static',marker_tree=self.marker_tree, center_oneat = self.center_oneat)
                    ...
                    if sum_time_prediction is not None:
                        ...
                        for i in range(0, sum_time_prediction.shape[0]):
                            ...
                            boxprediction = yoloprediction(ally[p], allx[p], time_prediction, self.stride,
                                                           ...
                                                           'static', marker_tree = self.marker_tree, center_oneat = False)
