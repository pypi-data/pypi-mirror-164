...
class SWeight:
    ...
    def __init__(
        ...
        if not hasattr(pdfs, '__iter__'):
            raise TypeError(
                "The passed pdf list is not an iterable type. It should be a list or tuple"
            )
        self.pdfs = pdfs
        ...
        if method=='summation' or method=='integration':
            for i, pdf in enumerate(self.pdfs):
                if not callable(pdf):
                    raise TypeError(
                        f"The pdf passed at index {i} is not callable"
                    )

        # check yields is iterable
        if not hasattr(yields, '__iter__'):
            raise TypeError(
                "The passed yield list is not an iterable type. It should be a list or tuple"
            )
        self.yields = yields
        ...
        if method=='summation' or method=='integration':
            for i, yld in enumerate(self.yields):
                if not np.isscalar(yld):
                    raise TypeError(
                        f"The yield passed at index {i} is not of scalar type (pass int or float type)"
                    )
