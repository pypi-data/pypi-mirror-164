...
from johnsnowlabs.auto_install.install_info import InstallFileInfoBase, JvmInstallInfo, PyInstallInfo, \
    InstallFileInfovBase2, \
...
from johnsnowlabs.domain_models.url_dependency import UrlDependency
import os
...
def is_jsl_home_outdated():
    if os.path.exists(settings.root_info_file):
        ...
        if old_infos.version.as_str() == settings.version:
            return False
        else:
            return True
...
def download_deps_and_create_info(deps: List[UrlDependency],
    ...
    if info:
        InstallFolder(**{'infos': info}).write(info_file_path, indent=4)
...
def setup_jsl_home(
    ...
    RootInfo(version=settings.version, run_from=sys.executable).write(settings.root_info_file, indent=4)
    if secrets:
        #     # TODO overwrite old creds or not?   Or Store Multipel!!!!
        secrets.write(settings.creds_file, indent=4)
...
def get_install_suite_from_jsl_home(create_jsl_home_if_missing: bool = True,
    ...
    suite = InstallSuite(
        nlp=LocalPy4JLib(java_lib=java_folder.get_product_entry(ProductName.nlp) if java_folder else None,
                         py_lib=py_folder.get_product_entry(ProductName.nlp) if py_folder else None),
        hc=LocalPy4JLib(java_lib=java_folder.get_product_entry(ProductName.hc) if java_folder else None,
                        py_lib=py_folder.get_product_entry(ProductName.hc) if py_folder else None),
        ocr=LocalPy4JLib(java_lib=java_folder.get_product_entry(ProductName.ocr) if java_folder else None,
    ...
    if any((not suite.ocr.java_lib and suite.secrets.OCR_LICENSE and ocr,
            not suite.hc.java_lib and suite.secrets.HC_LICENSE and hc,
            not suite.nlp.java_lib and nlp)) and not recurisve_call:
        print(
            f'🤓 The license data you provided has access to features for which no Jars are downloaded yet. Fetching...')
        setup_jsl_home(license_data, only_jars=only_jars, overwrite=True)
