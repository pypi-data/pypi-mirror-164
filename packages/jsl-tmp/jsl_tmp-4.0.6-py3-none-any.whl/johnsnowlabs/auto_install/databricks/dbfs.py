...
from johnsnowlabs.auto_install.install_info import  PyInstallInfo, JvmInstallInfo
...
def get_db_path(local_info: Union[JvmInstallInfo, PyInstallInfo, str]):
    ...
    if isinstance(local_info, JvmInstallInfo):
        ...
        if '.py' in local_info:
            return f"{settings.db_py_jobs_dir}/{local_info.split('/')[-1]}"
        elif '.jar' in local_info:
            return f"{settings.db_jar_jobs_dir}/{local_info.split('/')[-1]}"
        elif '.ipynb' in local_info:
            return f"{settings.db_py_notebook_dir}/{local_info.split('/')[-1]}"
        else:
