...
class BaseTableExpression(BaseTable):
    ...
    def go(self, page_size=100000, timeout: Optional[int] = 3600, keep_for: Optional[int] = 7200, quiet=False) -> DataFrame:
        ...
        """
        job = self.go_async(timeout=timeout, keep_for=keep_for)
    ...
    def go_async(self, timeout: Optional[int] = 3600, keep_for: Optional[int] = 7200, _print_fn=None) -> QueryJob:
        ...
        """
        sql = self.get_sql()
        ex_id = self._client.start_query(
