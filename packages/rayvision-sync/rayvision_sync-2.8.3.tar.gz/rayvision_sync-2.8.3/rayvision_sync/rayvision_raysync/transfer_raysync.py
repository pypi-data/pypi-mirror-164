...
from rayvision_sync.rayvision_raysync.exception import *
from rayvision_sync.rayvision_raysync.constants import *
...
class RayvisionTransferRaysync():
    ...
    def __init__(self, user_id, user_name, user_key, platform, logger=None, raysyncdirpath=None):
        """
        ...
        self._raysyncdirpath = raysyncdirpath if raysyncdirpath else os.path.join(os.path.dirname(__file__), RAYSYNC_NAME[self._system])
        self._raysyncexe = os.path.join(self._raysyncdirpath, RAYSYNCEXE[self._system])
        self.auto_download()
    ...
    def auto_download(self):
        ...
        try:
            if not os.path.exists(self._raysyncexe):
                self.logger.debug('No Raysyncweb is detected. Prepare to download the latest Raysyncweb automatically!')
                raysync_link = DOWNLINK[self._system]
                raysync_name = os.path.basename(raysync_link)
                zip_path = os.path.join(os.path.dirname(__file__), raysync_name)
                response = requests.get(raysync_link)
    ...
    def post(self, api_url, data):
        """Create a common request mode
            ...
            :param data: reuqest parameter eg: {
        ...
        if api_url not in [self._url.start_task, self._url.get_task_status,
            ...
            self.logger.debug('HTTP Headers: %s', pformat(self._headers, width=500))
            self.logger.debug('HTTP Body: %s', pformat(data, width=500))
        response = requests.post(request_address, json=data, headers=self._headers)
    ...
    def _port(self):
        ...
        if not os.path.exists(xml_path):
            raise NotfoundRaysyncInI('Port profile cluster.xml not found, Raysync engine package incomplete！')
        tree = parse(xml_path)
        root = tree.getroot()
        port = root[0][0].attrib['value']
        return port

    def set_transfer_speed(self, max_speed):
        ...
        response = self.post(self._url.set_transfer_speed, params)
    ...
    def listening_raysync_server(self):
        ...
        try:
            if not self.service_statu:
                self.close_raysync()
                self.logger.info('Start the Raysync-man.exe to listen for the process(port=%s)....' %(self._port))
                self.raysync_app = subprocess.Popen(self._raysyncexe)
                result_code = True
                while result_code:
                    try:
                        url = self._baseUrl + self._url.check_raysync_http
                        response = requests.post(url, headers=self._headers, json={"sign": "render-bus"}).json()
            ...
            raise LaunchRaysyncfailed("Description Failed to start the Raysync client!")
    ...
    def convert_upload(self, upload_path, input_id):
        ...
        upload_json_data = json.load(codecs.open(upload_path, "r", encoding="utf-8"))
        asset_list = upload_json_data.get('asset')
        for item in asset_list:
            item["server"] = "/input/%s-%s" % (input_id, self.user_id) + item["server"]
        upload_list_file = os.path.join(os.path.dirname(upload_path), "upload_list.json")
        upload_dict = {"list": asset_list}
        json.dump(upload_dict, codecs.open(upload_list_file, "w", encoding="utf-8"), separators=(',\n', ': '))
    ...
    def get_transfer_path(self, task_type, local_path, server_path, storage_id, task_id, file_type, downstorage):
        ...
        if task_type == "upload":
            if file_type == "normal":
                ...
                path_dict["target-path"] = "/input/%s-%s/%s" % (storage_id, self.user_id, server_path)
            elif file_type == "json":
                if not task_id:
                    raise TaskidNotexsit("The task ID must not be empty!")
                path_dict["source-path"] = local_path
                path_dict["target-path"] = "/input/%s-%s/%s/cfg" % (storage_id, self.user_id, task_id)
            else:
            ...
            path_dict["file-list"] = self.convert_upload(local_path, storage_id)
        elif task_type == "download":
            path_dict["source-path"] = "/%s/%s-%s/%s" % (downstorage, storage_id, self.user_id, server_path)
    ...
    def start_transfer(self, server_ip, server_port, local_path, server_path, storage_id, task_type=None,
                       task_id=None, file_type="normal", downstorage="output", max_speed=None, max_timeout=18000, network_mode=1):
        """
        ...
        self.set_transfer_speed(max_speed)
        if task_id not in self._task_failed_dict:  #first create task
            path_dict = self.get_transfer_path(task_type, local_path, server_path, storage_id, task_id, file_type, downstorage)
            ...
            self.logger.info('Transfer task has been created. The Transfer task ID is %s' %(tranfer_task_id))
        else:   #重新启动任务
            params = {
            ...
            self.logger.info('Render task_id is %s,Transfer task id %s already try again......' % (task_id, tranfer_task_id))
    ...
    def look_task_stauts(self, tranfer_task_id, task_id, max_timeout):
        ...
        while True:
            if now_seconds >= max_timeout/STATU_SLEEP:
                raise TransferTimeout("Transmission timeout! The maximum transmission time is 5 hours by default!")
            now_seconds += 1
            time.sleep(STATU_SLEEP)
            params = {"task-id": tranfer_task_id}
            response = self.post(self._url.get_task_status, params)
            statu = response["task-list"][0]["task-state"]
            if statu in ["ready", "start", "idle"]:
                continue
            else:
                if statu == "failed":
                    self._task_failed_dict[task_id] = tranfer_task_id
                elif statu == "successful":
                    self._task_failed_dict.pop(task_id, None)
                else:
                    raise CreatTaskFailed("create transfer task failed! message is %s" %(statu))
                break
        result_code = self._resultDict.get(statu)
        self.logger.info('The transmission task is complete and the status of the task is %s, result_code is %s!' % (statu, result_code))
        return result_code
