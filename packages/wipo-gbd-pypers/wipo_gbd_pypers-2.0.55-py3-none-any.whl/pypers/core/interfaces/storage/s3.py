...
class S3Storage:
    ...
    def do_store(self, file, bucket, location):
        s3_bucket = self.get_storage(bucket)
        s3_bucket.save_file_with_location(file, location)
...
class S3Interface(StorageBase):
    ...
    def __init__(self, bucket_name):
        ...
        worker = Thread(target=fast_upload, args=(enclosure_queue,))
        worker.setDaemon(True)
        worker.start()
        self.enclosure_queue.join()

    def fast_upload(self, q):
        s3t = s3transfer.create_transfer_manager(self.s3client, self.transfer_config)
        while True:
            raw_data = q.get()
            src = raw_data.get('src')
            dst = raw_data.get('dst')
            s3t.upload(
                src, self.bucket_name, dst,
            )
            q.task_done()
        s3t.shutdown()
    ...
    def save_file_with_location(self, local_path, key, string_input=None):
