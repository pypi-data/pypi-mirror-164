...
PyDoc_STRVAR(window_doc,
	...
	"\n"\
	"igeCore.window(show, width, hight, resizable, sync60frame)\n"\
	"\n"\
	"Parameters\n"\
	"----------\n"\
	"    show : bool\n"\
	...
	"    resizable : bool\n"\
	"        Make the window size variable\n"\
	"    fixedFrameRate : bool\n"\
	"        Fix frame rate to 1/60\n");
...
//deviceSize
PyDoc_STRVAR(deviceSize_doc,
	"Returns the physical resolution of the screen\n"\
	"\n"\
	"(w,h) = igeCore.deviceSize()\n"\
	"\n"\
	"Returns\n"\
	"-------\n"\
	"    w : float\n"\
...
PyDoc_STRVAR(showVirtualKeyboard_doc,
	"show the virtual keyboard\n"\
	"\n"\
	"igeCore.showVirtualKeyboard(value)\n"\
	"\n"\
	"Parameters\n"\
	"----------\n"\
...
PyDoc_STRVAR(hideVirtualKeyboard_doc,
    "hide the virtual keyboard\n"\
    "\n"\
    "igeCore.hideVirtualKeyboard()");

//get input text
PyDoc_STRVAR(getInputText_doc,
    "get input text\n"\
    "\n"\
    "result = igeCore.getInputText()"\
    "Returns\n"\
    "-------\n"\
    "    result : string");

//is virtual keyboard shown
PyDoc_STRVAR(isVirtualKeyboardShown_doc,
    "check if the virtual keyboard is shown\n"\
    "\n"\
    "result = igeCore.isVirtualKeyboardShown()"\
    "Returns\n"\
    "-------\n"\
    "    result : bool");
