...
def build_procedure(procedure_code, build_phases=False, graphics=True,
    ...
    if build_phases:
        for phase in procedure.phases:
            phase_files = next(pf for pf in procedure_files.phase_files if pf.phase_code == phase.code)
            build_phase(phase, phase_files, graphics, pip_graphics,
                        widget_graphics, thumbnails, step_numbers=step_numbers,
                        info_step=info_step)
...
def build_single_phase(phase_code, graphics=True, pip_graphics=True,
    ...
    build_phase(phase, phase_files, graphics, pip_graphics, widget_graphics,
                thumbnails, step_numbers=step_numbers, info_step=info_step,
                csv_version=csv_version)


def build_phase(phase, phase_files, graphics, pip_graphics, widget_graphics,
                thumbnails, step_numbers=True, info_step=True,
                supported_app="touchsurgery", csv_version=None):
...
def setup_procedure_files(procedure, procedure_files):
    ...
    if os.path.isdir(settings.BASE_DATA_DIR):
        delivery_files = DeliveryFileManager(procedure.code, procedure.phases)
