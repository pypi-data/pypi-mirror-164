...
from buoy_tracking import geo_tools, model_fetch


#DEFAULT_TOPAZ_PATH = (os.path.join(os.path.dirname(__file__), "topaz"))
DEFAULT_TOPAZ_PATH = tempfile.gettempdir()
...
def advanced_forecast(buoy_position, target_time, full_forecast=False, topaz_path=None,
    ...
    if topaz_path is None:
        topaz_path = DEFAULT_TOPAZ_PATH
    elif topaz_path == 'azure':
        a = 1
    ...
    if valid_file is None:
        dl_success, topaz_filename, topaz_start, topaz_end = fetch_topaz_forecast(buoy_position, target_time, topaz_path)
        # return just the last known point if the model download failed
        if not dl_success:
            logger.error("Error using advanced forecast, model download failed")
    ...
    try:
        ...
        if retry and not topaz_update:
            ...
            return advanced_forecast(buoy_position, target_time, topaz_path=topaz_path,
...
def fetch_topaz_forecast(buoy_position, target_time, output_dir):
    ...
    username = os.getenv('TOPAZ_USERNAME') #credentials.LOGIN['topaz_username']
    password = os.getenv('TOPAZ_PASSWORD') #credentials.LOGIN['topaz_password']
    
    # Authenticate with the model data server
    conn_session = model_fetch.copernicus_marine_login(url, username, password)
    # Define the model parameters we want to download
    payload = model_fetch.order_payload(date_min, date_max)
    # Request the data
    dl_success = model_fetch.download_data(url, payload, os.path.join(output_dir, output_filename), conn_session)
    ...
    #os.system(cmd)
    return dl_success, os.path.join(output_dir, output_filename), date_min, date_max


def check_existing_topaz(topaz_start, topaz_end, date_start, date_end, topaz_filename):
    """
