...
class BoxAccessTokenDomain:
    ...
    def load(
        self,
        request: request_.GetBoxRequest,
    ) -> result_.GetBoxResult:
    ...
    def reset(
        self,
        request: request_.ResetBoxRequest,
    ) -> result_.ResetBoxResult:
        request.with_namespace_name(self._namespace_name)
        request.with_access_token(self._access_token.token if self._access_token else None)
        request.with_prize_table_name(self._prize_table_name)
        r = self._client.reset_box(
            request,
        )
        return r
