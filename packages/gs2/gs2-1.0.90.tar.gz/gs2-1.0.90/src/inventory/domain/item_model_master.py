...
class ItemModelMasterDomain:
    ...
    def load(
        self,
        request: request_.GetItemModelMasterRequest,
    ) -> result_.GetItemModelMasterResult:
        request.with_namespace_name(self._namespace_name)
        request.with_inventory_name(self._inventory_name)
        request.with_item_name(self._item_name)
        r = self._client.get_item_model_master(
            request,
        )
        self._item_model_master_cache.update(r.item)
        return r

    def update(
        self,
        request: request_.UpdateItemModelMasterRequest,
    ) -> result_.UpdateItemModelMasterResult:
        request.with_namespace_name(self._namespace_name)
        request.with_inventory_name(self._inventory_name)
        request.with_item_name(self._item_name)
        r = self._client.update_item_model_master(
            request,
        )
        self._item_model_master_cache.update(r.item)
        return r
    ...
    def delete(
        self,
        request: request_.DeleteItemModelMasterRequest,
    ) -> result_.DeleteItemModelMasterResult:
        request.with_namespace_name(self._namespace_name)
        request.with_inventory_name(self._inventory_name)
        request.with_item_name(self._item_name)
        r = self._client.delete_item_model_master(
            request,
        )
        self._item_model_master_cache.delete(r.item)
        return r
