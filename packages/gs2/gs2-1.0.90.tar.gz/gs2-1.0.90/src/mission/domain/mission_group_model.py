...
from mission.domain.mission_task_model import MissionTaskModelDomain
...
class MissionGroupModelDomain:
    ...
    def __init__(
        ...
        mission_group_model_cache: MissionGroupModelDomainCache,
        namespace_name: str,
        ...
        self._mission_group_model_cache = mission_group_model_cache
        self._namespace_name = namespace_name
        self._mission_group_name = mission_group_name
        self._mission_task_model_cache = MissionTaskModelDomainCache()

    def load(
        self,
        request: request_.GetMissionGroupModelRequest,
    ) -> result_.GetMissionGroupModelResult:
        request.with_namespace_name(self._namespace_name)
        request.with_mission_group_name(self._mission_group_name)
        r = self._client.get_mission_group_model(
            request,
        )
        self._mission_group_model_cache.update(r.item)
        return r

    def mission_task_models(
        self,
    ) -> DescribeMissionTaskModelsIterator:
        return DescribeMissionTaskModelsIterator(
            self._mission_task_model_cache,
            self._client,
