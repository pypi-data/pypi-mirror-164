...
class SalesItemMasterDomain:
    ...
    _namespace_name: str
    _sales_item_name: str

    def __init__(
        self,
        session: Gs2RestSession,
        sales_item_master_cache: SalesItemMasterDomainCache,
        namespace_name: str,
        sales_item_name: str,
    ):
        ...
        self._sales_item_master_cache = sales_item_master_cache
        self._namespace_name = namespace_name
        self._sales_item_name = sales_item_name

    def load(
        self,
        request: request_.GetSalesItemMasterRequest,
    ) -> result_.GetSalesItemMasterResult:
        request.with_namespace_name(self._namespace_name)
        request.with_sales_item_name(self._sales_item_name)
        r = self._client.get_sales_item_master(
            request,
        )
        self._sales_item_master_cache.update(r.item)
        return r

    def update(
        self,
        request: request_.UpdateSalesItemMasterRequest,
    ) -> result_.UpdateSalesItemMasterResult:
        request.with_namespace_name(self._namespace_name)
        request.with_sales_item_name(self._sales_item_name)
        r = self._client.update_sales_item_master(
            request,
        )
        self._sales_item_master_cache.update(r.item)
        return r
    ...
    def delete(
        self,
        request: request_.DeleteSalesItemMasterRequest,
    ) -> result_.DeleteSalesItemMasterResult:
        request.with_namespace_name(self._namespace_name)
        request.with_sales_item_name(self._sales_item_name)
        r = self._client.delete_sales_item_master(
            request,
        )
        self._sales_item_master_cache.delete(r.item)
        return r
