...
class FriendAccessTokenDomain:
    ...
    def delete(
        self,
        request: request_.DeleteFriendRequest,
    ) -> result_.DeleteFriendResult:
        request.with_namespace_name(self._namespace_name)
        request.with_access_token(self._access_token.token if self._access_token else None)
        r = self._client.delete_friend(
            request,
        )
        return r
    ...
    def list(
        ...
        return DescribeFriendsIterator(
