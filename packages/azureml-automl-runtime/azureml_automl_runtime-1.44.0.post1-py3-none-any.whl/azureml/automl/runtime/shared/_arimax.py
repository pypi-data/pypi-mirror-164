...
from azureml.automl.runtime.shared.forecasting_ts_utils import extend_SARIMAX
...
class Arimax(_MultiGrainForecastBase):
    ...
    def _extend_single_series_impl(self, tsds_context: TimeSeriesDataSet, series_id: GrainType) -> Any:
        ...
        if len(self._transfer_exogenous_colnames) > 0:
            missing_cols = set(self._transfer_exogenous_colnames) - set(tsds_context.data.columns)
            ...
            X_new_df = tsds_context.data[self._transfer_exogenous_colnames].set_index(idx)
        else:
            X_new_df = None
