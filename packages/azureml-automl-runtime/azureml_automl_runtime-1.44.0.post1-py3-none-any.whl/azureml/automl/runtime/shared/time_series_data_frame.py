...
def construct_tsdf(
    ...
    if grain_column_names is None or len(grain_column_names) == 0 or \
            (constants.TimeSeriesInternal.DUMMY_GRAIN_COLUMN not in X.columns  # type: ignore
    ...
    try:
        ...
        X.drop(target_column_name, inplace=True, axis=1)  # type: ignore
        if has_dummy_grain:
            X.drop(constants.TimeSeriesInternal.DUMMY_GRAIN_COLUMN, inplace=True, axis=1)  # type: ignore
