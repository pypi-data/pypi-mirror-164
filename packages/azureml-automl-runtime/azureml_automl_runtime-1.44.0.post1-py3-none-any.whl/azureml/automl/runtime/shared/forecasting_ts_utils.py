...
def extend_SARIMAX(sarimax_results: Any, endog: Union[np.ndarray, pd.Series],
    ...
    # SARIMAX has an extension API, but it fails if the model was trained with the low memory option.
    # In that case, we can make it work by retrieving the latest values of the predicted state and its covariance
    # from the internal Kalman Filter object and applying the trained filter on the context.
    try:
        extended_results = sarimax_results.extend(endog, exog=exog)
    except TypeError:
        from statsmodels.tsa.statespace.mlemodel import Initialization
        kwargs: Dict[str, Any] = {}
        kwargs.setdefault('trend_offset', sarimax_results.nobs + 1)
        mod = sarimax_results.model.clone(endog, exog=exog, **kwargs)
        mod.ssm.initialization = \
            Initialization(mod.k_states, initialization_type='known',
                           constant=sarimax_results.filter_results.predicted_state[..., -1],
                           stationary_cov=sarimax_results.filter_results.predicted_state_cov[..., -1])
        extended_results = sarimax_results._apply(mod, refit=False, **kwargs)
