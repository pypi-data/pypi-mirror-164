...
class TimeSeriesImputer(AzureMLForecastTransformerBase, _GrainBasedStatefulTransformer):
    ...
    def __init__(self, input_column: Union[str, List[str]],
                 option: str = 'fillna',
                 method: Optional[Union[Dict[str, List[Any]], str]] = None,
                 value: Optional[Any] = None, limit: Optional[int] = None,
                 limit_direction: Optional[str] = 'forward', order: Optional[str] = None,
                 freq: Optional[Union[str, pd.DateOffset]] = None,
                 origin: Optional[str] = None, end: Optional[str] = None,
    ...
    @classmethod
    def _impute_with_interpolation_single_group(cls, single_group_data: pd.DataFrame,
    ...
    @classmethod
    def _impute_with_fillna_single_group(cls, single_group_data: pd.DataFrame,
                                         **kwargs: Any) -> pd.DataFrame:
        """
        Impute missing values for all columns which contains single group data.
    ...
    def _impute_missing_value_by_cols(
            cls, input_df: TimeSeriesDataSet, input_column: List[str], sort_index_by_col: str,
            by_cols: Optional[List[str]] = None,
            option: str = 'interpolate', freq: Optional[pd.DateOffset] = None,
            origin: Optional[str] = None, end: Optional[str] = None,
            external_datetime_gap_filler: bool = False, **kwargs: Any) -> TimeSeriesDataSet:
        ...
        if not external_datetime_gap_filler:
            input_df_filled = input_df_filled.fill_datetime_gap(
                freq=freq, origin=origin, end=end)

        if option == 'interpolate':
            if not by_cols:
                input_df_filled.data[input_column] = \
                    cls._impute_with_interpolation_single_group(
                        input_df_filled.data[input_column], **kwargs)
            else:
                input_df_filled.data[input_column] = input_df_filled.data.groupby(
    ...
    @classmethod
    def _condense_values_by_column(cls, input_df: pd.DataFrame,
                                   value_cols: List[str], by_cols: List[str]) -> pd.DataFrame:
        """
    ...
    def transform(self, X: TimeSeriesDataSet) -> TimeSeriesDataSet:
        ...
        if self.option == 'interpolate':
            ...
            kwargs = {'method': self.method, 'limit': self.limit,
                      ...
                      'order': self.order}  # type: Dict[str, Any]
            ...
            kwargs = {'method': self.method, 'value': self.value,
                      'limit': self.limit, 'known_df': known_df, 'time_index': X.time_column_name}
        else:
        ...
        if X.origin_time_column_name is None:
            return self._impute_missing_value_by_cols(
                ...
                by_cols=X.time_series_id_column_names, option=self.option, freq=freq,
            ...
            for col in self.input_column:
                ...
                if self._check_value_same_by_column(
                        input_df=X.data, value_col=col,
                        by_cols=grain_and_time):
            ...
            if len(cols_same_given_gt) > 0:
                ...
                gt_condensed_df = self._condense_values_by_column(
                    input_df=X.data[cols_same_given_gt],
                    value_cols=cols_same_given_gt,
                    by_cols=grain_and_time)
                gt_condensed_ds = TimeSeriesDataSet(
                    gt_condensed_df, time_column_name=X.time_column_name,
                ...
                gt_condensed_ds = self._impute_missing_value_by_cols(
                    ...
                    sort_index_by_col=X.time_column_name,
                    by_cols=X.time_series_id_column_names, option=self.option,
                ...
                X_copy[cols_same_given_gt] = self._join_df_with_multiindex(
                    gt_condensed_ds.data, X.data.index)
            ...
            if len(cols_same_given_got) > 0:
                ...
                got_condensed_df = self._condense_values_by_column(
                    input_df=X.data[cols_same_given_got],
                    value_cols=cols_same_given_got,
                    by_cols=grain_and_origin)
                got_condensed_ds = TimeSeriesDataSet(
                    got_condensed_df, time_column_name=X.origin_time_column_name,
                    time_series_id_column_names=X.time_series_id_column_names)
                # impute missing value
            ...
            if len(other_cols) > 0:
                if not self.impute_by_horizon:
                    ...
                    if not X.time_series_id_column_names:
                        ...
                        grain_and_horizon_colnames = X.time_series_id_column_names + [
                            HORIZON_COLNAME]
                    tsds_other = TimeSeriesDataSet(
                        X_copy, time_column_name=X.time_column_name,
                        time_series_id_column_names=X.time_series_id_column_names,
                        origin_time_column_name=X.origin_time_column_name
                    )
                    tsds_other = self._impute_missing_value_by_cols(
    ...
    def _get_last_non_nan_value(tsds: TimeSeriesDataSet, input_col: List[str]) -> pd.DataFrame:
        ...
        if not tsds.time_series_id_column_names:
            ...
            df_list = [grain_df for _, grain_df in tsds.data.groupby(
                tsds.time_series_id_column_names, group_keys=False)]
    ...
    def _get_last_non_nan_value_single_grain_list(
        ...
        """
        indices_df_list = [
            TimeSeriesImputer._get_last_non_nan_value_single_grain(df, input_col) for df in df_list]
    ...
    def _validate_default_tsdf(
            default_tsds: TimeSeriesDataSet,
            ffill_col: List[str]
    ) -> None:
        """
