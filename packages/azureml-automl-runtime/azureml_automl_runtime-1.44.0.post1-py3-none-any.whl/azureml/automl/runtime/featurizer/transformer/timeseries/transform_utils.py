...
class OriginTimeMixin(object):
    ...
    def create_origin_times(self, X: TimeSeriesDataSet, max_horizon: Union[int, Dict[Any, int]],
                            freq: Optional[Union[str, pd.DateOffset]] = None,
                            origin_time_colname: str = ORIGIN_TIME_COLNAME_DEFAULT,
                            horizon_colname: Optional[str] = None) -> TimeSeriesDataSet:
        """
        ...
        def add_origins_single_grain(gr: Union[str, Tuple[str]],
            ...
            return pd.concat([add_origins_for_horizon(Xgr, h)
                              for h in range(1, h_max + 1)], sort=False)
        ...
        X_df_origins = X.data.merge(origins_df, how='left',
                                    left_index=True, right_index=True)

        # Create a time-series data set with metadata
        #  set appropriately
        return TimeSeriesDataSet(X_df_origins,
    ...
    def detect_max_horizons_by_grain(self,
        ...
        horizons = get_period_offsets_from_dates(X.data.index.get_level_values(X.origin_time_column_name),
                                                 X.time_index,
                                                 freq)
        ...
        max_horizon_series = (horizon_series
                              .groupby(level=X.time_series_id_column_names,
                                       group_keys=False)
                              .max())
