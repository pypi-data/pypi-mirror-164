...
def get_next_scheduled_time(cron_string, use_timezone=False):
    """Calculate the next scheduled time by creating a crontab object
    with a cron string"""
    if isinstance(use_timezone, str):
        tz = dateutil.tz.gettz(use_timezone)
    else:
        tz = dateutil.tz.tzlocal() if use_timezone else dateutil.tz.UTC
    now = datetime.utcnow().replace(tzinfo=dateutil.tz.UTC).astimezone(tz)
    cron = crontab.CronTab(cron_string)
    next_time = cron.next(now=now, return_datetime=True).replace(tzinfo=tz)
    return next_time
