...
class ComponentMessage(Message):
    ...
    def __init__(self, *, state, channel, data, ephemeral=False, interaction: Interaction = None):
    ...
    async def edit(
        ...
        **fields,
        ...
        if suppress is not None and not self.ephemeral:
        ...
        if data:
            await state.http.request(
                Route(
                    ...
                    ) if not self.ephemeral else Route(
                    "PATCH",
                    f"/webhooks/{self.bot.user.id}/{self.interaction.interaction_token}/messages/{self.id}",
                ),
                json=data,
            )

        if delete_after is not None and not self.ephemeral:
