...
class ZoneSenderStone(ZoneSenderFramework):
    ...
    def OnSomeipStateChange(self,timestamp: 'float',service : 'tuple',state : 'bool') -> None :
        ''' [需要用户重写实现定制功能]\n
        订阅的服务状态回调
        :param timestamp: flaot 收到该数据的时间戳
    ...
    def OnLinMessage(self,timestamp: 'float', lin_message: 'LinMessage') ->None :
        ''' [需要用户重写实现定制功能]\n
    ...
    def OnLinFrame(self,timestamp: 'float', lin_frame: 'LinFrame') ->None :
        ''' [需要用户重写实现定制功能]\n
        当收到一个 LIN 报文应该做什么，此函数在收到任何订阅过的 LIN 报文的时候触发
    ...
    def OnLinSignal(self,timestamp: 'float', lin_signal: 'LinSignal') ->None :
        ''' [需要用户重写实现定制功能]\n
