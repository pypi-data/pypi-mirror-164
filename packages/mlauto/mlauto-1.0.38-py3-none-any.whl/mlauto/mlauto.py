...
def send_data_to_db(data):
  ...
  client.sendall(bytes(data,encoding="utf-8")) #a bytes-like object is required, not 'str'
...
def lr_range_finder(network, train_loader, name):
  ...
  metrics = {'lr':str(lr), 'train_loss_lr':str(train_loss_lr), 'name': name, 'task':'initLR', 'username': os.environ["username"], 'key': os.environ["key"]}
