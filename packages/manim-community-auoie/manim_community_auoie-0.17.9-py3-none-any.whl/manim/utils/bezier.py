...
def proportions_along_bezier_curve_for_point(
    ...
    real_roots: list[list[Complex]] = [
        [root for root in rootlist if root.imag == 0] for rootlist in roots
    ]
    reduced_roots = reduce(
        np.intersect1d, cast(np.ndarray, real_roots)
    )  # Get common roots.
