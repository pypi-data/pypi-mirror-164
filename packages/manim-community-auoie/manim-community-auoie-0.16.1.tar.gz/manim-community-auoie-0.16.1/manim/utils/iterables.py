...
def adjacent_n_tuples(objects: Sequence[T], n: int) -> Iterable[tuple[T, ...]]:
...
def adjacent_pairs(objects: Sequence[T]) -> zip[tuple[T, T]]:
    ...
    Normal usage::
        ...
        # returns [(1, 2), (2, 3), (3, 4), (4, 1)]
    """
    return cast(Any, adjacent_n_tuples(objects, 2))
...
def batch_by_property(
    ...
    """
    batch_prop_pairs: list[tuple[list[T], Optional[S]]] = []
    curr_batch: list[T] = []
    curr_prop = None
    for item in items:
...
def make_even(
    iterable_1: Iterable[T], iterable_2: Iterable[S]
) -> tuple[list[T], list[S]]:
...
def make_even_by_cycling(
    iterable_1: Collection[T], iterable_2: Collection[S]
) -> tuple[list[T], list[S]]:
    """Extends the shorter of the two iterables with duplicate values until its
...
def remove_list_redundancies(lst: Reversible[T]) -> list[T]:
    """Used instead of ``list(set(l))`` to maintain order.
    Keeps the last occurrence of each element.
    """
    reversed_result: list[T] = []
    used = set()
...
def remove_nones(sequence: Iterable[T]) -> list[T]:
