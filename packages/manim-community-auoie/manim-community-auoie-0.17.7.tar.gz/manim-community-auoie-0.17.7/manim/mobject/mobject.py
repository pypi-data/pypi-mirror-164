...
TimeBasedUpdater = Callable[[T, float], T]
NonTimeUpdater = Callable[[T], T]
...
class Mobject:
    ...
    animation_overrides: dict[
        type[Animation], Callable[[Mobject, types.EllipsisType], Animation]
    ] = {}
    ...
    ) -> Callable[[Mobject, types.EllipsisType], Animation] | None:
    ...
    def add_animation_override(
        cls,
        animation_class: type[Animation],
        override_func: Callable[[Mobject, types.EllipsisType], Animation],
    ):
    ...
    def animate(self: T):
        ...
        """
        return cast(T, _AnimationBuilder(self))
    ...
    def add(self, *mobjects: Mobject):
        ...
        mobjects_list = remove_list_redundancies(mobjects)
        if len(mobjects_list) != len(mobjects):
            logger.warning(
                "Attempted adding some Mobject as a child more than once, "
                "this is not possible. Repetitions are ignored.",
            )
        self.submobjects = list_update(self.submobjects, mobjects_list)
        return self
    ...
    def get_image(self, camera: None | Camera = None):
        if camera is None:
            from ..camera.camera import Camera
    ...
    def show(self, camera: None | Camera = None):
        self.get_image(camera=camera).show()

    def save_image(self, name: str | None = None):
    ...
    def generate_target(self: T, use_deepcopy=False) -> T:
    ...
    def update(self, dt: float = 0.0, recursive: bool = True):
        """Apply all updaters.
        ...
        for updater in self.updaters:
            ...
            if "dt" in parameters:
                cast(Any, updater)(self, dt)
            else:
                cast(Any, updater)(self)
        if recursive:
    ...
    def add_updater(
        ...
        if call_updater:
            cast(Any, update_function)(self, 0)
        return self

    def remove_updater(self, update_function: Updater | None):
        """Remove an updater.
    ...
    def rotate_about_origin(self, angle: float, axis=OUT, axes=[]):
    ...
    def apply_complex_function(self, function: Callable[[complex], complex], **kwargs):
        ...
        def R3_func(point: tuple[float, float, float]):
            x, y, z = point
    ...
    def wag(
        self, direction: np.ndarray = RIGHT, axis: np.ndarray = DOWN, wag_factor=1.0
    ):
        for mob in self.family_members_with_points():
    ...
    def next_to(
        self,
        mobject_or_point: Mobject | Object3D | OpenGLMobject | np.ndarray,
        direction=RIGHT,
        buff=DEFAULT_MOBJECT_TO_MOBJECT_BUFFER,
        aligned_edge=ORIGIN,
        submobject_to_align: None | Mobject = None,
    ...
    def rescale_to_fit(self, length, dim: int, stretch=False, **kwargs):
    ...
    def set_color(self, color: str = YELLOW_C, family: bool = True):
    ...
    def reduce_across_dimension(self, points_func, reduce_func, dim: int):
    ...
    def length_over_dim(self, dim: int):
    ...
    def arrange_in_grid(
        ...
        **kwargs,
        ...
        if rows * cols < len(mobs):
            raise ValueError("Too few rows and columns to fit all submobjects.")
        ...
        def init_alignments(
            ...
            if alignments is None:
                ...
                return np.array([cell_alignment * dir] * num)
            if len(alignments) != num:
                raise ValueError(f"{name}_alignments has a mismatching size.")
            alignments_chars: list[str] = list(alignments)
            alignments_res: list[np.ndarray] = []
            for i in range(num):
                alignments_res.append(mapping[alignments_chars[i]])
            return np.array(alignments_res)

        row_alignments_arr = init_alignments(
            row_alignments,
            rows,
        ...
        col_alignments_arr = init_alignments(
            col_alignments,
            cols,
        ...
        # Reverse row_alignments_arr and row_heights. Necessary since the
        ...
        row_alignments_arr = reverse(row_alignments_arr)
        ...
        for r in range(rows):
            ...
            for c in range(cols):
                if grid[r][c] is not placeholder:
                    alignment = row_alignments_arr[r] + col_alignments_arr[c]
                    line = Line(
                        x * RIGHT + y * UP,
