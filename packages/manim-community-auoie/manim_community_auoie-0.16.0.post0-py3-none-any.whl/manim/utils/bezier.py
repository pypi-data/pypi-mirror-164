...
from typing import Iterable, Sized, cast, overload
...
@overload
def bezier(points: list[float]) -> typing.Callable[[float], float]:
    ...


@overload
def bezier(points: np.ndarray) -> typing.Callable[[float], np.ndarray]:
    ...


def bezier(
    points: np.ndarray | list[float],
) -> typing.Callable[[float], np.ndarray | float]:
...
def quadratic_bezier_remap(triplets: list[Iterable[float]], new_number_of_curves: int):
...
@overload
def interpolate(start: float, end: float, alpha: np.ndarray) -> np.ndarray:
    ...
...
def interpolate(
    start: float | np.ndarray, end: float | np.ndarray, alpha: float | np.ndarray
) -> float | np.ndarray:
...
def is_closed(points: np.ndarray) -> bool:
...
def proportions_along_bezier_curve_for_point(
    ...
    np_control_points = np.array(control_points)
    n = len(np_control_points) - 1

    roots: list[np.ndarray] = []
    for dim, coord in enumerate(point):
        control_coords = np_control_points[:, dim]
        terms: list[float] = []
        ...
        polynom_roots: np.ndarray = bezier_polynom.roots()
        if len(polynom_roots) > 0:
            polynom_roots = np.around(polynom_roots, int(np.log10(1 / round_to)))
        roots.append(polynom_roots)

    real_roots: list[list[Complex]] = [[root for root in rootlist if root.imag == 0] for rootlist in roots]
    reduced_roots = reduce(np.intersect1d, cast(np.ndarray, real_roots))  # Get common roots.
    return np.array([r.real for r in reduced_roots if 0 <= r.real <= 1])
