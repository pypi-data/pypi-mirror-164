...
class Circle(Arc):
    ...
    @staticmethod
    def from_three_points(p1: np.ndarray, p2: np.ndarray, p3: np.ndarray, **kwargs):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(label, str):
            ...
            rendered_label: SingleStringMathTex | Text | Tex = MathTex(
                label, color=BLACK
            )
        else:
        ...
        if radius is None:
            radius_not_none = 0.1 + max(rendered_label.width, rendered_label.height) / 2
        else:
            radius_not_none = radius
        super().__init__(radius=radius_not_none, **kwargs)
...
class ArcPolygon(VMobject, metaclass=ConvertToOpenGL):
    ...
    def __init__(
        ...
        *vertices: list | np.ndarray,
        angle: float = PI / 4,
        radius: float | None = None,
        arc_config: list[dict[str, float]] | None = None,
        **kwargs,
    ):
        n = len(vertices)
        ...
        if not arc_config:
            if radius:
                all_arc_configs: itertools.repeat[dict[str, float]] | list[
                    dict[str, float]
                ] = itertools.repeat({"radius": radius}, len(point_pairs))
            else:
