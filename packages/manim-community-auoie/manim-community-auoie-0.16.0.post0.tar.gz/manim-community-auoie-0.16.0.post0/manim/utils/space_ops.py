...
def angle_axis_from_quaternion(quaternion: list[float]) -> Tuple[float, np.ndarray]:
    ...
    angle = cast(float, 2 * np.arccos(quaternion[0]))
    if angle > TAU / 2:
        angle = TAU - angle
    return angle, axis
...
def angle_of_vector(vector: Sequence[float] | np.ndarray):
...
def normalize(vect: np.ndarray | list[float], fall_back=None) -> np.ndarray:
