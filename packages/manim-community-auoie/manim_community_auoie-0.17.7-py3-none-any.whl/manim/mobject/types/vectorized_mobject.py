...
import sys
import typing
from typing import Any, Generator, Optional, Sequence, Tuple, Union, cast
...
class VMobject(Mobject):
    ...
    def __init__(
        self,
        fill_color: colour.Color | None = None,
        fill_opacity=0.0,
        stroke_color=None,
        stroke_opacity=1.0,
    ...
    def set_color(self, color: str, family=True):
    ...
    def append_points(self, new_points: np.ndarray | list[np.ndarray]):
    ...
    def add_smooth_curve_to(self, *points: np.ndarray):
        ...
        if self.has_new_path_started():
            ...
            self.throw_error_if_no_points()
            last_h2, last_a2 = cast(Tuple[np.ndarray, np.ndarray], self.points[-2:])
            last_tangent: np.ndarray = last_a2 - last_h2
    ...
    def add_points_as_corners(self, points: np.ndarray | list[np.ndarray]) -> None:
        for point in points:
            self.add_line_to(point)

    def set_points_as_corners(self, points: Sequence[np.ndarray]):
        ...
        self.set_anchors_and_handles(
            *(
                interpolate(np_points[:-1], np_points[1:], a)
...
class VGroup(VMobject, metaclass=ConvertToOpenGL):
    ...
    def add(self, *vmobjects: Mobject):
