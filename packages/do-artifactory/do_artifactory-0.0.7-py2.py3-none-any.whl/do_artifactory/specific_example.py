...
def main():
    ...
    ##############
    fw_revision = "0.1"
    to_download = "{}_Firmware_rev_{}".format("Perseus", fw_revision.replace(".", "p"))
    zip_name = "{}.zip".format(to_download)
    print(f'zip_name:{zip_name}')
    rep = 'Perseus_Optical_Evb'
    # Download "dirB" as "download.zip"
    #arti.download(_path_source=zip_name, _repo=rep)
    arti.download_file(_repo=rep, _path='', _name=zip_name)
    ##############
