...
class AnalysisSTEAM:
    ...
    def set_up_working_folders(self):
        ...
        if not os.path.isdir(output_path):
            ...
            if self.verbose: print('Folder {} did not exist. It was made now.'.format(output_path))

        # Check if temporary folder is present. If so, delete it.
        if os.path.isdir(temp_path):
            shutil.rmtree(temp_path)
            if self.verbose: print('Folder {} already existed. It was removed.'.format(temp_path))
    ...
    def step_make_model(self, step, verbose: bool = False):
        ...
        if (str(self.library_path).endswith('steam-models')) or (str(self.library_path).endswith('model_library')):
