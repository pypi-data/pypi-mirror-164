...
class ReaxFF(object):
  def __init__(self,libfile='ffield',dataset={},
      ...
      # if self.InitCheck:
      self.ic = Init_Check(re=self.re,clip=clip,nanv=nanv)
  ...
  def clip_parameters(self):
      ...
      for k in self.v:
          ...
          if key == 'n.u.':
             self.p[k] = self.v[k]
          elif k in self.clip:                 ############ clip
             lo,hi = self.clip[k]
             if key in self.punit:
                lo = lo*self.unit
                hi = hi*self.unit
             self.p[k] = tf.clip_by_value(self.v[k],lo,hi)
          elif key in self.clip:               ############ clip
             lo,hi = self.clip[key]
             if key in self.punit:
                lo = lo*self.unit
                hi = hi*self.unit
             ...
             self.p[k] = tf.clip_by_value(self.v[k],-9999.9999,9999.9999)
          elif key=='acut':
             self.p[k] = tf.clip_by_value(self.v[k],0.0000999,0.05)
          elif key=='hbtol':
             self.p[k] = tf.clip_by_value(self.v[k],0.0000999,0.05)
          elif key=='cutoff':
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.5,16.0)
          elif key == 'vdw1':
             self.p[k] = tf.clip_by_value(self.v[k],0.36,9.0)
          elif key == 'gamma':
             self.p[k] = tf.clip_by_value(self.v[k],0.001,6.0)
          elif key == 'pen1':
             self.p[k] = tf.clip_by_value(self.v[k],-69.0*self.unit,199.0*self.unit)
          elif key == 'Devdw':
             D_up= 9.9 if self.vdwnn else 6.0
             D_lo= 0.001  if self.vdwnn else 0.001
             self.p[k] = tf.clip_by_value(self.v[k],D_lo*self.unit,D_up*self.unit)
          elif key == 'Dehb':
             self.p[k] = tf.clip_by_value(self.v[k],-3.0*self.unit,3.0*self.unit)
          elif key == 'Desi':
             self.p[k] = tf.clip_by_value(self.v[k],60.0*self.unit,999.0*self.unit)
          elif key== 'Depi':
             self.p[k] = tf.clip_by_value(self.v[k],0.000*self.unit,999.0*self.unit)
          elif key== 'Depp':
             self.p[k] = tf.clip_by_value(self.v[k], 0.000*self.unit,999.0*self.unit)
          elif key=='tor1':
             self.p[k] = tf.clip_by_value(self.v[k],-9.9900,-0.0)
          elif key== 'ovun2':
             self.p[k] = tf.clip_by_value(self.v[k],-50.00,-0.01)
          elif key in ['tor3','val7']:
             self.p[k] = tf.clip_by_value(self.v[k],0.00,10.0)
             ...
             self.p[k] = tf.clip_by_value(self.v[k],-99.9900*self.unit,290.0*self.unit)
          elif key in ['bo1','bo3','bo5']:
             if self.nn:
                if key =='bo1':
                   self.p[k] = tf.clip_by_value(self.v[k],-0.25,-0.005)
                elif key =='bo3':
                   self.p[k] = tf.clip_by_value(self.v[k],-0.25,-0.005)
                elif key =='bo5':
                   self.p[k] = tf.clip_by_value(self.v[k],-0.25,-0.005)
             ...
             if self.nn:
                if key =='bo2':
                   self.p[k] = tf.clip_by_value(self.v[k],3.0,16.0)
                elif key =='bo4':
                   self.p[k] = tf.clip_by_value(self.v[k],3.0,16.0)
                elif key =='bo6':
                   self.p[k] = tf.clip_by_value(self.v[k],3.0,16.0)
             ...
             if self.vdwnn:
                ...
                self.p[k] = tf.clip_by_value(self.v[k],0.1,16.0)
          elif key== 'lp1':
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.1,9.0)
         ...
         #     self.p[k] = tf.clip_by_value(self.v[k],0.80*self.re[bd],1.2*self.re[bd]) # 0.95 1.2
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.8*self.p_['rosi_'+vn[1]],0.99*self.p_['rosi_'+vn[1]])
          elif key == 'ropp':
             self.p[k] = tf.clip_by_value(self.v[k],0.8*self.p_['ropi_'+vn[1]],0.99*self.p_['ropi_'+vn[1]])
          elif key == 'rohb':
             self.p[k] = tf.clip_by_value(self.v[k],1.5,3.6)
          elif key == 'rvdw':
             rvdw_ =  self.p_['rosi_'+vn[1]]*1.25
             ...
             a   = bd_.split('-')
             if a[1]!='H':
                self.p[k] = tf.clip_by_value(self.v[k],1.0,200.0*self.unit)
             else:
                self.p[k] = self.v[k]
          elif key=='val2':
             self.p[k] = tf.clip_by_value(self.v[k],0.2,8.0)
          elif key=='val9':
             self.p[k] = tf.clip_by_value(self.v[k],0.3,6.0)
          elif key=='val':
             sp = vn[1]
             self.p[k] = tf.clip_by_value(self.v[k],0.2,8.0)
          elif key=='vale':
             ...
             sp = vn[1]
             self.p[k] = tf.clip_by_value(self.v[k],0.0,2.0*self.p_['val_'+sp])
          else:
             self.p[k] = self.v[k]
