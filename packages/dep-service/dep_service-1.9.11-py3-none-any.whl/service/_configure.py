...
from typing import Callable, Coroutine, Dict, List, Type, Tuple
...
from ._jobs import CronJob, CronManager, init_cron
...
def prepare_cron(app: types.App, cron_jobs: List[CronJob]):
    """Prepare cron."""
    cron = CronManager()
    app.cron = cron
...
def prepare_builtin_middleware(app: types.App, spec: types.Spec):
    ...
    app.add_middleware(
        ...
        allowed=tuple(spec.i18n.all_codes),
    )
...
def create(
    ...
    if cron_jobs:
        prepare_cron(app=app, cron_jobs=cron_jobs)

    if builtin_middleware:
        prepare_builtin_middleware(app=app, spec=spec)

    prepare_routers(app=app, routers=routers, service_routes=service_routes)
