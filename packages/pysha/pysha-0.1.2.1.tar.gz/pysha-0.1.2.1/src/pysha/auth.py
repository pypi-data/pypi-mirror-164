...
class Auth:
    ...
    def auth(self, c=True, mask_color=fore["reset"], inp_color=fore["reset"], mask='*'):
        ...
        if c:
            ...
            if self.mode == 0:
                uname = colorprompt(
                    colorize(self.first_prompt), char_color=inp_color)
                ...
                uname = colorprompt(
                    colorize(self.first_prompt), char_color=inp_color)
                pwd = passprompt(colorize(self.second_prompt),
                                 mask_color=mask_color, mask=mask)
                ans.append(uname)
                ans.append(pwd)
            elif self.mode == 2:
                uname = colorprompt(
                ...
                raise ModeError(
                    f"Mode {self.mode} Is Not Available For Authentication. Just 0,1,2.")
            return ans
        else:
            ans = []
            if self.mode == 0:
                ...
                uname = colorprompt((self.first_prompt), char_color=inp_color)
                pwd = passprompt(prompt=(self.second_prompt),
                                 mask=mask_color + tar + fore["reset"])
                ans.append(uname)
                ans.append(pwd)
            elif self.mode == 2:
                uname = colorprompt((self.first_prompt), char_color=inp_color)
