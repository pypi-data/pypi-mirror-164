...
#     # await asyncio.gather(*async_request)
# async def post_signal( json_data ):
#     url = "http://grid_subscriber:8889/main"
#     await requests.post( url, json=json_data)
#     # rs = (grequests.get(u) for u in async_request)
#     # grequests.map(rs)

# def push_data_to_pubsub(signal_data):
#     db = firestore.Client()
#     strategy_id = signal_data["strategy_id"]
#     doc_ref = db.collection_group(u'strategys').where(u'strategy_id', u'==', strategy_id ).where(u'strategy_type', u'==', u'Grid').where(u'bot_type', u'==', u'copy')
#     docs = doc_ref.stream()
#     datas = []
#     result_data = {}
#     result_data.update({"system_status": signal_data["system_status"]})
#     result_data.update({"exe": signal_data["exe"]})
#     result_data.update({"signal_data": signal_data["grid_data"]})
#     #logger.error(f'Start running...', severity="INFO")
#     for doc in docs:
#         #ret_msg += f'{doc.id},'
#         data = doc.to_dict()
#         noti_ref = db.collection_group(u'notifications').where(u'user_id', u'==', data.get("user_id") ).where(u'noti_type', u'==', u'Line')
#         notis = noti_ref.stream()
#         line_token = ''
#         for noti in notis:
#             noti_ = noti.to_dict()
#             line_token = noti_.get("line_token")
...
#                 "accountName": data.get("sub_account_name"),
#                 "subAccountFlag": data.get("subAccountFlag"),
#                 "line_token": line_token,#data.get("line_token"),
#                 "admin_line_token": signal_data["admin_line_token"],#data.get("admin_Line_token"),
#                 "bot_status": data.get("system_status")
#             }
#         result_data.update({"subscribe_info": datas})
#         push_pubsub(result_data)

# def push_pubsub(json_data):    
#     # Instantiates a Pub/Sub client
#     publisher = pubsub_v1.PublisherClient()
#     topic_path = publisher.topic_path(GCLOUD_PROJECT, PUBSUB_ENDPOINT)
