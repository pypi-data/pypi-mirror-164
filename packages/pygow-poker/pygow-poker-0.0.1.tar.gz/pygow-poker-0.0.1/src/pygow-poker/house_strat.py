...
def house_strat(hand:list):
	...
	def two_pair_strat():
		...
		if int(MULTIPLES_KEYS[1]) >= 12: ##if high pair are As, Ks, or Qs then split
			...
			if  HIGH_CARD_ORDER[-1][:2] == '13' or HIGH_CARD_ORDER[-1][:2] == '14' or  HIGH_CARD_ORDER[-1] == 'JKR': 
				...
				low_hand = MULTIPLES[MULTIPLES_KEYS[0]]
				high_hand = subtract_hand(HAND, low_hand)
		elif 5 >= int(MULTIPLES_KEYS[1]) >= 3: ##if high pair are 5s, 4s, or 3s - has Q or higher keep together; else split
	...
	def get_best_low_hand(type:list):
		...
		if STRAIGHT_FLUSHES:
			for i, h in enumerate(STRAIGHT_FLUSHES):
				...
				elif read_hand(subtract_hand(HAND, h))['rank_points'] > read_hand(low_hand)['rank_points']:
					low_hand = subtract_hand(HAND, h)
			for i, h in enumerate(FLUSHES):
				if read_hand(subtract_hand(HAND, h))['rank_points'] > read_hand(low_hand)['rank_points']:
					low_hand = subtract_hand(HAND, h)
	...
	def straight_flush_strat(type:list):
		...
		if not MULTIPLES_KEYS: ##no pair - keep type; highest possible hand in low hand
			...
			low_hand = get_best_low_hand(type)
			if read_hand(low_hand)['rank'] == 'Pair':
				high_hand = subtract_hand(HAND, low_hand)
			else:
			...
			low_hand = get_best_low_hand(type)
			high_hand = subtract_hand(HAND, low_hand)
		elif len(MULTIPLES_KEYS) == 3 and len(MULTIPLES[MULTIPLES_KEYS[0]]) == len(MULTIPLES[MULTIPLES_KEYS[1]]) == len(MULTIPLES[MULTIPLES_KEYS[2]]) == 2: ## three pair - highest pair in low hand
	...
	elif len(MULTIPLES_KEYS) == 3 and len(MULTIPLES[MULTIPLES_KEYS[0]]) == len(MULTIPLES[MULTIPLES_KEYS[1]]) == len(MULTIPLES[MULTIPLES_KEYS[2]]) == 2: ## three pair without straight, flush, or straight flush
		low_hand = MULTIPLES[MULTIPLES_KEYS[2]] ##highest pair in low hand
		high_hand = subtract_hand(HAND, low_hand)
