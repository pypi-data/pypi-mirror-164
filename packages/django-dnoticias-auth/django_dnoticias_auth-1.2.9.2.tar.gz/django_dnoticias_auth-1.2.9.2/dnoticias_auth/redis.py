...
class KeycloakSessionStorage:
    ...
    def create_or_update(self, session_data: Optional[dict] = dict()):
        ...
        body = {"sessions": self.django_session_id, "payload": session_data}

        if self.exists():
            body = self.load()

            if self.django_session_id in body.get("sessions", ""):
                logger.debug(
            ...
            body["sessions"] = f"{body.get('sessions', '')},{self.django_session_id}"
            self.delete()
