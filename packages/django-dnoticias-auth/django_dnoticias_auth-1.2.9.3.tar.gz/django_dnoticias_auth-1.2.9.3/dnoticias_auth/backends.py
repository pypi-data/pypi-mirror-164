...
class ExtraClaimsOIDCAuthenticationBackend(OIDCAuthenticationBackend):
    ...
    def authenticate(self, request, **kwargs):
        ...
        expires_in = self.__get_isoformat_from_timestamp(token_info.get('expires_in'))
        refresh_expires_in = \
            self.__get_isoformat_from_timestamp(token_info.get('refresh_expires_in'))
    ...
    def store_keycloak_tokens(self):
        """Store the keycloak tokens in the session storage."""
        try:
            logger.debug("Storing keycloak tokens...")
            keycloak_session = KeycloakSessionStorage(
                self.request.session["keycloak_session_id"],
                self.request.session.session_key,
            )

            session_data = {key: value for key, value in self.request.session.items()}
            keycloak_session.create_or_update(session_data)
        except:
            logger.exception("Error storing keycloak tokens")
    ...
    def store_tokens(
        ...
        if self.get_settings('OIDC_STORE_ACCESS_TOKEN_EXPIRATION', False):
            session['oidc_expires_in'] = expires_in
