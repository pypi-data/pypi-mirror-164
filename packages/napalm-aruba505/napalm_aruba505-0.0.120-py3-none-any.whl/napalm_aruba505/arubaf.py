...
class ArubaFDriver(NetworkDriver):
    ...
    def get_config(self, retrieve="all", full=False, sanitized=False):
        ...
        if retrieve.lower() in ('running', 'all'):
            ...
            #output_ = self.device.send_command(command)
    ...
    def get_facts(self):
        ...
        interface_list = ["eth0", "eth1", "eth2", "eth3", "eth4"]
        configs = {}
        #show_version_output = self.device.send_command("show version")
        #show_summary_output = self.device.send_command("show summary")
        show_version_output = self.device._send_command("show version")
        show_summary_output = self.device._send_command("show summary")
    ...
    def get_ping(self):
        ...
        #output_ = self.device.send_command(command)
        output_ = self.device._send_command(command)
        output_ = str(output_)
        ...
        if output_:
            for i in bad_responses:
                if i in output_:
                    ...
                    break
        elif output_ and len(output_) > 10:
    ...
    def get_lldp_neighbors(self):
        ...
        result = self.device._send_command(command)
    ...
    def get_environment(self):
        ...
        cpu_output = self.device._send_command(cpu_cmd)
        # process cpu outputs
        ...
        #output_ = self.device.send_command(mem_cmd)
        output_ = self.device._send_command(mem_cmd)
        available_ram = None
        free_ram = None
