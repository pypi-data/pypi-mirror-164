...
DEFAULT_DOCKER_TEMPLATE = Template(
    """
FROM subcom/bio.bitia:bioarchlinux
...
class ComposeFile(object):
    ...
    def dockerfile_str(self, **kwargs):
        return DEFAULT_DOCKER_TEMPLATE.render(**kwargs)
    ...
    def _setup_docker_file(self, commands: T.List[str]):
        for command in commands:
            self.dockerfile_params[
                "run_steps"
            ] += f"RUN pacman -S --noconfirm $(pkgfile {command})\n"
    ...
    def _setup_compose_file(self):
        self.compose["command"] = self._user_input_to_command()
    ...
    def run(self, check: bool = False):
        ...
        self.finalize()
        cwd = self.composefile_path.parent
        logging.info(f"Running container in {cwd}")
        return itertools.chain(
            bitia.common.run_command("ls -ltr", cwd=cwd),
            bitia.common.run_command("docker-compose up", cwd=cwd, check=check),
        )
