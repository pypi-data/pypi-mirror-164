...
class DataEda:
    ...
    def eda(self):
        ...
        print('Have finished EDA process, the result is in dir: {} !'.format(self.output_dir))


def coco_eda(data_dir, output_dir):
    from csp.datatool import transform
    voc_dir = "./tmp/voc"
    transform(data_dir=data_dir, task="obj_det", form="coco", output=voc_dir)


def det_eda(data_dir, form: str, output_dir):
    assert form.lower() in ["voc", "coco"]
    if form.lower() == "coco":
        from csp.datatool.image_detection.transform import det_transform
        voc_dir = os.path.join(output_dir, "voc")
        os.makedirs(voc_dir, exist_ok=True)
        det_transform(folder=data_dir, form="coco", output=voc_dir)
        data_dir = voc_dir
        data_eda = DataEda(data_dir, output_dir)
        data_eda.eda()
...
if __name__ == '__main__':
    ...
    voc_dir = "C:/Users/xgy/Desktop/CSPTools/0507/20220507154756390569/train_eda"
    output = "C:/Users/xgy/Desktop/CSPTools/0507/20220507154756390569/voc_eda2"

    data_eda = DataEda(voc_dir, output)
    data_eda.eda()
