            ...
            for txt in txt_list:
                ...
                if xml_name in xmls_in_anno and filename in img_in_folder:
                    ...
                    raise FileNotFoundError(
                        "{} exists in {}, but not found in {}".format(filename, txt_file, self.voc_xml_dir))
                elif xml_name in xmls_in_anno and filename not in img_in_folder:
                    raise FileNotFoundError(
                        "{} exists in {}, but not found in {}".format(filename, txt_file, self.voc_img_dir))
                elif xml_name not in xmls_in_anno and filename not in img_in_folder:
                    raise FileNotFoundError(
                        "{} exists in {}, but not found in {} or not found in {}".format(filename, txt_file, self.voc_xml_dir, self.voc_img_dir))
        if num_trainval != len(xmls_in_anno):
            logger.warning("Unequal number of annotations in {} and files in trainval.txt!".format(self.voc_xml_dir))
        if num_trainval != len(img_in_folder):
            logger.warning("Unequal number of annotations in trainval.txt and images in {}".format(self.voc_img_dir))
    ...
    def convert(self, xml_files, json_path, categories):
        ...
        for i, xml_file in enumerate(xml_files):
            ...
            for obj in root.findall("object"):
                ...
                erro_msg = "in file {} there are some errors hanppend in xmin, xmax, ymin, ymax: {} {} {} {}".format(xml_file, xmin, xmax, ymin, ymax)
            ...
            if os.path.exists(origin_Images_dir):
                ...
                for item in json_data["images"]:
                    ...
                    if item_name in img_names_in_folder:
                        ...
                        raise FileNotFoundError("{} exists in {}, but not found in {}".format(item, annpath, origin_Images_dir))
                if len(img_names_in_folder) != num_img_in_json:
                    logger.warning("Unequal number of images in {} and {}!".format(annpath, origin_Images_dir))
