...
class PsqlCmdHook(BaseHook):
    ...
    def __init__(self, dataset_name: Optional[str] = None, db_target_schema=None, conn_id="postgres_default", *args, **kwargs):
        """Initialize.
    ...
    def run(self, sql_files):
        ...
        db_conn_string = partial(pg_params,
        ...
        subprocess.run(
            f'cat {paths} | psql {db_conn_string()}', shell=True, check=True
        )
    ...
    def recreate_schema(self, db_target_schema, connection_uri):
        ...
        self.log.info(
            f"Creating the DB target schema '{db_target_schema}' if not present")
        subprocess.run(
