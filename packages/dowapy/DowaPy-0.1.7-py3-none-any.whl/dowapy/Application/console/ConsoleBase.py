...
from dowapy.Application.console.CommandBase import CommandBaseClass

def BuildModuleImportCode(Module):
    ResultImportCode = ''
    ResultPath = ''
...
def BuildInitializeCode(RequireModules:list, CommandClass):
    RequirePath, ImportCode = BuildImportCode(RequireModules, CommandClass)
    ResultCode = f'''
import sys
...
class ConsoleBase:
    ...
    def Boot(self, RequireModules, CommandClass, CommandFilter='>>'):
        self.CommandFilter = CommandFilter
        InitCode = BuildInitializeCode(RequireModules, CommandClass)
        # @TODO : 임시 파일 지워지는 지 확인 필요.
        f, fpath = tempfile.mkstemp()
        os.write(f, InitCode.encode('utf-8'))
        self.Process = subprocess.Popen(f'py {fpath}', stdin=subprocess.PIPE, stdout=subprocess.PIPE, text=True, shell=False)
