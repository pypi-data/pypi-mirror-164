...
class ElementSpecs:
    ...
    Review = {
        ...
        "at": ElementSpec(None, [5, 0], lambda v: (datetime.fromtimestamp(v)).strftime("%d/%m/%Y %H:%M:%S")),
        "replyContent": ElementSpec(None, [7, 1]),
        "repliedAt": ElementSpec(None, [7, 2, 0], lambda v: (datetime.fromtimestamp("%d/%m/%Y %H:%M:%S")).strftime("%d/%m/%Y %H:%M:%S")),
    }
