...
class GameBotClient:
    ...
    def __init__(self, server_id=None, host=None, port=None, timeout=5):
        ...
        self.host = host or config['NETWORK']['host'] or '127.0.0.1'
        self.port = int(port) or int(config['NETWORK']['port']) or 65432
    ...
    def ping(self):
        ...
        from_addr = f"{self.client_address}"
        to_addr = f"{self.server_address}"
        stream = Stream(from_address=from_addr, to_address=to_addr, type='client', function='ping', body=self.server_id)
        self.socket.sendall(stream.encode())
