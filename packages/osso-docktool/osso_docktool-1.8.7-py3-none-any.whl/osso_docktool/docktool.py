...
class AutoServer:
    ...
    def handle_client(self, connection):
        ...
        if client_data:
            ...
            try:
                ...
                print(f'Received bogus notification: {notification!r}')
                return

            device = Path(device)
            if device.is_block_device():
                if self.is_ignored(device, force=bool(cmd == 'device-force')):
    ...
    def is_ignored(self, device, force):
        hdd_dock = BaseStorageDevice.from_devname(device)
        serial = hdd_dock.get_serial_number()
        if not force and serial in self.seen_devices:
            print(f'Device {device} with serial {serial} has been processed')
            print(
                f'Use "osso-docktool auto-notify {device} --force" to start')
            return True
        self.seen_devices.add(serial)
    ...
    def start_gui(self, device):
        ...
        if os.fork() == 0:
            args = [f'osso-docktool-terminal [{device}]', '-e']
...
def auto_notify(device, force=False):
    notification = f'device-force:{device}' if force else f'device:{device}'
    with socket(AF_INET, SOCK_STREAM) as sock:
        try:
...
def main():
    ...
    try:
        if args.command == 'dev':
            ...
            auto_notify(args.device, args.force)
    except KeyboardInterrupt:
        sys.exit(0)
    except SystemExit:
