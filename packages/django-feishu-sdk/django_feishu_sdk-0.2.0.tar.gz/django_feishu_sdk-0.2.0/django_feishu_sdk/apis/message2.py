...
class MessageAPI2(BaseAPI):
    ...
    @allow_async_call
    def send_text(self, text: str, receive_id_type: str, receive_id: str) -> Optional[str]:
        """发送文本消息
        Url:
        ...
        if text.strip():
            ...
            self.logger.warning(f"text为空, 文本消息未发送: msg={text}")
    ...
    def batch_send_all(self, message: Union[Message, dict], department_ids: List[str], open_ids: List[str],
        ...
        """
        slice_size = api_settings.FEISHU_BATCH_SEND_SIZE
