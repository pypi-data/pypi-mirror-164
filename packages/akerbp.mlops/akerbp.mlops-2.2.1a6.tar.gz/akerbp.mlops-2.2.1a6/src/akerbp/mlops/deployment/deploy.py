...
def deploy_model(
    ...
    try:
        ...
        if c.platform == "local" and c.test_file:
            ...
            logging.warning(
                "Skipping local tests. Either test file not found, or deploying remotely in pipeline"
            )

        if c.platform == "cdf":
            logging.debug(f"local_deployment={local_deployment}")
            logging.debug(f"env={env}")
            if (env == "test" or env == "prod") and local_deployment is True:
                logging.info("Running tests only, will not deploy model(s)")
                command = [sys.executable, "-m", "akerbp.mlops.services.test_service"]
                subprocess.check_output(command, encoding="UTF-8")
            elif (env == "test" or env == "prod") and local_deployment is False:
                logging.info(f"Deploying to {c.platform}")
