...
class dbclient:
    ...
    def __init__(self, configs):
        ...
        self._is_azure = configs['is_azure'].strip()
        self._verify_ssl = configs['verify_ssl'].strip()
        self._verbosity = configs['verbosity'].strip()
        self._cluster_id=configs['clusterid'].strip()
        self._token = ''
        self._raw_token = ''
        self._master_name_scope=configs['master_name_scope'].strip()
        self._master_name_key=configs['master_name_key'].strip()
        self._master_password_scope=configs['master_pwd_scope'].strip()
        self._master_password_key=configs['master_pwd_key'].strip()
        self._master_name=''
        self._master_password=''
        self._workspace_pat_scope=configs['workspace_pat_scope'].strip()
        self._workspace_pat_token=configs['workspace_pat_token'].strip()
        ...
        if(self._master_name_scope == ''):
            raise ValueError('Master scope - name cannot be empty')
        if(self._master_name_key == ''):
            raise ValueError('Master key - name cannot be empty')
        if(self._master_password_scope == ''):
            raise ValueError('Master scope - password cannot be empty')
        if(self._master_password_key == ''):
            raise ValueError('Master key - password scope cannot be empty')
        if(self._workspace_pat_scope == ''):
            raise ValueError('Workspace PAT scope cannot be empty')
        if(self._workspace_pat_token == ''):
            raise ValueError('Workspace PAT token key cannot be empty')            
    ...
    def test_connection(self, master_acct=False):
            if master_acct: #master acct may use a different credential
                ...
                if self._master_name.strip() == '':
                    raise Exception('Secrets not setup for Master Name')
                if self._master_password.strip() == '':
                    raise Exception('Secrets not setup for Master Password')                                                       
                ...
                if self._raw_token.strip() == '':
                    raise Exception('Secrets not setup for Token')
