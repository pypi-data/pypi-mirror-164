...
class FeaturesFilteringManager:
    def get_latest(
        ...
        features_data = features_data.select(id_column, *features)
        features_data = self.__get_complete_rows(features_data, skip_incomplete_rows)
    ...
    def get_for_target(
        ...
        features_data = features_data.select(*join_columns, *features)
        features_data = self.__get_complete_rows(features_data, skip_incomplete_rows)
