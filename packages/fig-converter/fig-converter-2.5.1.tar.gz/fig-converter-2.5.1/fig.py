...
@click.option('-fr', '--fps-reduction', default=1,
              ...
              type=int)
@click.option('-q', '--quality', is_flag=True, default=False, help='prioritize quality over speed and size',
              show_default=True)
...
def main(filename, gif2video, output, width, fps_reduction, quality, dither, shit_optimize, ffmpeg, text, text_style):
    if gif2video:
        if filename[-4:] != ".gif":
            raise click.BadParameter(f"File '{filename}' is not a gif.", param_hint='FILENAME')
        figconverter.gif2video(filename, output, width, fps_reduction, quality, dither, shit_optimize, text,
                               TextStyle(text_style), True)
        return

    figconverter.video2gif(filename, output, width, fps_reduction, quality, dither, shit_optimize, ffmpeg, text,
                           TextStyle(text_style), True)
