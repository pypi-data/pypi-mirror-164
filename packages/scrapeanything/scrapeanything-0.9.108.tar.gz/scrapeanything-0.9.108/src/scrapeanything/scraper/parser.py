...
class Parser:
    ...
    def __init__(self, config: Config, user_dir: str=None, headless: bool=True, window: dict={}) -> None:
        self.config = config
        self.scraper = self.get_scraper(config=config, headless=headless, window=window, user_dir=user_dir)
    ...
    def get_scraper(self, config: Config, user_dir: str=None, headless: bool=True, window: dict={}) -> Scraper:
        ...
        return Utils.instantiate(module_name=f'scrapeanything.scraper.scrapers.{module_name}', class_name=class_name, args={ 'headless': headless, 'window': window, 'config': config, 'user_dir': user_dir })