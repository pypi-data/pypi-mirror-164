...
#######   SIZE DISTRIBUTION PLOTS !!!
    ...
    for title in ins:
        ...
        for i in range(len(ins[title])):
            ...
            for n in x:
                if not n:
                    continue
                if n.count(',')!=1:
                    print('\n  Wrong file format! Each line should contain exactly one comma!\n')
                    sys.exit()
                a,b=n.split(',')
                if a[0]=='(':
                    a=a.replace(' ',',')
        ...
        for n in sorted(imap):
            imap[n]=[imap[n][i]/s[i]*100 for i in range(len(s))]
        enr={}
        if len(ins[title])>1:
            for n in imap:
                if max(imap[n])<thr:
                ...
                for i in range(1,len(s)):
                    x.append(math.log10(imap[n][i]/max(imap[n][i-1],1/s[i]*100)))
                if max(x)>0:
                    enr[n]=x
            for n in enr:
                glenr[title].append(imap[n][-1])
        ...
        if enr:
            ...
            plt.clim(-lim, lim)
            plt.colorbar(label='Log(enrichment)',shrink=0.75)
            plt.xlabel('Genome position')
    ...
    z=0.94/len(compl)
    w=z*0.9
    colors,fig=dbl.plot_start(c,None,'Global insertion site complexity')
    plt.bar(locs,[compl[n][0] for n in compl],width=w,label='All sites')
    plt.bar([n+z for n in locs],[compl[n][1] for n in compl],width=w,label='sites with frequency >= '+str(thr)+'%')
    plt.xticks([n+z/2 for n in locs],compl.keys())
    plt.yscale('log')
    plt.ylabel('Number of insertion sites')
    plt.legend()
...
def anaconf(fname,args):
    ...
    if sd:
        ...
        for m in set([k[k.rfind('--')+2:k.rfind('_sd.csv')] for k in y]):
