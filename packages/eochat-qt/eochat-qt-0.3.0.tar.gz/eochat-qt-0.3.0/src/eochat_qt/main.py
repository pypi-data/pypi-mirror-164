...
# Eochat-QT
from eochat_qt.eo_defs import *
from eochat_qt.helpers import *
from eochat_qt.config import *
from eochat_qt.mainwindow import *
...
def display(message):
    global MAIN_WINDOW, log_name, logfp, logging, LOG_TIMESTAMP_FMT, LOG_FILE_FMT
...
async def sender(conn):
    ...
    try:
        while True:
            ...
            while MAIN_WINDOW.message_obtained:
                msg = get_ui_input()
                await conn.send(build_chat(msg))
                MAIN_WINDOW.message_obtained = False
...
def init_mainwindow(loader, ui_file):
    global MAIN_WINDOW
    MAIN_WINDOW = MainWindow(loader.load(ui_file))
    ui_file.close()
    if not MAIN_WINDOW:
...
def main():
    ...
    init_mainwindow(*init_qt_uic_loader(MAIN_UI_FILE))
