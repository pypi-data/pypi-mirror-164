...
def parse_user_logformat(cfg):
    ...
    # CFG_LOG_TIMESTAMP_FMT   = DEFAULT_TIMESTAMP_FMT
    # CFG_LOG_FILE_FMT        = DEFAULT_LOG_FILE_FMT
    ...
    if (cfg_eochat_qt):
        ...
        if (cfg_log):
            ...
            # CFG_LOG_TIMESTAMP_FMT = cfg_tsformat
            TIMESTAMP_FMT = cfg_tsformat
            LOG_FILE_FMT = cfg_fpformat

    # CFG_LOG_TIMESTAMP_FMT   = cfg.get('eochat-qt').get('log').format('tsformat')
    # CFG_LOG_FILE_FMT        = cfg.get('eochat-qt').get('log').format('fpformat')

    # TIMESTAMP_FMT   = override_values(DEFAULT_TIMESTAMP_FMT, CFG_LOG_TIMESTAMP_FMT, None)
    # LOG_FILE_FMT    = override_values(DEFAULT_LOG_FILE_FMT, CFG_LOG_FILE_FMT, None)

    # log_name = f'chat-{timestamp(DEFAULT_LOG_FILE_FMT)}'
    # log_name = f'chat-{timestamp(TIMESTAMP_FMT)}'
    log_name = f'chat-{timestamp(LOG_FILE_FMT)}'
    logfp = get_log_name(log_name)

    # CFG_PATH_WINDOWS        = expand(r'%APPDATA%\eochat\eo_chat-qt\logs\{log_name}')
    # CFG_PATH_LINUX          = expand('~/.config/eochat/eo_chat-qt/logs/{log_name}')

    # logfp = expand(CFG_PATH_WINDOWS) if is_win else expand(CFG_PATH_LINUX)
    ...
    # log = f'{logfp}/{}
    # Log format
    # return log_name, logfp, DEFAULT_LOG_FILE_FMT, DEFAULT_TIMESTAMP_FMT
