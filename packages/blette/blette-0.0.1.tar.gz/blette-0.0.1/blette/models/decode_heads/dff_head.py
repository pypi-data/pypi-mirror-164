...
from ..utils import LocationAdaptiveLearner
from ..builder import HEADS
from ..losses import calc_metrics


@HEADS.register_module()
class DFFHead(BaseDecodeHead):
    def __init__(
        self,
        **kwargs,
    ):
        super(DFFHead, self).__init__(
    ...
    def forward(self, inputs, img_metas):
        ...
        ada_weights = self.ada_learner(s4_w)
        slice4 = s4[:, 0:1, :, :]
        ...
        fuse = fuse.view(fuse.size(0), self.num_classes, -1, fuse.size(2), fuse.size(3))
        fuse = torch.mul(fuse, ada_weights)
