...
from .pipelines import DefaultFormatBundle, LoadAnnotations, LoadEdges
from .base_dataset import BaseMultiLabelDataset
from .builder import DATASETS


@DATASETS.register_module()
class CustomMultiLabelDataset(BaseMultiLabelDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(CustomMultiLabelDataset, self).__init__(**kwargs)
...
@DATASETS.register_module()
class OTFCustomMultiLabelDataset(BaseMultiLabelDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(OTFCustomMultiLabelDataset, self).__init__(**kwargs)

        assert seg_dir is not None, f"ERR: seg_dir is not valid: {seg_dir}"
        if inst_sensitive:
        ...
        # join paths if data_root is specified
        if self.data_root is not None:
            if not (self.seg_dir is None or osp.isabs(self.seg_dir)):
                self.seg_dir = osp.join(self.data_root, self.seg_dir)
            if not (self.inst_dir is None or osp.isabs(self.inst_dir)):
                self.inst_dir = osp.join(self.data_root, self.inst_dir)

        # load annotations
        assert self.seg_dir is not None
        self.img_infos = self.load_annotations(
            img_dir=self.img_dir,
            img_suffix=self.img_suffix,
    ...
    def pre_pipeline(self, results):
        results["seg_prefix"] = self.seg_dir
