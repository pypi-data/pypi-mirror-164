...
def DictObjToList(Dict):
    for K,V in Dict.items():
        if type(V) == dict: Dict[K] = DictObjToList(V)

        elif type(V) == np.ndarray and V.dtype == np.dtype('O'):
            Dict[K] = V.tolist()
            if K in ('p', 'p.adj') and None in V:
                Dict[K] = np.array([_ if _ is not None else np.nan for _ in V])
