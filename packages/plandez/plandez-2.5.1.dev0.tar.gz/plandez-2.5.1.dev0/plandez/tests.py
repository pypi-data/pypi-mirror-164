...
class TestMBTilesBuilder(unittest.TestCase):
    temp_cache = os.path.join(tempfile.gettempdir(), 'plandez/stileopenstreetmaporg_z_x_ypng')
    temp_dir = os.path.join(tempfile.gettempdir(), 'plandez/tiles')
    ...
    def test_init(self):
        ...
        self.assertEqual(mb.cache.folder, self.temp_cache)
        self.assertEqual(mb.tmp_dir, os.path.join(tempfile.gettempdir(), 'plandez/toto'))
...
class TestCache(unittest.TestCase):
    temp_path = os.path.join(tempfile.gettempdir(), 'plandez/stileopenstreetmaporg_z_x_ypng')
    ...
    def test_remove(self):
        ...
        self.assertTrue(os.path.exists('/tmp/plandez/stileopenstreetmaporg_z_x_ypng/1/1/0.png'))
        mb.cache.remove((1, 1, 1))
        self.assertFalse(os.path.exists('/tmp/plandez/stileopenstreetmaporg_z_x_ypng/1/1/0.png'))
        mb.cache.clean()
...
class TestLayers(unittest.TestCase):
    def test_cache_folder(self):
        ...
        self.assertEqual(mb.cache.folder, '/tmp/plandez/server')
        over = TilesManager(tiles_url='http://toto')
        self.assertEqual(over.cache.folder, '/tmp/plandez/toto')
        mb.add_layer(over)
        self.assertEqual(mb.cache.folder, '/tmp/plandez/servertoto10')
        mb.add_layer(over, 0.5)
        self.assertEqual(mb.cache.folder, '/tmp/plandez/servertoto10toto05')
...
class TestFilters(unittest.TestCase):
    def test_cache_folder(self):
        ...
        self.assertEqual(mb.cache.folder, '/tmp/plandez/servercolortoalphaffffff')
