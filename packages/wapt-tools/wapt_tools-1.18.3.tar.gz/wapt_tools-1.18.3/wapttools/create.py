...
upstream_master = '![{text}]({url}/wapt/packages/{package}/-/jobs/artifacts/master/raw/upstream-master.svg?job=version-check)'  # noqa
upstream_develop = '![{text}]({url}/wapt/packages/{package}/-/jobs/artifacts/develop/raw/upstream-develop.svg?job=version-check)'  # noqa
...
def create(package, verbose=False):
    ...
    with open('README.md', 'w') as file:
        ...
        file.write(upstream_master.format(text='Upstream', url=gl.url, package=package))
        file.write(pipeline.format(text='DEV', branch='develop', url=gl.url, package=package))
        file.write(upstream_develop.format(text='Upstream', url=gl.url, package=package))
    ...
    # Create schedule named AutoVersionCheckerMaster
    found = False
    for sched in scheds:
        if sched.description == 'AutoVersionCheckerMaster':
            found = True

    if not found:
        sched = project.pipelineschedules.create({
            'ref': 'master',
            'description': 'AutoVersionCheckerMaster',
            'cron_timezone': 'Europe/Paris',
            'cron': '{} 5 * * *'.format(random.randint(0, 59)),
            'active': True
        })
