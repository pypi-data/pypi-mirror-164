...
class Ruler(Aml972Common):
    ...
    def get_board_macro(self):
        ...
        if "81" in str(board_type):
            ret = self.get_macro_line("CVT_DEF_BOARD_TYPE", board_macro)
            ret = ret +self.get_macro_line("CVT_DEF_CURRENT_REF_DUTY", "0") 
            ret = ret +self.get_macro_line("CVT_EN_81F", "TRUE")                          
        else:
            ret = self.get_macro_line("CVT_DEF_BOARD_TYPE", board_macro)
        return ret
    ...
    def get_ocs_require(self):
        ...
        ret += '// language & region' + '\n'
        ret += self.get_macro_line("CVT_DEF_JPE_LANGUAGE_GROUP", "ID_CONFIG_JPE_LANG_GRP_ARENFAFRRU")
