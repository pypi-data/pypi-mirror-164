...
def vib_func(args):
    ...
    if args.gaussian_log:
        ...
        ho = Harmonic.from_vasp_phonopy(
            args.poscar, args.force_sets, supercell=args.distortion_expansion,
...
def gen_charges_func(args):
    ...
    print("******************************")
    print("The asymmetric unit (ASU) contains")
...
def parse_charges_func(args):
    ...
    # Recalculate repair shifts for new centred system
    scell.repair_shifts = scell.find_repair_shifts()

    xyzp.save_xyz(
        "supercell_o_shifted.xyz",
        scell.atom_labels,
        scell.cart_coords
    )

    xyzp.save_xyz(
        "supercell_o_shifted_repaired.xyz",
        scell.atom_labels,
        scell.cart_coords+scell.repair_shifts
    )
    ...
    np.savetxt(
        "supercell_charges.dat",
        supercell_charges
    )

    sc_charges_coords = np.vstack(
        [
            supercell_charges,
            scell.cart_coords.T
        ]
    ...
    np.savetxt(
        "environment_coords_charges.dat",
        [sc_charges_coords[wci] for wci in without_central_indices]
    )

    np.savetxt("central_cell_indices.dat", central_indices, fmt='%i')
    np.savetxt(
        "environment_cell_indices.dat", without_central_indices, fmt='%i'
    )
