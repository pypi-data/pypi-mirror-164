...
class KamibotPi:
    ...
    # -------------------------------TOP 스텝퍼 모드 ----------------------------------------
    def top_motor_degree(self,  dir, value=90, speed=50,torque=1):
        ...
        command[PacketIndex.DATA3] = speed
        command[PacketIndex.DATA4] = torque
        command[PacketIndex.INDEX] = self.__get_idx()
        try:
            self.sr.write(bytes(bytearray(command)))
            self.sr.flush()
    ...
    def top_motor_abspos(self,  degree=0, speed=50,torque=1):
        """탑모터 절대 각도 위치로 이동시키기
    ...
    def top_motor_time(self, dir, value=3, speed=50,torque=1):
    ...
    def top_motor_round(self,  dir, value=1, speed=50,torque=1,infinite=0):
        ...
        Args:
            ...
            speed (int): 회전 속도
            torque (int): 모터 힘 (1~5)
            infinite (int) : 무한 회전 ,  '0':지정 횟수만큼 회전, '1': 무한회전
        Returns:
            None
        """
        if self.__verbose:
    ...
    def draw_arc(self, len, time=1,mode=0):
        """주어진 시간만큼 원호 그리기

        Args:
           len (int): 원의 반지름
           time (int):  mode==0 이면 시간 (초)
                        mode==1 이면 각도 (0~360)
        ...
        self.__draw_shape(0x09, len, time & 0x00ff, (time >> 8) & 0x00ff)
