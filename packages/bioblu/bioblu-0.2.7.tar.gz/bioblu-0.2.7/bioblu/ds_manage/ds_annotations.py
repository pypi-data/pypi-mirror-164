...
def line_is_yolo_line(line: str) -> bool:
    """
    Returns true if a line is in yolo format (e.g. 0 0.32232 0.73222 0.78888 0.1888 0.8001). The last value (confidence)
    of the line is optional.
    """
...
def count_yolo_annotations(fdir: str, recursive=True) -> int:
    """
    Returns the number of yolo annotations in the text files contained in the given folder (and subfilders, if
    recursive=True).
    ...
    for fpath in txt_fpaths:
        ...
        for line in lines:
            annotation_count += line_is_yolo_line(line)  # True = 1
...
def load_yolo_line_as_dict(line: str) -> dict:
    assert line_is_yolo_line(line)
    # Using named regex groups
    pattern = re.compile(r"(?P<category>\d) (?P<center_x>\d\.\d+) (?P<center_y>\d\.\d+) (?P<width>\d\.\d+) "
...
def load_all_yolo_annotations_only(fdir_annotations):
    files = [os.path.join(fdir_annotations, fname) for fname in os.listdir(fdir_annotations)
...
def remove_material_annotations_from_labelme_files(fdir, material_to_remove):
    ...
    for fpath_json in json_fpaths:
        ...
        for annotation in annotations["shapes"]:
            if annotation["label"] == material_to_remove:
                file_changed = True
            else:
...
# ToDo: after this: get predictions per obj (as a curve, with iou as x)
def cvt_yolo_annotations_to_coco_json(fdir_label_txts: str, fdir_imgs: str, fpath_dst: str, materials_dict: dict):
    txt_files = get_all_fpaths_by_extension(fdir_label_txts, (".txt",))
    img_files = get_all_fpaths_by_extension(fdir_imgs, YOLO_IMG_FORMATS)
...
def get_labelme_categories(fpath_json)  -> List[str]:
...
def find_labelme_files_with_category(fdir: str, category: str) -> List[str]:
    ...
    for file in json_files:
        classes = get_labelme_categories(file)
...
if __name__ == '__main__':
    ...
    # fdir = "/media/findux/DATA/Documents/Malta_II/datasets/TACO_ORIG_SHAPED/"
    # save_to_json(list(get_materials_dict_from_labelme_jsons(fdir).values()),
    # os.path.join("/media/findux/DATA/Documents/Malta_II/datasets/taco_classes.json"))
    # print(count_yolo_annotations("/media/findux/DATA/Documents/Malta_II/datasets/dataset_01/labels/train/"))
    # print(count_yolo_annotations("/media/findux/DATA/Documents/Malta_II/datasets/dataset_01/labels/valid/"))
    # print(count_yolo_annotations("/media/findux/DATA/Documents/Malta_II/datasets/dataset_01/labels/test/"))
    # print(count_yolo_annotations("/media/findux/DATA/Documents/Malta_II/datasets/dataset_02_mixed/"))
    # print(count_yolo_annotations("/media/findux/DATA/Documents/Malta_II/datasets/dataset_04_gnejna_with_duplicates/labels/train/"))
    # print(count_yolo_annotations("/media/findux/DATA/Documents/Malta_II/datasets/dataset_04_gnejna_with_duplicates/labels/valid/"))
    # print(count_yolo_annotations("/media/findux/DATA/Documents/Malta_II/datasets/dataset_04_gnejna_with_duplicates/labels/test/"))

    # fdir = "/media/findux/DATA/Documents/Malta_II/datasets/TACO_ORIG_SHAPED_RELEVANT/"
    #
    # update_all_labelme_materials(fdir, "Clothes pin", "Other plastic")
    # update_all_labelme_materials(fdir, "Corrugated carton", "Cardboard")
    # update_all_labelme_materials(fdir, "Crisp packet", "Plastic wrapper & foil")
    # update_all_labelme_materials(fdir, "Clear plastic bottle", "Plastic bottle")
    # update_all_labelme_materials(fdir, "Foam cup", "Plastic cup")
    # update_all_labelme_materials(fdir, "Foam food container", "Plastic container")
    # update_all_labelme_materials(fdir, "Paper cup", "Cardboard cup")
    # update_all_labelme_materials(fdir, "Pizza box", "Cardboard")
    # update_all_labelme_materials(fdir, "Magazine paper", "Paper")
    # update_all_labelme_materials(fdir, "Normal paper", "Paper")
    # update_all_labelme_materials(fdir, "Food can", "Metal can")
    # update_all_labelme_materials(fdir, "Paper bag", "Paper")
    # update_all_labelme_materials(fdir, "Paper straw", "Straws")
    # update_all_labelme_materials(fdir, "Other carton", "Cardboard")
    # update_all_labelme_materials(fdir, "Other plastic bottle", "Plastic bottle")
    # update_all_labelme_materials(fdir, "Other plastic container", "Plastic container")
    # update_all_labelme_materials(fdir, "Other plastic cup", "Plastic cup")
    # update_all_labelme_materials(fdir, "Other plastic wrapper", "Plastic wrapper & foil")
    # update_all_labelme_materials(fdir, "Plastic glooves", "Other plastic")
    # update_all_labelme_materials(fdir, "Styrofoam piece", "Other plastic")
    # update_all_labelme_materials(fdir, "Plastic film", "Plastic wrapper & foil")
    # update_all_labelme_materials(fdir, "Plastic lid", "Other plastic")
    # update_all_labelme_materials(fdir, "Wrapping paper", "Plastic wrapper & foil")
    # update_all_labelme_materials(fdir, "Aerosol", "Metal can")
    # update_all_labelme_materials(fdir, "Drink can", "Metal can")
    # update_all_labelme_materials(fdir, "Aluminium blister pack", "Plastic wrapper & foil")
    # update_all_labelme_materials(fdir, "Bicycle tyre", "Tyre")
    # update_all_labelme_materials(fdir, "Car tyre", "Tyre")
    # update_all_labelme_materials(fdir, "Cardboard straw", "Straws")
    # update_all_labelme_materials(fdir, "Plastic straw", "Straws")
    # update_all_labelme_materials(fdir, "Polypropylene bag", "Plastic bag")
    # update_all_labelme_materials(fdir, "Single-use carrier bag", "Plastic bag")
    # update_all_labelme_materials(fdir, "Six pack rings", "Other plastic")
    # update_all_labelme_materials(fdir, "Spread tub", "Plastic container")
    # update_all_labelme_materials(fdir, "Tupperware", "Plastic container")
    # update_all_labelme_materials(fdir, "Meal carton", "Cardboard")
    # update_all_labelme_materials(fdir, "Garbage bag", "Plastic bag")
    # update_all_labelme_materials(fdir, "Clothes pin", "Other plastic")
    # update_all_labelme_materials(fdir, "Crisp packet", "Plastic wrapper & foil")
    # update_all_labelme_materials(fdir, "Disposable food container", "Plastic container")
    # update_all_labelme_materials(fdir, "Disposable plastic cup", "Plastic cup")
    # update_all_labelme_materials(fdir, "Corrugated carton", "Cardboard")
    # update_all_labelme_materials(fdir, "Food Can", "Metal can")
    #

    # fdir = "/media/findux/DATA/Documents/Malta_II/datasets/TACO_ORIG_SHAPED_RELEVANT/"
    # mats = get_materials_dict_from_labelme_jsons(fdir)
    # print(json.dumps(mats, indent=0))
    # print(find_labelme_files_with_category(fdir, "Metal lid"))

    fdir = "/home/findux/Desktop/test/"
    n_annot = count_yolo_annotations(fdir)
    line = "0 0.956771 0.974537 0.08125 0.0509259"
    line_dict = load_yolo_line_as_dict(line)
    print(line_dict)
