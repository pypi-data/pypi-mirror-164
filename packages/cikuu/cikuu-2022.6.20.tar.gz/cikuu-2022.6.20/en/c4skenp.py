...
def run(infile):
	...
	outfile = infile.split('.')[0] + f".c4skenp"
	start = time.time()
		...
		for sid, line in enumerate(fileinput.input(infile,openhook=fileinput.hook_compressed)): 
			try:
				...
				for i,snt in enumerate(tdoc.sents):
					...
					skenp = "_^ " + " ".join([ "_NP" if t.ent_type_ == 'NP' else f"{t.text}_{t.lemma_}_{t.pos_}_{t.tag_}" for t in doc]) + " _$"
					fw.write(json.dumps({'_id': f"{did}-{i}-skenp", '_source': {"skenp":skenp, "url":url} }) + "\n") 
