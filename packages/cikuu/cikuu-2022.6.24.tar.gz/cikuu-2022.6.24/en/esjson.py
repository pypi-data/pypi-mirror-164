...
def kps(sid, snt, doc, with_tok:bool=False): 
	''' 2022.8.19 '''
	postag = "_^ " + ' '.join([ f"_{t.pos_}_{t.tag_}" if t.pos_ in ('PROPN','NUM','X','SPACE','PUNCT') else f"{t.text.lower()}_{t.lemma_}_{t.pos_}_{t.tag_}{_oneself(t)}"  for t in doc])
...
def kps_esjson(infile, outfile=None,  with_tok:bool=False):
