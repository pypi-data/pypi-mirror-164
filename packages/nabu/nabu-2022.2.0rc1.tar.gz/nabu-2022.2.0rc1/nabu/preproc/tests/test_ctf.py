...
class TestCtf:
    ...
    def check_result(self, res, ref, error_message):
        assert np.abs(res - ref).max() < self.abs_tol * (np.abs(ref).mean()), error_message
    ...
    def test_ctf_id16_way(self):
        ...
        ctf_filter = ctf.CtfFilter(
            self.dark.shape, geo_pars, self.delta_beta,
            padded_shape=self.padded_img_shape_vh, translation_vh=my_shift, normalize_by_mean=True,
            lim1=1.0e-5, lim2=0.2
        )
        phase = ctf_filter.retrieve_phase(my_img)

        self.check_result(
            phase, self.expected_result,
            "retrieved phase and reference result differ beyond the accepted tolerance"
        )
    ...
    def test_ctf_id16_class(self):
        ...
        ctf_filter = ctf.CtfFilter(
            img.shape, geo_pars, self.delta_beta,
            padded_shape=self.padded_img_shape_vh, translation_vh=shift, normalize_by_mean=True,
            lim1=1.0e-5, lim2=0.2
        )
    ...
    def test_ctf_plain_way(self):
        ...
        flatfielder = FlatFieldArrays(
            ...
            {0: self.flats[0], 1200: self.flats[1]},
            {0: self.dark}, radios_indices=[self.ipro]
        )
    ...
    def test_cuda_ctf(self):
        ...
        geo_pars = ctf.GeoPars(
            z2=distance_m,
            pix_size_det=pix_size_m,
            wavelength=1.23984199e-9 / energy_kev
        )
        ...
        for normalize in [True, False]:
            ...
            cuda_ctf_filter = CudaCTFPhaseRetrieval(
                data.shape,
                geo_pars, delta_beta=delta_beta, use_rfft=True, normalize_by_mean=normalize,
            )
