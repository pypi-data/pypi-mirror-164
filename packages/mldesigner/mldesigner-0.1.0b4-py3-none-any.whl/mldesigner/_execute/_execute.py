...
def _get_source_file_from_base_node(node):
    ...
    for input_name, input in node.inputs.items():
        ...
        if meta._is_primitive_type:
            value = input._data or meta.default
            if value or meta.optional is True:
                node_inputs[input_name] = value
        else:
            value = input._data.path if input._data else None
            if value or meta.optional is True:
                node_inputs[input_name] = value
...
def _get_executor_from_source_file(source, name):
    ...
    execute_logger.info(f"Looking for target component file: {abs_file_path}")
    if not file.exists():
        raise MldesignerExecutionError(f"Source file does not exist: {abs_file_path}")
    execute_logger.info(f"Fetching target component: {name!r}")
    if name is None:
...
def _execute_component_from_executor(executor, inputs, outputs):
    ...
    execute_logger.info(f"Mldesigner starts executing target component: '{executor._name}'...")
    ...
    if len(res) != 0:
        ...
        for k, v in res.items():
            execute_logger.info(f"\t{k}: {v}")

    # warning about the user input args that did not get used.
    addtional_args = executor._additional_args
    if isinstance(addtional_args, dict) and len(addtional_args) > 0:
        execute_logger.warning(f"Additional args found and ignored: {list(addtional_args.items())}")
    return res
