...
from mldesigner._generate._generators._param_generator import ParamGenerator, EnumGenerator
...
class BaseComponentGenerator:
    ...
    def _get_component_type_cls(self):
        ...
        if node_cls is None:
            raise UserErrorException(f"Unsupported component type: {type(self._entity)}")
    ...
    def _get_component_inputs(self):
        ...
        for in_name, val in self._entity.inputs.items():
            ...
            # because the component fucntion call is case insensitive, but node params are case sensitive
            if isinstance(val, Input):
                if val._is_enum() or self._is_internal_component_enum(val):
