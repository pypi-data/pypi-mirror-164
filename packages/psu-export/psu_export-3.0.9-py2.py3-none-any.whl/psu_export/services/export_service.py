        ...
        try:
            ...
            for mm in models:
                ...
                if not model_definition:
                    log.info(f"Adding model definition: {app}.{mm}")
                ...
                for ff in field_list:
                    ...
                    if not field_definition:
                        log.info(f"Adding field definition: {field_name}")
    ...
    for app, models in apps.all_models.items():
        ...
        for mm in models:
            ...
            if not model_definition:
                ...
                continue
            if not model_definition.export_enabled:
        ...
        if not audit_mode:
            ...
            def auto_export_date_definition():
                return [
                    ...
                    "cognos_export_date",
                    "datetime",
                    f"{str(last_model).upper()}_EXPORT_DATE",
                    "varchar2(255)",
                    "date",
                    None,
...
def export_model(model_definition):
    ...
    try:
        ...
        for result in model_instance.objects.all():
            ...
            for field in model_fields:
                ...
                if field_name not in columns:
                    skipped_columns.append(field_name)
                    continue
        ...
        # Show skipped columns for debugging
        if skipped_columns:
            skipped_columns = list(set(skipped_columns))
            log.info(f"Skipped columns in {model_name}: {skipped_columns}")
            log.info(f"Included columns in {model_name}: {columns}")
...
def _create_s3_file(file_path, data, columns):
    try:
        ...
        # Get debugging info on column/data mismatch
    ...
    Bad Row Example:
        {bad_rows[0] if bad_rows else None}
        """)

        if good_rows and bad_rows:
            log.info("Exporting only the good rows...")
            _create_s3_file(file_path, good_rows, columns)
            log.warning("Exported only the good rows.")
