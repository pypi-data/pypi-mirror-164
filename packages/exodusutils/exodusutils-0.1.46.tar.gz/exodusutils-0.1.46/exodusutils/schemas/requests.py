...
class SimpleIIDTrainRequest(BaseModel):
    ...
    def __exit__(self, *args):
        self.training_file.delete(self._minio)
        if self.holdout_file is not None:
            self.holdout_file.delete(self._minio)
        if self.validation_file is not None:
            self.validation_file.delete(self._minio)
        return self
    ...
    def load(self):
        ...
        training_data = self.training_file.upload(self._minio)
        validation_data = None if self.validation_file is None else self.validation_file.upload(self._minio)
        holdout_data = None if self.holdout_file is None else self.holdout_file.upload(self._minio)
