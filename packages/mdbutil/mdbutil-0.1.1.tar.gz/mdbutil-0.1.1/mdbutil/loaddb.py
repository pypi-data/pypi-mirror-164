...
@click.option("-d", "--dialect", help="SQL CSV/TSV dialect definition (default \"LINES TERMINATED BY '\\r\\n' IGNORE 1 ROWS\")", default="LINES TERMINATED BY '\\r\\n' IGNORE 1 ROWS")
def load_db(host: str, user: str, password: str, database: str, table: str, input: str, dialect: str):
    ...
    with pymysql.connect(host=host, user=user, password=password, database=database, charset='utf8mb4', local_infile=True,
        ...
        if input.endswith(".gz"):
            ...
            p.start()
            try:
                cur.execute(f"LOAD DATA LOCAL INFILE 'pipe.tsv' INTO TABLE {table} {dialect}")
            ...
            cur.execute(f"LOAD DATA LOCAL INFILE '{input}' INTO TABLE {table} {dialect}")
            print(f"Ingested {cur.rowcount} rows.")
            cur.execute("SHOW WARNINGS")
            print(f"Errors and warnings encountered: {cur.fetchall()}")
