...
def setup_propagator(v: Tensor,
    ...
    n_batch = get_n_batch([source_locations, receiver_locations] +
                          list(wavefields))
    ay, ax, by, bx = \
        setup_pml(pml_width, fd_pad, dy, dx, dt, v, max_vel, pml_freq)
    nt *= step_ratio
...
def setup_pml(pml_width: List[int], fd_pad: int, dy: float, dx: float,
              dt: float, v: Tensor, max_vel: float, pml_freq: Optional[float],
    ...
    a = []
    b = []
    a0 = math.exp(-math.pi * pml_freq * abs(dt))
    for width, ds in zip(pml_width, [dy, dy, dx, dx]):
        if width == 0:
            a.append(a0 * torch.ones(fd_pad, device=device, dtype=dtype))
            b.append(torch.zeros(fd_pad, device=device, dtype=dtype))
            continue
        width = width + fd_pad
        sigma = (
            3
            * max_vel
            * -math.log(R) / (2 * width * ds)
            * (torch.arange(width + 1, device=device, dtype=dtype)
               / width) ** 2
        )
        alpha = (
            math.pi * pml_freq
            * (1 - torch.arange(width + 1, device=device, dtype=dtype)
               / width)
        )
    ...
    ay[:fd_pad + pml_width[0] + 1] = a[0].flip(0)
    ay[len(ay) - pml_width[1] - 1 - fd_pad:] = a[1]
    ax[:fd_pad + pml_width[2] + 1] = a[2].flip(0)
    ax[len(ax) - pml_width[3] - 1 - fd_pad:] = a[3]
    by[:fd_pad + pml_width[0] + 1] = b[0].flip(0)
    by[len(by) - pml_width[1] - 1 - fd_pad:] = b[1]
    bx[:fd_pad + pml_width[2] + 1] = b[2].flip(0)
    bx[len(bx) - pml_width[3] - 1 - fd_pad:] = b[3]
    return ay, ax, by, bx
