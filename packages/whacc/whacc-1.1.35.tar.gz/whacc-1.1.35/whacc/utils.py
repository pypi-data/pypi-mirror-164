...
def foo_predict_mods(mod_list, h5_list, chunk_size=10000):
    for mod_str in make_list(mod_list, True):
        ...
        for h5 in make_list(h5_list, True):
            ...
            write_key = 'YHAT__' + os.path.basename(mod_str).split('.')[0]
...
def foo_predict(mod, h5_dir, write_key='temp_yhat'):
    # mod = load_obj(r'X:\PHILLIP\Samsons_models\model_saves_2\model_save.pkl') # replace with the model file full path
    # h5_dir= '\WhACC_PROCESSING_FOLDER\Session77_FINISHED\\' # replace this with a directory to any of the h5 filesimport sys, os
    #
    # # Disable
    # def blockPrint():
...
def batch_process_videos_on_colab(bd, local_temp_dir, video_batch_size=2, copy_image_files_to_final_h5=False):
    ...
    while True:  # keep going until there are no more files to process
        ...
        process_these_videos = np.where(file_dict['is_processed'] == False)[0][
                               :video_batch_size]  # fix001 - make not equal to True so that skip string can be in there
...
def combine_final_h5s(h5_file_list_to_combine, delete_extra_files=False):
    ...
    with h5py.File(fn, 'r+') as h:
        ...
        if h5_key_exists(h5_file_list_to_combine[0], 'images'):
            for i, k in enumerate(h5_file_list_to_combine):
    ...
    overwrite_h5_key(fn, 'trial_nums_and_frame_nums',
                     image_tools.get_h5_key_and_concatenate(h5_file_list_to_combine[0], 'trial_nums_and_frame_nums'))
...
'/Users/phil/Dropbox/HIRES_LAB/GitHub/whacc/whacc/whacc_data/feature_data/start_end_nan_locations.pkl'
