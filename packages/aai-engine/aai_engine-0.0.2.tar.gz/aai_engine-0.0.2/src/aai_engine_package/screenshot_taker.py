...
class ScreenShotTaker():
    ...
    """
    def __init__(self, master, save_location, haystack=SCREENSHOT, editing=False, temp_screenshot_path='my_screenshot.png'):
        """
    ...
    def view_offset_picker(self):
        ...
        if not self.editing:
            ...
            self.tk_cropped = ImageTk.PhotoImage(self.cropped)
            ...
            self.cropped.save('my_cropped.png', 'PNG')
            self.tk_cropped = ImageTk.PhotoImage(self.im)
            w, h = self.master.winfo_screenwidth(), self.master.winfo_screenheight()
            self.w = w
            self.h = h
            w = w / 4
            ...
            # drawing default cross at center
            targetImage = PngImageFile(self.haystack)
    ...
    def view_confidence_picker(self):
        ...
        self.init_canvas_screenshot(SCREENSHOT)
    ...
    def confirm_name(self):
        ...
        self.save_path = ''.join([self.save_location, "/img/", self.textBox.get().strip(), ".png"])
    ...
    def calculate_all_matches(self):
        ...
        """
        confidence_float = float(self.confidence) / 100.0
        imloc = pyautogui.locateAll('my_cropped.png', 'my_screenshot.png', confidence=confidence_float)
        self.matches = []
