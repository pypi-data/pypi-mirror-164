...
class TaskWrapper():
    ...
    def execute(self):
        ...
        p = subprocess.Popen(['python3', filepath], cwd=self.cwd) #, startupinfo=info)
        stdout, stderr = p.communicate()
        log_time = datetime.strftime(datetime.now(), '%Y%m%d-%hh%mm%ss')
        log_file_stdout = os.path.join(self.log_dir, log_time, "stdout.txt")
        log_file_stderr = os.path.join(self.log_dir, log_time, "stdout.txt")
        
        with open(log_file_stdout, "w") as log_file:
            log_file.write(stdout)

        with open(log_file_stderr, "w") as log_file:
