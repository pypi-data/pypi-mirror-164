...
def _handle(response):
    if response.status_code != 200:
        ...
        if not response["success"]:
            raise RuntimeError(f"Billy returned error {response['result']}")
        else:
...
class Subscription:
    @staticmethod
    @checkUrlIsSet
    def create(customerId: str, productId: str, option: Union[str,int] = "base", cancelExistingSubscriptions: bool = True, invoiceNow: bool = True, prorate: bool = True, cancelAtPeriodEnd: bool = False, trialUntil: int = None, paymentMethodTimeout: int = 1, paymentMethodTypes: str = "card"):
