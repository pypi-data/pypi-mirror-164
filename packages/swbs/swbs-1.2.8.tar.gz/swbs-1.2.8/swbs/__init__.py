...
from Cryptodome.Cipher import AES, _mode_eax
...
class Security:
    ...
    def encrypt(key: Union[str, bytes], message: Union[str, bytes]) -> list:
        ...
        encryptor: _mode_eax.EaxMode = \
            AES.new(key, AES.MODE_EAX)  # type: ignore
    ...
    def decrypt(key: Union[str, bytes], message: bytes, tag: bytes,
        ...
        try:
            ...
            if return_bytes is True:
                return AES.new(key, AES.MODE_EAX,   # type: ignore
                               nonce).decrypt_and_verify(message, tag)
            return AES.new(key, AES.MODE_EAX,  # type: ignore
                           nonce).decrypt_and_verify(message, tag).decode(
                            "utf-8", "replace")
        ...
        while self.thread_listen_kill_flag is False:
            ...
            if hasattr(self.connection_handler, "__bases__") and \
                ...
                thread = threading.Thread(
                    ...
                    args=(self, connection_socket, self.clients_handled,))
            self.clients.update(
