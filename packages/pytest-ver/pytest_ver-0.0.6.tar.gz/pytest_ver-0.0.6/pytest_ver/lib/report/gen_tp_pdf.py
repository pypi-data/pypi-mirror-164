...
class GenTpPdf(GenBasePdf):
    ...
    def __init__(self, protocols, do_results=True):
        ...
        path = os.path.join(services.cfg.outdir, f'{fname}.pdf')
        self._doc_init(path)
    ...
    def _gen_protocol_info(self, tbl, protocol):
        ...
        tbl.append([
            ...
            '', '', '',
            Paragraph(f'<b>Requirements:</b>: see below', self._stbody),
            '', '',
        ])
    ...
    def _gen_step(self, tbl, stepno, step, requirements):
        ...
        stepno = Paragraph(str(stepno), self._strow)
        desc = Paragraph(step['desc'], self._strow)
        expected = Paragraph(str(rs.expected_formatted), self._stmono)

        if self._do_results:
            actual = Paragraph(str(rs.actual_formatted), self._stmono)
            result = Paragraph(str(rs.result), self._strow)
        else:
            actual = Paragraph('', self._stmono)
            result = Paragraph('', self._strow)

        if self._do_results:
            details = Paragraph(str(step['dts']) + '<br/>' + str(rs.location), self._strow)
        else:
            details = Paragraph('' + '<br/>' + str(rs.location), self._strow)

        if rs.reqids is None:
            reqids_str = Paragraph('N/A', self._strow)
        else:
            for reqid in rs.reqids:
                if reqid not in requirements:
                    requirements.append(reqid)
            if len(rs.reqids) == 1:
                reqids_str = Paragraph(rs.reqids[0], self._strow)
            else:
                reqids_str = Paragraph('<br/>'.join(rs.reqids), self._strow)
