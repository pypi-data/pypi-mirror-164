...
class Client():
    ...
    def create(
        ...
        **kwargs
        ...
        self.history = [create_history_event(member=self.__dict__, event_type='alta', source='checkout')]
    ...
    def set_dates(self, cobro):
        ...
        self.fecha_vigencia = None if self.status == 'pago en proceso' else set_next_vigency(npd)
    ...
    def push_to_db(self, db):
        client = db.clientes.find_one({'documento': self.documento})
        if not client:
            inserted = db.clientes.insert_one(self.__dict__)
            self._id = inserted.inserted_id
        else:
            db.clientes.update_one({'documento': self.documento}, {'$set': self.__dict__})
