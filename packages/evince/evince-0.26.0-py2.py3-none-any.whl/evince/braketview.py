...
class BraketView(widgets.DOMWidget):
    ...
    _view_module_version = Unicode('^0.26.0').tag(sync=True)

    # Version of the front-end module containing widget model
    _model_module_version = Unicode('^0.26.0').tag(sync=True)
    ...
    def __init__(self, ket_instance, surface_view = False, bg_color = [0.0,0.0,0.0]):
    ...
    float cs = 0.016031349546793448*tex[2]*(pow(tex[0], 2.0) - pow(tex[1], 2.0))*exp(-0.10000000000000001*q);
     gl_FragColor = gl_FragColor + vec4(cs*sin(time), 0.01, -1.0*cs*cos(time), .1);
        ...
        if np.sum(np.array(ket_instance.energy)**2.0)>1e-10:
            time_dependent = True
        self.fragment_shader, self.nd = generate_webgl_shader(ket_instance, time_dependent=time_dependent)
        if self.nd<3:
