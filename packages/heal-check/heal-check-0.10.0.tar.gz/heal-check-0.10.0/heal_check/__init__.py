...
from heal_check.exceptions import CheckWarning, UrlWarning, ResponseWarning, TimestampWarning, StatusWarning, ExpectedModeWarning, DictionaryError, StatusError, WarningTimeoutError
...
def check(url: str, request_timeout: float = REQUEST_TIMEOUT_DEFAULT, heal_timeout: float = HEAL_TIMEOUT_DEFAULT, ignore_tls: bool = False) -> RemoteData:
    ...
    _check_timestamp(remote_data.timestamp, heal_timeout)
...
def warning_tolerant_check(url: str,
                           ...
                           heal_timeout: float = HEAL_TIMEOUT_DEFAULT,
    ...
    try:
        while True:
            try:
                remote_data = check(url, request_timeout, heal_timeout, ignore_tls)
...
def main(args: Optional[List[str]] = None) -> None:
    ...
    print(warning_tolerant_check(args.url, event, args.expected_mode, args.delay, args.warning_timeout, args.request_timeout, args.heal_timeout, args.ignore_tls).to_json())
