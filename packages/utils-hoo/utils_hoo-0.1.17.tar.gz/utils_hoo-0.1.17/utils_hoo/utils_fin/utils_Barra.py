...
from utils_hoo import logger_show
...
def get_exp_decay_weight(N, adjust=True, logger=None, **kwargs):
    ...
    Parameters
    ----------
    adjust: bool，若为False，则采用精确方法计算权重，True采用近似方法
    kwargs: 设置计算最新一期权重a的参数
    ...
    Return
    ------
    w: 权重序列，返回中w[-1]为最近一期权重，w[0]为最远一期权重
...
def get_regression_beta(vals_y, vals_x, weights=None, intercept=True,
    ...
    Parameters
    ----------
    vals_y, vals_x: 因变量和自变量，均为一维
    weights: 若为None则进行普通回归；若为一维权重向量，则进行加权回归
    intercept: bool，设置回归是否加入截距项
    GLS: 是否用GLS回归（weights不为None时不适用）
    algo: 计算回归系数算法设置
        'ori': 用原始样本数据直接计算
...
def get_Barra_beta(data, col_pct, col_pct_mkt, rf=3/100, halflife=63, N=252,
                   intercept=True, GLS=False, algo='ori'):
    '''
    beta因子计算

    Parameters
    ----------
    data: pd.DataFrame，须包含col_pct和col_pct_mkt两列
    col_pct: col_pct列为个股收益率序列数据
    col_pct_mkt: col_pct_mkt列为指数收益率数据
    rf: 无风险收益（年化）
    halflife: 半衰期
    N: 一年交易天数
    intercept: 回归是否考虑截距
    GLS: 是否使用GLS回归
    algo: 回归算法选择，参见get_regression_beta函数
    '''

    df = data.reindex(columns=[col_pct, col_pct_mkt])
    rf = pow(1+rf, 1/N) - 1 # 每日无风险收益收益率
    ...
    # 滚动计算beta
    df['beta'] = np.nan
    weights = get_exp_decay_weight(N, halflife=halflife)
    for k in range(df.shape[0]-N+1):
        vals_x = df['expct_mkt'].iloc[k: k+N]
        vals_y = df['expct'].iloc[k: k+N]
        beta = get_regression_beta(vals_y, vals_x, weights=weights,
                                   intercept=intercept, GLS=GLS, algo=algo)
        df.loc[k+N-1, 'beta'] = beta
