...
def get_pre_val_Fcond(data, col_val, col_Fcond, Fcond):
    ...
    获取上一个满足指定条件的行中col_val列的值，条件为：
        该行中col_Fcond列的值x满足Fcond(x)为True
    返回结果为pd.Series
    '''
...
def get_tushare_api(token=None):
    ...
    if token is None:
        ts_token = ('595253d60e483e240a4924784c23e1e092e7df7193655'
                    'a66b9ed6862')
...
def get_aligned_data(data, col, since=None, until=None,
    ...
    tmp['nextlastholiday'] = get_pre_val_Fcond(tmp, 'day', 'tmp',
                                               lambda x: x == -1)
