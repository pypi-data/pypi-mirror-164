...
def calIC(data, colFactor, colP='close', colT=None, colCode=None,
    ...
    if isnull(colCode):
        df['code'] = 'code0'
        colCode = 'code'
    df = df.reindex(columns=[colCode, colT, colP, colFactor])
    df_ = []
    def get_pct(df):
        df.sort_values(colT, inplace=True)
        df['pct'] = df[colP].shift(-lag) / df[colP] - 1
        return df
    for code in df[colCode].unique():
        df_.append(get_pct(df[df[colCode] == code].copy()))
...
def simPrice_MC(his_prices, nT, nCut=240, Nsim=1000,
                random_seed=None, keepP0=True, return_mid=False,
    ...
    for k in range(1, nT_+1):
        df_sim_['P{}'.format(k)] = np.random.randn(Nsim)
        df_sim_['P{}'.format(k)] = df_sim_['P{}'.format(k-1)] + df_sim_['P{}'.format(k-1)] * \
                           (mu_deltaT + df_sim_['P{}'.format(k)] * sigma_deltaT_sqrt)
    if keepP0:
        cols = ['P{}'.format(k*nCut) for k in range(0, nT+1)]
    else:
        cols = ['P{}'.format(k*nCut) for k in range(1, nT+1)]
...
def MAsOrder(series, lags=[3, 5, 15, 20, 30, 60]):
    ...
    def get_order_type(x):
        ...
        if order_type == len(lags) - 1:
            ...
            return 0
    df['order_type'] = df[['ma_{}'.format(lag) for lag in lags]].apply(lambda x:
