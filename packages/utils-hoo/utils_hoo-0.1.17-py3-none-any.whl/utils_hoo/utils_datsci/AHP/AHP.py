    ...
    for i in range(JM.shape[0]):
        for j in range(JM.shape[1]):
            if abs(JM[i, j] * JM[j, i] - 1) > tol:
                if logger != False:
                    if isnull(logger):
                        print('判断矩阵可能错误的行和列位置: {}, {}'.format(i, j))
                    else:
                        logger.warning('判断矩阵可能错误的行和列位置: {}, {}'.format(i, j))
                return False
    return True
...
def cal_weights(judge_mat, RI_dict=None, checkJMtol=0.001, logger=None):
    ...
    if logger != False and not isnull(logger):
        ...
        logger.info('特征值：\n{}'.format(lambdas.real))
        logger.info('特征向量：\n{}'.format(vectors.real))
        logger.info('最大特征值：{}'.format(lmdmax.real))
        logger.info('最大特征值对应特向量：\n{}'.format(max_vector.real))
        logger.info('标准化权重：\n{}'.format(w))
        logger.info('RI: {}'.format(RI))
        logger.info('CI: {}'.format(CI))
...
def cal_weights_sim(judge_mat, RI_dict=None, checkJMtol=0.001, logger=None):
    ...
    if logger != False and not isnull(logger):
        logger.info('判断矩阵：\n{}'.format(judge_mat))
        logger.info('最大特征值：{}'.format(lmdmax))
        logger.info('最大特征值对应特向量：\n{}'.format(max_vector))
        logger.info('标准化权重：\n{}'.format(w))
        logger.info('RI: {}'.format(RI))
        logger.info('CI: {}'.format(CI))
    ...
    for k in range(len(judge_mats)):
        ...
        if isOK:
            ...
            if skipBadJM:
                if logger != False:
                    if isnull(logger):
                        print('第{}个判断矩阵不能通过一致性检验，将被舍弃！'.format(k+1))
                    else:
                        logger.info('第{}个判断矩阵不能通过一致性检验，将被舍弃！'.format(k+1))
            else:
                if logger != False:
                    if isnull(logger):
                        print('第{}个判断矩阵不能通过一致性检验！'.format(k+1))
                    else:
                        logger.warning('第{}个判断矩阵不能通过一致性检验！'.format(k+1))
                us.append(CI)
                JMs.append(judge_mats[k])
