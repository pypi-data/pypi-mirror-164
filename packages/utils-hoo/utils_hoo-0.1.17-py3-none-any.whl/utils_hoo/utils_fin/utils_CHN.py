...
def get_target_date_by_delta(end_date, n, out_format='str'):
    '''给定一个报告期，往前推n期'''
...
def get_last_effect_finreport_dates(date, force_q1q3=False, wait_finished=False,
                                    semi_annual_only=False, annual_only=False,
                                    adjust_report=False):
    '''
    获取距离date最近的财报日期，即date可以取到最新财报数据对应的财报日期
    ...
    if not isinstance(date, int):
        _, joiner = utl_dttm.get_date_format(date)
        date = int(utl_dttm.date_reformat(date, ''))
        int_ = False
    else:
        int_ = True
    year = date // 10000
    ...
    if not int_:
        if isinstance(dates, list):
            dates = [utl_dttm.date_reformat(str(x), joiner=joiner) for x in dates]
        elif isinstance(dates, dict):
            dates = {utl_dttm.date_reformat(str(k), joiner=joiner):
                     utl_dttm.date_reformat(str(v), joiner=joiner) \
...
def get_trade_dates_ChnCal(start_date, end_date, joiner='-'):
    ...
    注：若chinese_calendar库统计的周内工作日与交易日有差异或没更新，可能导致结果不准确
    '''
    _, joiner = utl_dttm.get_date_format(start_date)
...
def get_trade_dates(start_date, end_date=None, trade_dates_df_path=None,
    ...
    _, joiner1 = utl_dttm.get_date_format(start_date)
    if isnull(end_date):
    ...
    if isnull(trade_dates_df_path):
        ...
        _, joiner2 = utl_dttm.get_date_format(end_date)
