...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        duxlp__nrt = self.overloads[sig]
        pfbra__sgcl = duxlp__nrt.metadata.get('distributed_diagnostics', None)
        if pfbra__sgcl is None:
            bom__ckry = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(bom__ckry)
        pfbra__sgcl.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        mtx__ani = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(mtx__ani)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        zxre__awpwc = options.pop('distributed')
        pipeline_class = (pipeline_class if zxre__awpwc else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        cbf__bfchr = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if cbf__bfchr else
            pipeline_class)
    qavhr__roghp = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(qavhr__roghp, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(qavhr__roghp)
        else:
            return master_mode_wrapper(qavhr__roghp)
    else:
        return qavhr__roghp
