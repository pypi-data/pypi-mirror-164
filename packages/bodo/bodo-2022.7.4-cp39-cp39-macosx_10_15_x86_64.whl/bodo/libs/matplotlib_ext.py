...
def _install_mpl_types():
    ptx__qgy = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for aausm__sia, oeju__sul in ptx__qgy:
        install_py_obj_class(types_name=aausm__sia, python_type=oeju__sul,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    mpg__jcgjk = ', '.join(f'e{jma__djqpq}' for jma__djqpq in range(len(args)))
    if mpg__jcgjk:
        mpg__jcgjk += ', '
    quuu__nkm = ', '.join(f"{qurb__jxm} = ''" for qurb__jxm in kws.keys())
    ognx__jrl = 'matplotlib_obj, ' if obj_typ is not None else ''
    ptysl__zvu = f'def mpl_stub({ognx__jrl} {mpg__jcgjk} {quuu__nkm}):\n'
    ptysl__zvu += '    pass\n'
    tez__nuog = {}
    exec(ptysl__zvu, {}, tez__nuog)
    yzvhe__kaw = tez__nuog['mpl_stub']
    edc__dthae = numba.core.utils.pysignature(yzvhe__kaw)
    izy__qfgfk = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(izy__qfgfk)).replace(pysig=
        edc__dthae)


def generate_axes_typing(mod_name, nrows, ncols):
    eomhm__cvixb = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(eomhm__cvixb.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(eomhm__cvixb.format(mod_name, 'ncols'))
    ubdvd__mux = get_overload_const_int(nrows)
    dhozr__kpsmm = get_overload_const_int(ncols)
    if ubdvd__mux < 1:
        raise BodoError(eomhm__cvixb.format(mod_name, 'nrows'))
    if dhozr__kpsmm < 1:
        raise BodoError(eomhm__cvixb.format(mod_name, 'ncols'))
    if ubdvd__mux == 1 and dhozr__kpsmm == 1:
        ixo__hqizv = types.mpl_axes_type
    else:
        if dhozr__kpsmm == 1:
            ozsd__utsbd = types.mpl_axes_type
        else:
            ozsd__utsbd = types.Tuple([types.mpl_axes_type] * dhozr__kpsmm)
        ixo__hqizv = types.Tuple([ozsd__utsbd] * ubdvd__mux)
    return ixo__hqizv


def generate_pie_return_type(args, kws):
    mvuf__xqgx = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if mvuf__xqgx == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    trpwr__lcvl = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(trpwr__lcvl):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(trpwr__lcvl):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        zhl__srit = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, zhl__srit]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    zhl__srit = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    aausm__sia = str(zhl__srit)
    if not hasattr(types, aausm__sia):
        aausm__sia = f'objmode_type{ir_utils.next_label()}'
        setattr(types, aausm__sia, zhl__srit)
    ptysl__zvu = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{aausm__sia}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    tez__nuog = {}
    exec(ptysl__zvu, {'numba': numba, 'np': np}, tez__nuog)
    impl = tez__nuog['impl']
    return impl
