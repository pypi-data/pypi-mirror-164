...
def _global_except_hook(exctype, value, traceback):
    ...
    etgp__fgmhn = MPI.COMM_WORLD
    swkxl__dzkja = etgp__fgmhn.Get_rank()
    bhyo__mvvad = 3.0
    xxpru__xpls = True
    uixx__hurt = etgp__fgmhn.Ibarrier()
    wnju__olvbd = time.time()
    while time.time() - wnju__olvbd < bhyo__mvvad:
        time.sleep(0.1)
        if uixx__hurt.Test():
            xxpru__xpls = False
            break
    try:
        ...
        if xxpru__xpls:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {swkxl__dzkja}. \n')
...
def set_numba_environ_vars():
    if (dcoi__regxs := os.environ.get('BODO_PLATFORM_CACHE_LOCATION')
        ) is not None:
        if 'NUMBA_CACHE_DIR' in os.environ and os.environ['NUMBA_CACHE_DIR'
            ...
            numba.config.CACHE_DIR = dcoi__regxs
            os.environ['NUMBA_CACHE_DIR'] = dcoi__regxs
    numba.config.DISABLE_PERFORMANCE_WARNINGS = 1
    hrj__uthr = {'NUMBA_DISABLE_PERFORMANCE_WARNINGS': '1'}
    os.environ.update(hrj__uthr)
