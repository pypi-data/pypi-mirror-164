...
def decimal_to_str_codegen(context, builder, signature, args, scale):
    ...
    kwri__tcg = cgutils.create_struct_proxy(types.unicode_type)(context,
        builder)
    phwu__kvja = lir.FunctionType(lir.VoidType(), [lir.IntType(128), lir.
        IntType(8).as_pointer().as_pointer(), lir.IntType(64).as_pointer(),
        lir.IntType(32)])
    mpzhg__tivp = cgutils.get_or_insert_function(builder.module, phwu__kvja,
        name='decimal_to_str')
    builder.call(mpzhg__tivp, [val, kwri__tcg._get_ptr_by_name('meminfo'),
        kwri__tcg._get_ptr_by_name('length'), scale])
    kwri__tcg.kind = context.get_constant(types.int32, numba.cpython.
        unicode.PY_UNICODE_1BYTE_KIND)
    kwri__tcg.is_ascii = context.get_constant(types.int32, 1)
    kwri__tcg.hash = context.get_constant(numba.cpython.unicode._Py_hash_t, -1)
    kwri__tcg.data = context.nrt.meminfo_data(builder, kwri__tcg.meminfo)
    kwri__tcg.parent = cgutils.get_null_value(kwri__tcg.parent.type)
    return kwri__tcg._getvalue()
...
def str_to_decimal_codegen(context, builder, signature, args):
    val, zdltg__uic, zdltg__uic = args
    val = bodo.libs.str_ext.gen_unicode_to_std_str(context, builder, val)
    phwu__kvja = lir.FunctionType(lir.IntType(128), [lir.IntType(8).
        as_pointer()])
    mpzhg__tivp = cgutils.get_or_insert_function(builder.module, phwu__kvja,
        name='str_to_decimal')
    gvyla__jhvc = builder.call(mpzhg__tivp, [val])
    return gvyla__jhvc
...
def decimal128type_to_int64_tuple(typingctx, val):
    ...
    def codegen(context, builder, signature, args):
        guyh__aej = cgutils.alloca_once(builder, lir.ArrayType(lir.IntType(
            64), 2))
        builder.store(args[0], builder.bitcast(guyh__aej, lir.IntType(128).
            as_pointer()))
        return builder.load(guyh__aej)
...
def decimal128type_cmp(typingctx, val1, scale1, val2, scale2, func_name):
    ...
    bege__vckaz = get_overload_const_str(func_name)

    def codegen(context, builder, signature, args):
        val1, scale1, val2, scale2, zdltg__uic = args
        phwu__kvja = lir.FunctionType(lir.IntType(1), [lir.IntType(128),
            lir.IntType(64), lir.IntType(128), lir.IntType(64)])
        mpzhg__tivp = cgutils.get_or_insert_function(builder.module,
            phwu__kvja, name=bege__vckaz)
        return builder.call(mpzhg__tivp, (val1, scale1, val2, scale2))
...
def decimal_create_cmp_op_overload(op):
    ...
    def overload_cmp(lhs, rhs):
        if isinstance(lhs, Decimal128Type) and isinstance(rhs, Decimal128Type):
            bege__vckaz = 'decimal_cmp_' + op.__name__
            scale1 = lhs.scale
            scale2 = rhs.scale

            def impl(lhs, rhs):
                return decimal128type_cmp(lhs, scale1, rhs, scale2, bege__vckaz
                    )
            return impl
    return overload_cmp
...
def lower_constant_decimal(context, builder, ty, pyval):
    iym__ywkgx = numba.njit(lambda v: decimal128type_to_int64_tuple(v))(pyval)
    xjv__ner = [context.get_constant_generic(builder, types.int64, v) for v in
        iym__ywkgx]
    vfu__ksdfe = cgutils.pack_array(builder, xjv__ner)
    guyh__aej = cgutils.alloca_once(builder, lir.IntType(128))
    builder.store(vfu__ksdfe, builder.bitcast(guyh__aej, lir.ArrayType(lir.
...
def unbox_decimal(typ, val, c):
    phwu__kvja = lir.FunctionType(lir.VoidType(), [lir.IntType(8).
        as_pointer(), lir.IntType(128).as_pointer()])
    mpzhg__tivp = cgutils.get_or_insert_function(c.builder.module,
        phwu__kvja, name='unbox_decimal')
    guyh__aej = cgutils.alloca_once(c.builder, c.context.get_value_type(
        int128_type))
    c.builder.call(mpzhg__tivp, [val, guyh__aej])
    uiagu__brs = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    anpm__otwmy = c.builder.load(guyh__aej)
    return NativeValue(anpm__otwmy, is_error=uiagu__brs)


@box(Decimal128Type)
def box_decimal(typ, val, c):
    vky__zrnyp = decimal_to_str_codegen(c.context, c.builder, bodo.
        string_type(typ), (val,), typ.scale)
    kqp__dyrff = c.pyapi.from_native_value(bodo.string_type, vky__zrnyp, c.
        env_manager)
    lqvr__lex = c.context.insert_const_string(c.builder.module, 'decimal')
    uvky__iaxg = c.pyapi.import_module_noblock(lqvr__lex)
    guyh__aej = c.pyapi.call_method(uvky__iaxg, 'Decimal', (kqp__dyrff,))
    c.pyapi.decref(kqp__dyrff)
    c.pyapi.decref(uvky__iaxg)
    return guyh__aej
...
class DecimalArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        lobxh__cfh = [('data', data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, lobxh__cfh)
...
def init_decimal_array(typingctx, data, null_bitmap, precision_tp, scale_tp
    ...
    def codegen(context, builder, signature, args):
        ojxcs__pey, fpcf__qmmmn, zdltg__uic, zdltg__uic = args
        owg__wtcro = cgutils.create_struct_proxy(signature.return_type)(context
            , builder)
        owg__wtcro.data = ojxcs__pey
        owg__wtcro.null_bitmap = fpcf__qmmmn
        context.nrt.incref(builder, signature.args[0], ojxcs__pey)
        context.nrt.incref(builder, signature.args[1], fpcf__qmmmn)
        return owg__wtcro._getvalue()
    precision = get_overload_const_int(precision_tp)
    scale = get_overload_const_int(scale_tp)
    ocr__cql = DecimalArrayType(precision, scale)
    ndpdd__qwg = ocr__cql(data, null_bitmap, precision_tp, scale_tp)
    return ndpdd__qwg, codegen


@lower_constant(DecimalArrayType)
def lower_constant_decimal_arr(context, builder, typ, pyval):
    n = len(pyval)
    idjq__ssnxh = context.get_constant(types.int64, n)
    hyeo__xps = bodo.utils.utils._empty_nd_impl(context, builder, types.
        Array(int128_type, 1, 'C'), [idjq__ssnxh])
    ypb__xzdri = np.empty(n + 7 >> 3, np.uint8)

    def f(arr, idx, val):
        arr[idx] = decimal128type_to_int128(val)
    for thpql__ykgor, zdtpa__ezfs in enumerate(pyval):
        obzz__hsao = pd.isna(zdtpa__ezfs)
        bodo.libs.int_arr_ext.set_bit_to_arr(ypb__xzdri, thpql__ykgor, int(
            not obzz__hsao))
        if not obzz__hsao:
            context.compile_internal(builder, f, types.void(types.Array(
                int128_type, 1, 'C'), types.int64, Decimal128Type(typ.
                precision, typ.scale)), [hyeo__xps._getvalue(), context.
                get_constant(types.int64, thpql__ykgor), context.
                get_constant_generic(builder, Decimal128Type(typ.precision,
                typ.scale), zdtpa__ezfs)])
    kjpy__xmhr = context.get_constant_generic(builder, nulls_type, ypb__xzdri)
    owg__wtcro = context.make_helper(builder, typ)
    owg__wtcro.data = hyeo__xps._getvalue()
    owg__wtcro.null_bitmap = kjpy__xmhr
    return owg__wtcro._getvalue()


@numba.njit(no_cpython_wrapper=True)
def alloc_decimal_array(n, precision, scale):
    qte__fwydp = np.empty(n, dtype=int128_type)
    ciprk__qhk = np.empty(n + 7 >> 3, dtype=np.uint8)
    return init_decimal_array(qte__fwydp, ciprk__qhk, precision, scale)
...
def box_decimal_arr(typ, val, c):
    awxcn__jpyik = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    qte__fwydp = c.context.make_array(types.Array(int128_type, 1, 'C'))(c.
        context, c.builder, awxcn__jpyik.data)
    mcpce__jgzn = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, awxcn__jpyik.null_bitmap).data
    n = c.builder.extract_value(qte__fwydp.shape, 0)
    scale = c.context.get_constant(types.int32, typ.scale)
    phwu__kvja = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(128).as_pointer(), lir.IntType(8).as_pointer(), lir.IntType
        (32)])
    cerg__cuqia = cgutils.get_or_insert_function(c.builder.module,
        phwu__kvja, name='box_decimal_array')
    xwfz__hzysz = c.builder.call(cerg__cuqia, [n, qte__fwydp.data,
        mcpce__jgzn, scale])
...
def unbox_decimal_arr(typ, val, c):
    owg__wtcro = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    obb__knvcj = c.pyapi.call_method(val, '__len__', ())
    n = c.pyapi.long_as_longlong(obb__knvcj)
    c.pyapi.decref(obb__knvcj)
    epjqe__osd = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(
        64), 7)), lir.Constant(lir.IntType(64), 8))
    hyeo__xps = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types
        .Array(int128_type, 1, 'C'), [n])
    qya__soc = bodo.utils.utils._empty_nd_impl(c.context, c.builder, types.
        Array(types.uint8, 1, 'C'), [epjqe__osd])
    phwu__kvja = lir.FunctionType(lir.VoidType(), [lir.IntType(8).
        as_pointer(), lir.IntType(64), lir.IntType(128).as_pointer(), lir.
        IntType(8).as_pointer()])
    mpzhg__tivp = cgutils.get_or_insert_function(c.builder.module,
        phwu__kvja, name='unbox_decimal_array')
    c.builder.call(mpzhg__tivp, [val, n, hyeo__xps.data, qya__soc.data])
    owg__wtcro.null_bitmap = qya__soc._getvalue()
    owg__wtcro.data = hyeo__xps._getvalue()
    uiagu__brs = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(owg__wtcro._getvalue(), is_error=uiagu__brs)
...
def decimal_arr_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl(A, ind):
            mjzby__ess, zwpsp__jixr = array_getitem_bool_index(A, ind)
            return init_decimal_array(mjzby__ess, zwpsp__jixr, precision, scale
                )
        return impl
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        ...
        def impl(A, ind):
            mjzby__ess, zwpsp__jixr = array_getitem_int_index(A, ind)
            return init_decimal_array(mjzby__ess, zwpsp__jixr, precision, scale
                )
        return impl
    if isinstance(ind, types.SliceType):
        ...
        def impl_slice(A, ind):
            mjzby__ess, zwpsp__jixr = array_getitem_slice_index(A, ind)
            return init_decimal_array(mjzby__ess, zwpsp__jixr, precision, scale
                )
        return impl_slice
    raise BodoError(
