...
def _install_mpl_types():
    bvsl__qtm = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for knkk__qmxu, wrvi__uiw in bvsl__qtm:
        install_py_obj_class(types_name=knkk__qmxu, python_type=wrvi__uiw,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    kanvq__dygjm = ', '.join(f'e{qvaab__ksqo}' for qvaab__ksqo in range(len
        (args)))
    if kanvq__dygjm:
        kanvq__dygjm += ', '
    zzxd__cxnoj = ', '.join(f"{fnz__oezb} = ''" for fnz__oezb in kws.keys())
    yrd__tdh = 'matplotlib_obj, ' if obj_typ is not None else ''
    uqw__rsby = f'def mpl_stub({yrd__tdh} {kanvq__dygjm} {zzxd__cxnoj}):\n'
    uqw__rsby += '    pass\n'
    qbunn__ssbpf = {}
    exec(uqw__rsby, {}, qbunn__ssbpf)
    qnne__msah = qbunn__ssbpf['mpl_stub']
    lalm__mpr = numba.core.utils.pysignature(qnne__msah)
    jmaj__nnyd = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(jmaj__nnyd)).replace(pysig=
        lalm__mpr)


def generate_axes_typing(mod_name, nrows, ncols):
    naaj__gtjwe = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(naaj__gtjwe.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(naaj__gtjwe.format(mod_name, 'ncols'))
    pekh__mihou = get_overload_const_int(nrows)
    apbyt__oloy = get_overload_const_int(ncols)
    if pekh__mihou < 1:
        raise BodoError(naaj__gtjwe.format(mod_name, 'nrows'))
    if apbyt__oloy < 1:
        raise BodoError(naaj__gtjwe.format(mod_name, 'ncols'))
    if pekh__mihou == 1 and apbyt__oloy == 1:
        yxog__txsx = types.mpl_axes_type
    else:
        if apbyt__oloy == 1:
            kgfez__eqp = types.mpl_axes_type
        else:
            kgfez__eqp = types.Tuple([types.mpl_axes_type] * apbyt__oloy)
        yxog__txsx = types.Tuple([kgfez__eqp] * pekh__mihou)
    return yxog__txsx


def generate_pie_return_type(args, kws):
    pkqm__hlcm = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if pkqm__hlcm == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    rjt__vxo = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(rjt__vxo):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(rjt__vxo):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        vth__wqany = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, vth__wqany]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    vth__wqany = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    knkk__qmxu = str(vth__wqany)
    if not hasattr(types, knkk__qmxu):
        knkk__qmxu = f'objmode_type{ir_utils.next_label()}'
        setattr(types, knkk__qmxu, vth__wqany)
    uqw__rsby = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{knkk__qmxu}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    qbunn__ssbpf = {}
    exec(uqw__rsby, {'numba': numba, 'np': np}, qbunn__ssbpf)
    impl = qbunn__ssbpf['impl']
    return impl
