...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        ivck__fbfzg = self.overloads[sig]
        xddln__qdkyy = ivck__fbfzg.metadata.get('distributed_diagnostics', None
            )
        if xddln__qdkyy is None:
            ozohy__tkj = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(ozohy__tkj)
        xddln__qdkyy.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        udxi__ryjdp = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(udxi__ryjdp)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        tbygo__dsjf = options.pop('distributed')
        pipeline_class = (pipeline_class if tbygo__dsjf else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        gld__fiqr = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if gld__fiqr else
            pipeline_class)
    nqnea__ywqx = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(nqnea__ywqx, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(nqnea__ywqx)
        else:
            return master_mode_wrapper(nqnea__ywqx)
    else:
        return nqnea__ywqx
