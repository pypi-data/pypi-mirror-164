...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    kqup__jpdr = pq_node.out_vars[0].name
    hjtoe__yxkv = pq_node.out_vars[1].name
    if kqup__jpdr not in lives and hjtoe__yxkv not in lives:
        return None
    elif kqup__jpdr not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
        pq_node.is_live_table = False
    elif hjtoe__yxkv not in lives:
