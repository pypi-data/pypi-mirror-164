...
def box_regex_flag(typ, val, c):
    iogeb__gigt = c.box(types.int64, val)
    ehrpi__hnoe = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(ehrpi__hnoe, (iogeb__gigt,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    iogeb__gigt = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, iogeb__gigt)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    ioiwt__kcch = context.get_python_api(builder)
    tvt__fkkh = builder.icmp_signed('==', val, ioiwt__kcch.borrow_none())
    with builder.if_then(tvt__fkkh):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    saatw__kkw = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(saatw__kkw)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        mhgg__srqsq = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if mhgg__srqsq > 1:
            typ = types.List(types.Tuple([string_type] * mhgg__srqsq))
...
def overload_match_group(m, *args):
    ...
    qrx__iixtd = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            ydpfc__ucgws = args[0]
            with numba.objmode(out=qrx__iixtd):
                out = m.group(ydpfc__ucgws)
            return out
        return _match_group_impl_one
    tlzr__lng = 'tuple_str_{}'.format(len(args))
    setattr(types, tlzr__lng, types.Tuple([qrx__iixtd] * len(args)))
    owb__jcqo = ', '.join('group{}'.format(zzu__jcqc + 1) for zzu__jcqc in
        range(len(args)))
    fbk__qaucm = 'def _match_group_impl(m, *args):\n'
    fbk__qaucm += '  ({}) = args\n'.format(owb__jcqo)
    fbk__qaucm += "  with numba.objmode(out='{}'):\n".format(tlzr__lng)
    fbk__qaucm += '    out = m.group({})\n'.format(owb__jcqo)
    fbk__qaucm += '  return out\n'
    lxx__fmx = {}
    exec(fbk__qaucm, globals(), lxx__fmx)
    jqt__nshv = lxx__fmx['_match_group_impl']
    return jqt__nshv
...
def overload_match_groups(m, default=None):
    bfsnt__wfyj = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=bfsnt__wfyj):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(jhswm__fhcw is None for jhswm__fhcw in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=qrx__iixtd):
