...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        vmr__knewx = []
        fyli__vmt = h5g_get_num_objs(obj_id)
        for wcbi__sux in range(fyli__vmt):
            jbce__gtnhg = h5g_get_objname_by_idx(obj_id, wcbi__sux)
            vmr__knewx.append(jbce__gtnhg)
        return vmr__knewx
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    kgdh__vmz = parse_dtype(dtype)
    nvj__ixjyc = np.int32(numba_to_c_type(kgdh__vmz))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            mtdlm__dfjl = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    pxqin__anfb = tuple_to_ptr(starts)
    xxf__mxan = tuple_to_ptr(counts)
    vug__iuaq = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, pxqin__anfb, xxf__mxan,
        is_parallel, out_arr.ctypes, vug__iuaq)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    xxf__mxan = tuple_to_ptr(counts)
    vug__iuaq = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, pxqin__anfb, xxf__mxan,
        is_parallel, out_arr.ctypes, vug__iuaq)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        ysp__eytwc = types.Array(dtype, ndim, 'C')
        vyr__xflej = args[3]
        ysp__eytwc = self.context.resolve_function_type(operator.getitem, [
            ysp__eytwc, vyr__xflej], {}).return_type.copy(layout='C')
        return signature(ysp__eytwc, *args)
...
def get_filter_read_indices(bool_arr):
    dken__ktd = bool_arr.nonzero()[0]
    jlwnz__nwppx = bodo.libs.distributed_api.get_rank()
    elhwu__zzzzt = bodo.libs.distributed_api.get_size()
    qbls__vjrma = np.empty(elhwu__zzzzt, np.int64)
    krz__mnifv = len(bool_arr)
    bodo.libs.distributed_api.allgather(qbls__vjrma, krz__mnifv)
    giotb__vzigm = qbls__vjrma.cumsum()[jlwnz__nwppx] - krz__mnifv
    dken__ktd += giotb__vzigm
    kptv__kfei = bodo.libs.distributed_api.dist_reduce(len(dken__ktd), np.
        int32(sum_op))
    gkc__cbe = bodo.libs.distributed_api.gatherv(dken__ktd)
    if jlwnz__nwppx == 0:
        mta__awnax = gkc__cbe
    else:
        mta__awnax = np.empty(kptv__kfei, dken__ktd.dtype)
    bodo.libs.distributed_api.bcast(mta__awnax)
    fgz__gas = bodo.libs.distributed_api.get_start(kptv__kfei, elhwu__zzzzt,
        jlwnz__nwppx)
    yltxy__iiuc = bodo.libs.distributed_api.get_end(kptv__kfei,
        elhwu__zzzzt, jlwnz__nwppx)
    return mta__awnax[fgz__gas:yltxy__iiuc]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        fii__zvf = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], fii__zvf)
        return builder.bitcast(fii__zvf, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    xxf__mxan = tuple_to_ptr(counts)
    vug__iuaq = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, pxqin__anfb,
        xxf__mxan, is_parallel, out_arr.ctypes, vug__iuaq, read_indices.
