...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        obbp__klje = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, obbp__klje)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            vmp__burw = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(vmp__burw.dtype)
            file_write(unicode_to_utf8(fname), vmp__burw.ctypes, dtype_size *
                vmp__burw.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            vmp__burw = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(vmp__burw.dtype)
            wismq__fwhhl = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(vmp__burw.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), vmp__burw.ctypes,
                start, count, wismq__fwhhl)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            oldh__vlem = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                oldh__vlem = min(oldh__vlem, count * dtype_size)
            if oldh__vlem < 0:
                return -1
            return oldh__vlem
        return impl
