...
def get_pytz_type_info(pytz_type):
    if isinstance(pytz_type, pytz._FixedOffset):
        okpi__map = pd.Timedelta(pytz_type._offset).value
    else:
        okpi__map = pytz_type.zone
        if okpi__map not in pytz.all_timezones_set:
            raise BodoError(
...
def nanoseconds_to_offset(nanoseconds):
    qghv__ohe = nanoseconds // (60 * 1000 * 1000 * 1000)
    return pytz.FixedOffset(qghv__ohe)
...
class PandasDatetimeArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        bbwys__dhktf = [('data', fe_type.data_array_type)]
        models.StructModel.__init__(self, dmm, fe_type, bbwys__dhktf)
...
def unbox_pd_datetime_array(typ, val, c):
    urwe__tsyeo = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    yug__tqeik = c.pyapi.string_from_constant_string('datetime64[ns]')
    tsjl__nhadi = c.pyapi.call_method(val, 'to_numpy', (yug__tqeik,))
    urwe__tsyeo.data = c.unbox(typ.data_array_type, tsjl__nhadi).value
    c.pyapi.decref(tsjl__nhadi)
    nfhba__jdd = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(urwe__tsyeo._getvalue(), is_error=nfhba__jdd)
...
def box_pd_datetime_array(typ, val, c):
    urwe__tsyeo = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    c.context.nrt.incref(c.builder, typ.data_array_type, urwe__tsyeo.data)
    wdy__lgduf = c.pyapi.from_native_value(typ.data_array_type, urwe__tsyeo
        .data, c.env_manager)
    rcp__lxc = c.context.get_constant_generic(c.builder, types.unicode_type,
        'ns')
    okns__pjvq = c.pyapi.from_native_value(types.unicode_type, rcp__lxc, c.
        env_manager)
    if isinstance(typ.tz, str):
        doso__hnq = c.context.get_constant_generic(c.builder, types.
            unicode_type, typ.tz)
        vryz__ksiuy = c.pyapi.from_native_value(types.unicode_type,
            doso__hnq, c.env_manager)
    else:
        oniq__wzmqs = nanoseconds_to_offset(typ.tz)
        vryz__ksiuy = c.pyapi.unserialize(c.pyapi.serialize_object(oniq__wzmqs)
            )
    ukfof__vdo = c.context.insert_const_string(c.builder.module, 'pandas')
    ongz__mdkaw = c.pyapi.import_module_noblock(ukfof__vdo)
    gtm__crj = c.pyapi.call_method(ongz__mdkaw, 'DatetimeTZDtype', (
        okns__pjvq, vryz__ksiuy))
    dre__xpl = c.pyapi.object_getattr_string(ongz__mdkaw, 'arrays')
    hvamg__naoy = c.pyapi.call_method(dre__xpl, 'DatetimeArray', (
        wdy__lgduf, gtm__crj))
    c.pyapi.decref(wdy__lgduf)
    c.pyapi.decref(okns__pjvq)
    c.pyapi.decref(vryz__ksiuy)
    c.pyapi.decref(ongz__mdkaw)
    c.pyapi.decref(gtm__crj)
    c.pyapi.decref(dre__xpl)
...
def init_pandas_datetime_array(typingctx, data, tz):
    ...
    def codegen(context, builder, sig, args):
        data, tz = args
        foyl__ycnq = cgutils.create_struct_proxy(sig.return_type)(context,
            builder)
        foyl__ycnq.data = data
        context.nrt.incref(builder, sig.args[0], data)
        return foyl__ycnq._getvalue()
    if is_overload_constant_str(tz) or is_overload_constant_int(tz):
        doso__hnq = get_literal_value(tz)
    else:
        raise BodoError('tz must be a constant string or Fixed Offset')
    xect__kbf = DatetimeArrayType(doso__hnq)
    sig = xect__kbf(xect__kbf.data_array_type, tz)
    return sig, codegen
...
def lower_constant_pd_datetime_arr(context, builder, typ, pyval):
    ykp__mdrup = context.get_constant_generic(builder, typ.data_array_type,
        pyval.to_numpy('datetime64[ns]'))
    zvxt__qmj = lir.Constant.literal_struct([ykp__mdrup])
    return zvxt__qmj
...
def overload_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl_bool(A, ind):
            ...
            ecswi__fsebr = ensure_contig_if_np(A._data[ind])
            return init_pandas_datetime_array(ecswi__fsebr, tz)
        return impl_bool
...
def unwrap_tz_array_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    pehud__rzn = args[0]
    if equiv_set.has_shape(pehud__rzn):
        return ArrayAnalysis.AnalyzeResult(shape=pehud__rzn, pre=[])
    return None
