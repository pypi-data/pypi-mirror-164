...
class TupleArrayType(types.ArrayCompatible):
    ...
    @property
    def dtype(self):
        return types.BaseTuple.from_types(tuple(ivzdc__ivoz.dtype for
            ivzdc__ivoz in self.data))
...
class TupleArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        wqzca__vbqhs = [('data', StructArrayType(fe_type.data))]
        models.StructModel.__init__(self, dmm, fe_type, wqzca__vbqhs)
...
def init_tuple_arr(typingctx, data_typ=None):
    ...
    ndbwt__tznpd = TupleArrayType(data_typ.data)

    def codegen(context, builder, sig, args):
        hnxh__apr, = args
        cbvch__xwjpg = context.make_helper(builder, ndbwt__tznpd)
        cbvch__xwjpg.data = hnxh__apr
        context.nrt.incref(builder, data_typ, hnxh__apr)
        return cbvch__xwjpg._getvalue()
    return ndbwt__tznpd(data_typ), codegen


@unbox(TupleArrayType)
def unbox_tuple_array(typ, val, c):
    data_typ = StructArrayType(typ.data)
    kng__udcpm = unbox_struct_array(data_typ, val, c, is_tuple_array=True)
    hnxh__apr = kng__udcpm.value
    cbvch__xwjpg = c.context.make_helper(c.builder, typ)
    cbvch__xwjpg.data = hnxh__apr
    bptfx__xkqp = kng__udcpm.is_error
    return NativeValue(cbvch__xwjpg._getvalue(), is_error=bptfx__xkqp)


@box(TupleArrayType)
def box_tuple_arr(typ, val, c):
    data_typ = StructArrayType(typ.data)
    cbvch__xwjpg = c.context.make_helper(c.builder, typ, val)
    arr = box_struct_arr(data_typ, cbvch__xwjpg.data, c, is_tuple_array=True)
    return arr
...
def tuple_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        ulbg__oymwa = 'def impl(arr, ind):\n'
        imphk__sokt = ','.join(f'get_data(arr._data)[{stz__xyg}][ind]' for
            stz__xyg in range(len(arr.data)))
        ulbg__oymwa += f'  return ({imphk__sokt})\n'
        gwy__pjd = {}
        exec(ulbg__oymwa, {'get_data': bodo.libs.struct_arr_ext.get_data},
            gwy__pjd)
        ufxs__nzltq = gwy__pjd['impl']
        return ufxs__nzltq
...
def tuple_arr_setitem(arr, ind, val):
    ...
    if val == types.none or isinstance(val, types.optional):
        return
    if isinstance(ind, types.Integer):
        snpi__yvjmx = len(arr.data)
        ulbg__oymwa = 'def impl(arr, ind, val):\n'
        ulbg__oymwa += '  data = get_data(arr._data)\n'
        ulbg__oymwa += '  null_bitmap = get_null_bitmap(arr._data)\n'
        ulbg__oymwa += '  set_bit_to_arr(null_bitmap, ind, 1)\n'
        for stz__xyg in range(snpi__yvjmx):
            ulbg__oymwa += f'  data[{stz__xyg}][ind] = val[{stz__xyg}]\n'
        gwy__pjd = {}
        exec(ulbg__oymwa, {'get_data': bodo.libs.struct_arr_ext.get_data,
            'get_null_bitmap': bodo.libs.struct_arr_ext.get_null_bitmap,
            'set_bit_to_arr': bodo.libs.int_arr_ext.set_bit_to_arr}, gwy__pjd)
        ufxs__nzltq = gwy__pjd['impl']
        return ufxs__nzltq
