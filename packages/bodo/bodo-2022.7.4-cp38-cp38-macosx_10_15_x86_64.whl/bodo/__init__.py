...
def _global_except_hook(exctype, value, traceback):
    ...
    uqvk__fsvf = MPI.COMM_WORLD
    irop__kjhbq = uqvk__fsvf.Get_rank()
    hfho__xps = 3.0
    hrqsp__lyq = True
    vkg__rgp = uqvk__fsvf.Ibarrier()
    ofwt__xrp = time.time()
    while time.time() - ofwt__xrp < hfho__xps:
        time.sleep(0.1)
        if vkg__rgp.Test():
            hrqsp__lyq = False
            break
    try:
        ...
        if hrqsp__lyq:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {irop__kjhbq}. \n')
...
def set_numba_environ_vars():
    if (gxuew__hal := os.environ.get('BODO_PLATFORM_CACHE_LOCATION')
        ) is not None:
        if 'NUMBA_CACHE_DIR' in os.environ and os.environ['NUMBA_CACHE_DIR'
            ...
            numba.config.CACHE_DIR = gxuew__hal
            os.environ['NUMBA_CACHE_DIR'] = gxuew__hal
    numba.config.DISABLE_PERFORMANCE_WARNINGS = 1
    yhh__meikz = {'NUMBA_DISABLE_PERFORMANCE_WARNINGS': '1'}
    os.environ.update(yhh__meikz)
