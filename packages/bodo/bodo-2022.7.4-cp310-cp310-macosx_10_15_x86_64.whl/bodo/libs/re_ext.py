...
def box_regex_flag(typ, val, c):
    vfo__ishbn = c.box(types.int64, val)
    ewssk__xzj = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(ewssk__xzj, (vfo__ishbn,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    vfo__ishbn = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, vfo__ishbn)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    hhs__zftde = context.get_python_api(builder)
    ewesb__wbcsi = builder.icmp_signed('==', val, hhs__zftde.borrow_none())
    with builder.if_then(ewesb__wbcsi):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    iza__gykb = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(iza__gykb)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        puzj__pyg = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if puzj__pyg > 1:
            typ = types.List(types.Tuple([string_type] * puzj__pyg))
...
def overload_match_group(m, *args):
    ...
    qsjn__ivkab = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            tzqeu__liltg = args[0]
            with numba.objmode(out=qsjn__ivkab):
                out = m.group(tzqeu__liltg)
            return out
        return _match_group_impl_one
    vhcw__cpt = 'tuple_str_{}'.format(len(args))
    setattr(types, vhcw__cpt, types.Tuple([qsjn__ivkab] * len(args)))
    tbta__zpkd = ', '.join('group{}'.format(wphac__hcvns + 1) for
        wphac__hcvns in range(len(args)))
    hha__jsy = 'def _match_group_impl(m, *args):\n'
    hha__jsy += '  ({}) = args\n'.format(tbta__zpkd)
    hha__jsy += "  with numba.objmode(out='{}'):\n".format(vhcw__cpt)
    hha__jsy += '    out = m.group({})\n'.format(tbta__zpkd)
    hha__jsy += '  return out\n'
    hzexr__xukil = {}
    exec(hha__jsy, globals(), hzexr__xukil)
    cwkf__dmn = hzexr__xukil['_match_group_impl']
    return cwkf__dmn
...
def overload_match_groups(m, default=None):
    zvi__jsz = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=zvi__jsz):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(bzeqj__aoti is None for bzeqj__aoti in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=qsjn__ivkab):
