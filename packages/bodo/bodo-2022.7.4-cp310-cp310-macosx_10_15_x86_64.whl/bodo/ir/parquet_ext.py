...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    wkb__fpfr = pq_node.out_vars[0].name
    dqz__mkhi = pq_node.out_vars[1].name
    if wkb__fpfr not in lives and dqz__mkhi not in lives:
        return None
    elif wkb__fpfr not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
        pq_node.is_live_table = False
    elif dqz__mkhi not in lives:
