...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        fynvk__bfw = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, fynvk__bfw)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            dct__qev = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(dct__qev.dtype)
            file_write(unicode_to_utf8(fname), dct__qev.ctypes, dtype_size *
                dct__qev.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            dct__qev = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(dct__qev.dtype)
            egciv__tjqf = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(dct__qev.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), dct__qev.ctypes,
                start, count, egciv__tjqf)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            giiia__owd = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                giiia__owd = min(giiia__owd, count * dtype_size)
            if giiia__owd < 0:
                return -1
            return giiia__owd
        return impl
