        ...
        def wrapper(self, *args, **kwargs):
            ...
            try:
                ...
                if isfunction(after_exception):
                    after_exception(self, *args, **kwargs, remark=remark, current_times=current_times, max_retry=max_retry)
                ...
                if max_retry == -1:
                    ...
                    if isfunction(retry_error_callback):
                        # 重试结束，执行回调函数
                        return retry_error_callback(self, *args, **kwargs, remark=remark, current_times=current_times, max_retry=max_retry)
                    else:
                        # 没有指定回调函数时，弹出错误
                        raise e
                else:
