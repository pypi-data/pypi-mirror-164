...
class Craft(object):
    ...
    def components(self, crate_id: str = None) -> Crate:
        ...
        for crate in self._cache.crates:
            ...
            items_to_append = crate["items"]
            # Append in the crate id so the component can make network queries alone
            for n in items_to_append:
                n["crate_id"] = crate["id"]
            items += items_to_append

        # Make a crate with the current items in the craft, plus network callbacks that it needs
        return Crate(items, requested_crate_ids, {k: self._network_callbacks[k] for k in ["component_resync_query",
                                                                                          "component_resync_execute",
                                                                                          "component_attribute_edit",
                                                                                          "component_attribute_remove",
                                                                                          ]})
