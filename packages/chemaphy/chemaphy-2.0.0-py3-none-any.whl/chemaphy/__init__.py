...
class Statistics:
    ...
    def RunningMean(args:(list|np.ndarray),size:int = 2):
        r"""
        ...
        if type(size) == int:
            ...
            return "size should be integer only"
    ...
    def LR(args:(list|np.ndarray),kwargs:(list|np.ndarray)):
        ...
        if len(args) == len(kwargs):
            ...
            if type(args) == (list|np.ndarray) & type(kwargs) == (list|np.ndarray):
                for i in range(0,len(args)):
                    a = args[i]**2
                    b = args[i]*kwargs[i]
                    xy += b
    ...
    def StandardError(args:(list|np.ndarray)):
        if type(args) == (list|np.ndarray):
            dev = Statistics.StandardDeviation(args)
        else:
            X = np.asarray(args)
            dev = Statistics.StandardDeviation(X)
    ...
    def MeanSquaredError(actual:(list|np.ndarray),predicted:(list|np.ndarray)):
        ...
        if len(actual) == len(predicted):
            errors = []
            if type(actual) == (list|np.ndarray) & type(predicted) == (list|np.ndarray):
    ...
    def Accuracy(actual:(list|np.ndarray),predicted:(list|np.ndarray)):
        if len(actual) == len(predicted):
            ...
            if (type(actual) == int|np.ndarray) & (type(predicted) == int|np.ndarray):
                for i in range(0,len(actual)):
                ...
                y = np.asarray(predicted)
                for i in range(0,len(x)):
                    rel_actual.append(round(x[i],0))
                    rel_predicted.append(round(y[i],0))
    ...
    def Intercept(x:(list|np.ndarray),y:(list|np.ndarray)):
        ...
        if len(x) == len(y):
            X,Y,XY,X2 = 0,0,0,0
            if type(x) == (list|np.ndarray) & type(y) == (list|np.ndarray):
                for i in range(0,len(x)):
                    Y += y[i]
                    X += x[i]
                    XY += (x[i]*y[i])
