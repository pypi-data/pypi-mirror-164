...
from syndb_cassandra.settings import SyndbCassandraSettings, get_settings
...
def create_syndb_tables(settings: Optional[SyndbCassandraSettings] = None) -> None:
    ...
    partial_mv = partial(create_materialized_view, current_connection, SYNDB_NEURO_DATA_KEYSPACE)
    brain_structure_materialized_views_metadata = orjson.loads(
        pkgutil.get_data("syndb_cassandra", "assets/brain_structure_materialized_views.json")
    )
...
def create_roles(settings: Optional[SyndbCassandraSettings] = None) -> None:
    ...
    current_connection.execute(f"CREATE ROLE IF NOT EXISTS {NEURO_DATA_MODIFY_ROLE_NAME}")
    current_connection.execute(f"GRANT ALTER on KEYSPACE {SYNDB_NEURO_DATA_KEYSPACE} to {NEURO_DATA_MODIFY_ROLE_NAME}")
    current_connection.execute(f"GRANT SELECT on KEYSPACE {SYNDB_NEURO_DATA_KEYSPACE} to {NEURO_DATA_MODIFY_ROLE_NAME}")

    current_connection.execute(f"CREATE ROLE IF NOT EXISTS {NEURO_DATA_READ_ROLE_NAME}")
    current_connection.execute(f"GRANT SELECT on KEYSPACE {SYNDB_NEURO_DATA_KEYSPACE} to {NEURO_DATA_READ_ROLE_NAME}")
