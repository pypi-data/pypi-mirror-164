            ...
            for j, sample in enumerate(self.samples):
                for k, label in enumerate(sample.geoms):
                    ...
                    if (
                        ...
                        if (
                            k == len(sample.geoms) - 1 and j == len(self.samples) - 1
                        ):  # On the last sample, set the range of the value being plotted on the radial axis.
                            delta = max_r - min_r
                            self.ax.set_ylim(min_r - delta / 10, max_r + delta / 10)
                            self.ax.set_rgrids(np.round(np.arange(min_r, max_r + delta / 10, delta * 0.75), 3))
                            self.ax.set_yticks(np.round(np.arange(min_r, max_r + delta / 10, delta * 0.75), 3))
                            ...
                            with plt.style.context("default"):
                                self.white_ax.set_ylim(min_r - delta / 10, max_r + delta / 10)
                                self.white_ax.set_rgrids(np.round(np.arange(min_r, max_r + delta / 10, delta * 0.75), 3))
                                self.white_ax.set_yticks(np.round(np.arange(min_r, max_r + delta / 10, delta * 0.75), 3))
