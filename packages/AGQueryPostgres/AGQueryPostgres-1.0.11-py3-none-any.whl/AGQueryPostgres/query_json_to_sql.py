...
class AGJsonToSQL:
    ...
    # SELECT
    #
    def _get_column_name(self, column_name: str, alias_name: str) -> str:
        if "." in column_name:
            result = column_name
        else:
    ...
    # GROUP BY
    #
    def _get_group_by_column_list(self, select, alias):
        column_list = ""
        for f in select:
    ...
    def _get_group_by_format(self, c: Coder, select: list, format_list: list, alias: str, function: str) -> Coder:
        format_column_list = self._get_group_by_column_list(format_list, alias)
        ro_select: list = []
        for s in select:
            if s not in format_list:
                ro_select.append(s)

        if len(ro_select) > 0:
            column_list = self._get_group_by_column_list(ro_select, alias)
            c.a(0, "GROUP BY " + column_list + ", " + function + " (" + format_column_list + ")")
        else:
            c.a(0, "GROUP BY " + function + " (" + format_column_list + ")")
    ...
    def _get_group_by(self, alias, select, total) -> Coder:
        ...
        if len(total) > 0:
            column_list = self._get_group_by_column_list(select, alias)

            if len(self.rollup) > 0:
                c = self._get_group_by_format(c, select, self.rollup, alias, "ROLLUP")
            elif len(self.cube) > 0:
                c = self._get_group_by_format(c, select, self.cube, alias, "CUBE")
            else:
