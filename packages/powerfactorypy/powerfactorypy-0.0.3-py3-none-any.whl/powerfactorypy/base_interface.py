...
from powerfactorypy.exceptions import PFNonExistingObjectError
...
class PFBaseInterface:
  ...
  def get_obj(self,path,condition=None,parent_folder=None,error_if_non_existent=True,
    ...
    The parent_folder can be a PF container object or a string:
      parent_folder = "user" means 'path' is relative to the active user.
  ...
  def handle_non_existing_obj(self,path,parent_folder,error_if_non_existent):
    ...
    if not error_if_non_existent:
      ...
      raise powerfactorypy.PFPathError(existing_path,non_existing_child)
  ...
  def handle_condition_of_obj_not_met(self,path,obj,error_if_non_existent):
    ...
    if not error_if_non_existent:
      ...
      head,tail = os_path.split(path)
      raise powerfactorypy.PFNonExistingObjectError(obj[0].GetParent(),tail,condition=True)
  ...
  def get_active_project(self):
    ...
    if active_project:
      ...
      raise powerfactorypy.PFNotActiveError("a project")
  ...
  def get_by_condition(self,objects,condition):
    ...
    for obj in objects:
      try:
        ...
        raise powerfactorypy.PFAttributeError(obj,e,self)
      except(TypeError) as e:
        object_str = powerfactorypy.PFStringManipuilation.format_full_path(str(obj),self)
  ...
  def add_results_variable(self,obj,variables,results_obj=None):
    ...
    """
    if not results_obj:
      results_obj_name = powerfactorypy.PFTranslator.get_default_result_object_name(self.language)
