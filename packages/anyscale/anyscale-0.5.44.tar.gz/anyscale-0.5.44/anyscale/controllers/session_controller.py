...
class SessionController(BaseController):
    ...
    def download_log_files(
        ...
        parallelism: int = 10,
        read_timeout: timedelta = timedelta(minutes=30),
    ) -> None:
        """Download all log files based on the passed parameters, by repeatedly calling get_log_files_api_v2,
        with each call returning a *page_size* of results.

        Parameters
        ----------
        base_folder: Optional[str]
            Base folder to download files into. If not provided, then logs will be downloaded in the current working directory.
        glob: Optional[str]
            Glob expression to filter logs.
        session_id: Optional[str]
            DB id of the session.
        node_id: Optional[str]
            Id of a node, usually a worker node.
        job_id: Optional[str]
            the job id for which the logs should be retrieved.
        job_run: Optional[str]
            the job run name for which the logs should be retrieved.
        node_type: Optional[NodeType]
            either head-node or worker-nodes
        page_size: Optional[int]
            Max number of files to fetch at a time from the API.
        ttl_seconds: Optional[int]
            TTL of the presigned URL for downloading logs.
        parallelism: int
            Number of log files to download in parallel at a time.
        read_timeout: timedelta
            The read timeout while downloading log file.
        """
        next_page_token: Optional[str] = None
        while True:
            log_details: LogDetails = self.api_client.get_log_files_api_v2_sessions_session_id_get_log_files_post(
                session_id or "latest",
            ...
            log_files: List[LogDetail] = log_details.log_files
            asyncio.run(
                self._download_files(base_folder, log_files, parallelism, read_timeout)
            )
            if log_details.next_page_token is None:
                break
            next_page_token = log_details.next_page_token

    def download_ray_result_files(
        self,
        base_folder: Optional[str] = None,
        session_id: Optional[str] = None,
        job_id: Optional[str] = None,
        ...
        """Download ray tune result files based on the passed parameters.

        Parameters
        ----------
        base_folder: Optional[str]
            Base folder to download files into. If not provided, then results will be downloaded in the current working directory.
        session_id: Optional[str]
            DB id of the session.
        job_id: Optional[str]
            the job id for which the results should be retrieved.
        job_run: Optional[str]
            the job run name for which the results should be retrieved.
        parallelism: int
            Number of result files to download in parallel at a time.
        read_timeout: timedelta
            The read timeout while downloading result file.
        """
        log_details: LogDetails = self.api_client.get_ray_result_files_api_v2_sessions_session_id_get_ray_result_files_get(
            session_id or "latest", job_id=job_id, job_run=job_run,
        ).result
        log_files: List[LogDetail] = log_details.log_files
        asyncio.run(
            self._download_files(base_folder, log_files, parallelism, read_timeout)
    ...
    async def _download_files(
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            for pos, log_detail in enumerate(log_files):
                downloads.append(
                    asyncio.create_task(
                        self._download_file(
                            sem,
                            pos,
                            os.path.join(
                                base_folder or "", log_detail.name.lstrip("/")
                            ),
                            log_detail.download_url,
                            log_detail.size,
                            session,
                            read_timeout,
                        )
