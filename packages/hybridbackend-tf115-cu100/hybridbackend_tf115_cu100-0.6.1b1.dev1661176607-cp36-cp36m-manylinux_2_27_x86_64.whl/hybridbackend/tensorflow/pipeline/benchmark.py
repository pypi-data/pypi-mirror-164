...
@hb.function(mode=tf.estimator.ModeKeys.TRAIN)
def build_bench_op(params):
  ...
  @hb.pipeline.compute_embedding()
  def build_emb_layer(features):
    columns = {
  ...
  embeddings_out = build_emb_layer(features)
  loss = build_loss(embeddings_out)
  opt = hb.train.GradientDescentOptimizer(learning_rate=params.lr)
